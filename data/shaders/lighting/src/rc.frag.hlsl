// SHADER_BUILD_TAG: rc_field_tag_h
...
//   t0  Emitters — StructuredBuffer<float4>, emitter ei = float4[ei*4 .. ei*4+3]
//   t1  SdfBuf   — StructuredBuffer<float>, x-major sdf[x*map_h+y]
// (shadercross reflection must show samplers=0 storage_buffers=2.)
//
// D3D12 NOTE: this shader is sampler-less. Two patterns crash SDL_GPU's D3D12
// pipeline creation (E_INVALIDARG) while Metal/Vulkan tolerate them — both have
// been removed here:
//   1. a user-struct StructuredBuffer  → use flat StructuredBuffer<float4>.
//   2. a dynamically-indexed local array (indexable temp) → use scalar counters.

// Emitter records are 4×float4 (pos_radius, color_falloff, cone_shape, misc);
// rc.frag uses only pos_radius (+0) and color_falloff (+1).
...
// P2: per-probe shadow-march budget. The first RC_K in-range emitters get a full
// trace_shadow; the rest add unshadowed only. Bounds the pass at
// probes*RC_K*steps regardless of horde size — the horde-TDR fix.
...
float4 main(VS_OUT i) : SV_Target0 {
    ...
    // Single pass, array-free K budget: every in-range emitter adds its
    // UNSHADOWED contribution; the first RC_K of them additionally get a full SDF
    // shadow trace (the cost cap). The previous form tracked the K-strongest in a
    // dynamically-indexed local array, but that indexable-temp pattern produced
    // DXIL the D3D12 driver rejects at pipeline creation (E_INVALIDARG) — a flat
    // scalar counter avoids it. For indirect GI, "first K in-range" vs "K
    // strongest" is visually negligible (every emitter still fills the unshadowed
    // base; only the shadow refinement is budgeted).
    uint traced = 0u;
    [loop] for(uint ei = 0u; ei < me; ++ei) {
        ...
        gi += rgb * atten;
        // First RC_K in-range emitters: refine by replacing the unshadowed term
        // with the shadowed one (add atten*(shadow-1)).
        if(traced < RC_K) {

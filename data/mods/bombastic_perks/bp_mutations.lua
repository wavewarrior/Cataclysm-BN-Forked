...
function Mutation.new(config)
  ...
  self.conditional = config.conditional or nil
  return self
end
...
local function add_perk(config)
...
add_perk({
  ...
  conditional = {
    type = "damage_buff",
    buff_id = "bp_vengeful_buff",
    buff_duration = 2,
    trigger = "on_damage_taken",
    effect = { melee_damage = 0.15 },
  },
})
...
add_perk({
  ...
  conditional = {
    type = "dodge_buff",
    buff_id = "bp_twice_shy_buff",
    buff_duration = 5,
    trigger = "on_damage_taken",
    effect = { dodge = 1, dodge_skill = 2 },
  },
})
...
add_perk({
  ...
  stat_bonuses = {},
  conditional = {
    type = "downed_recovery",
    chance = 0.75,
    trigger = "on_downed_check",
  },
})
...
add_perk({
  ...
  stat_bonuses = {},
  conditional = {
    type = "bleed_damage_scaling",
    trigger = "on_every_5_minutes",
    max_multiplier = 0.50,
  },
})
...
local CONDITIONAL_PERK_IDS = {}

local function register_perk_ids(perk)
  ...
  if perk.conditional then table.insert(CONDITIONAL_PERK_IDS, perk_id) end
end

for id, perk in pairs(PERKS) do
  register_perk_ids(perk)
end
...
return {
  ...
  CONDITIONAL_PERK_IDS = CONDITIONAL_PERK_IDS,

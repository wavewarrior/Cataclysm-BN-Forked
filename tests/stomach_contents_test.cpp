...
static void clear_stomach(player& p) { p.stomach.empty(); }
...
static void print_stomach_contents(player& p, const bool print) {
    if (!print) { return; }
    cata_printf("stomach: %d player: %d/%d\n", p.stomach.get_calories(), p.get_stored_kcal(),
                p.max_stored_kcal());
...
static void eat_all_nutrients(player& p) {
    // Vitamin target: 100% DV -- or 96 vitamin "units" since all vitamins currently decay every
    // 15m. Energy target: 2100 kcal -- debug target will be completely sedentary.
...
TEST_CASE("starve_test_hunger3", "[starve][slow]") {
    ...
    reset_time();
    clear_stomach(dummy);
    while (!(dummy.has_trait(trait_id("HUNGER3")))) { dummy.mutate_towards(trait_id("HUNGER3")); }
    clear_stomach(dummy);
...
TEST_CASE("all_nutrition_starve_test", "[!mayfail][starve][slow]") {
    ...
    if (print_tests) { cata_printf("\n\n"); }

    for (unsigned int day = 0; day <= 20; day++) {
        if (print_tests) { cata_printf("day %u: %d\n", day, dummy.get_stored_kcal()); }
        pass_time(dummy, 1_days);
        dummy.set_thirst(0);
        dummy.set_fatigue(0);
        eat_all_nutrients(dummy);
    ...
    if (print_tests) {
        cata_printf(
            "vitamins: vitA %d vitB %d vitC %d calcium %d iron %d\n",
...
TEST_CASE("Stomach calories become stored calories after less than 1 day", "[stomach]") {
    ...
    for (time_point now = start; now < end; now += 30_minutes) { dummy.update_body(30_minutes); }
...
TEST_CASE("Eating above max kcal causes bloating", "[stomach]") {
    ...
    WHEN("Character consumes calories above max") {
        dummy.eat(food, true);
        THEN("They become bloated") { CHECK(dummy.has_effect(effect_bloated)); }
    }
    WHEN("Bloated character consumes calories") {
        dummy.eat(food, true);
        THEN("They are no longer bloated") { CHECK(dummy.has_effect(effect_bloated)); }

...
static_assert( classify_tile_light( tile_light_query{
    .world_present = true,
    .lighting_ready = true,
} ) == sprite_light_mode::gpu_lit, "classify_tile_light must be a constant expression" );
...
TEST_CASE( "classify_tile_light picks the sprite composite mode", "[lighting]" )
{
    SECTION( "memorized world tile with lighting ready cross-fades to memory" ) {
        CHECK( classify_tile_light( tile_light_query{
            .world_present = true,
            .lighting_ready = true,
            .memorized = true,

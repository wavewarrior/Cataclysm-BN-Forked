...
auto cut_up_yields(const std::string& target) -> void {
    ...
    const units::mass salvaged_mass = std::accumulate(
        salvaged_items.begin(), salvaged_items.end(), 0_gram,
        [](const units::mass acc, const item* salvage) { return acc + salvage->weight(); });
...
TEST_CASE("cut_up_yields_all", "[.]") {
    ...
    for (const itype* itp : item_controller->all()) {
        i++;
        SECTION(string_format("Item number %d", i)) { cut_up_yields(itp->get_id().str()); }
    }
}

...
{
    return R"({"t":42,"d":{"vid":)" + std::to_string( vid ) +
    R"(,"ax":)" + std::to_string( ax ) +
    R"(,"ay":)" + std::to_string( ay ) +
    R"(,"az":)" + std::to_string( az ) +
...
TEST_CASE( "vehicle: vehicle_id_map persists across multiple updates",
           "[coop][vehicle]" )
{
    inproc_harness h;
    h.setup();

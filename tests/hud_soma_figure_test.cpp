...
auto &at( hud_figure::body_state &b, limb l )
{
    return b.limbs[static_cast<int>( l )];
}
...
TEST_CASE( "figure_fill_class_walks_the_ladder_and_stays_visible", "[hud_figure]" )
{
    ...
    const auto splint = hud_figure::fill_class( { .cur = 0, .max = 30, .broken = true,
                        .splinted = true } );

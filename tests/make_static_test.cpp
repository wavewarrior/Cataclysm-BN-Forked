...
TEST_CASE("make_static_macro_test", "[make_static_macro]") {
    ...
    auto get_static = []() -> const auto& { return STATIC(std::string("test")); };
    ...
    static_assert(
        std::is_same_v<std::decay_t<decltype(test11)>, std::string>, "type must be std::string");
...
TEST_CASE("make_static_macro_benchmark_string_id", "[.][make_static_macro][benchmark]") {
    ...
    BENCHMARK("static variable outside") { return test_id.is_empty(); };
    ...
    BENCHMARK("inline const") { return test_id_type("test").is_empty(); };
    ...
    BENCHMARK("make_static macro") { return STATIC(test_id_type("test")).is_empty(); };
}

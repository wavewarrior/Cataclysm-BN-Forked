...
template <typename T> static void test_serialization(const T& val, const std::string& s) {
    CAPTURE(val);
    {
...
static void test_translation_text_style_check(Matcher&& matcher, const std::string& json) {
    ...
    JsonIn jsin(iss);
    translation trans;
    const std::string dmsg = capture_debugmsg_during([&]() { jsin.read(trans); });
...
template <typename Matcher>
static void test_string_error_throws_matches(
    Matcher&& matcher, const std::string& json, const int offset) {
    CAPTURE(json);
    CAPTURE(offset);
    std::istringstream iss(json);
    JsonIn jsin(iss);
...
static bool approximatelyEqual(T a, T b, T eps, const unsigned ulps) {
    ...
    if (std::abs(a - b) <= ulps * limits::denorm_min()) { return true; }
...
template <typename T> static void test_number_roundtrip(T val, T eps, int ulps) {
    T read_val;
    CAPTURE(val);
    {
...
TEST_CASE("serialize_integers", "[json]") {
    ...
    SECTION("random_unsigned") {
        ...
        std::uniform_int_distribution<uint64_t>
            distrib(std::numeric_limits<uint64_t>::min(), std::numeric_limits<uint64_t>::max());

        for (int i = 0; i < 100000; i++) { test_number_roundtrip<uint64_t>(distrib(gen), 0, 0); }
    }
    ...
    SECTION("random_signed") {
        ...
        std::uniform_int_distribution<int64_t>
            distrib(std::numeric_limits<int64_t>::min(), std::numeric_limits<int64_t>::max());

        for (int i = 0; i < 100000; i++) { test_number_roundtrip<int64_t>(distrib(gen), 0, 0); }
    }
}
...
TEST_CASE("serialize_doubles", "[json]") {
    ...
    SECTION("random_doubles_large") {
        ...
        std::uniform_real_distribution<>
            distrib(std::numeric_limits<double>::min(), std::numeric_limits<double>::max());

        for (int i = 0; i < 100000; i++) { test_number_roundtrip(distrib(gen), eps, 4); }
    }

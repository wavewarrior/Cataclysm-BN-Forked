...
TEST_CASE("player_morale", "[morale]") {
    ...
    GIVEN("an empty morale") { CHECK(m.get_level() == 0); }
        ...
        WHEN("took prozac") {
            ...
            THEN("it's not so bad") {
                ...
                AND_WHEN("the effect ends") {
                    ...
                    THEN("guilt returns") { CHECK(m.get_level() == -100); }
                }
            }
        }
    }
    ...
    GIVEN("a set of super fancy bride's clothes") {
        ...
        const item& dress_wedding =
            *item::spawn_temporary("dress_wedding", calendar::start_of_cataclysm);
        const item& veil_wedding =
            *item::spawn_temporary("veil_wedding", calendar::start_of_cataclysm);
        ...
        WHEN("not a stylish person") {
            ...
            THEN("just don't care (even if man)") { CHECK(m.get_level() == 0); }
            ...
            AND_WHEN("wearing yet another wedding gown") {
                ...
                THEN("it adds nothing") {
                    ...
                    AND_WHEN("taking it off") {
                        THEN("your fanciness remains the same") { CHECK(m.get_level() == 19); }
                    }
                }
            }
            AND_WHEN("tries to be even fancier") {
                const item& watch =
                    *item::spawn_temporary("sf_watch", calendar::start_of_cataclysm);
                m.on_item_wear(watch);
                THEN("there's a limit") { CHECK(m.get_level() == 20); }
            }
        }
    }
    ...
    GIVEN("masochist trait") {
        ...
        WHEN("in an insufferable pain") {
            ...
            THEN("there's a limit") { CHECK(m.get(MORALE_PERM_MASOCHIST) == 25); }
        }
    }
    ...
    GIVEN("cenobite trait") {
        ...
        WHEN("in an insufferable pain") {
            ...
            THEN("there's no limit") { CHECK(m.get(MORALE_PERM_MASOCHIST) == 48); }

            AND_WHEN("took prozac") {
                m.on_effect_int_change(effect_took_prozac, 1);
                THEN("it spoils all fun") { CHECK(m.get(MORALE_PERM_MASOCHIST) == 16); }
            }
        }
    }
    ...
    GIVEN("a humanoid plant") {
        ...
        WHEN("wearing a legpouch") {
            ...
            THEN("half of the roots are suffering") { CHECK(m.get(MORALE_PERM_CONSTRAINED) == -5); }
        }
        ...
        WHEN("wearing a pair of boots") {
            ...
            THEN("all of the roots are suffering") { CHECK(m.get(MORALE_PERM_CONSTRAINED) == -10); }

            AND_WHEN("even more constrains") {
                const item& hat =
                    *item::spawn_temporary("tinfoil_hat", calendar::start_of_cataclysm);

                m.on_item_wear(hat);
                THEN("it can't be worse") { CHECK(m.get(MORALE_PERM_CONSTRAINED) == -10); }
            }
        }
    }
    ...
    GIVEN("tough temperature conditions") {
        WHEN("chilly") {
            for (auto bp : temperature_body_parts) { m.on_effect_int_change(effect_cold, 1, bp); }

            AND_WHEN("no time has passed") { CHECK(m.get_level() == 0); }
        ...
        WHEN("cold") {
            for (auto bp : temperature_body_parts) { m.on_effect_int_change(effect_cold, 2, bp); }
        ...
        WHEN("warm") {
            for (auto bp : temperature_body_parts) { m.on_effect_int_change(effect_hot, 1, bp); }
        ...
        WHEN("hot") {
            for (auto bp : temperature_body_parts) { m.on_effect_int_change(effect_hot, 2, bp); }
...
TEST_CASE("player_morale_from_effects", "[morale][effect]") {
    ...
    GIVEN("effects with morale bonuses and penalties") {
        ...
        WHEN("intensity increases") {
            ...
            THEN("so does the bonus") {
                ...
                AND_WHEN("intensity increases again") {
                    ...
                    THEN("bonus increases by expected amount") { CHECK(m.get_level() == 25); }
                }
            }
        }
        ...
        WHEN("intensity increases and drops back") {
            m.on_effect_int_change(drunk, 2);
            m.on_effect_int_change(drunk, 1);
            THEN("bonus stays the same") { CHECK(m.get_level() == 5); }
        }
        ...
        WHEN("a second effect with the same morale type, but negative sign, is added") {
            ...
            THEN("the morale effect is the sum of highest bonus and highest penalty") {
                ...
                AND_WHEN("first effect increases in intensity") {
                    m.on_effect_int_change(drunk, 2);
                    THEN("morale effect rises by expected amount") { CHECK(m.get_level() == 5); }
                }
                AND_WHEN("second effect increases in intensity") {
                    m.on_effect_int_change(hangover, 2);
                    THEN("morale effect drops by expected amount") { CHECK(m.get_level() == -25); }
                }
                AND_WHEN("both effect increase in intensity") {
                    m.on_effect_int_change(drunk, 2);
                    m.on_effect_int_change(hangover, 2);
                    THEN(
                        "the morale effect is the sum of the new highest bonus and new highest penalty") {
                        CHECK(m.get_level() == -15);
                    }
                }
            }
    ...
    GIVEN("effect with bonus, item with bonus and independent bonus") {
        m.on_effect_int_change(drunk, 1);
        m.add(MORALE_FOOD_GOOD, 10, 0, 1_hours, 1_hours, false, *itype_id("test_pine_nuts"));
        m.add(MORALE_BOOK, 10);
        THEN("square root of sum of squares rule applies") { CHECK(m.get_level() == 15); }
    }
}

...
TEST_CASE( "nc_apt_the_beam_turns_once_per_period_and_loops", "[newchar][aptitude]" )
{
    ...
    CHECK( nc_apt::beam_at( nc_apt::sweep_secs * 0.25F ) ==
           Catch::Approx( turn * 0.25F ).margin( 1e-4 ) );
...
TEST_CASE( "nc_apt_the_glow_is_continuous_across_the_wrap", "[newchar][aptitude]" )
{
    ...
    for( float theta = 0.3F; theta < turn - 0.3F; theta += 0.05F ) {
        CHECK( nc_apt::glow( below, theta ) ==
               Catch::Approx( nc_apt::glow( above, theta ) ).margin( 1e-2 ) );
    }
    ...
    // The wrap is HALF-OPEN, and that is not pedantry: subtracting two bearings a hair apart gives a
    // tiny negative, and adding a turn to a tiny negative rounds to exactly a turn in float. A
    // `behind` of one whole turn is "further back than any tail", so every dot the beam crosses would
    // go dark for the one frame its bearing sits a hair ahead of the head — a seam travelling round
    // the disc.
    ...
    for( int n = 1; n <= 9; n++ ) {
        ...
        for( int row = 0; row < nc_apt::grid; row++ ) {
            for( int col = 0; col < nc_apt::grid; col++ ) {
                ...
                int claims = 0;
                for( int j = 0; j < n; j++ ) {
                    if( nc_apt::in_sector( off, j, n, nc_apt::disc_radius() ) ) {
                        claims++;
                    }
                }
                CHECK( claims == 1 );
        ...
        for( int k = 0; k < n; k++ ) {
            CHECK( seen[static_cast<size_t>( k )] > 0 );
        }
    }
}
...
TEST_CASE( "nc_apt_sector_zero_starts_at_twelve_o_clock", "[newchar][aptitude]" )
{
    ...
    // finds at the top. A half-step offset would still look like a pie while mislabelling all of it.
...
{
    // Buying a skill level deepens a wedge. If membership were not monotone in reach, a dot could go
    // dark as the aptitude it reports goes UP.
    constexpr int n = 6;
        ...
        for( int row = 0; row < nc_apt::grid; row++ ) {
            for( int col = 0; col < nc_apt::grid; col++ ) {
                ...
                for( float reach = 0.0F; reach <= max_reach; reach += 0.25F ) {
                    ...
                    lit = lit || hit;
                    if( hit ) {
                        // Never beyond the reach: a wedge that overshoots reports an aptitude nobody
                        // bought.
...
TEST_CASE( "nc_apt_rings_sit_at_thirds_of_the_radius", "[newchar][aptitude]" )
{
    ...
    for( int k = 1; k <= nc_apt::rings; k++ ) {
        const float r = static_cast<float>( nc_apt::mid ) * static_cast<float>( k ) /

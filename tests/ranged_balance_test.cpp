...
static void equip_shooter(npc& shooter, const std::vector<std::string>& apparel) {
    ...
    shooter.worn.clear();
    shooter.inv_clear();
    for (const std::string& article : apparel) { shooter.wear_item(item::spawn(article)); }
}
...
static dispersion_sources get_dispersion(npc& shooter, const int aim_time) {
    ...
    shooter.aim();
    if (aim_time > 0) { REQUIRE(shooter.recoil < MAX_RECOIL); }
    dispersion.add_range(shooter.recoil);

    return dispersion;
}

static void test_shooting_scenario(
    npc& shooter, const int min_quickdraw_range, const int min_good_range,
    const int max_good_range) {
    {
        const dispersion_sources dispersion = get_dispersion(shooter, 0);
        std::vector<firing_statistics> minimum_stats = firing_test(
            dispersion, min_quickdraw_range,
            {Threshold(accuracy_grazing, 0.2), Threshold(accuracy_standard, 0.1)});
        INFO(dispersion);
        INFO("Range: " << min_quickdraw_range);
        INFO("Max aim speed: " << ranged::
                 aim_per_move(shooter, shooter.primary_weapon(), MAX_RECOIL));
        INFO("Min aim speed: " << ranged::
    ...
    }
    {
        ...
        firing_statistics good_stats =
            firing_test(dispersion, min_good_range, Threshold(accuracy_goodhit, 0.5));
        INFO(dispersion);
    ...
    }
    {
        ...
        firing_statistics good_stats =
            firing_test(dispersion, max_good_range, Threshold(accuracy_goodhit, 0.1));
        INFO(dispersion);
...
static void test_fast_shooting(npc& shooter, const int moves, float hit_rate) {
    ...
    firing_statistics fast_stats =
        firing_test(dispersion, fast_shooting_range, Threshold(accuracy_standard, hit_rate));
    firing_statistics fast_stats_upper =
        firing_test(dispersion, fast_shooting_range, Threshold(accuracy_standard, hit_rate_cap));
    INFO(dispersion);
...
static void assert_encumbrance(npc& shooter, int encumbrance) {
    for (const body_part bp : all_body_parts) {
        ...
        REQUIRE(shooter.encumb(convert_bp(bp))
                == std::max(encumbrance - shooter.get_skill_level(skill_id("swimming")), 0));
    }
}
...
static void range_test(const Threshold& test_threshold, bool write_data = false) {
    ...
    for (int r = 1; r <= 60; ++r) {
        ...
        // We carry forward prev_dispersion because we never expet the next tier of range to hit the
        // target accuracy level with a lower dispersion.
        for (int d = prev_dispersion; d >= 0; --d) {
            ...
            // The intent here is to skip over dispersion values proportionally to how far from
            // converging we are. As long as we check several adjacent dispersion values before a
            // hit, we're good.
    ...
    if (write_data) {
        const bool similar_to_previous_test_results = std::equal(
            data.begin(), data.end(), Creature::dispersion_for_even_chance_of_good_hit.begin(),
...
TEST_CASE("synthetic_range_test", "[.]") {
    clear_all_state();
    SECTION("quickdraw thresholds") { range_test(Threshold(accuracy_grazing, 0.1)); }
    SECTION("max range thresholds") { range_test(Threshold(accuracy_goodhit, 0.1)); }
    SECTION("good hit thresholds") { range_test(Threshold(accuracy_goodhit, 0.5), true); }
}

...
static void filesystem_test_group(
    int serial, const std::string& s1, const std::string& s2, const std::string& s3) {
    CAPTURE(serial);
    CAPTURE(s1);
    ...
    std::string base =
        g->get_active_world()->info->folder_path() + "/fs_test_" + get_pid_string() + "_"
    ...
    {
        std::vector<std::string> got = get_files_from_path(".json", base, true, true);
...
TEST_CASE("filesystem_ascii", "[filesystem]") {
    // English
    filesystem_test_group(
        1, conv_str(u8R"(DwarfFort)"), conv_str(u8R"(warf)"), conv_str(u8R"(fFor)"));
}

TEST_CASE("filesystem_utf8", "[filesystem]") {
    // Russian
    filesystem_test_group(
        2, conv_str(u8R"(МамаМылаРаму)"), conv_str(u8R"(амаМ)"), conv_str(u8R"(ылаР)"));
    // Chinese
    filesystem_test_group(3, conv_str(u8R"(你好)"), conv_str(u8R"(你)"), conv_str(u8R"(好)"));
}
...
TEST_CASE("filesystem_utf8_comp", "[filesystem][!mayfail]") {
    ...
    filesystem_test_group(4, conv_str(u8R"(नमस्ते)"), conv_str(u8R"(स्ते)"), conv_str(u8R"(मस्)"));
    // French (should not decompose into char + combining char)
    filesystem_test_group(
        5, conv_str(u8R"(crème brûlée)"), conv_str(u8R"(rèm)"), conv_str(u8R"(rûlé)"));
    // Spice it up a bit
    filesystem_test_group(
        6, conv_str(u8R"(Dw你ы)"), conv_str(u8R"(лаस्तेlé)"), conv_str(u8R"(मябस्or好)"));
}

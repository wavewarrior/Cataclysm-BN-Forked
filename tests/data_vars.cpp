...
TEST_CASE("load_store_numeric_arrays", "[data_vars]") {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        CHECK(std::ranges::equal(p, q, [](auto& lhs, auto& rhs) {
            return std::fabs(lhs - rhs) == 0;
        }));
    }
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        for (int j = 0; j < distrib2(gen); j++) { p.push_back(distrib(gen)); }
        dv.set(key, p);
        auto q = dv.get<decltype(p)>(key);
        CHECK(std::ranges::equal(p, q, [](auto& lhs, auto& rhs) {
            return std::fabs(lhs - rhs) == 0;
...
TEST_CASE("load_store_string_arrays", "[data_vars]") {
    ...
    {
        auto key = std::to_string(rand() % 10);
        auto p = gen_array<std::string>(
            [](size_t i) { return "item" + std::to_string(i); }, std::make_index_sequence<10>());
        dv.set(key, p);
        auto q = dv.get<decltype(p)>(key);
        CHECK(p == q);
    }

...
//
// This test covers is_night, is_dawn, is_day, is_dusk, and their behavior in relation to time of
// day.
...
TEST_CASE("sunlight and moonlight", "[sun][sunlight][moonlight][.]") {
    ...
    SECTION("moonlight") {
        ...
        WHEN("the moon is new") {
            ...
            THEN("moonlight is 1.0") { CHECK(1.0f == sunlight(new_moon)); }
        }

        WHEN("the moon is full") {
            REQUIRE(get_moon_phase(full_moon) == MOON_FULL);
            THEN("moonlight is 10.0") { CHECK(10.0f == sunlight(full_moon)); }
        }
    }
}

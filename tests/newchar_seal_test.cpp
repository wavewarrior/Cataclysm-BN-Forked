...
//
//   * the RING itself: it must be closed (no cell of it isolated from its neighbours) and every node
//     must sit ON it, because a node off the thread breaks the ring visibly at that node;
//   * the seven nodes must land in seven DISTINCT cells, or two steps share a glyph socket and one of
...
//   * the strike: `struck` is a flag rather than `arc == 2pi`, because nc_apt::wrap_turn is half-open
...
TEST_CASE( "nc_seal_the_ring_is_closed_and_one_dot_thick", "[newchar][seal]" )
{
    ...
    // Enough dots to read as a circle rather than as a polygon of blobs, and few enough that the band
    // has not silently widened into a disc.
    CHECK( ring.size() >= 24 );
    CHECK( ring.size() <= 48 );
...
TEST_CASE( "nc_seal_the_bead_runs_once_per_cycle_and_loops", "[newchar][seal]" )
{
    ...
    CHECK( nc_seal::at( nc_seal::weld_total * 0.5F ).arc ==
           Catch::Approx( turn * 0.5F ).margin( 1e-4 ) );
    ...
    CHECK( nc_seal::at( 6.0F * nc_seal::cycle_secs + 0.7F ).struck ==
           nc_seal::at( 0.7F ).struck );
}
...
TEST_CASE( "nc_seal_the_glow_trails_the_bead_and_leaves_a_floor", "[newchar][seal]" )
{
    ...
    // At the bead: full. Ahead of it: nothing at all — this pass has not reached that bearing, and a
    // symmetric falloff would make the direction of travel ambiguous.
    ...
    CHECK( nc_seal::glow_at( { .arc = 0.0F, .struck = false, .hub = 0.0F }, 0.0F ) ==
           Catch::Approx( 1.0F ) );
...
TEST_CASE( "nc_seal_the_hub_flares_at_the_strike_and_the_cycle_closes_on_itself",
           "[newchar][seal]" )
{
    ...
    // The strike is a STEP, and it is the only one in the cycle: the flare is the seal being struck,
    // so it is deliberately not eased.
...
TEST_CASE( "nc_seal_node_alpha_never_reaches_zero", "[newchar][seal]" )
{
    ...
    // thirds of every cycle reads as a rendering fault, so nodes keep a floor where thread does not.
    ...
    // Clamped rather than trusted, the same reason nc_apt::alpha_of is: an easing curve overshooting
    // 1 would otherwise wrap the byte and the brightest socket would render transparent.

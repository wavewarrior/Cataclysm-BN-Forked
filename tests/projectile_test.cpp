...
static tripoint_bub_ms projectile_end_point(
    ...
    for (const ammo_effect_str_id& ae_id : gun.ammo_effects()) { test_proj.add_effect(ae_id); }
...
TEST_CASE("projectiles_through_obstacles", "[projectile]") {
    ...
    std::vector<tripoint_bub_ms> range =
        {tripoint_bub_ms::zero(), tripoint_bub_ms::east(), tripoint_bub_ms(2, 0, 0)};
...
TEST_CASE("projectiles_stop_at_reality_bubble_edge", "[projectile][ballistics]") {
    ...
    auto& here = get_map();
    const auto shooter_pos = tripoint_bub_ms(2, 2, 0);
    const auto target_pos = tripoint_bub_ms(-1, 2, 0);
    const auto clear_points = std::vector<
        tripoint_bub_ms>{shooter_pos, tripoint_bub_ms(1, 2, 0), tripoint_bub_ms(0, 2, 0)};
...
TEST_CASE("adjacent_friendly_fire_prevention", "[projectile][ballistics]") {
    ...
    const auto shooter_pos = tripoint_bub_ms(50, 60, 0);
    const auto friendly_pos = tripoint_bub_ms(51, 60, 0);
    const auto target_pos = tripoint_bub_ms(55, 60, 0);

    // Clear the area
    for (int x = 50; x <= 55; ++x) {
        auto pt = tripoint_bub_ms(x, 60, 0);
        REQUIRE(here.inbounds(pt));
        here.ter_set(pt, ter_id("t_dirt"));
        here.furn_set(pt, furn_id("f_null"));
    }
...
TEST_CASE("npc_adjacent_friendly_fire_prevention", "[projectile][ballistics]") {
    ...
    // Set up test area: NPC shooter at (50,60), friendly NPC adjacent at (51,60), target at
    // (55,60). Positions near map center so spawned NPCs are within load_npcs radius (4 submaps).
    const auto shooter_pos = tripoint_bub_ms(50, 60, 0);
    const auto friendly_pos = tripoint_bub_ms(51, 60, 0);
    const auto target_pos = tripoint_bub_ms(55, 60, 0);
...
TEST_CASE("npc_protects_adjacent_player", "[projectile][ballistics]") {
    ...
    // Positions near map center so spawned NPCs are within load_npcs radius (4 submaps).
    const auto shooter_pos = tripoint_bub_ms(50, 60, 0);
    const auto player_pos = tripoint_bub_ms(51, 60, 0);
    const auto target_pos = tripoint_bub_ms(55, 60, 0);
...
TEST_CASE("monster_adjacent_ally_fire_prevention", "[projectile][ballistics]") {
    ...
    const auto shooter_pos = tripoint_bub_ms(50, 60, 0);
    const auto ally_pos = tripoint_bub_ms(51, 60, 0);
    const auto target_pos = tripoint_bub_ms(55, 60, 0);
...
TEST_CASE("hostile_npc_adjacent_ally_fire_prevention", "[projectile][ballistics]") {
    ...
    // Set up test area: hostile NPC shooter at (50,60), allied hostile NPC at (51,60), target at
    // (55,60). Positions near map center so spawned NPCs are within load_npcs radius (4 submaps).
    const auto shooter_pos = tripoint_bub_ms(50, 60, 0);
    const auto ally_pos = tripoint_bub_ms(51, 60, 0);
    const auto target_pos = tripoint_bub_ms(55, 60, 0);
...
TEST_CASE("friendly_monster_iff_respects_adjacent_player", "[projectile][monster][iff]") {
    ...
    const auto monster_pos = tripoint_bub_ms::zero();
    const auto player_pos = tripoint_bub_ms::east();
    const auto hostile_pos = tripoint_bub_ms(2, 0, 0);

    // Clear the area
    for (int x = 0; x <= 3; ++x) {
        auto pt = tripoint_bub_ms(x, 0, 0);
        REQUIRE(here.inbounds(pt));
        here.ter_set(pt, ter_id("t_dirt"));
        here.furn_set(pt, furn_id("f_null"));
    }

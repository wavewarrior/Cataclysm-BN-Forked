...
int get_remaining_charges(const std::string& tool_id) {
    ...
    for (const item* const& instance : items) { remaining_charges += instance->ammo_remaining(); }
...
void clear_avatar() { clear_character(g->u); }

void process_activity(player& dummy) {
    do {
        dummy.moves += dummy.get_speed();
        while (dummy.moves > 0 && dummy.activity) { dummy.activity->do_turn(dummy); }
...
void give_and_activate_bionic(player& p, const bionic_id& bioid) {
    ...
    if (bio.id->has_flag(STATIC(flag_id("BIONIC_TOGGLED"))) && !bio.powered) {
        ...
        if (!fuel_opts.empty()) { p.set_value(fuel_opts.front().str(), "2"); }
        p.activate_bionic(bio);
        INFO("bionic " + bio.id.str() + " is active ");
        REQUIRE(p.has_active_bionic(bioid));
        if (!fuel_opts.empty()) { p.remove_value(fuel_opts.front().str()); }
    }
}
...
void arm_character(
    ...
    for (const auto& mod : mods) { gun.put_in(item::spawn(itype_id(mod))); }
    shooter.wield(gun);
}

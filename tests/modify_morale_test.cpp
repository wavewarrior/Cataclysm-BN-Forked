...
TEST_CASE("food enjoyability", "[food][modify_morale][fun]") {
    ...
    GIVEN("food with positive fun") {
        detached_ptr<item> det =
            item::spawn("jihelucake", calendar::start_of_cataclysm, item::default_charges_tag{});
        item& jihelucake = *det;
    ...
    GIVEN("food with negative fun") {
        detached_ptr<item> det =
            item::spawn("garlic", calendar::start_of_cataclysm, item::default_charges_tag{});
        item& garlic = *det;
        dummy.i_add(std::move(det));
...
TEST_CASE("cannibalism", "[food][modify_morale][cannibal]") {
    ...
    detached_ptr<item> det =
        item::spawn("bone_human", calendar::start_of_cataclysm, item::default_charges_tag{});
    item& human = *det;
    dummy.i_add(std::move(det));
...
TEST_CASE("sweet junk food", "[food][modify_morale][junk][sweet]") {
    ...
    GIVEN("some sweet junk food") {
        detached_ptr<item> det =
            item::spawn("neccowafers", calendar::start_of_cataclysm, item::default_charges_tag{});
        item& necco = *det;
        dummy.i_add(std::move(det));
        ...
        WHEN("character has a sweet tooth") {
            ...
            THEN("they get a morale bonus from its sweetness") {
                ...
                AND_THEN("they enjoy it") { CHECK(dummy.get_morale(MORALE_FOOD_GOOD) > 0); }
            }
        }
...
TEST_CASE("junk food that is not ingested", "[modify_morale][junk][no_ingest]") {
    ...
    detached_ptr<item> det =
        item::spawn("caff_gum", calendar::start_of_cataclysm, item::default_charges_tag{});
    item& caff_gum = *det;
    dummy.i_add(std::move(det));
    ...
    GIVEN("character has a sweet tooth") {
        ...
        THEN("they do not get an extra morale bonus for chewing gum") {
            ...
            AND_THEN("they still enjoy it") { CHECK(dummy.get_morale(MORALE_FOOD_GOOD) > 0); }
        }
    }
...
TEST_CASE("food allergies and intolerances", "[food][modify_morale][allergy]") {
    ...
    GIVEN("character is vegetarian") {
        ...
        THEN("they get a morale penalty for eating meat") {
            detached_ptr<item> det =
                item::spawn("meat", calendar::start_of_cataclysm, item::default_charges_tag{});
            item& meat = *det;
            dummy.i_add(std::move(det));
    ...
    GIVEN("character is lactose intolerant") {
        ...
        THEN("they get a morale penalty for drinking milk") {
            detached_ptr<item> det =
                item::spawn("milk", calendar::start_of_cataclysm, item::default_charges_tag{});
            item& milk = *det;
            dummy.i_add(std::move(det));
    ...
    GIVEN("character is grain intolerant") {
        ...
        THEN("they get a morale penalty for eating wheat") {
            detached_ptr<item> det =
                item::spawn("wheat", calendar::start_of_cataclysm, item::default_charges_tag{});
            item& wheat = *det;
            dummy.i_add(std::move(det));
    ...
    GIVEN("character hates vegetables") {
        ...
        THEN("they get a morale penalty for eating vegetables") {
            detached_ptr<item> det =
                item::spawn("broccoli", calendar::start_of_cataclysm, item::default_charges_tag{});
            item& veggy = *det;
            dummy.i_add(std::move(det));
    ...
    GIVEN("character hates fruit") {
        ...
        THEN("they get a morale penalty for eating fruit") {
            detached_ptr<item> det =
                item::spawn("apple", calendar::start_of_cataclysm, item::default_charges_tag{});
            item& fruit = *det;
            dummy.i_add(std::move(det));
    ...
    GIVEN("character has a junk food intolerance") {
        ...
        THEN("they get a morale penalty for eating junk food") {
            detached_ptr<item> det = item::
                spawn("neccowafers", calendar::start_of_cataclysm, item::default_charges_tag{});
            item& junk = *det;
            dummy.i_add(std::move(det));
...
TEST_CASE("saprophage character", "[food][modify_morale][saprophage]") {
    ...
    GIVEN("character is a saprophage, preferring rotted food") {
        ...
        AND_GIVEN("some rotten chewable food") {
            detached_ptr<item> det = item::
                spawn("jihelucake", calendar::start_of_cataclysm, item::default_charges_tag{});
            item& jihelucake = *det;
            dummy.i_add(std::move(det));
...
TEST_CASE("ursine honey", "[food][modify_morale][ursine][honey]") {
    ...
    detached_ptr<item> det =
        item::spawn("honeycomb", calendar::start_of_cataclysm, item::default_charges_tag{});
    item& honeycomb = *det;
    dummy.i_add(std::move(det));

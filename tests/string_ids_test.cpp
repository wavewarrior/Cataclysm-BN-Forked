...
TEST_CASE("string_id_sorting_test", "[string_id]") {
    ...
    SECTION("map sorting") {
        std::map<id, int> m;
        for (int i = 0; i < 10; ++i) { m.insert({id("id" + std::to_string(i)), i}); }
    ...
    SECTION("vector of pairs sorting") {
        ...
        for (int i = 9; i >= 0; i--) { vec.push_back({id("id" + std::to_string(i)), i}); }
    ...
    SECTION("vector ids sorting") {
        std::vector<id> vec;
        for (int i = 0; i < 10; ++i) { vec.push_back(id("id" + std::to_string(i))); }
    ...
    SECTION("set of pairs sorting") {
        std::set<id> s;
        for (int i = 0; i < 10; ++i) { s.insert(id("id" + std::to_string(i))); }

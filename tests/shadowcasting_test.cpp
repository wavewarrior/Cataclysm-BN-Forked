...
static void oldCastLight(
    ...
    const float (&input_array)[MAPSIZE * SEEX][MAPSIZE * SEEY], const int xx, const int xy,
    ...
    for (int distance = row; distance <= radius && !blocked; distance++) {
        ...
        for (delta.x() = -distance; delta.x() <= 0; delta.x()++) {
            const point
                current(offsetX + delta.x() * xx + delta.y() * xy,
            ...
            if (rl_dist(tripoint_rel_ms::zero(), delta) <= radius) {
            ...
            if (blocked) {
                ...
                if (input_array[current.x][current.y] == LIGHT_TRANSPARENCY_SOLID
                    ...
                    blocked = true;
                    oldCastLight(output_cache, input_array, xx, xy, yx, yy, offsetX, offsetY,
                                 offsetDistance, distance + 1, start, leftSlope);
                    newStart = rightSlope;
                }
            }
        }
...
static bool bresenham_visibility_check(
    ...
    bool visible = true;
    const int junk = 0;
    bresenham(p.raw(), offset.raw(), junk, [&transparency_cache, &visible](const point& new_point) {
        if (transparency_cache[new_point.x][new_point.y] <= LIGHT_TRANSPARENCY_SOLID) {
            visible = false;
            return false;
        }
        return true;
...
bool grids_are_equivalent(
    ...
    for (int x = 0; x < MAPSIZE * SEEX; ++x) {
        for (int y = 0; y < MAPSIZE * SEEY; ++y) {
            ...
            if (is_nonzero(control[x][y]) != is_nonzero(experiment[x][y])) { return false; }
        }
    }
...
void print_grid_comparison(
    const point_bub_ms& offset, float (&transparency_cache)[MAPSIZE * SEEX][MAPSIZE * SEEY],
    float control[MAPSIZE * SEEX][MAPSIZE * SEEY], Exp experiment[MAPSIZE * SEEX][MAPSIZE * SEEY]) {
    for (int x = 0; x < MAPSIZE * SEEX; ++x) {
        for (int y = 0; y < MAPSIZE * SEEX; ++y) {
            ...
            if (transparency_cache[x][y] == LIGHT_TRANSPARENCY_SOLID) { output = '#'; }
            if (x == offset.x() && y == offset.y()) { output = '@'; }
...
static const light_model k_sight_model =
    {sight_calc,        sight_check,
     ...
     sight_from_lookup, accumulate_transparency};
...
static void shadowcasting_runoff(const int iterations, const bool test_bresenham = false) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        oldCastLight(seen_squares_control, transparency_cache, 0, 1, 1, 0, offset.x(), offset.y(),
                     0);
        oldCastLight(seen_squares_control, transparency_cache, 1, 0, 0, 1, offset.x(), offset.y(),
                     0);
...
static void shadowcasting_3d_2d(const int iterations) {
    ...
    static float seen_squares_experiment[MAPSIZE * SEEX][MAPSIZE * SEEY];
    static float transparency_cache[MAPSIZE * SEEX][MAPSIZE * SEEY];
    static char floor_cache[MAPSIZE * SEEX][MAPSIZE * SEEY]; // zero-initialized once; never written
                                                             // by algorithms.
    static diagonal_blocks blocked_cache[MAPSIZE * SEEX][MAPSIZE * SEEY];
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        castLightAll(
            ...
            MAPSIZE * SEEX, MAPSIZE * SEEY, offset.xy(), 0, VISIBILITY_FULL, k_sight_model);
    }
    const auto end1 = std::chrono::high_resolution_clock::now();
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        cast_zlight(seen_caches, transparency_caches, floor_caches, blocked_caches, origin, 0, 1.0f,
                    k_sight_model);
    }
    const auto end2 = std::chrono::high_resolution_clock::now();
    ...
    if (!passed) {
        print_grid_comparison(
            offset.xy(), transparency_cache, seen_squares_control, seen_squares_experiment);
    }
...
static void run_spot_check(const grid_overlay& test_case, const grid_overlay& expected_result) {
    ...
    for (int x = 0; x < MAPSIZE * SEEX; ++x) {
        for (int y = 0; y < MAPSIZE * SEEY; ++y) {
            transparency_cache[x][y] = test_case.get_global(tripoint_bub_ms(x, y, 0));
        }
    }

    castLightAll(&seen_squares[0][0], &transparency_cache[0][0], &blocked_cache[0][0],
                 MAPSIZE * SEEX, MAPSIZE * SEEY, ORIGIN.xy(), 0, VISIBILITY_FULL, k_sight_model);
    ...
    for (int y = 0; y < expected_result.height(); ++y) {
        for (int x = 0; x < expected_result.width(); ++x) {
            INFO("x:" << x << " y:" << y << " expected:" << expected_result.data[y][x] << " actual:"
                      << seen_squares
                             [expected_result.offset.x() + x][expected_result.offset.y() + y]);
            if (V == expected_result.get_local(tripoint_bub_ms(x, y, 0))) {
                CHECK(seen_squares[expected_result.offset.x() + x][expected_result.offset.y() + y]
                      > 0);
            } else {
                CHECK(seen_squares[expected_result.offset.x() + x][expected_result.offset.y() + y]
                      == 0);
...
TEST_CASE("shadowcasting_slope_inversion_regression_test", "[shadowcasting]") {
    ...
    test_case.data =
        {{T, T, T, T, T, T, T, T, T, T}, {T, O, T, T, T, T, T, T, T, T},
         {T, O, T, T, T, T, T, T, T, T}, {T, O, O, T, O, T, T, T, T, T},
         {T, T, T, T, T, T, T, T, T, T}, {T, T, T, T, T, T, T, T, T, T},
         {T, T, T, T, T, T, T, T, T, T}, {T, T, T, T, T, T, T, T, O, T},
         {T, T, T, T, T, T, O, T, O, T}, {T, T, T, T, T, T, O, O, O, T},
         {T, T, T, T, T, T, T, T, T, T}};

    grid_overlay expected_results({7, 8}, LIGHT_TRANSPARENCY_OPEN_AIR);
    expected_results.data =
        {{O, O, O, V, V, V, V, V, V, V}, {O, V, V, O, V, V, V, V, V, V},
         {O, O, V, V, V, V, V, V, V, V}, {O, O, V, V, V, V, V, V, V, V},
         {O, O, V, V, V, V, V, V, V, V}, {O, O, O, V, V, V, V, V, V, O},
         {O, O, O, O, V, V, V, V, V, O}, {O, O, O, O, O, V, V, V, V, O},
         {O, O, O, O, O, O, V, X, V, O}, {O, O, O, O, O, O, V, V, V, O},
         {O, O, O, O, O, O, O, O, O, O}};
...
TEST_CASE("shadowcasting_pillar_behavior_cardinally_adjacent", "[shadowcasting]") {
    ...
    test_case.data =
        {{T, T, T, T, T, T, T, T, T}, {T, T, T, T, T, T, T, T, T}, {T, T, T, T, T, T, T, T, T},
         {T, T, T, T, T, T, T, T, T}, {T, T, O, T, T, T, T, T, T}, {T, T, T, T, T, T, T, T, T},
         {T, T, T, T, T, T, T, T, T}, {T, T, T, T, T, T, T, T, T}, {T, T, T, T, T, T, T, T, T}};

    grid_overlay expected_results({1, 4}, LIGHT_TRANSPARENCY_OPEN_AIR);
    expected_results.data =
        {{V, V, V, V, V, V, V, O, O}, {V, V, V, V, V, V, O, O, O}, {V, V, V, V, V, O, O, O, O},
         {V, V, V, V, O, O, O, O, O}, {V, X, V, O, O, O, O, O, O}, {V, V, V, V, O, O, O, O, O},
         {V, V, V, V, V, O, O, O, O}, {V, V, V, V, V, V, O, O, O}, {V, V, V, V, V, V, V, O, O}};

...
static void populate_map_with_content(map& here) {
    ...
    for (int x = 10; x < 160; x += 8) {
        for (int y = 10; y < 160; y += 8) {
            here.add_item_or_charges(tripoint_bub_ms(x, y, 0), item::spawn("flashlight_on"));
            here.add_item_or_charges(tripoint_bub_ms(x + 1, y, 0), item::spawn("rock"));
        }
    }
...
static auto run_shift_benchmark(int iterations) -> std::pair<double, double> {
    ...
    for (int i = 0; i < iterations; ++i) {
        const int edge_x = (i % 2 == 0) ? g_half_mapsize_x + SEEX - 1 : g_half_mapsize_x;

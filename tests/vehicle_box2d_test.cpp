...
static auto make_box_body(
    b2WorldId world, float px, float py, float vx, float vy, float hw, float hh, float mass_kg,
...
TEST_CASE("Box2D VV: momentum conserved in head-on equal-mass collision", "[vehicle][box2d]") {
...
TEST_CASE("Box2D VV: off-centre hit produces angular velocity on target", "[vehicle][box2d]") {
    const auto mass = 1000.0f;
...
TEST_CASE("Box2D Phase5 VT: glancing terrain hit produces angular velocity", "[vehicle][box2d]") {
...
TEST_CASE(
    "Box2D Phase10: kinematic body integrates from velocity then reads back", "[vehicle][box2d]") {
    const auto world = make_test_world();
...
TEST_CASE(
    "Box2D Phase10 full: dynamic body generates begin-touch event against static tile",
    "[vehicle][box2d]") {
...
TEST_CASE("Box2D Phase7: mount rotation formula gives correct offsets", "[vehicle][box2d]") {
    // Helper: apply the same formula as refresh_precalc / part_world_offset.
    const auto rotate = [](float mx, float my, float angle_rads) -> std::pair<int, int> {
        const float c = std::cos(angle_rads);
        const float s = std::sin(angle_rads);
        return {static_cast<int>(std::round(mx * c - my * s)),
...
TEST_CASE("Box2D Phase10 Step1-3: VT hit events fire, VV contacts silent", "[vehicle][box2d]") {
    ...
    // ── Part A: vehicle approaches terrain — VT hit event must fire ───────────
    {
        ...
        // Static terrain body to the east.
        {
            ...
            sdef.enableContactEvents = true; // explicit — beginCount is the filter-validation
                                             // signal
...
TEST_CASE(
    "Box2D Phase10 Step5: dynamic body at TILE_M m/s gives physics_pos tile delta 1",
    "[vehicle][box2d]") {
    const auto world = make_test_world();
    ...
    for (int i = 0; i < 60; ++i) { b2World_Step(world, 1.0f / 60.0f, 4); }
...
TEST_CASE(
    "Box2D Phase10 Step5: sub-tile physics_pos rounds to zero, >=0.5 to one", "[vehicle][box2d]") {

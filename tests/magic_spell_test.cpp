...
TEST_CASE("spell level", "[magic][spell][level]") {
    ...
    GIVEN("spell level 0") {
        ...
        WHEN("spell levels up once") {
            ...
            THEN("it is level 1") { CHECK(pew_spell.get_level() == 1); }

            AND_WHEN("spell levels up to max_level") {
                pew_spell.set_level(pew_type.max_level);

                THEN("it is maximum level") { CHECK(pew_spell.get_level() == pew_type.max_level); }
...
TEST_CASE("known magic remembers the last cast spell", "[magic][spell][save]") {
    ...
    deserialize_wrapper([&](JsonIn& jsin) { loaded_magic.deserialize(jsin); }, serialized_magic);
    ...
    GIVEN("spell is level 0") {
        ...
        THEN("exp_to_next_level returns experience needed to reach level 1") {
            ...
            WHEN("experience is 10 points away from level 1") {
                ...
                THEN("exp_to_next_level is 10") { CHECK(lava_spell.exp_to_next_level() == 10); }
            }
            ...
            WHEN("experience is just enough to reach level 1") {
                ...
                THEN("spell is level 1") { CHECK(lava_spell.get_level() == 1); }

...
struct coop_ctrl_server {
    ...
    auto try_recv_line(std::string& out) -> bool {
        ...
        const int n = static_cast<int>(
            ::recv(conn_fd, tmp, static_cast<int>(sizeof(tmp)), MSG_DONTWAIT));
...
struct coop_ctrl_client {
    ...
    auto try_recv_line(std::string& out) -> bool {
        ...
        char tmp[1024];
        const int n = static_cast<int>(
            ::recv(fd, tmp, static_cast<int>(sizeof(tmp)), MSG_DONTWAIT));
        if (n > 0) { recv_buf.append(tmp, static_cast<std::size_t>(n)); }
        const auto nl = recv_buf.find('\n');
        if (nl == std::string::npos) { return false; }
        out = recv_buf.substr(0, nl);
...
static auto run_host_movement(coop_server& srv, coop_ctrl_server& ctrl) -> void {
    ...
    if (proxy_reached_target) {
        ...
        oss << "PROXY_POS " << proxy_final.x() << " " << proxy_final.y() << " " << proxy_final.z();
...
static auto run_client_pickup(coop_client& cli, coop_ctrl_client& ctrl) -> void {
    ...
    manifest << "{\"items\":[{\"tx\":" << item_abs.x() << ",\"ty\":" << item_abs.y() << ",\"tz\":"
             << item_abs.z() << ",\"type\":\"knife_combat\",\"charges\":0,\"qty\":1}]}";
...
static auto run_host_disconnect(coop_server& srv, coop_ctrl_server& ctrl) -> void {
    ...
    const auto drop_deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(30'000);
...
static auto run_host_submap_shift(coop_server& srv, coop_ctrl_server& ctrl) -> void {
    ...
    const auto move_deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(30'000);
    bool proxy_reached_target = false;
    while (std::chrono::steady_clock::now() < move_deadline) {
        srv.coop_world_tick();
        SDL_Delay(50);
...
static auto run_client_submap_shift(coop_client& cli, coop_ctrl_client& ctrl) -> void {
    ...
    const tripoint_abs_ms actual = g->u.abs_pos();
    const int drift =
        std::abs(actual.x() - expected_proxy_pos.x())
...
static auto run_host_death(coop_server& srv, coop_ctrl_server& ctrl) -> void {
    ...
    const auto dead_deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(10'000);
...
static auto run_host_melee_family(
    coop_server& srv, coop_ctrl_server& ctrl, const std::string& done_signal) -> void {
    ...
    ctrl.send_signal(
        "MONSTER_POS " + std::to_string(target_abs.x()) + " " + std::to_string(target_abs.y()) + " "
    ...
    bool done = false;
    int post_ticks = 0;
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(20'000);
    while (std::chrono::steady_clock::now() < deadline) {
        srv.coop_world_tick();
...
static auto run_client_melee_family(
    coop_client& cli, coop_ctrl_client& ctrl, const std::string& action_key,
    ...
    for (int i = 0; i < COOP_HIT_ACTIONS + 3; ++i) {
        cli.coop_world_tick();
        SDL_Delay(50);
    }
    ctrl.send_signal(done_signal);

    const auto done_deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(10'000);
...
static auto run_host_ranged(coop_server& srv, coop_ctrl_server& ctrl) -> void {
    ...
    ctrl.send_signal(
        "MONSTER_POS " + std::to_string(zombie_abs.x()) + " " + std::to_string(zombie_abs.y()) + " "
    ...
    int post_ticks = 0;
    const auto fire_deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(30'000);
    while (std::chrono::steady_clock::now() < fire_deadline) {
        srv.coop_world_tick();
        min_hp = std::min(min_hp, zomb.get_hp()); // capture trough before regen
        SDL_Delay(50);
        std::string sig;
        if (!fire_done && ctrl.try_recv_line(sig) && sig == "FIRE_DONE") { fire_done = true; }
...
static auto run_host_terrain_change(coop_server& srv, coop_ctrl_server& ctrl) -> void {
    ...
    ctrl.send_signal(
        "TILE_POS " + std::to_string(tile_abs.x()) + " " + std::to_string(tile_abs.y()) + " "
...
static auto run_host_reconnect(coop_server& srv, coop_ctrl_server& ctrl) -> void {
    ...
    const auto drop_deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(15'000);
...
TEST_CASE("coop integration: host role", "[.][coop_role_host]") {
    ...
    if (scenario == "pickup") {
        g->m.add_item(
            PICKUP_TILE, item::spawn(PICKUP_KNIFE_ID, calendar::turn, item::solitary_tag{}));

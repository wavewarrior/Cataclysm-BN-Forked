...
TEST_CASE("stats_tracker_with_event_statistics", "[stats]") {
    ...
    SECTION("movement") {
        ...
        const cata::event walk = cata::event::make<
            event_type::avatar_moves>(no_monster, t_null, character_movemode::CMM_WALK, false, 0);
        const cata::event ride = cata::event::make<
            event_type::avatar_moves>(horse, t_null, character_movemode::CMM_WALK, false, 0);
        const cata::event run = cata::event::make<
            event_type::avatar_moves>(no_monster, t_null, character_movemode::CMM_RUN, false, 0);
        const cata::event crouch = cata::event::make<
            event_type::avatar_moves>(no_monster, t_null, character_movemode::CMM_CROUCH, false, 0);
        const cata::event swim = cata::event::make<event_type::avatar_moves>(
            no_monster, t_water_dp, character_movemode::CMM_WALK, false, 0);
        const cata::event swim_underwater = cata::event::make<event_type::avatar_moves>(
            no_monster, t_water_dp, character_movemode::CMM_WALK, true, 0);
...
struct watch_stat: stat_watcher {
    void new_value(const cata_variant& v, stats_tracker&) override { value = v; }
    cata_variant value;
};
...
TEST_CASE("achievements_tracker", "[stats]") {
    ...
    SECTION("time") {
        ...
        const cata::event avatar_wakes_up = cata::event::make<event_type::character_wakes_up>(u_id);
    ...
    SECTION("hidden_kills") {
        ...
        for (int i = 0; i < 10; ++i) { b.send(avatar_zombie_kill); }
        CHECK(!a.is_hidden(&*a_kill_10));
        CHECK(!a.is_hidden(&*a_kill_100));
    }
    ...
    SECTION("kills") {
        ...
        if (time_since_game_start < 1_minutes) {
            CHECK(
                a.ui_text_for(&*a_kill_in_first_minute)
    ...
    SECTION("movement") {
        ...
        const cata::event run = cata::event::make<
            event_type::avatar_moves>(no_monster, t_null, character_movemode::CMM_RUN, false, 0);
        const cata::event sharp_move = cata::event::make<event_type::avatar_moves>(
            no_monster, t_shrub_raspberry, character_movemode::CMM_WALK, false, 0);
        const cata::event swim = cata::event::make<event_type::avatar_moves>(
            no_monster, t_water_dp, character_movemode::CMM_WALK, false, 0);
        const cata::event swim_underwater = cata::event::make<event_type::avatar_moves>(
            no_monster, t_water_dp, character_movemode::CMM_WALK, true, 0);
        const cata::event swim_underwater_deep = cata::event::make<event_type::avatar_moves>(
            no_monster, t_water_dp, character_movemode::CMM_WALK, true, -5);
        const cata::event walk_max_z = cata::event::make<event_type::avatar_moves>(
            no_monster, t_null, character_movemode::CMM_WALK, false, OVERMAP_HEIGHT);
        const cata::event walk_min_z = cata::event::make<event_type::avatar_moves>(
            no_monster, t_null, character_movemode::CMM_WALK, false, -OVERMAP_DEPTH);
        ...
        SECTION("achievement_marathon") {
            ...
            GIVEN("a new game") {
                ...
                WHEN("the avatar runs the required distance") {
                    for (int i = 0; i < 42196; i++) { b.send(run); }
        ...
        SECTION("achievement_traverse_sharp_terrain") {
            ...
            GIVEN("a new game") {
                ...
                WHEN("the avatar traverses the required number of sharp terrain") {
                    for (int i = 0; i < 100; i++) { b.send(sharp_move); }
                    THEN("the achivement should be achieved") {
        ...
        SECTION("achievement_swim_merit_badge") {
            ...
            GIVEN("a new game") {
                ...
                WHEN("the avatar does the required actions") {
                    for (int i = 0; i < 10000; i++) { b.send(swim); }
                    for (int i = 0; i < 1000; i++) { b.send(swim_underwater); }

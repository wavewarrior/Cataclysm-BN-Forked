...
TEST_CASE( "coop_admit_action enforces the depth cap keeping newest", "[coop][inputwindow]" )
{
    ...
    for( int i = 0; i < 10; ++i ) {
        coop_admit_action( q, act( std::to_string( i ).c_str(), i ) );
    }
...
TEST_CASE( "coop_expire_stale_actions drops old input but keeps the newest",
           "[coop][inputwindow]" )
{
...
TEST_CASE( "coop_expire_stale_actions never empties a single-entry queue",
           "[coop][inputwindow]" )
{

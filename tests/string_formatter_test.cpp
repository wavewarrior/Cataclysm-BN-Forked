...
template <typename... Args>
void importet_test(
    const int serial, const char* const expected, const char* const format, Args&&... args) {
    CAPTURE(serial);
    CAPTURE(format);
...
template <typename... Args> void test_for_error(const char* const format, Args&&... args) {
    CAPTURE(format);
...
template <typename... Args>
void test_new_old_pattern(
    const char* const old_pattern, const char* const new_pattern, Args&&... args) {
    CAPTURE(old_pattern);
    CAPTURE(new_pattern);
...
TEST_CASE("string_formatter") {
    ...
    // importet_test( 178, "2345", "%hx", 74565 ); // 74565 is not a valid short int, as required by
    // %hx

...
TEST_CASE("full backpack drop", "[activity][drop_token]") {
    ...
    GIVEN("a character with a backpack full of items and no other containers") {
        ...
        WHEN("he considers dropping the backpack") {
            ...
            THEN("he will try to drop all carried items") {
                // TODO: Check that all items will be dropped. inv.size() doesn't work because
                // stacks
                AND_THEN(
                    "all of them will have identical drop tokens, marking the backpack as parent") {
                ...
                AND_THEN(
                    "the backpack will have non-zero drop cost, while all contents will have zero drop cost") {
                ...
                AND_THEN(
                    "total volume dropped will equal volume of backpack and all items in inventory") {
                    units::volume total_dropped = std::accumulate(
                        drop_list.begin(), drop_list.end(), 0_ml,
                        [](units::volume acc, const pickup::act_item& ait) {
                            return acc + ait.loc->volume();
                        });
                    units::volume inventory_volume = dummy.volume_carried();
                    units::volume worn_volume = std::accumulate(
                        dummy.worn.begin(), dummy.worn.end(), 0_ml,
    ...
    GIVEN(
        "a character with two duffel bags and two backpacks full of items and no other containers") {
        ...
        WHEN("he considers dropping one duffel bag and one backpack") {
            drop_locations drop;
            auto first_duffel_iter =
                std::find_if(dummy.worn.begin(), dummy.worn.end(), [&](const item* const& it) {
                    return it->typeId() == duffel_bag.typeId();
                });
            auto first_backpack_iter =
                std::find_if(dummy.worn.begin(), dummy.worn.end(), [&](const item* const& it) {
            ...
            THEN("he will try to drop some, but not all of the carried items") {
                ...
                units::volume inventory_volume = dummy.volume_carried();
                units::volume worn_dropped_volume =
                    (*first_duffel_iter)->volume() + (*first_backpack_iter)->volume();
                CHECK(total_dropped - worn_dropped_volume < inventory_volume);

                AND_THEN(
                    "dropped duffel bag and backpack will have non-zero drop cost, while all contents will have zero drop cost") {
                    auto drop_duffel_iter = std::find_if(
                        drop_list.begin(), drop_list.end(), [&](const pickup::act_item& ait) {
                            return ait.loc->typeId() == duffel_bag.typeId();
                        });
                    auto drop_backpack_iter = std::find_if(
                        drop_list.begin(), drop_list.end(), [&](const pickup::act_item& ait) {
                    ...
                    AND_THEN(
                        "both containers will be followed by enough same-drop-token items to fill them") {
                        ...
                        for (auto iter = std::next(drop_duffel_iter); iter != drop_list.end();
                             iter++) {
                            if (iter->consumed_moves == 0
                                && iter->loc->typeId() == an_item.typeId()) {
                        ...
                        for (auto iter = std::next(drop_backpack_iter); iter != drop_list.end();
                             iter++) {
                            if (iter->consumed_moves == 0
                                && iter->loc->typeId() == an_item.typeId()) {
                        ...
                        // +1 because we aren't checking tokens, but the last item is still zero
                        // cost
                        ...
                        AND_THEN(
                            "when actually dropping items, each container will be followed by enough items of same token to fill it, "
                            "followed by one un-contained item") {
                            dummy.set_moves(1000);
                            std::vector<detached_ptr<item>> tokenized_dropped =
                                obtain_and_tokenize_items(dummy, drop_list);
                            auto tokenized_duffel_iter = std::find_if(
                                tokenized_dropped.begin(), tokenized_dropped.end(),
                                [&](const detached_ptr<item>& it) {
                                    return it->typeId() == duffel_bag.typeId();
                                });
                            auto tokenized_backpack_iter = std::find_if(
                                tokenized_dropped.begin(), tokenized_dropped.end(),
                                [&](const detached_ptr<item>& it) {
                                    return it->typeId() == backpack.typeId();
                                });
                            ...
                            REQUIRE(*(*tokenized_duffel_iter)->drop_token
                                    != *(*tokenized_backpack_iter)->drop_token);
                            int actual_duffel_tokens = std::count_if(
                                tokenized_dropped.begin(), tokenized_dropped.end(),
                                [&](const detached_ptr<item>& it) {
                                    return it->drop_token->is_child_of(
                                        *(*tokenized_duffel_iter)->drop_token);
                                });
                            int actual_backpack_tokens = std::count_if(
                                tokenized_dropped.begin(), tokenized_dropped.end(),
                                [&](const detached_ptr<item>& it) {
                                    return it->drop_token->is_child_of(
                                        *(*tokenized_backpack_iter)->drop_token);
    ...
    GIVEN("a character with two duffel bags full of items") {
        ...
        WHEN("he considers dropping only one of the bags, but all of the items") {
            ...
            dummy.dump_inv(dump);
            for (item* it : dump) { drop.push_back(drop_location(*it, 1)); }
            std::list<pickup::act_item> drop_list = pickup::reorder_for_dropping(dummy, drop);
            THEN("at most half of the non-bag items will have zero drop cost") {
                const size_t actual_zero_cost = std::
                    count_if(drop_list.begin(), drop_list.end(), [&](const pickup::act_item& ait) {
    ...
    GIVEN("a backpack full of items lying on a ground tile") {
        ...
        *parent.drop_token = token_provider.make_next(calendar::turn);
        for (units::volume remaining_storage = backpack.get_storage();
             remaining_storage > an_item.volume(); remaining_storage -= an_item.volume()) {
        ...
        WHEN("a character with enough volume and weight capacity tries to pick it up") {
            units::mass weight_sum = std::accumulate(
                stack.begin(), stack.end(), 0_gram,
                [](units::mass acc, const item* const& it) { return acc + it->weight(); });
            units::volume volume_sum = std::accumulate(
                stack.begin(), stack.end(), 0_ml,
                [](units::volume acc, const item* const& it) { return acc + it->volume(); });
            ...
            std::vector<pickup::pick_drop_selection> targets =
                pickup::optimize_pickup(target_locations, quantities);
            ...
            THEN("it succeeds and costs only as much moves as picking the backpack itself would") {
                ...
                bool no_items_left = mc.items_with([](const item&) { return true; }).empty();
                CHECK(no_items_left);
...
TEST_CASE("drop overflow prefers non-favorites", "[activity][drop_token][favorite]") {
    ...
    for (auto i = 0; i < non_favorite_count; i++) { dummy.i_add(item::spawn(non_favorite_item)); }
    ...
    const auto dropped_favorites =
        std::count_if(drop_list.begin(), drop_list.end(), [&](const pickup::act_item& ait) {
            return !dummy.is_worn(*ait.loc) && ait.loc->is_favorite;
        });
    const auto dropped_non_favorites =
        std::count_if(drop_list.begin(), drop_list.end(), [&](const pickup::act_item& ait) {
            return !dummy.is_worn(*ait.loc) && !ait.loc->is_favorite;
        });
...
TEST_CASE(
    "backpack drop keeps favorites when non-favorites are sufficient",
...
TEST_CASE("pickup_ui_stacking", "[activity][drop_token]") {
    ...
    GIVEN("A mix of items on the ground, with some of them being in a backpack") {
        ...
        *(*insert_parent_iter)->drop_token = parent_token;
        ...
        auto post_scramble_parent_iter = std::find_if(
            unstacked.begin(), unstacked.end(), [&parent_token](const item_stack::iterator& it) {
        ...
        WHEN("The items are restacked in preparation for pickup display") {
            std::vector<pickup::stacked_items> stacked_nonflat = pickup::stack_for_pickup_ui(
                unstacked);
            ...
            THEN("Backpack with children doesn't stack with empty ones") {
                ...
                auto stacked_parent_iter =
                    std::find(stacked.begin(), stacked.end(), list_with_just_parent);
        ...
        *(*second_backpack_iter)->drop_token = token_provider.make_next(calendar::turn);
        ...
        WHEN("The items are restacked in preparation for pickup display") {
            std::vector<pickup::stacked_items> stacked_nonflat = pickup::stack_for_pickup_ui(
                unstacked);
            std::vector<std::list<item_stack::iterator>> stacked = pickup::flatten(stacked_nonflat);
            THEN("The two backpacks stack") { CHECK(stacked.size() == 1); }
        }
    }

    GIVEN(
        "Multiple identical items with tokens with different turns and parents set, but with no parents on the item stack") {
        testing_stack the_stack;
        ...
        WHEN("The items are restacked in preparation for pickup display") {
            std::vector<pickup::stacked_items> stacked_nonflat = pickup::stack_for_pickup_ui(
                unstacked);
            std::vector<std::list<item_stack::iterator>> stacked = pickup::flatten(stacked_nonflat);
            THEN("All of those items stack") { CHECK(stacked.size() == 1); }
        }
    }
}

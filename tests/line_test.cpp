...
static bool check_bresenham_far(const tripoint& source, const tripoint& destination, int length) {
    ...
    std::stringstream ss;
    ss << '\n';
    for (const tripoint& t : generated_path) { ss << t << '\n'; }
...
static void check_bresenham_cube(const tripoint& src, int dist) {
    ...
    for (int x = -1; x <= 1; x += 2) {
        for (int y = -1; y <= 1; y += 2) {
            for (int z = -1; z <= 1; z += 2) {
                tripoint sign(x, y, z);
                if (sign == sign_positive) { continue; }
    ...
    for (int x = -2; x <= 2; ++x) {
        for (int y = -2; y <= 2; ++y) {
            for (int z = -2; z <= 2; ++z) {
                ...
                CHECK(direction_from(tripoint_zero, p) == direction_from(tripoint_abs_omt(), c));
...
TEST_CASE("direction_name", "[line]") {
    CHECK(direction_name(direction_from(tripoint_north_east + tripoint_above))
          == "northeast and above");
    CHECK(direction_name_short(direction_from(tripoint_north_east + tripoint_above)) == "UP_NE");
}
...
static void line_to_comparison(const int iterations) {
    ...
    for (int i = 0; i < RANDOM_TEST_NUM; ++i) {
        const point
            p1(rng(-COORDINATE_RANGE, COORDINATE_RANGE), rng(-COORDINATE_RANGE, COORDINATE_RANGE));
        const point
            p2(rng(-COORDINATE_RANGE, COORDINATE_RANGE), rng(-COORDINATE_RANGE, COORDINATE_RANGE));
        int t1 = 0;
        int t2 = 0;
        REQUIRE(line_to(p1, p2, t1) == canonical_line_to(p1, p2, t2));
    }

    {
        const point
            p12(rng(-COORDINATE_RANGE, COORDINATE_RANGE), rng(-COORDINATE_RANGE, COORDINATE_RANGE));
        const point
            p22(rng(-COORDINATE_RANGE, COORDINATE_RANGE), rng(-COORDINATE_RANGE, COORDINATE_RANGE));
        const int t1 = 0;
        const int t2 = 0;
        int count1 = 0;
        ...
        for (int j = -60; j < 60; ++j) {
            ...
            for (int k = -1; k <= max_start_offset; ++k) {
                ...
                if (line.back() != point(i, j)) {
                    WARN("Expected ("
                         << i << "," << j << ") but got (" << line.back().x << "," << line.back().y
                         << ") with t == " << k);
                }
...
TEST_CASE("line_to_regression", "[line]") { line_to_comparison(1); }

TEST_CASE("line_to_performance", "[.]") { line_to_comparison(10000); }

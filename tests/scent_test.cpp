...
void old_scent_map_update(
    ...
    std::array<std::array<bool, MAPSIZE_Y>, MAPSIZE_X> blocks_scent; // currently only
                                                                     // TFLAG_NO_SCENT blocks scent
    ...
    for (int x = scentmap_minx; x <= scentmap_maxx; ++x) {
        for (int y = scentmap_miny; y <= scentmap_maxy; ++y) {
            ...
            if (!blocks_scent[x][y]) {
                ...
                const int squares_used =
                    squares_used_y[y][x - 1] + squares_used_y[y][x] + squares_used_y[y][x + 1];

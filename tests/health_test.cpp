...
static void test_diet(
    const time_duration& dur, npc& dude, const std::array<int, 4>& hmod_changes_per_day, int min,
    int max) {
    ...
    for (time_duration i = 0_turns; i < dur; i += 1_days) {
        ...
        for (size_t index = 0u; index < 4u; ++index) {
            ...
            dude.mod_healthy_mod(
                hmod_changes_per_day[index], sgn(hmod_changes_per_day[index]) * 200);
            // And update health every 30m over the 6h period
            for (time_duration n = 0_turns; n < 6_hours; n += 30_minutes) { dude.update_health(); }
            health_samples.emplace_back(dude.get_healthy());
        }
    }

    std::stringstream ss;
    for (int i : health_samples) { ss << i << ", "; }

...
static void write_dummy_file(const std::string& path) {
    const auto writer = [](std::ostream& out) { out << "test"; };
...
static void remove_graveyard_subdir(const std::string& graveyard_dir, const std::string& dest_dir) {
...
TEST_CASE("move_save_to_graveyard_with_open_player_db", "[graveyard]") {
    ...
    w->write_player_mm_omt(tripoint_abs_mmr::zero(), [](std::ostream& out) { out << "{}"; });

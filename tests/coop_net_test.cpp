...
TEST_CASE("coop_net poll: returns false when no data, true when data present", "[coop][net]") {
    LoopbackFixture fix;
...
TEST_CASE(
    "coop_net recv: unexpected packet type received successfully (transport is agnostic)",
    "[coop][net]") {
    LoopbackFixture fix;
    if (!setup_loopback(fix, k_adv_port)) {
        WARN("loopback unavailable — skipping");
...
TEST_CASE("coop_net recv: duplicate seq packet received twice without corruption", "[coop][net]") {
    LoopbackFixture fix;
    if (!setup_loopback(fix, k_adv_port)) {
        WARN("loopback unavailable — skipping");
        return;

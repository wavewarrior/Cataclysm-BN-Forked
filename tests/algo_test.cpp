...
static void check_cycle_finding(
    std::unordered_map<int, std::vector<int>>& g, std::vector<std::vector<int>>& expected) {
    CAPTURE(g);
...
TEST_CASE("flat_map", "[ranges]") {
    ...
    SECTION("general case") {
        ...
        auto output = input | flat_map(fn) | std::ranges::to<std::vector>();
...
TEST_CASE("complex pipeline", "[ranges]") {
    ...
    auto input = std::vector{1, 2, 3};
    auto output =
        input | std::views::filter(is_even)
        | flat_map([](const int i) -> auto { return std::vector{i, i * 2}; })
        | std::views::transform(dup) | max();

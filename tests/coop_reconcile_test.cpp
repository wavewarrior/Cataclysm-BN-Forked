...
TEST_CASE("coop_reconcile_pos direction: MOVE_NE", "[coop][reconcile]") {
    CHECK(coop_reconcile_pos(bub(0, 0), 0, one(1, "MOVE_NE")) == bub(0, 0) + tripoint_north_east);
}

TEST_CASE("coop_reconcile_pos direction: MOVE_NW", "[coop][reconcile]") {
    CHECK(coop_reconcile_pos(bub(0, 0), 0, one(1, "MOVE_NW")) == bub(0, 0) + tripoint_north_west);
}

TEST_CASE("coop_reconcile_pos direction: MOVE_SE", "[coop][reconcile]") {
    CHECK(coop_reconcile_pos(bub(0, 0), 0, one(1, "MOVE_SE")) == bub(0, 0) + tripoint_south_east);
}

TEST_CASE("coop_reconcile_pos direction: MOVE_SW", "[coop][reconcile]") {
    CHECK(coop_reconcile_pos(bub(0, 0), 0, one(1, "MOVE_SW")) == bub(0, 0) + tripoint_south_west);
}
...
TEST_CASE(
    "coop_reconcile_pos operator+: N+E+S+W round-trip returns to origin",
    "[coop]["
    "reconcile]") {
...
TEST_CASE("coop_reconcile_pos: no-op when prediction is correct", "[coop][reconcile]") {
    ...
    CHECK(coop_reconcile_pos(server, 3, pending) == bub(10, 10) + tripoint_north + tripoint_north);
}
...
TEST_CASE(
    "coop_reconcile_pos: snap correction with pending actions after the blocked one",
    "[coop][reconcile]") {
    ...
    CHECK(coop_reconcile_pos(server, 3, pending) == bub(10, 10) + tripoint_north + tripoint_east);
}

// Gap 7b: multiple pending actions all replay from corrected server position.
TEST_CASE(
    "coop_reconcile_pos: snap + three-action replay from corrected position", "[coop][reconcile]") {
    const auto server = bub(5, 5);
    const std::vector<reconcile_action> pending = {{10, "MOVE_N"}, {11, "MOVE_N"}, {12, "MOVE_N"}};
...
TEST_CASE("coop_reconcile_pos: mixed movement and outcome actions", "[coop][reconcile]") {
    ...
    const std::vector<reconcile_action> pending =
        {reconcile_action{1, "MOVE_N"}, smash2, reconcile_action{3, "MOVE_E"}};
    // Only MOVE_N and MOVE_E affect position; SMASH with outcome is a no-op
    CHECK(coop_reconcile_pos(server, 0, pending) == bub(0, 0) + tripoint_north + tripoint_east);
}

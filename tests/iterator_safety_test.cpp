...
auto make_temp_rect_range(const int width, const int height, const point count)
    -> rect_range<test_rect> {
...
TEST_CASE("vehicle_part_range_iterator_safety", "[iterator][safety]") {
    ...
    // Part indices are not guaranteed to be 0, 1, 2 sequentially depending on installation
    // internals, but iteration count should be 3.
    int count = 0;
...
TEST_CASE("vehicle_part_with_feature_range_iterator_safety", "[iterator][safety]") {
    ...
    // Even though the temporary range is destroyed, the iterator carries its own copy of the
    // filter.
...
TEST_CASE("vehicle_part_range_for_temporary_safety", "[iterator][safety]") {
    ...
    for (const auto& part : v.get_all_parts()) { indices.push_back(part.part_index()); }

    REQUIRE(indices.size() == 3);
}

TEST_CASE("vehicle_part_filtered_range_for_temporary_safety", "[iterator][safety]") {
    vehicle v;
...
TEST_CASE("vehicle_part_range_std_ranges_algorithms", "[iterator][safety][ranges]") {
    ...
    std::vector<int> indices;
    for (auto part : v.get_all_parts()) { indices.push_back(part.part_index()); }

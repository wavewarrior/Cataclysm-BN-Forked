...
auto luma( std::string_view hex ) -> double
{
    ...
    const auto chan = [hex]( std::size_t k ) -> double {
        return static_cast<double>( std::stoi( std::string( hex.substr( 1 + 2 * k, 2 ) ),
        nullptr, 16 ) );
    };
            ...
            for( const int lines : log_lines ) {
                for( const bool veh : {
                    ...
                    const auto l = hud_runic::layout_for( {
                        .ctx_w_dp = sz.w, .ctx_h_dp = sz.h,
                        .sidebar_right = right, .log_lines = lines, .show_vehicle = veh } );
...
TEST_CASE( "runic_ink_ladder_never_collapses", "[hud_runic]" )
{
    ...
    for( const ink i : ladder ) {
        lumas.push_back( luma( hud_runic::hex( i ) ) );
    }
...
TEST_CASE( "runic_legend_item_drops_the_separator_on_a_bare_value", "[hud_runic]" )
{
    ...
    const auto alarm = hud_runic::legend_item( { .label = "SAFE MODE", .value = "OFF",
                       .alarm = true } );
    ...
    for( const auto &sz : sizes ) {
        for( const bool right : {
            ...
            const auto opts = hud_runic::layout_options{
                .ctx_w_dp = sz.w, .ctx_h_dp = sz.h, .sidebar_right = right,
                .log_lines = 6, .show_vehicle = false };
            auto open = opts;
            ...
            const auto rows = ( collapsed.soma.h - hud_runic::head_h - hud_runic::chrome_h ) /
                              hud_runic::row_h;

...
static void move_item(
    player& p, item& it, const inventory_location from, const inventory_location to) {
    switch (from) {
        case GROUND:
            switch (to) {
                case GROUND:
...
static void invlet_test(player& dummy, const inventory_location from, const inventory_location to) {
    ...
    for (int id = 0; id < INVLET_STATE_NUM * INVLET_STATE_NUM * TEST_ACTION_NUM; ++id) {
        ...
        const invlet_state second_invlet_state = invlet_state(
            id / INVLET_STATE_NUM % INVLET_STATE_NUM);
        // the test steps
        const test_action action = test_action(
            id / INVLET_STATE_NUM / INVLET_STATE_NUM % TEST_ACTION_NUM);

        // the final expected invlet state of the two items
        invlet_state expected_first_invlet_state =
            second_invlet_state == NONE ? first_invlet_state : NONE;
        ...
        invlet_state
            final_first_invlet_state = check_invlet(dummy, *final_first, invlet),
            final_second_invlet_state = check_invlet(dummy, *final_second, invlet);

        INFO(test_action_desc(
            action, from, to, first_invlet_state, second_invlet_state, expected_first_invlet_state,
            expected_second_invlet_state, final_first_invlet_state, final_second_invlet_state));
...
static void stack_invlet_test(player& dummy, inventory_location from, inventory_location to) {
    ...
    if (from != INVENTORY || (to != WORN && to != WIELDED_OR_WORN)) { FAIL("unimplemented"); }
    ...
    ss << "actually the two items have "
       << (tshirt1.invlet != tshirt2.invlet ? "different" : "the same") << " invlets" << '\n';
    INFO(ss.str());
...
static void merge_invlet_test(player& dummy, inventory_location from) {
    ...
    for (int id = 0; id < INVLET_STATE_NUM * INVLET_STATE_NUM; ++id) {
        ...
        invlet_state expected_merged_invlet_state =
            first_invlet_state != NONE ? first_invlet_state : second_invlet_state;
        char expected_merged_invlet =
            first_invlet_state != NONE ? invlet_1
            : second_invlet_state != NONE
                ? invlet_2
                : 0;
        ...
        ss << "expect the stack in the inventory to have "
           << invlet_state_desc(expected_merged_invlet_state) << " invlet "
           << expected_merged_invlet << '\n';
...
TEST_CASE("Inventory letter test", "[invlet]") {
    ...
    invlet_test_autoletter_off(
        "Wielding and wearing items from the ground", dummy, GROUND, WIELDED_OR_WORN);
    invlet_test_autoletter_off("Wearing items from inventory", dummy, INVENTORY, WORN);

    stack_invlet_test_autoletter_off(
        "Wearing item from a stack in inventory", dummy, INVENTORY, WORN);
    stack_invlet_test_autoletter_off(
        "Wielding item from a stack in inventory", dummy, INVENTORY, WIELDED_OR_WORN);

    swap_invlet_test_autoletter_off(
        "Swapping invlets of two worn items of the same type", dummy, WORN);

    merge_invlet_test_autoletter_off(
        "Merging wielded item into an inventory stack", dummy, WIELDED_OR_WORN);
...
static void verify_invlet_consistency(const invlet_favorites& fav) {
    for (const auto& p : fav.get_invlets_by_id()) {
        for (const char invlet : p.second) { CHECK(fav.contains(invlet, p.first)); }
    }
}

    ...
    GIVEN("Names loaded from tests/data/name.json") {
        ...
        WHEN("Getting a male backer name") {
            for (int i = 0; i < 8; ++i) {
                ...
                CHECK_THAT(name, IsOneOf({"Male Backer", "Unisex Backer", "Male 'Nick' Backer",
                                          "Unisex 'Nick' Backer"}));
            }
        }
        WHEN("Getting a female backer name") {
            for (int i = 0; i < 8; ++i) {
                ...
                CHECK_THAT(name, IsOneOf({"Female Backer", "Unisex Backer", "Female 'Nick' Backer",
                                          "Unisex 'Nick' Backer"}));
            }
        }
        ...
        WHEN("Generating a male name") {
            for (int i = 0; i < 16; ++i) {
                ...
                CHECK_THAT(
                    name,
                    IsOneOf({"Male Backer", "Unisex Backer", "Male Family", "Unisex Family",
                             "Male 'Nick' Backer", "Unisex 'Nick' Backer", "Male 'Nick' Family",
                             "Unisex 'Nick' Family"}));
            }
        }
        WHEN("Generating a female name") {
            for (int i = 0; i < 16; ++i) {
                ...
                CHECK_THAT(
                    name,
                    IsOneOf({"Female Backer", "Unisex Backer", "Female Family", "Unisex Family",
                             "Female 'Nick' Backer", "Unisex 'Nick' Backer", "Female 'Nick' Family",
                             "Unisex 'Nick' Family"}));
            }
        }
    }

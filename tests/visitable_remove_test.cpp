...
TEST_CASE("visitable_remove", "[visitable]") {
    ...
    const auto suitable = [](const tripoint_bub_ms& pos, const int radius) {
        ...
        return std::all_of(tiles.begin(), tiles.end(), [](const tripoint_bub_ms& e) {
            if (!g->m.inbounds(e)) { return false; }
    ...
    item& obj = *item::in_container(
        temp_liquid.type->default_container.value_or(itype_id::NULL_ID()),
        std::move(temp_liquid_d));
    ...
    GIVEN("A player with several bottles of water") {
        for (int i = 0; i != count; ++i) { p.i_add(item::spawn(obj)); }
        ...
        WHEN("all the bottles are removed") {
            ...
            THEN("the correct number of items were removed") {
                ...
                AND_THEN("the removed items were all bottles") {
                    CHECK(
                        std::all_of(del.begin(), del.end(), [&container_id](detached_ptr<item>& e) {
                ...
                AND_THEN("the removed items all contain water") {
                    CHECK(std::all_of(del.begin(), del.end(), [&liquid_id](detached_ptr<item>& e) {
                        return e->contents.num_item_stacks() == 1
                            && e->contents.front().typeId() == liquid_id;
                    }));
                }
            }
        }
        ...
        WHEN("one of the bottles is wielded") {
            ...
            AND_WHEN("all the bottles are removed") {
                ...
                THEN("the correct number of items were removed") {
                    ...
                    AND_THEN("the removed items were all bottles") {
                        CHECK(std::all_of(
                            del.begin(), del.end(), [&container_id](detached_ptr<item>& e) {
                                return e->typeId() == container_id;
                            }));
                    }
                    AND_THEN("the removed items all contain water") {
                        CHECK(
                            std::all_of(del.begin(), del.end(), [&liquid_id](detached_ptr<item>& e) {
                                return e->contents.num_item_stacks() == 1
                                    && e->contents.front().typeId() == liquid_id;
                            }));
                    }
            ...
            AND_WHEN("all but one of the bottles is removed") {
                ...
                THEN("there is only one bottle remaining in the players possession") {
                    ...
                    AND_THEN("the remaining bottle is currently wielded") {
                        ...
                        AND_THEN(
                            "the remaining water is contained by the currently wielded bottle") {
    ...
    GIVEN("A player surrounded by several bottles of water") {
        ...
        WHEN("all the bottles are removed") {
            ...
            THEN("no bottles remain on the map") { REQUIRE(count_items(sel, container_id) == 0); }
            THEN("no water remains on the map") { REQUIRE(count_items(sel, liquid_id) == 0); }
            THEN("the correct number of items were removed") {
                REQUIRE(del.size() == count);
    ...
    GIVEN("An adjacent vehicle contains several bottles of water") {
        ...
        REQUIRE(
            std::count_if(
                ...
                [](const tripoint_bub_ms& e) { return static_cast<bool>(g->m.veh_at(e)); })
            == 1);
        ...
        for (int i = 0; i != count; ++i) { v->add_item(part, item::spawn(obj)); }
        ...
        WHEN("all the bottles are removed") {
            std::vector<detached_ptr<item>> del;
            sel.remove_items_with([&container_id, &del](detached_ptr<item>&& e) {
                if (e->typeId() == container_id) { del.push_back(std::move(e)); }
                return VisitResponse::NEXT;
            });

    ...
    GIVEN("A character with debug hammerspace trait") {
        ...
        WHEN("The character tries to craft a no-dachi") {
            ...
            THEN("The craft item is created") {
                ...
                AND_WHEN(
                    "The character spends a second performing the activity set when starting the craft") {
                    dummy.set_moves(100);
...
TEST_CASE("tool selection ui", "[crafting][ui]") {
    ...
    GIVEN(
        "Two compatible tools in map inventory, one with enough charges for entire craft and one with half") {

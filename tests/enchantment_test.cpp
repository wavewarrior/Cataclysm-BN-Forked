...
TEST_CASE("Enchantments grant mutations", "[magic][enchantment][trait][mutation]") {
    ...
    WHEN("Character doesn't have trait") {
        ...
        AND_WHEN("Character receives relic") {
            ...
            THEN("Character gains trait") { CHECK(guy.has_trait(trait_CARNIVORE)); }
            AND_WHEN("Turn passes") {
                advance_turn(guy);
                THEN("Character still has trait") { CHECK(guy.has_trait(trait_CARNIVORE)); }
                AND_WHEN("Character loses relic") {
                    clear_items(guy);
                    THEN("Character loses trait") { CHECK_FALSE(guy.has_trait(trait_CARNIVORE)); }
                    AND_WHEN("Turn passes") {
                        advance_turn(guy);
    ...
    WHEN("Character has trait") {
        ...
        AND_WHEN("Character receives relic") {
            give_item(guy, s_relic);
            THEN("Nothing changes") { CHECK(guy.has_trait(trait_CARNIVORE)); }
            AND_WHEN("Turn passes") {
                advance_turn(guy);
                THEN("Nothing changes") { CHECK(guy.has_trait(trait_CARNIVORE)); }
                AND_WHEN("Character loses relic") {
...
static void tests_stats(
    Character& guy, int s_base, int d_base, int p_base, int i_base, int s_exp, int d_exp, int p_exp,
    int i_exp) {
    guy.str_max = s_base;
    guy.dex_max = d_base;
    guy.per_max = p_base;
    ...
    auto check_stats_base = [&]() { check_stats(s_base, d_base, p_base, i_base); };
    ...
    WHEN("Character receives relic") {
        give_item(guy, s_relic);
        THEN("Stats remain unchanged") { check_stats_base(); }
        AND_WHEN("Turn passes") {
            advance_turn(guy);
            ...
            AND_WHEN("Character loses relic") {
                clear_items(guy);
                THEN("Stats remain unchanged") { check_stats(s_exp, d_exp, p_exp, i_exp); }
                AND_WHEN("Turn passes") {
                    advance_turn(guy);
                    THEN("Stats return to normal") { check_stats_base(); }
                }
            }
        }
    }
...
TEST_CASE("Enchantments modify stats", "[magic][enchantment][character]") {
    ...
    SECTION("base stats 8") { tests_stats(guy, 8, 8, 8, 8, 20, 6, 5, 0); }
    SECTION("base stats 12") { tests_stats(guy, 12, 12, 12, 12, 28, 10, 7, 1); }
    SECTION("base stats 4") { tests_stats(guy, 4, 4, 4, 4, 12, 2, 3, 0); }
}
...
static void tests_speed(Character& guy, int sp_base, int sp_exp) {
    ...
    auto check_speed_is_base = [&] { check_speed(sp_base, sp_base); };

    WHEN("Character has no relics") {
        THEN("Speed and moves gain equel base") { check_speed_is_base(); }
    }
    WHEN("Character receives relic") {
        give_item(guy, s_relic);
        THEN("Nothing changes") { check_speed_is_base(); }
        AND_WHEN("Turn passes") {
            guy.set_moves(0);
            advance_turn(guy);
            THEN("Speed changes, moves gain changes") { check_speed(sp_exp, sp_exp); }
            AND_WHEN("Character loses relic") {
                clear_items(guy);
                THEN("Nothing changes") { check_speed(sp_exp, sp_exp); }
                AND_WHEN("Turn passes") {
                    guy.set_moves(0);
                    advance_turn(guy);
                    THEN("Speed and moves gain return to normal") { check_speed_is_base(); }
                }
            }
        }
    }
    WHEN("Character receives 10 relics") {
        for (int i = 0; i < 10; i++) { give_item(guy, s_relic); }
        THEN("Nothing changes") { check_speed_is_base(); }
        AND_WHEN("Turn passes") {
            guy.set_moves(0);
            advance_turn(guy);
...
TEST_CASE("Enchantments modify speed", "[magic][enchantment][speed]") {
    ...
    SECTION("base = 100") { tests_speed(guy, 100, 75); }
    SECTION("base = 80") { tests_speed(guy, 80, 65); }
    SECTION("base = 120") { tests_speed(guy, 120, 85); }
}

static void tests_attack_cost(
    Character& guy, const item& weap, int item_atk_cost, int guy_atk_cost, int exp_guy_atk_cost) {
    advance_turn(guy);
    ...
    WHEN("Character receives relic") {
        give_item(guy, s_relic);
        THEN("Attack cost changes") { CHECK(guy.attack_cost(weap) == exp_guy_atk_cost); }
        AND_WHEN("Character loses relic") {
            clear_items(guy);
            THEN("Attack cost returns to normal") { CHECK(guy.attack_cost(weap) == guy_atk_cost); }
        }
    }
    WHEN("Character receives 10 relics") {
        for (int i = 0; i < 10; i++) { give_item(guy, s_relic); }
        THEN("Attack cost does not drop below 25") { CHECK(guy.attack_cost(weap) == 25); }
    }
}
...
static void tests_move_cost(Character& guy, int tile_move_cost, int move_cost, int exp_move_cost) {
    ...
    WHEN("Character receives relic") {
        give_item(guy, s_relic);
        THEN("Move cost changes") { CHECK(guy.run_cost(tile_move_cost) == exp_move_cost); }
        AND_WHEN("Character loses relic") {
            clear_items(guy);
    ...
    WHEN("Character receives 15 relics") {
        for (int i = 0; i < 15; i++) { give_item(guy, s_relic); }
        THEN("Move cost does not drop below 20") { CHECK(guy.run_cost(tile_move_cost) == 20); }
    }
}
...
TEST_CASE("Enchantments modify move cost", "[magic][enchantment][move]") {
    ...
    SECTION("Naked character") {
        SECTION("tile move cost = 100") { tests_move_cost(guy, 100, 100, 90); }
        SECTION("tile move cost = 120") { tests_move_cost(guy, 120, 120, 108); }
    }
    SECTION("Naked character with PADDED_FEET") {
        ...
        SECTION("tile move cost = 100") { tests_move_cost(guy, 100, 90, 81); }
        SECTION("tile move cost = 120") { tests_move_cost(guy, 120, 108, 97); }
    }
}
...
TEST_CASE("Enchantments modify metabolic rate", "[magic][enchantment][metabolism]") {
    ...
    SECTION("Clean character") { tests_metabolic_rate(guy, 1.0f, 0.9f); }
...
static void tests_stamina(
    Character& guy, int cap_norm, int cap_exp, float rate_norm, float rate_exp) {
    advance_turn(guy);
...
TEST_CASE("Enchantments modify stamina", "[magic][enchantment][stamina]") {
    ...
    SECTION("Clean character") { tests_stamina(guy, 10000, 9000, 20.0f, 18.0f); }
...
template <typename F>
static void tests_need_rate(
    Character& guy, const std::string& s_relic, float norm, float exp, F getter) {
    advance_turn(guy);
    ...
    WHEN("Character receives relic") {
        ...
        THEN("Need rate changes") {
            ...
            AND_WHEN("Character loses relic") {
                clear_items(guy);
                THEN("Need rate goes back to normal") { CHECK(getter(guy) == Catch::Approx(norm)); }
            }
        }
    }
    WHEN("Character receives 15 relics") {
        for (int i = 0; i < 15; i++) { give_item(guy, s_relic); }
        THEN("Need rate does not go below 0") { CHECK(getter(guy) == Catch::Approx(0.0f)); }
    }
}
...
TEST_CASE("Enchantments modify thirst rate", "[magic][enchantment][thirst]") {
    ...
    const auto getter = [](const Character& guy) -> float { return guy.calc_needs_rates().thirst; };
    ...
    SECTION("Clean character") { tests_need_rate(guy, s_relic, 1.0f, 0.9f, getter); }
...
static void tests_num_dodges(Character& guy) {
    ...
    WHEN("Character receives relic") {
        give_item(guy, s_relic);
        THEN("Nothing changes") { check_num_dodges(guy, 1); }
        AND_WHEN("Turn passes") {
            guy.moves = 1;
            guy.dodges_left = 0;
            advance_turn(guy);
            THEN("Dodge bonus changes, dodges increase") { check_num_dodges(guy, 2); }
            AND_WHEN("Character loses relic") {
                clear_items(guy);
                THEN("Nothing changes") { check_num_dodges(guy, 2); }
                AND_WHEN("Turn passes") {
                    guy.moves = 1;
                    guy.dodges_left = 0;
                    advance_turn(guy);
    ...
    WHEN("Character receives 10 relics") {
        ...
        AND_WHEN("Turn passes") {
            ...
            guy.dodges_left = 0;
            advance_turn(guy);
            THEN("Dodge bonus and dodge gain increase by 10") { check_num_dodges(guy, 11); }
        }
    }
}
...
TEST_CASE("Armor enchantments", "[magic][enchantment][armor]") {
    ...
    SECTION("Armor item with enchantment that trades bash armor for cut armor") {
        ...
        SECTION("Cut") {
            // 10 (incoming) + (10 * -0.5 + 3) (enchantment) - 4 (base item cut armor) = 4 (6
            // absorbed)
            CHECK(calc_damage_absorb(guy, damage_type::DT_CUT, 10) == 6);
        }
        SECTION("Bash") {
            // 10 (incoming) + (10 * 0.5 - 3) (enchantment) - 4 (base item bash armor) = 8 (2
            // absorbed)
    ...
    SECTION("Armor item with no enchantments + socks of protection") {
        ...
        SECTION("Stab") {
            // 10 (incoming) + (10 * -0.1 - 3) (enchantment) - 3 (base item stab armor) = 3 (7
            // absorbed)

...
TEST_CASE("coop_sim_transport: message delivered at correct sim time", "[coop][simtransport]") {
...
TEST_CASE(
    "coop_sim_transport: message not delivered before latency elapses", "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
    a.latency_ms = 100;
...
TEST_CASE("coop_sim_transport: zero-latency delivers immediately", "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
    a.latency_ms = 0;
...
TEST_CASE(
    "coop_sim_transport: multiple messages arrive in-order at correct times",
    "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
    a.latency_ms = 10;
...
TEST_CASE("coop_sim_transport: loss_rate=1.0 drops every message", "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
    a.loss_rate = 1.0f;
...
TEST_CASE("coop_sim_transport: loss_rate=0.0 delivers all messages", "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
    a.loss_rate = 0.0f;

    for (int i = 0; i < 5; ++i) { a.send(std::string(1, static_cast<char>('A' + i))); }
    b.advance(0);
...
TEST_CASE(
    "coop_sim_transport: reorder flag swaps last two enqueued messages", "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
    a.reorder = true;
...
TEST_CASE(
    "coop_sim_transport: bidirectional wiring delivers cross-direction", "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
    b.wire_peer(&a);
    a.latency_ms = 20;
...
TEST_CASE(
    "coop_sim_transport: close_abruptly drains inbox and severs reverse reference",
    "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
...
TEST_CASE(
    "coop_sim_transport: recv returns false when inbox empty regardless of timeout",
    "[coop][simtransport]") {
    coop_sim_transport a, b;
    a.wire_peer(&b);
...
TEST_CASE("Gap 4: ring buffer capped at 32 — oldest seq dropped on overflow", "[coop][ringbuf]") {
    coop_client cli;

    // Queue 35 actions; next_seq_ starts at 1 so seqs are 1..35
    for (int i = 0; i < 35; ++i) { cli.queue_action("MOVE_N"); }
...
TEST_CASE("Gap 4: ring buffer at exact cap (32) — no drop", "[coop][ringbuf]") {
    coop_client cli;
    for (int i = 0; i < 32; ++i) { cli.queue_action("MOVE_N"); }
    CHECK(cli.pending_actions_size_for_test() == 32);
    CHECK(cli.pending_actions_front_seq_for_test() == 1u); // no drop yet
}

TEST_CASE("Gap 6: next_seq_ wraps from UINT32_MAX to 0 without crash or UB", "[coop][ringbuf]") {
    coop_client cli;

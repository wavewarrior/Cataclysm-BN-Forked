...
TEST_CASE("generic_factory_insert_convert_valid", "[generic_factory]") {
    ...
    // technically, lookup by non-valid int_id may result undefined behavior, if factory is
    // non-empty this is expected
...
TEST_CASE("string_ids_comparison", "[generic_factory][string_id]") {
    ...
    if (first_valid) { test_factory.insert({id1, "value"}); }
    if (second_valid) { test_factory.insert({id2, "value"}); }
    if (first_cached) {
        // is_valid should update cache internally
        test_factory.is_valid(id1);
    }
    if (second_cached) { test_factory.is_valid(id2); }
    ...
    DYNAMIC_SECTION(
        ...
        << "___" << "first_" << id_info(first_cached, first_valid)
        << "___"
           "second_"
        << id_info(second_cached, second_valid)) {
        if (equal) {
            CHECK(id1 == id2);
        } else {
            CHECK_FALSE(id1 == id2);
...
TEST_CASE("generic_factory_lookup_benchmark", "[.][generic_factory][benchmark]") {
    ...
    BENCHMARK("single lookup") { return test_factory.obj(id_200).value; };
}

TEST_CASE("string_id_compare_benchmark", "[.][generic_factory][string_id][benchmark]") {
    std::string prefix;
    SECTION("short id") {}
    SECTION("long id") { prefix = "log_common_prefix_"; }
    ...
    BENCHMARK("ids are equal, invalid version") { return id_200 == id_200_; };

    CHECK_FALSE(id_200 == id_300);
    BENCHMARK("ids are not equal, invalid version") { return id_200 == id_300; };
    ...
    CHECK(id_200 == id_200_);
    BENCHMARK("ids are equal, valid version") { return id_200 == id_200_; };

    CHECK_FALSE(id_200 == id_300);
    BENCHMARK("ids are not equal, valid version") { return id_200 == id_300; };
}
...
TEST_CASE("string_and_int_ids_benchmark", "[.][generic_factory][int_id][string_id][benchmark]") {
    ...
    for (const auto& f : all_flags) {
        if (xor_rand() % all_flags_n <= flags_in_item) { item.set_flag(f.id); }
    }
    ...
    if (hits) {
        ...
        for (const auto& f : item.std_set_string_ids) { test_flags.push_back(f); }
    } else {
    ...
    for (const auto& f : test_flags) { test_dyn_str_ids.push_back(dyn_str_id(f.str())); }
    ...
    BENCHMARK("baseline: string_id::obj()") { return test_flags[0].obj().was_loaded; };

    BENCHMARK("baseline: string_id -> int_id") { return test_flags[0].id().to_i(); };

    BENCHMARK("\"enum\" bitset") { return !!item.bit_flags[(i++) % bit_flags_size]; };

...
TEST_CASE("soak: 500-tick random walk with checksum convergence", "[coop][soak][.]") {
    inproc_harness h;
    h.setup();
...
TEST_CASE("soak: 50-item pickup/drop churn conserves total items", "[coop][soak][.]") {
    inproc_harness h;
    h.setup();
    ...
    for (int i = 0; i < 50; ++i) { g->m.add_item(TILE, item::spawn(ITEM_TYPE, calendar::turn)); }

    // Count items on tile.
    auto count_items = [&]() -> int { return static_cast<int>(g->m.i_at(TILE).size()); };

    const int initial = count_items();
    CHECK(initial == 50);

    // Simulate 100 ticks of alternating pickup/drop (via apply_* static methods).
    for (int t = 0; t < 100; ++t) { h.tick(); }
...
TEST_CASE("soak: disconnect and rewire cycles without crash", "[coop][soak][.]") {
    inproc_harness h;
    h.setup();

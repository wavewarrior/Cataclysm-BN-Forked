...
static void wield_check_internal(
    player& dummy, item& the_item, const char* section_text, const std::string& var_name,
...
#define wield_check_inv(section_text, dummy, the_item, expected_cost)                       \
    SECTION(section_text) {                                                                 \
        ...
        wield_check_internal(                                                               \
            dummy, obj, #section_text, #the_item, generating_cases ? -1 : (expected_cost)); \
    }
...
static void do_test(const bool generating_cases) {
    ...
    wield_check_inv(
        "Wielding halberd from inventory while unencumbered", dummy, item::spawn("halberd"), 287);
    ...
    wield_check_floor("Wielding metal tank from outside inventory while wearing boxing gloves",
                      spot, dummy, item::spawn("metal_tank"), 400);
}

...
|---|---|---|---|
| **CL-RANGED** | Proxy arming | `spawn_proxy_npc()` did not clear `primary_weapon()` — proxy fired wrong gun or nothing. **Fixed (commit 38d18c3802):** `remove_primary_weapon()` added to `spawn_proxy_npc()`; `weapon_id`+`ammo_id` embedded in FIRE ctx_json at fire time; server arms proxy via `item::spawn(wid)+ammo_set+wield` before each `resolve_fire_at_seq`, disarms after. Consolidated into `arm_proxy_from_ctx(proxy, ctx, require_gun=true)` (commit 25f6238a1d). Verified by `COOP_SCENARIO=ranged` E2E. |
| **CL-MELEE-WEAPON** | Proxy melee weapon | Proxy always fought bare-fisted — client's melee weapon ignored. **Fixed (commit 25f6238a1d):** `weapon_id` embedded in MELEE/SMASH ctx_json at attack time; server arms proxy via `arm_proxy_from_ctx(proxy, ctx, require_gun=false)` before `melee_attack()`, disarms after. Verified by `COOP_SCENARIO=melee` and `COOP_SCENARIO=smash` E2E (knife_combat weapon relay exercised). |
| **CL-POPUP** | "Waiting for client" popup | **Fixed (commit 4c8b5f628c).** Replaced `static_popup` with `ui_adaptor` + catacurses, bypassing the RmlUI model system. |

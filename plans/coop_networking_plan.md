...
| **CL-POPUP** | "Waiting for client" popup | `static_popup` relies on the query_popup RmlUI model (`g_rml_query_popup_model_active`). The loading chain leaves that model active, so `rml_open()` fails silently — popup frame renders but has no bound text. Confirmed: main menu visible behind popup, no world-picker (single world auto-selected), H/J/0/1/2 ignored (only QUIT/Escape registered in COOP_WAIT). | **Fixed (commit 4c8b5f628c):** replaced `static_popup` with `ui_adaptor` + catacurses (`fold_and_print` + `draw_border`), bypassing the RmlUI model system. Pending user verification. |

...
| S11 | `5ea9b1a7c9` | landed — `origin/main` content-tail merge (`c3090ca8f0`, depth 96) plus 16 parallel decomposition-conflict batches. 3 of 4 `~[.]` shards (0, 1, 3) plus `[coop]` verified clean: same 5-test failure set already accepted as baseline (`flung creatures stop at the reality bubble edge`, `vision_wall_obstructs_light`, `vision_single_tile_skylight`, `vision_see_out_of_vehicle`, `vision_see_into_vehicle`) — confirmed by running all 5 in isolation too (4 vision tests fail standalone, same assertions; `flung creatures...` passes standalone, matching the S6 gate's "order-sensitive" note) — zero new failures in the ~941 cases covered. Shard 2 (314 cases) hits a real SIGSEGV partway through, reproduced on a clean serial re-run: crash is in `map::build_absorption_cache(int)` (via `wipe_map_terrain`/`clear_map`/`build_map_cache`, hit while running `tree_terrain_supports_climbing_destination_above` at declaration position #22), matching the exact crash site already carried forward from S6 as an open issue (`plans/vehicle-cache-sigsegv-handoff.md`) — not attributable to any S11 hunk. One real regression found and fixed during verification: landing upstream's itemgroup-postprocessor feature exposed a load-order bug (`mod.genome` set in `main.lua` but usable from `finalize`/`check_consistency`, which run before `main.lua`); fixed by moving the require+assignment into `preload.lua`. |

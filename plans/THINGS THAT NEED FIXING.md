...
- the soundwavefront shader effect is too small. the rings need to be a bit larger, add in a knob in the debug UI panel so we can adjust params. `g_sound_wave_speed`/`g_sound_wave_min_radius`/`g_sound_wave_max_radius` are already bound and tunable in `sdl_lighting_devui.cpp:183-185,946-948` — only the 'feels too small' taste tweak remains open.
...
- Piper TTS is not working or inactive? when conversing with NPC i expect it to trigger and work. Re-verify in-game before treating as still open — `src/tts_piper_synthesizer.{h,cpp}` and `tts_voice_registry.*` are a full non-stub backend, wired into `npc.cpp:1488-1494` and `npctalk.cpp:2123-2132`, with a `feat(tts)`/`fix(tts)` commit series already on the branch tip (as of 2026-09-14 this may already be fixed).
...
- Windows should act as light portals as done in many other games, to let more daylight seep in in an indoor scenario. also add shader effect for godrays entering through windows with dust particles floating in it. The godray half has landed (`feat(rendering): indoor godrays through window openings`, `volumetric_pass.h:47-51`, `sdl_render_frame.cpp:1094-1095`) — only the 'dust particles floating in it' half remains open.

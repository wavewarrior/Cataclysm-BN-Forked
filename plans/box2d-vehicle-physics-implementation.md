...
The vehicle-vehicle (VV) collision system in `map::vehicle_vehicle_collision()` (map.cpp:1394) has two correctness gaps:
...
static constexpr float TILE_M = 1.78816f;  // vehicles::cmps_per_tile / 100
    ...
    -> vv_cluster_result
{
    ...
    for( vehicle *v : veh_list ) {
        ...
        // face_vec() returns normalized (cos θ, sin θ); b2Rot stores (c, s) — direct assignment.
        ...
        bdef.angularVelocity = v->angular_velocity_rads;  // carry residual from prior solve
...
./out/build/linux-full/tests/cata_test-tiles "[vehicle][box2d]"
```

---

## Phase 3: Persistent World Infrastructure
...
- Replace pairwise loop at map.cpp:1208–1210 with `solve_vv_cluster()` call
- `vehicle_vehicle_collision()` gains a `precomputed` path for Box2D results vs analytic fallback
- `angular_velocity_rads` field added to `vehicle`; consumed in `vehicle::on_move()` during transition
- Damage code unchanged — `impulse_ns` from solver feeds existing damage thresholds

---

## Phase 5: Vehicle-Terrain Collision
...
## Phase 6: Continuous Position Data Model
...
## Phase 7: Continuous Rotation, Part Precalc, and Rendering
...
3. Migrate callsites in priority order:
   - **Hot (every tick):** tile-occupancy cache (`occupied_points` rebuild), FoV blocking, collision detection — migrate first; these are performance-sensitive
   - **Warm (on vehicle move):** `part_at()` lookup, creature adjacency checks — migrate second
...
## Phase 9: Physics State Serialization
...
|---|---|---|
| `physics_pos` (float metres) | `jo.member("physics_pos", physics_pos)` | read → `b2Body_SetTransform` on load |
...
**Save format version:** a version bump is required. Old saves (no physics fields) deserialize with zero velocity and tile-anchor position; the vehicle is stationary on load — acceptable degradation.

**Decision D4 (walkthrough):** whether the integer tile anchor or the float `physics_pos` is the save-format canonical position (impacts old-save compatibility).

---

## Phase 10: Vehicle Movement Migration
...
- Vehicles promoted from `b2_kinematicBody` to `b2_dynamicBody` in persistent world
- Engine thrust → `b2Body_ApplyForceToCenter()`; steering torque → `b2Body_ApplyTorque()`
- Box2D steps at ~60Hz; game reads `b2Body_GetPosition()` + `b2Body_GetRotation()` at each tick boundary
- Integer tile anchor updated from float physics position via rounding rule (Phase 6/D2)
- `tileray face` kept as render-only quantized shadow of Box2D angle (D3: quantize to nearest 15°)
- VV and terrain contacts detected by Box2D contact events; tile-step detection loop retired
- Creature collision (`veh_coll_body`) remains game-logic driven
- `of_turn`, `velocity` (integer cm/s), `turn_dir`, and discrete `turn()` calls retired
...
### Step — Fix `vehicle::part_collision()` creature-phase loop exit

**File: `src/vehicle_move.cpp`** — `else` branch at line 889 (inside `veh_coll_body` case):
...
```cpp
    ...
    velocity = 0;
    smashed  = true;  // forces do-while exit; prevents re-eval at v=0 (phasing)
    break;
}
```

The `do { } while( !smashed && sgn(coll_velocity) == vel_sign )` at line 912 already handles `smashed == true` as exit. Setting it prevents a zero-velocity loop iteration that was the phasing mechanism.

**Verification:** Drive at ~10 mph into a heavy creature (bear/moose) 20 times. Vehicle stops adjacent; no tile overlap.

---

## Critical Files & Anchors

| File | Symbol / Region | Phase |
|---|---|---|
| `CMakeLists.txt:736` | After `FetchContent_MakeAvailable(RmlUi)` | Phase 1 |
| `src/CMakeLists.txt:190` | After `target_link_libraries(...rmlui)` | Phase 1 |
| `src/map.cpp:1208–1210` | VV dispatch loop in `move_vehicle()` | Phase 4 |
| `src/map.cpp:1394–1582` | `map::vehicle_vehicle_collision()` | Phase 4 |
| `src/vehicle_move.cpp:636` | `veh_coll_body` (creature case) | Phase 13 |
| `src/vehicle_move.cpp:656` | `veh_coll_bashable` | Phase 5 |
| `src/vehicle_move.cpp:662` | `veh_coll_other` | Phase 5 |
| `src/vehicle_move.cpp:721–912` | do-while bash loop | Phase 5 |
| `src/vehicle_move.cpp:889` | Creature-phase loop exit | Phase 13 |
| `src/vehicle.h:~1720` | Vehicle data fields | Phases 4, 6 |
| `src/savegame_json.cpp:3166,3374` | serialize/deserialize | Phase 9 adds physics fields; transitional fields NOT serialized before Phase 9 |
| `vehicles::cmps_per_tile = 178.816f` | `src/vehicle.h:59` | TILE_M = 1.78816f |
...
### Phase 13 — Creature fix
Drive at ~10 mph into bear/moose 20 times. Vehicle stops adjacent every time; no overlap.
...
**`bdef.rotation` uses `b2Rot{fv.x, fv.y}`**: `face_vec()` returns normalized `(cos θ, sin θ)`. `b2Rot` stores `{c, s}`. Direct assignment is correct.

...
static void erase_char(std::string& s, const char& c) {
    ...
    while ((pos = s.find(c)) != std::string::npos) { s.erase(pos, 1); }
}
#endif
...
static void write_array(JsonIn& jsin, JsonOut& jsout, int depth, bool force_wrap) {
    ...
    jsin.start_array();
    while (!jsin.end_array()) { format(jsin, jsout, depth); }
    jsout.end_array();
}
    ...
    while (!jsin.end_object()) {
        ...
        if (name == "rows" || name == "blueprint") {
            ...
            bool ate_seperator = jsin.get_ate_separator();
            {
                ...
                if (arr.size() > 1) { override_wrap = true; }
            }
...
static void format_collection(
    JsonIn& jsin, JsonOut& jsout, int depth,
    std::function<void(JsonIn&, JsonOut&, int, bool)> write_func, bool force_wrap) {
    if (depth > 1 && !force_wrap) {
        ...
        if (jsout.tell() - out_start_pos <= 120) {
            ...
            if (need_separator) { jsout.set_need_separator(); }
        }
    }
    ...
    if (jsin.test_array()) {
        ...
        *jsout.get_stream() << str;
        ...
        if (str_form.find('.') == std::string::npos) {
            ...
            if (double_str.back() == '.') { double_str += "0"; }
            jsout.write_separator();

...
static auto isInteger(const std::string& bind) {
    return expr(anyOf(integerLiteral(),
...
static auto testWhetherParentIsVarDecl() {
    return expr(anyOf(hasParent(varDecl().bind("parentVarDecl")), anything()));
}
...
void UseNamedPointConstantsCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        cxxConstructExpr(
            ...
            testWhetherConstructingTemporary(), testWhetherParentIsVarDecl(), argumentCountIs(2),
            hasArgument(0, isInteger("xexpr")), hasArgument(1, isInteger("yexpr")))
            .bind("constructorCall"),
        this);
    Finder->addMatcher(
        cxxConstructExpr(
            hasDeclaration(isPointConstructor().bind("constructorDecl")),
            testWhetherConstructingTemporary(), testWhetherParentIsVarDecl(), argumentCountIs(3),
            hasArgument(0, isInteger("xexpr")), hasArgument(1, isInteger("yexpr")),
...
static const std::map<std::pair<int, int>, std::string> PointConstants = {
    {{0, 0}, "point_zero"},        {{0, -1}, "point_north"},     {{1, -1}, "point_north_east"},
    {{1, 0}, "point_east"},        {{1, 1}, "point_south_east"}, {{0, 1}, "point_south"},
    {{-1, 1}, "point_south_west"}, {{-1, 0}, "point_west"},      {{-1, -1}, "point_north_west"},
};
...
static void CheckConstructor(
    UseNamedPointConstantsCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    auto insertArgIf = [&](const Expr* E, const char* Key) {
        ...
        if (E == nullptr) {
            ...
            Value = Literal->getValue().getZExtValue();
            if (UOp->getOpcode() == UO_Minus) { Value = -Value; }
        } else {
            assert(false);
        }
    ...
    if (ZExpr) {
        ...
        if (it != TripointConstants.end()) { Replacement = it->second; }
    } else {
        auto it = PointConstants.find(std::make_pair(Args["x"], Args["y"]));
        if (it != PointConstants.end()) { Replacement = it->second; }
    }

    if (Replacement.empty()) { return; }

    // Avoid replacing the definitions of the constants themselves
    if (VarDeclParent && VarDeclParent->getName() == Replacement) { return; }

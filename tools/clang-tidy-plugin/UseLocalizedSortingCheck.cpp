    ...
    switch (Arg.getKind()) {
        ...
        case TemplateArgument::Pack:
            for (const TemplateArgument& PackArg : Arg.getPackAsArray()) {
                if (IsStringish(PackArg)) { return true; }
            }
            break;
        default:
            break;
...
static bool IsStringish(QualType T) {
    ...
    if (Name == "basic_string") { return true; }
    if (Name == "pair" || Name == "tuple") {
        const ClassTemplateSpecializationDecl* SpecDecl = dyn_cast<ClassTemplateSpecializationDecl>(
            TTag);
        if (!SpecDecl) {
        ...
        for (const TemplateArgument& Arg : Args.asArray()) {
            if (IsStringish(Arg)) { return true; }
        }
    }
    return false;
}
...
void UseLocalizedSortingCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        cxxOperatorCallExpr(
            hasArgument(0, expr(hasType(qualType().bind("arg0Type"))).bind("arg0Expr")),
    ...
    Finder->addMatcher(
        callExpr(callee(namedDecl(hasAnyName("std::sort", "std::stable_sort"))), argumentCountIs(2),
                 hasArgument(0, expr(hasType(qualType(
                                    anyOf(pointerType(pointee(qualType().bind("valueType"))),
...
static void CheckOpCall(UseLocalizedSortingCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (!Call || !Arg0Type || !Arg0Expr) { return; }

    if (!IsStringish(*Arg0Type)) { return; }

    StringRef Arg0Text = getText(Result, Arg0Expr);
    if (Arg0Text.ends_with("id")) { return; }
...
static void CheckSortCall(UseLocalizedSortingCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (!Call || (!BoundValueType && !IteratorDecl)) { return; }
    ...
    if (BoundValueType) { ValueType = *BoundValueType; }

    if (!IsStringish(ValueType)) { return; }

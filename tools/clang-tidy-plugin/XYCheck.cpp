...
void XYCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        fieldDecl(hasType(isInteger()), isXParam(),
                  hasParent(cxxRecordDecl(forEachDescendant(fieldDecl(isYParam()).bind("yfield")))
                                .bind("record")))
            .bind("xfield"),
        this);
    Finder->addMatcher(
        parmVarDecl(hasType(isInteger()), isXParam(), hasAncestor(functionDecl().bind("function")))
            .bind("xparam"),
        this);
}
...
static void CheckField(XYCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (!XVar || !YVar || !Record) { return; }
    const NameConvention NameMatcher(XVar->getName());
    if (!NameMatcher) { return; }
    if (NameMatcher.Match(YVar->getName()) != NameConvention::YName) { return; }
    ...
    if (ZVar) { Check.diag(ZVar->getLocation(), "declaration of %0", DiagnosticIDs::Note) << ZVar; }
}

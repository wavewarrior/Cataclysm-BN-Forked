...
void PointInitializationCheck::registerMatchers(MatchFinder* Finder) {
    ...
    const CxxConstructorMatcher ZeroConstructor =
        cxxConstructExpr(
            anyOf(
                allOf(argumentCountIs(1),
                      hasArgument(0, declRefExpr(to(varDecl(
                                         anyOf(hasName("point_zero"), hasName("tripoint_zero"))))))),
                allOf(hasArgument(0, integerLiteral(equals(0))),
                      hasArgument(1, integerLiteral(equals(0))),
                      anyOf(argumentCountIs(2),
                            allOf(hasArgument(2, integerLiteral(equals(0))), argumentCountIs(3))))))
            .bind("expr");
    Finder->addMatcher(
        varDecl(unless(parmVarDecl()), hasType(isPointType()), hasInitializer(ZeroConstructor))
            .bind("decl"),
        this);
    Finder->addMatcher(
        cxxCtorInitializer(forField(hasType(isPointType())), withInitializer(ZeroConstructor))
            .bind("init"),
        this);
}
...
static void CheckDecl(PointInitializationCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    Check.diag(MatchedExpr->getBeginLoc(),
               "Unnecessary initialization of %0. %1 is zero-initialized by default.")
        << MatchedDecl << Type << FixItHint::CreateReplacement(ToReplace, Replacement);
}

...
void UsePointApisCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        callExpr(
            forEachArgumentWithParam(
                expr().bind("xarg"), parmVarDecl(hasType(isInteger()), isXParam()).bind("xparam")),
    ...
    Finder->addMatcher(
        cxxConstructExpr(
            forEachArgumentWithParam(
                expr().bind("xarg"),
                parmVarDecl(anyOf(hasType(asString("int")), hasType(asString("const int"))),
                            isXParam())
                    .bind("xparam")),
            hasDeclaration(cxxMethodDecl(unless(ofClass(isPointOrCoordPointType()))).bind("callee")))
...
static bool doFunctionsMatch(
    ...
    for (; CalleeParamI < Callee->getNumParams(); ++CalleeParamI, ++OtherCalleeParamI) {
        ...
        const ParmVarDecl* OtherCalleeParam = OtherCallee->getParamDecl(OtherCalleeParamI);
        ...
        if (CalleeParamI == MinArg - SkipArgs) {
            ...
            if (OtherCalleeParam->getType().getAsString() != ExpectedTypeName) { return false; }
...
static void CheckCall(UsePointApisCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (!XParam || !XArg || !(Call || ConstructorCall) || !Callee) { return; }
    ...
    const FunctionDecl* ContainingFunction =
        getContainingFunction(Result, Call ? static_cast<const Expr*>(Call) : ConstructorCall);
    ...
    for (const NamedDecl* OtherDecl : Context->lookup(Callee->getDeclName())) {
        if (const FunctionDecl* OtherCallee = dyn_cast<FunctionDecl>(OtherDecl)) {
            if (OtherCallee == Callee || OtherCallee == ContainingFunction) { continue; }

            if (doFunctionsMatch(Callee, OtherCallee, NumCoordParams, SkipArgs, MinArg, !!ZArg)) {
                NewCallee = OtherCallee;
                break;
            }
        }
        if (const FunctionTemplateDecl* OtherTmpl = dyn_cast<FunctionTemplateDecl>(OtherDecl)) {
            const FunctionTemplateDecl* Tmpl = Callee->getPrimaryTemplate();

            if (!Tmpl || Tmpl == OtherTmpl) { continue; }
    ...
    if (ZArg) { Replacement = ("tri" + Replacement + ", " + getText(Result, ZArg)).str(); }
    Replacement += " )";
    ...
    SourceRange
        SourceRangeToReplace(GetCallArg(MinArg)->getBeginLoc(), GetCallArg(MaxArg)->getEndLoc());
...
void UsePointApisCheck::check(const MatchFinder::MatchResult& Result) { CheckCall(*this, Result); }

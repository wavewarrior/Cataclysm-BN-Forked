    ...
    Finder->addMatcher(
        callExpr(
            ...
            hasAncestor(
                callExpr(
                    callee(decl(anyOf(
                        functionDecl(hasAnyName("_", "gettext", "pgettext", "vgettext", "vpgettext"))
                            .bind("translationFunc"),
                        functionDecl(hasAnyName("to_translation", "pl_translation")))))
                    // no_translation is ok, it's used to load generated names such as artifact
                    // names
                    )
...
void JsonTranslationInputCheck::check(const MatchFinder::MatchResult& Result) {
    ...
    if (jsonInputCall && translationCall) {
        const FunctionDecl* translationFunc = Result.Nodes.getNodeAs<FunctionDecl>(
            "translationFunc");
        if (translationFunc) {
            diag(translationCall->getBeginLoc(),
            ...
            diag(jsonInputCall->getBeginLoc(), "value read from json", DiagnosticIDs::Note);
        } else {
            diag(translationCall->getBeginLoc(),
                 "read translation directly instead of constructing it from "
                 "json strings to ensure consistent format in json.");
            diag(jsonInputCall->getBeginLoc(), "string read from json", DiagnosticIDs::Note);
        }
    }
}

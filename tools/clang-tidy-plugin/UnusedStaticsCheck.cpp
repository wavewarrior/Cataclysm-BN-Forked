...
void UnusedStaticsCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        varDecl(anyOf(hasParent(namespaceDecl()), hasParent(translationUnitDecl())),
                hasStaticStorageDuration())
            .bind("decl"),
        this);
    Finder->addMatcher(declRefExpr(to(varDecl().bind("decl"))).bind("ref"), this);
}
...
void UnusedStaticsCheck::check(const MatchFinder::MatchResult& Result) {
    ...
    if (Ref) { used_decls_.insert(ThisDecl); }
    ...
    if (ThisDecl->getPreviousDecl()) { return; }

    // Ignore cases that are not static linkage
    Linkage Lnk = ThisDecl->getFormalLinkage();
    if (Lnk != Linkage::Internal && Lnk != Linkage::UniqueExternal) { return; }
...
void UnusedStaticsCheck::onEndOfTranslationUnit() {
    for (const VarDecl* V : decls_) {
        if (used_decls_.count(V)) { continue; }

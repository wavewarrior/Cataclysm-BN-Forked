...
static auto isRefactorableExpr(const std::string& member_) {
    return ignoringParenCasts(
        anyOf(memberExpr(member(hasName(member_))), binaryOperator(), cxxOperatorCallExpr()));
}
...
void UsePointArithmeticCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        cxxConstructExpr(
            ...
            testWhetherConstructingTemporary(), argumentCountIs(2),
...
struct ExpressionComponent {
    ...
    ExpressionComponent(
        const MatchFinder::MatchResult& Result, const Expr* E, const CXXRecordDecl* MemberOf,
        bool IsArrowRef)
        : objectRef(getText(Result, E)),
          coefficient(1),
          isMember(true),
...
static std::vector<ExpressionComponent> decomposeExpr(
    ...
    switch (E->getStmtClass()) {
        case Stmt::BinaryOperatorClass: {
            ...
            std::vector<ExpressionComponent> Lhs = decomposeExpr(Binary->getLHS(), Member, Result);
            std::vector<ExpressionComponent> Rhs = decomposeExpr(Binary->getRHS(), Member, Result);
            if (Lhs.empty() || Rhs.empty()) { return {}; }
            if (!anyHaveMember(Lhs) && !anyHaveMember(Rhs)) { return {{Result, E}}; }
            ...
            switch (Binary->getOpcode()) {
                case BO_Add:
                    ...
                    return Lhs;
                case BO_Sub:
                    for (ExpressionComponent& Component : Rhs) { Component.coefficient *= -1; }
                    Lhs.insert(Lhs.end(), Rhs.begin(), Rhs.end());
                    return Lhs;
                case BO_Mul:
        ...
        case Stmt::CXXOperatorCallExprClass: {
            ...
            if (Op->getNumArgs() == 1) {
                ...
                std::vector<ExpressionComponent> Lhs = decomposeExpr(Op->getArg(0), Member, Result);
                std::vector<ExpressionComponent> Rhs = decomposeExpr(Op->getArg(1), Member, Result);
                if (Lhs.empty() || Rhs.empty()) { return {}; }
                switch (Op->getOperator()) {
                    case OO_Plus:
                        Lhs.insert(Lhs.end(), Rhs.begin(), Rhs.end());
        ...
        case Stmt::ParenExprClass: {
            ...
            if (!anyHaveMember(result)) { return {{Result, E}}; }
            return result;
        }
        case Stmt::UnaryOperatorClass: {
            ...
            if (Inner.empty()) { return {}; }
            if (!anyHaveMember(Inner)) { return {{Result, E}}; }

            switch (Unary->getOpcode()) {
                case UO_Minus:
                    for (ExpressionComponent& Component : Inner) { Component.coefficient *= -1; }
                    return Inner;
                default:
                    return {};
            }
...
// (second) a tripoint.
static const std::unordered_map<std::string, std::pair<std::string, std::string>> MemberAccessor = {
    {"x", {".x", ".x"}},   {"y", {".y", ".y"}}, {"z", {".z", ".z"}},
    {"xy", {"", ".xy()"}}, {"xyz", {"", ""}},
};
...
static void appendComponent(
    std::string& Result, const ExpressionComponent& Component, const std::string& MemberKey) {
...
static void appendMismatchedComponent(
    std::string& Result, const std::map<std::string, std::vector<ExpressionComponent>>& Components,
...
static void CheckConstructor(
    UsePointArithmeticCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (!ConstructorCall || !ConstructorDecl) { return; }

    std::map<std::string, std::vector<ExpressionComponent>> Components;

    auto insertComponentsIf = [&](const Expr* E, const char* Key) {
        if (E) { Components.insert({Key, consolidateComponents(decomposeExpr(E, Key, Result))}); }
    };
    ...
    if (isPointMethod(getContainingFunction(Result, ConstructorCall))) { return; }
    ...
    for (auto const& Component : Components) {
        if (Component.second.size() > 1) { AnyWithMultipleComponents = true; }
        Positions.insert({Component.first, Component.second.begin()});
    }

    if (!AnyWithMultipleComponents) { return; }

    while (true) {
        bool XYAtEnd = false;
        for (auto const& Component : Components) {
            if (Component.first == "z") { continue; }
        ...
        for (auto const& Position : Positions) {
            const std::string& Key = Position.first;
            if (Position.second != Components.at(Key).end() && *Position.second < *CurrentMin) {
                MinKey = Key;
        ...
        for (auto const& Position : Positions) {
            ...
            if (!AtEnd && *Position.second < *CurrentMin) {
                ...
                AllEqual = false;
                if (Key != "z") { XYEqual = false; }
            } else {
                if (Key == "z") { HasZ = true; }
                if (Position.second->coefficientInvolvesMacro) { InvolvesMacro = true; }
                if (Position.second->coefficient != AnyCoefficient) { CoefficientsEqual = false; }
            }
        }
        ...
        if (CurrentMin->isMember && XYEqual) {
            ...
            if (CoefficientsEqual) {
                ...
                if (AllEqual && HasZ) {
                    appendMismatchedComponent(Joined, Components, Positions, *CurrentMin, "xyz");
                } else {
    ...
    std::set<std::string> Keys;
    for (const auto& Component : Components) { Keys.insert(Component.first); }
    ...
    if (!AccumulatedLeftovers.empty()) { Joined += " + " + AccumulatedLeftovers; }

    const unsigned int LastArg = ConstructorCall->getNumArgs() - 1;
    SourceRange SourceRangeToReplace(
        ConstructorCall->getArg(0)->getBeginLoc(), ConstructorCall->getArg(LastArg)->getEndLoc());

    if (TempParent) { SourceRangeToReplace = ConstructorCall->getSourceRange(); }
    ...
    Check.diag(ConstructorCall->getBeginLoc(),
        ...
        << ConstructorDecl->getParent() << FixItHint::CreateReplacement(CharRangeToReplace, Joined);
}

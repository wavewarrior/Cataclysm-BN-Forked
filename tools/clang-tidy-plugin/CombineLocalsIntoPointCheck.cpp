...
void CombineLocalsIntoPointCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(varDecl(hasType(isInteger()), isXParam()).bind("xdecl"), this);
    Finder->addMatcher(
        declRefExpr(hasDeclaration(varDecl(hasType(isInteger())).bind("decl"))).bind("declref"),
        this);
}
...
static void CheckDecl(CombineLocalsIntoPointCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (StringRef(XDecl->getType().getAsString()).ends_with("ool")) { return; }

    if (XDecl->getType().getTypePtr()->isUnsignedIntegerType()) { return; }

    NameConvention NameMatcher(XDecl->getName());

    if (!NameMatcher || NameMatcher.Match(YDecl->getName()) != NameConvention::YName) { return; }

    if (ZDecl && NameMatcher.Match(ZDecl->getName()) != NameConvention::ZName) { ZDecl = nullptr; }

    auto GetGrandparent = [&](const Decl* D) -> const Stmt* {
        const Stmt* ParentStmt = getParent<Stmt>(Result, D);
        if (!ParentStmt) { return nullptr; }
    ...
    if (XGrandparentStmt != YGrandparentStmt) { return; }
    ...
    auto FuncInsert =
        Check.alteredFunctions.emplace(Func, FunctionReplacementData{XDecl, YDecl, ZDecl, {}, {}});
    if (!FuncInsert.second) { return; }
    FunctionReplacementData& FuncReplacement = FuncInsert.first->second;

    if (ZDecl) {
        const Stmt* ZGrandparentStmt = GetGrandparent(ZDecl);

        if (XGrandparentStmt != ZGrandparentStmt) { ZDecl = nullptr; }
    }
    ...
    if (!XInit || !YInit || (ZDecl && !ZInit)) { return; }

    std::string NewVarName = NameMatcher.getRoot();

    if (!NewVarName.empty() && NewVarName.front() == '_') { NewVarName.erase(NewVarName.begin()); }

    if (!NewVarName.empty() && NewVarName.back() == '_') { NewVarName.pop_back(); }

    if (NewVarName.empty()) { NewVarName = "p"; }

    if (!isalpha(NewVarName.front())) { NewVarName = "p" + NewVarName; }

    // Ensure we don't use a keyword
    if (isKeyword(NewVarName)) { NewVarName += "_"; }
    ...
    if (ZDecl) { Check.usageReplacements.emplace(ZDecl, NewVarName + ".z"); }

    // Construct replacement text
    std::string Replacement =
        ("point " + NewVarName + "( " + getText(Result, XInit) + ", " + getText(Result, YInit))
            .str();
    if (ZDecl) { Replacement = ("tri" + Replacement + ", " + getText(Result, ZInit)).str(); }
    Replacement += " )";
    ...
    if (XDecl->isStaticLocal()) { Replacement = "static " + Replacement; }
...
static void CheckDeclRef(
    CombineLocalsIntoPointCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (!DeclRef || !Decl) { return; }

    auto NewName = Check.usageReplacements.find(Decl);
    if (NewName == Check.usageReplacements.end()) { return; }

    if (getText(Result, DeclRef) != Decl->getName()) { return; }
    ...
    Replacement.RefReplacements.push_back(DeclRefReplacementData{DeclRef, Decl, NewName->second});
    ...
    for (const std::pair<const FunctionDecl* const, FunctionReplacementData>& p :
                ...
                : "Variables %0 and %1 could be combined into a single 'point' variable.";
        {
            DiagnosticBuilder Diag =
                diag(data.XDecl->getBeginLoc(), Message) << data.XDecl << data.YDecl << data.ZDecl;
            for (const FixItHint& FixIt : data.Fixits) { Diag << FixIt; }
        }
        diag(data.YDecl->getLocation(), "%0 variable", DiagnosticIDs::Note) << data.YDecl;
        if (data.ZDecl) {
            diag(data.ZDecl->getLocation(), "%0 variable", DiagnosticIDs::Note) << data.ZDecl;
        }
        for (const DeclRefReplacementData& RefReplacment : data.RefReplacements) {
            diag(RefReplacment.DeclRef->getBeginLoc(), "Update %0 to '%1'.", DiagnosticIDs::Note)

...
void NoStaticGettextCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        callExpr(
            ...
            callee(functionDecl(hasAnyName("_", "gettext", "pgettext", "vgettext", "vpgettext"))))
            .bind("gettextCall"),
        this);
}

void NoStaticGettextCheck::check(const MatchFinder::MatchResult& Result) {
    const CallExpr* gettextCall = Result.Nodes.getNodeAs<CallExpr>("gettextCall");
    if (!gettextCall) { return; }
    diag(

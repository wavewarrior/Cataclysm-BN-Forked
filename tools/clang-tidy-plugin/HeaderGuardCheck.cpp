...
CataHeaderGuardCheck::CataHeaderGuardCheck(StringRef Name, ClangTidyContext* Context)
...
public:
    ...
    void FileChanged(
        SourceLocation Loc, FileChangeReason Reason, SrcMgr::CharacteristicKind FileType,
        FileID) override {
    ...
    void MacroDefined(const Token& MacroNameTok, const MacroDirective* MD) override {
        ...
        FileInfos[GetFileName(Loc)].Macros.push_back(MacroInfo_{MacroNameTok, MD->getMacroInfo()});
    }
    ...
    void EndOfMainFile() override {
        ...
        for (const std::string& FileName : Files) {
            if (!isHeaderFileName(FileName)) { continue; }

            const FileInfo& Info = FileInfos[FileName];

            if (Info.Macros.empty() || Info.Ifndefs.empty() || Info.EndIfs.empty()) { continue; }
            ...
            if (EndifIt == Info.EndIfs.end()) { continue; }
            ...
            std::string NewGuard = checkHeaderGuardDefinition(
                Ifndef.MacroNameLoc, DefineLoc, FileName, CurHeaderGuard, FixIts);
    ...
    bool wouldFixEndifComment(
        ...
        if (FindEscapedNewline != StringRef::npos && EndIfStr[FindEscapedNewline] == '\\') {
            return false;
        }
    ...
    static std::string checkHeaderGuardDefinition(
        ...
        if (Ifndef.isValid() && CurHeaderGuard != CPPVar) {
            FixIts.push_back(FixItHint::CreateReplacement(
                CharSourceRange::
                    getTokenRange(Ifndef, Ifndef.getLocWithOffset(CurHeaderGuard.size())),
                CPPVar));
            FixIts.push_back(FixItHint::CreateReplacement(
                CharSourceRange::
                    getTokenRange(Define, Define.getLocWithOffset(CurHeaderGuard.size())),
                CPPVar));
            return CPPVar;
        }
    ...
    void checkEndifComment(
        SourceLocation EndIf, StringRef HeaderGuard, std::vector<FixItHint>& FixIts) {
        size_t EndIfLen;
    ...
    void checkGuardlessHeaders(std::unordered_set<std::string> GuardlessHeaders) {
        ...
        for (const std::string& FileName : GuardlessHeaders) {
            ...
            if (StartLoc.isInvalid()) { continue; }

            std::string CPPVar = getHeaderGuard(FileName);
            if (CPPVar.empty()) { continue; }
            ...
            for (const auto& MacroEntry : Info.Macros) {
                ...
                if (Name == CPPVar && SM.isWrittenInSameFile(StartLoc, DefineLoc)) {
                    Check->diag(
                        DefineLoc,
            ...
            if (SeenMacro) { continue; }

            SourceLocation InsertLoc = StartLoc;

            if (Info.PragmaOnce.isValid()) {
                const char* PragmaData = PP->getSourceManager().getCharacterData(Info.PragmaOnce);

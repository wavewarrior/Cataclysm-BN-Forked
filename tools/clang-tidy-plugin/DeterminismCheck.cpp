...
void DeterminismCheck::registerMatchers(MatchFinder* Finder) {
    Finder->addMatcher(
        callExpr(
            callee(functionDecl(anyOf(
                hasName("::rand"), hasName("::srand"), hasName("::random"), hasName("::srandom"),
                hasName("::drand48"), hasName("::erand48"), hasName("::lrand48"),
                hasName("::mrand48"), hasName("::nrand48"), hasName("::jrand48"),
                hasName("::seed48"), hasName("::srand48")))))
            .bind("call"),
        this);
    ...
    const TypeMatcher IsRngTypedefDecl = typedefNameDecl(
        hasType(qualType(hasDeclaration(IsRngCoreDecl))));
    const TypeMatcher IsRngTypedef2Decl = typedefNameDecl(
        hasType(qualType(hasDeclaration(IsRngTypedefDecl))));
...
static void CheckInit(DeterminismCheck& Check, const MatchFinder::MatchResult& Result) {
    ...
    if (!FoundResultType) { return; }
    const QualType ConstructedType = Construction->getType();
    Check.diag(Construction->getBeginLoc(),

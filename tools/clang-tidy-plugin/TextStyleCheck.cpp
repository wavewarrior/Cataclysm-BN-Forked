    ...
    Finder->addMatcher(
        stringLiteral(
            ...
            unless(hasAncestor(callExpr(callee(
                functionDecl(hasAnyName("formatted_set_simple", "ter_bind", "furn_bind")))))),
            // __func__, etc
...
void TextStyleCheck::check(const MatchFinder::MatchResult& Result) {
    ...
    if ((!text.isWide() && !text.isUTF8()) || text.getCharByteWidth() != 1) { return; }
    ...
    const auto location =
        [&SrcMgr, &LangOpts, &Info](const StringLiteralIterator& it) -> SourceLocation {
        return SrcMgr.getSpellingLoc(it.toSourceLocation(SrcMgr, LangOpts, Info));
    };

    const auto text_style_check_callback =
        [this, fixit, location](
            const text_style_fix type, const std::string& msg, const StringLiteralIterator& /*beg*/,
            const StringLiteralIterator& /*end*/, const StringLiteralIterator& at,
                ...
                switch (type) {
                    case text_style_fix::removal: {
                        const CharSourceRange range =
                            CharSourceRange::getCharRange(location(from), location(to));
    ...
    text_style_check<StringLiteralIterator>(
        beg, end,
        ...
        EscapeUnicode ? escape_unicode::yes : escape_unicode::no, text_style_check_callback);
}

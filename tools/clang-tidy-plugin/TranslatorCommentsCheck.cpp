...
AST_POLYMORPHIC_MATCHER_P2(
    hasImmediateArgument, AST_POLYMORPHIC_SUPPORTED_TYPES(CallExpr, CXXConstructExpr), unsigned int,
    N, internal::Matcher<Expr>, InnerMatcher) {
...
public:
    TranslatorCommentsHandler(TranslatorCommentsCheck& Check)
        : Check(Check),
    ...
    bool HandleComment(Preprocessor& PP, SourceRange Range) override {
        if (Check.MatchingStarted) {
            ...
            Check.diag(
                Range.getBegin(), "AST Matching started before the end of comment preprocessing",
                DiagnosticIDs::Error);
        }

        const SourceManager& SM = PP.getSourceManager();
        StringRef Text =
            Lexer::getSourceText(CharSourceRange::getCharRange(Range), SM, PP.getLangOpts());

        if (!Match.match(Text)) { return false; }
        ...
        TranslatorComments.emplace(
            TranslatorCommentLocation{File, EndLine, EndCol},
...
public:
    ...
    void MacroExpands(
        const Token& MacroNameTok, const MacroDefinition&, SourceRange Range,
        ...
        if (RawStringInd >= Args->getNumMacroArguments()) {
            Check.diag(
                Range.getBegin(), "Translation marker doesn't have expected number of arguments",
                DiagnosticIDs::Error);
        }
            ...
            for (; Tok->isNot(tok::eof); ++Tok) {
                if (!tok::isStringLiteral(Tok->getKind())) {
                    if (is_marker) {
                        Check.diag(
                            Tok->getLocation(),
                            "Translation marker macros only accepts string literal arguments");
                    }
                    return;
                }
            }
...
void TranslatorCommentsCheck::registerMatchers(MatchFinder* Finder) {
    const auto stringLiteralArgumentBound = anyOf(
        stringLiteral().bind("RawText"),
        cxxConstructExpr(unless(isListInitialization()),
                         hasImmediateArgument(0, stringLiteral().bind("RawText"))));
    const auto stringLiteralArgumentUnbound = anyOf(
        stringLiteral(),
        cxxConstructExpr(unless(isListInitialization()), hasImmediateArgument(0, stringLiteral())));
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasAnyName("_", "gettext"))),
                 hasImmediateArgument(0, stringLiteralArgumentBound)),
        this);
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasName("vgettext"))),
                 hasImmediateArgument(0, stringLiteralArgumentBound),
                 hasImmediateArgument(1, stringLiteralArgumentUnbound)),
        this);
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasName("to_translation"))), argumentCountIs(1),
                 hasImmediateArgument(0, stringLiteralArgumentBound)),
        this);
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasName("pl_translation"))), argumentCountIs(2),
                 hasImmediateArgument(0, stringLiteralArgumentBound),
                 hasImmediateArgument(1, stringLiteralArgumentUnbound)),
        this);
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasAnyName("pgettext"))),
                 hasImmediateArgument(0, stringLiteralArgumentUnbound),
                 hasImmediateArgument(1, stringLiteralArgumentBound)),
        this);
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasAnyName("vpgettext"))),
                 hasImmediateArgument(0, stringLiteralArgumentUnbound),
                 hasImmediateArgument(1, stringLiteralArgumentBound),
                 hasImmediateArgument(2, stringLiteralArgumentUnbound)),
        this);
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasName("to_translation"))), argumentCountIs(2),
                 hasImmediateArgument(0, stringLiteralArgumentUnbound),
                 hasImmediateArgument(1, stringLiteralArgumentBound)),
        this);
    Finder->addMatcher(
        callExpr(callee(functionDecl(hasName("pl_translation"))), argumentCountIs(3),
                 hasImmediateArgument(0, stringLiteralArgumentUnbound),
                 hasImmediateArgument(1, stringLiteralArgumentBound),
                 hasImmediateArgument(2, stringLiteralArgumentUnbound)),
        this);
    Finder->addMatcher(stringLiteral(isMarkedString(this)).bind("RawText"), this);
}

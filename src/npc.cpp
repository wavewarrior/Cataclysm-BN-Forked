...
standard_npc::standard_npc(
    const std::string& name, const tripoint_bub_ms& pos, const std::vector<std::string>& clothing,
    int sk_lvl, int s_str, int s_dex, int s_int, int s_per) {
    this->name = name;
    // Resolve tripoint_min sentinel to the runtime bubble center.
    position =
        (pos == tripoint_bub_ms::min())
    ...
    for (const Skill& e : Skill::skills) { set_skill_level(e.ident(), std::max(sk_lvl, 0)); }

    for (const std::string& e : clothing) { wear_item(item::spawn(e)); }

    for (item* const& e : worn) {
        if (e->has_flag(flag_VARSIZE)) { e->set_flag(flag_FIT); }
    }
}
...
void npc_template::load(const JsonObject& jsobj) {
    ...
    if (jsobj.has_string("faction")) { guy.set_fac_id(jsobj.get_string("faction")); }
...
void npc_template::reset() { npc_templates.clear(); }

void npc_template::check_consistency() {
    for (const auto& e : npc_templates) {
        const auto& guy = e.second.guy;
        if (!guy->myclass.is_valid()) { debugmsg("Invalid NPC class %s", guy->myclass.c_str()); }
    }
}

template <> bool string_id<npc_template>::is_valid() const { return npc_templates.contains(*this); }

template <> const npc_template& string_id<npc_template>::obj() const {
...
void npc::load_npc_template(const string_id<npc_template>& ident) {
    ...
    randomize(myclass);
    if (!tem.name_unique.empty()) { name = tem.name_unique.translated(); }
    ...
    if (is_shopkeeper()) {
        ...
        if (from != item_group_id("EMPTY_GROUP")) {
            inv_clear();
            for (detached_ptr<item>& it : item_group::items_from(from)) { i_add(std::move(it)); }
        }
    }
...
void npc::randomize(const npc_class_id& type) {
    if (!getID().is_valid()) { setID(g->assign_npc_id()); }
    ...
    for (const trait_id& tid : trait_group::traits_from(myclass->traits, male)) {
        if (!has_trait(tid)) { toggle_trait(tid); }
    }

    // Run mutation rounds
    for (const auto& mr : type->mutation_rounds) {
        int rounds = mr.second.roll();
        for (int i = 0; i < rounds; ++i) { mutate_category(mr.first); }
    }
    ...
    for (const auto& bl : type->bionic_list) {
        ...
        if (rng(0, 100) <= chance) { add_bionic(bl.first); }
    }
    ...
    for (std::pair<spell_id, int> spell_pair : type->_starting_spells) {
        ...
        while (sp.get_level() < spell_pair.second && !sp.is_max_level()) { sp.gain_level(); }
    }
}
...
void npc::set_fac(const faction_id& id) {
    if (my_fac) { my_fac->remove_member(getID()); }
...
void npc::apply_ownership_to_inv() {
    for (auto& e : inv_dump()) { e->set_owner(*this); }
}
...
static detached_ptr<item> random_item_from(
    const npc_class_id& type, const std::string& what, const item_group_id& fallback) {
    auto result = item_group::item_from(item_group_id(type.str() + "_" + what), calendar::turn);
    if (!result || result->is_null()) { result = item_group::item_from(fallback, calendar::turn); }
    return result;
}
...
static detached_ptr<item> get_clothing_item(
    const npc_class_id& type, const std::string& what, bool male) {
...
void starting_clothes(npc& who, const npc_class_id& type, bool male) {
    ...
    for (detached_ptr<item>& it : ret) {
        if (!it) { continue; }
        if (it->has_flag(flag_VARSIZE)) { it->set_flag(flag_FIT); }
...
void starting_inv(npc& who, const npc_class_id& type) {
    ...
    if (who.primary_weapon().is_gun()) {
        detached_ptr<item> ammo = item::in_its_container(
            item::spawn(who.primary_weapon().ammo_default()));
    ...
    if (type == NC_ARSONIST) { res.emplace_back(item::spawn("molotov")); }
    ...
    while (qty-- != 0) {
        ...
        if (tmp && !tmp->is_null()) {
            if (!one_in(3) && tmp->has_flag(flag_VARSIZE)) { tmp->set_flag(flag_FIT); }
            if (who.can_pick_volume(*tmp)) { res.push_back(std::move(tmp)); }
        }
    }
...
void npc::set_activity_failure_message(const std::string& msg) { activity_failure_message = msg; }
...
std::string npc::peek_activity_failure_message() const { return activity_failure_message; }

npc_mission npc::get_previous_mission() { return previous_mission; }

npc_attitude npc::get_previous_attitude() { return previous_attitude; }

bool npc::get_known_to_u() { return known_to_u; }

void npc::set_known_to_u(bool known) {
    known_to_u = known;
    if (my_fac) { my_fac->add_to_membership(getID(), disp_name(), known_to_u); }
}

void npc::setpos(const tripoint_bub_ms& pos) { setpos(get_map().bub_to_abs(pos)); }
...
void npc::setpos(const tripoint_abs_ms& new_pos) {
    ...
    if (auto* _log = coop_mutation_log::current()) {
        _log->push({coop_event_type::creature_moved, abs_pos(), 0, getID().get_value()});
    }
#endif
}
...
void npc::travel_overmap(const tripoint_abs_sm& pos) {
    ...
    spawn_at_sm(pos);
    if (abs_omt_pos() == goal) { reach_omt_destination(); }
...
void npc::spawn_at_sm(const tripoint_abs_sm& p) {
    spawn_at_precise(p.xy(), tripoint_sm_ms(rng(0, SEEX - 1), rng(0, SEEY - 1), p.z()));
}
...
void npc::place_on_map() {
    ...
    if (g->is_empty(initial) || is_mounted()) { return; }
...
const std::map<skill_id, std::string> skill_to_weapons =
    {{skill_bashing, "bashing"}, {skill_cutting, "cutting"},  {skill_unarmed, "unarmed"},
     {skill_throw, "throw"},     {skill_archery, "archery"},  {skill_launcher, "launcher"},
     {skill_pistol, "pistol"},   {skill_shotgun, "shotgun"},  {skill_smg, "smg"},
     {skill_rifle, "rifle"},     {skill_stabbing, "stabbing"}};
...
bool npc::can_read(const item& book, std::vector<std::string>& fail_reasons) {
    if (!book.is_book()) {
        fail_reasons.push_back(
            string_format(_("This %s is not good reading material."), book.tname()));
        return false;
    }
...
int npc::time_to_read(const item& book, const Character& reader) const {
    const auto& type = book.type->book;
    const skill_id& skill = type->skill;
    // The reader's reading speed has an effect only if they're trying to understand the book as
    // they read it Reading speed is assumed to be how well you learn from books (as opposed to
    // hands-on experience)
    const bool try_understand =
        character_funcs::is_fun_to_read(reader, book)
    ...
    retval *= std::
        min(character_funcs::fine_detail_vision_mod(*this),
...
void npc::finish_read(item* it) {
    ...
    if (skill && get_skill_level(skill) < reading->level
        ...
        if (has_active_bionic(bio_memory)) { min_ex += 2; }
        if (max_ex < 2) { max_ex = 2; }
        if (max_ex > 10) { max_ex = 10; }
        if (max_ex < min_ex) { max_ex = min_ex; }
        ...
        if ((skill_level == reading->level || !skill_level.can_train())
            || ((has_trait(trait_SCHIZOPHRENIC) || has_artifact_with(AEP_SCHIZO)) && one_in(25))) {
            if (display_messages) {
                add_msg(m_info, _("%s can no longer learn from %s."), disp_name(),
                        book.type_name());
            }
        }
    } else if (skill) {
...
void npc::do_npc_read() {
    ...
    if (loc) {
        ...
        if (can_read(*loc, fail_reasons)) {
            if (g->u.sees(bub_pos())) { add_msg(m_info, _("%s starts reading."), disp_name()); }
            start_read(*loc, pl);
        } else {
            for (const auto& elem : fail_reasons) { say(elem); }
        }
    } else {
...
void npc::do_npc_craft(const std::optional<tripoint>&) {
    ...
    if (menu.ret == 0) {
        ...
        visit_items([&](item* node) {
            if (node->is_craft()) { found.push_back({node, bub_pos().raw(), true}); }
            return VisitResponse::NEXT;
        });

        map& here = get_map();
        for (const auto& adj : here.points_in_radius(bub_pos(), PICKUP_RANGE)) {
            if (!here.inbounds(adj) || here.dangerous_field_at(adj)) { continue; }
            for (item* itm : here.i_at(adj)) {
                if (itm->is_craft()) { found.push_back({itm, adj.raw(), false}); }
            }
        }
        ...
        for (size_t i = 0; i < found.size(); ++i) {
            const std::string where = found[i].in_inventory ? _("inventory") : _("ground");
            pick.addentry(
                static_cast<int>(i), true, MENU_AUTOASSIGN, "%s (%s)", found[i].it->tname(), where);
        }
        pick.query();
        if (pick.ret >= 0 && pick.ret < static_cast<int>(found.size())) {
            const resume_entry& sel = found[pick.ret];
            item* target = sel.it;
            auto target_pos = tripoint_bub_ms(sel.pos);

            if (!sel.in_inventory && can_pick_volume(*sel.it) && can_pick_weight(*sel.it, false)) {
                detached_ptr<item> det = here.i_rem(target_pos, sel.it);
                if (det) {
...
detached_ptr<item> npc::wear_if_wanted(detached_ptr<item>&& it, std::string& reason) {
    ...
    if (it->has_flag(flag_SPLINT)) {
        for (const bodypart_id& bp : get_all_body_parts(true)) {
            if (is_limb_broken(bp.id()) && !worn_with_flag(flag_SPLINT, bp.id())
                && it->covers(bp.id())) {
...
detached_ptr<item> npc::wield(detached_ptr<item>&& target) {
    if (!can_wield(*target).success()) { return std::move(target); }
    clear_npc_ai_info_cache(npc_ai_info::ideal_weapon_value);

    if (!unwield()) { return std::move(target); }
    if (!target || target->is_null()) { return std::move(target); }
    item& obj = *target;
...
void npc::drop(const drop_locations& what, const tripoint_bub_ms& target, bool stash) {
...
void npc::invalidate_range_cache() {
    if (primary_weapon().is_gun()) {
        confident_range_cache = confident_shoot_range(
            primary_weapon(), ranged::get_most_accurate_sight(*this, primary_weapon()));
    } else {
...
void npc::form_opinion(const Character& u) {
    ...
    // is your health low
    for (const std::pair<const bodypart_str_id, bodypart>& elem :
         get_player_character().get_body()) {
        const int hp_max = elem.second.get_hp_max();
        const int hp_cur = elem.second.get_hp_cur();
        if (hp_cur <= hp_max / 2) { op_of_u.fear--; }
    }
    ...
    if (u.has_trait(trait_TERRIFYING)) { op_of_u.fear += 6; }

    int u_ugly = 0;
    for (trait_id& mut : u.get_mutations()) { u_ugly += mut.obj().ugliness; }
    op_of_u.fear += u_ugly / 2;
    op_of_u.trust -= u_ugly / 3;

    if (u.get_stim() > 20) { op_of_u.fear++; }

    if (u.has_effect(effect_drunk)) { op_of_u.fear -= 2; }
    ...
    // TODO: More effects
    if (u.has_effect(effect_drunk)) { op_of_u.trust -= 2; }
    if (u.get_stim() > 20 || u.get_stim() < -20) { op_of_u.trust -= 1; }
    if (u.get_painkiller() > 30) { op_of_u.trust -= 1; }
    ...
    op_of_u.value = 0;
    for (const std::pair<const bodypart_str_id, bodypart>& elem : get_body()) {
        if (elem.second.get_hp_cur() < elem.second.get_hp_max() * 0.8f) { op_of_u.value++; }
    }
    decide_needs();
...
void npc::mutiny() {
    if (!my_fac || !is_player_ally()) { return; }
...
float npc::vehicle_danger(int radius) const {
    ...
    for (size_t i = 0; i < vehicles.size(); ++i) {
        ...
        if (wrapped_veh.v->is_moving()) {
            ...
            double normal = std::sqrt(
                static_cast<float>((b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y)));
            int closest = static_cast<int>(
                std::abs((bub_pos().x() - a.x) * (b.y - a.y) - (bub_pos().y() - a.y) * (b.x - a.x))
                / normal);

            if (size > closest) { danger = i; }
        }
    }
    return danger;
}

bool npc::turned_hostile() const { return (op_of_u.anger >= hostile_anger_level()); }

int npc::hostile_anger_level() const { return (20 + op_of_u.fear - personality.aggression); }
...
void npc::on_attacked(const Creature& attacker) {
    if (is_hallucination()) { die(nullptr); }
...
int npc::assigned_missions_value() {
    int ret = 0;
    for (auto& m : chatbin.missions_assigned) { ret += m->get_value(); }
    return ret;
}
...
std::vector<skill_id> npc::skills_offered_to(const Character& p) const {
    ...
    for (const auto& pair : *_skills) {
        ...
        if (p.get_skill_level(id) < pair.second.level()) { ret.push_back(id); }
    }
    return ret;
}
...
void npc::decide_needs() {
    ...
    for (auto& elem : needrank) { elem = 20; }
    if (primary_weapon().is_gun()) {
        ...
        if (ups_drain > 0) {
            ...
            needrank[need_ammo] =
                character_funcs::
                    get_ammo_items(*this, ammotype(*primary_weapon().type->gun->ammo.begin()))
                        .size();
        }
        needrank[need_ammo] *= 5;
    }
    if (!base_location) { needrank[need_safety] = 1; }
    ...
    for (int i = 1; i < num_needs; i++) {
        if (needrank[i] < 20) {
            ...
            if (j == needs.size()) { needs.push_back(static_cast<npc_need>(i)); }
        }
    }
}
...
void npc::say(const std::string& line, const sounds::sound_t spriority) const {
    ...
    if (has_trait(trait_MUTE)) { return; }
...
bool npc::wants_to_sell(const item& /*it*/, int at_price, int market_price) const {
    if (mission == NPC_MISSION_SHOPKEEP) { return true; }
...
bool npc::will_exchange_items_freely() const { return is_player_ally(); }
...
void npc::shop_restock() {
    ...
    if (is_player_ally()) { return; }

    const item_group_id& from = myclass->get_shopkeeper_items();
    if (from == item_group_id("EMPTY_GROUP")) { return; }

    units::volume total_space = volume_capacity();
    if (is_shopkeeper()) { total_space = units::from_liter(5000); }
    ...
    // we have items to restock with, so go ahead and pick up everything so we can clear out
    // properly If we don't restock for some reason don't clear out inventory since we'd end up not
    // having anything to trade
    if (!ret.empty()) {
...
void npc::update_worst_item_value() {
    ...
    if (inv_val < worst_item_value) { worst_item_value = inv_val; }
}
...
bool healing_options::all_false() { return !any_true(); }

bool healing_options::any_true() { return bandage || bleed || bite || infect || disinfect; }
...
healing_options npc::has_healing_options(healing_options try_to_fix) {
    ...
    visit_items([&fix_p, try_to_fix](item* node) {
        ...
        if (use == nullptr) { return VisitResponse::NEXT; }
        ...
        if (try_to_fix.bleed && !fix_p->bleed && actor.bleed > 0) { fix_p->bleed = true; }
        if (try_to_fix.bite && !fix_p->bite && actor.bite > 0) { fix_p->bite = true; }
        if (try_to_fix.infect && !fix_p->infect && actor.infect > 0) { fix_p->infect = true; }
        // if we've found items for everything we're looking for, we're done
        if ((!try_to_fix.bandage || fix_p->bandage) && (!try_to_fix.disinfect || fix_p->disinfect)
            && (!try_to_fix.bleed || fix_p->bleed) && (!try_to_fix.bite || fix_p->bite)
...
item& npc::get_healing_item(healing_options try_to_fix, bool first_best) {
    ...
    visit_items([&best, try_to_fix, first_best](item* node) {
        ...
        if ((try_to_fix.bandage && actor.bandages_power > 0.0f)
            ...
            || (try_to_fix.bleed && actor.bleed > 0) || (try_to_fix.bite && actor.bite > 0)
            || (try_to_fix.infect && actor.infect > 0)) {
            best = node;
            if (first_best) { return VisitResponse::ABORT; }
        }
...
bool npc::has_painkiller() { return inv.has_enough_painkiller(get_pain()); }

bool npc::took_painkiller() const {
    return (has_effect(effect_pkill1) || has_effect(effect_pkill2) || has_effect(effect_pkill3)
    ...
    if (p.is_player()) {
        ...
        if (faction_api_version < 2) {
            ...
            if (attitude == NPCATT_FOLLOW || attitude == NPCATT_LEAD || attitude == NPCATT_WAIT
                ...
                || mission == NPC_MISSION_GUARD_ALLY || has_companion_mission()) {
                return true;
            }
        }
    } else {
...
bool npc::is_player_ally() const { return is_ally(g->u); }

bool npc::is_friendly(const Character& p) const {
    return is_ally(p) || (p.is_player() && (is_walking_with() || is_player_ally()));
}

bool npc::is_minion() const { return is_player_ally() && op_of_u.trust >= 5; }
...
bool npc::is_leader() const { return attitude == NPCATT_LEAD; }
...
bool npc::is_patrolling() const { return mission == NPC_MISSION_GUARD_PATROL; }
...
void npc::npc_dismount() {
    ...
    for (const auto& elem : g->m.points_in_radius(bub_pos(), 1)) {
        if (g->is_empty(elem)) {
            pnt = elem;
            break;
        }
...
float npc::danger_assessment() { return ai_cache.danger_assessment; }

float npc::average_damage_dealt() {
    return static_cast<float>(npc_ai::melee_value(*this, primary_weapon()));
}

bool npc::bravery_check(int diff) { return (dice(10 + personality.bravery, 6) >= dice(diff, 4)); }

bool npc::emergency() const { return emergency(ai_cache.danger_assessment); }
...
bool npc::emergency(float danger) const {
    ...
    if (20 + std::max(danger, 0.0f) > stamina_percent + personality.bravery) { return true; }
...
bool npc::is_active() const { return g->critter_at<npc>(bub_pos()) == this; }
...
std::string npc::print_info_text() const {
    ...
    std::string head = colorize(bar.first, bar.second) + " " + colorize(name, basic_symbol_color());
    const std::string att_goal = npc_attitude_name(get_attitude());
    if (!att_goal.empty()) { head += "  " + colorize(att_goal, basic_symbol_color()); }
    ...
    const std::string senses_str = sees(g->u) ? _("Aware of your presence") : _("Unaware of you");
    lines.emplace_back(
        colorize(senses_str, sees(g->u) ? c_yellow : c_green) + "  "
    ...
    if (is_armed()) {
        lines.emplace_back(
            colorize(_("Wielding: "), c_light_gray) + colorize(primary_weapon().tname(), c_red));
    }
...
static void maybe_shift(tripoint_bub_ms& pos, point_rel_ms d) {
    if (pos != tripoint_bub_ms::min()) { pos = pos + d; }
}
...
bool npc::is_dead() const { return dead || is_dead_state(); }
...
void npc::die(Creature* nkiller) {
    ...
    if (in_vehicle) { g->m.unboard_vehicle(bub_pos(), true); }
    if (is_mounted()) {
...
#ifdef COOP_ENABLED
    if (auto* _log = coop_mutation_log::current()) {
        _log->push({coop_event_type::creature_died, abs_pos(), 0, getID().get_value()});
    }
#endif
    Character::die(nkiller);

    if (is_hallucination()) {
        if (g->u.sees(*this)) { add_msg(_("%s disappears."), name.c_str()); }
        return;
    }

    if (g->u.sees(*this)) { add_msg(_("%s dies!"), name); }
...
template <> std::string io::enum_to_string<npc_attitude>(npc_attitude att) {
    std::string result = npc_attitude_id(att);
    if (result == "NPCATT_INVALID") { abort(); }
    return result;
}
...
// message related stuff
void npc::add_msg_if_npc(const std::string& msg) const { add_msg(replace_with_npc_name(msg)); }
...
void npc::add_new_mission(class mission* miss) { chatbin.add_new_mission(miss); }

void npc::on_unload() { last_updated = calendar::turn; }
...
void npc::on_load() {
    ...
    if (has_trait(trait_HALLUCINATION)) { hallucination = true; }

    cata::run_hooks("on_creature_loaded", [this](sol::table& params) {
        params["creature"] = this;
    });
    cata::run_hooks("on_npc_loaded", [this](sol::table& params) { params["npc"] = this; });
}

void npc_chatbin::add_new_mission(mission* miss) {
    if (miss == nullptr) { return; }
...
bool npc::dispose_item(item& obj, const std::string&) {
        ...
        if (e->can_holster(obj)) {
            ...
            opts.emplace_back(dispose_option{
                item_store_cost(obj, *e, false, ptr->draw_cost), [this, ptr, &e, &obj] {
                    ...
                    g->m.add_item_or_charges(bub_pos(), std::move(failed));
                }});
        }
    }
    ...
    if (volume_carried() + obj.volume() <= volume_capacity()) {
        opts.emplace_back(dispose_option{
            item_handling_cost(obj), [this, &obj] {
    ...
    if (opts.empty()) {
        // Drop it
        g->m.add_item_or_charges(bub_pos(), obj.detach());
        return true;
    }

    const auto mn =
        std::ranges::min_element(opts, [](const dispose_option& lop, const dispose_option& rop) {
...
void npc::process_turn() {
    ...
    if (is_player_ally() && calendar::once_every(1_hours) && get_kcal_percent() > 0.95
        && get_thirst() < thirst_levels::very_thirsty && op_of_u.trust < 5) {
        ...
        int op_penalty =
            std::max(0, op_of_u.anger) + std::max(0, -op_of_u.value) + std::max(0, op_of_u.fear);
        // Being barely hungry and thirsty, not in pain and not wounded means good care
        int state_penalty =
            (max_stored_kcal() - get_stored_kcal()) / 10 + get_thirst() + (100 - hp_percentage())
            + get_pain();
        if (x_in_y(trust_chance, 240 + 10 * op_penalty + state_penalty)) { op_of_u.trust++; }

        // TODO: Similar checks for fear and anger
    }

    // TODO: Add decreasing trust/value/etc. here when player doesn't provide food
    // TODO: Make NPCs leave the player if there's a path out of map and player is
    // sleeping/unseen/etc.
}
...
bool npc::invoke_item(item* used) { return Character::invoke_item(used); }
...
template <> std::string io::enum_to_string<npc_need>(npc_need need) {
...
std::set<tripoint_bub_ms> npc::get_legacy_path_avoid() const {
    ...
    if (rules.has_flag(ally_rule::avoid_doors)) {
        for (const tripoint_bub_ms& p : g->m.points_in_radius(bub_pos(), 30)) {
            if (g->m.can_open_door(this, p, true)) { ret.insert(p); }
        }
    }
    if (rules.has_flag(ally_rule::hold_the_line)) {
        for (const tripoint_bub_ms& p : g->m.points_in_radius(g->u.bub_pos(), 1)) {
            if (g->m.close_door(p, true, true) || g->m.move_cost(p) > 2) { ret.insert(p); }
        }
    }
    return ret;
}

std::pair<PathfindingSettings, RouteSettings> npc::get_pathfinding_pair() const {
    return this->get_pathfinding_pair(false);
}

std::pair<PathfindingSettings, RouteSettings> npc::get_pathfinding_pair(bool no_bashing) const {
    ...
    path_settings.bash_strength_val =
        no_bashing ? 0 : smash_ability() / path_settings.bash_strength_quanta;
...
void npc::set_companion_mission(
    const tripoint_abs_omt& omt_pos, const std::string& role_id, const std::string& mission_id) {
...
void npc::set_mission(npc_mission new_mission) {
    ...
    if (mission == NPC_MISSION_ACTIVITY) { current_activity_id = activity->id(); }
}
...
npc_attitude npc::get_attitude() const { return attitude; }

void npc::set_attitude(npc_attitude new_attitude) {
    if (new_attitude == attitude) { return; }
    previous_attitude = attitude;
    if (new_attitude == NPCATT_FLEE) { new_attitude = NPCATT_FLEE_TEMP; }
    ...
    add_msg(m_debug, "%s changes attitude from %s to %s", name, npc_attitude_id(attitude),
            npc_attitude_id(new_attitude));
...
bool npc::is_hallucination() const { return hallucination; }
...
void npc_follower_rules::enable_override(ally_rule setit) {
    override_enable = static_cast<ally_rule>(
        static_cast<int>(override_enable) | static_cast<int>(setit));
}

void npc_follower_rules::disable_override(ally_rule clearit) {
    override_enable = static_cast<ally_rule>(
        static_cast<int>(override_enable) & ~static_cast<int>(clearit));
}
...
void npc_follower_rules::clear_override(ally_rule clearit) {
    overrides = static_cast<ally_rule>(static_cast<int>(overrides) & ~static_cast<int>(clearit));
}

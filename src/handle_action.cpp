...
inline static void pldrive(point_rel_veh d) { return pldrive(tripoint_rel_veh(d, 0)); }
...
static void smash() {
    ...
    if (should_pulp) {
        ...
        u.assign_activity(
            std::make_unique<player_activity>(ACT_PULP, calendar::INDEFINITELY_LONG, 0));
...
static void wait() {
    ...
    if (setting_alarm) {
        ...
        u.assign_activity(
            std::make_unique<player_activity>(actType, 100 * (to_turns<int>(time_to_wait)), 0),
            false);
    }
}
...
static void sleep() {
    ...
    as_m.entries.emplace_back(0, true, get_option<bool>("FORCE_CAPITAL_YN") ? 'Y' : 'y', _("Yes."));
    as_m.entries.emplace_back(
        1, g->get_moves_since_last_save(), get_option<bool>("FORCE_CAPITAL_YN") ? 'S' : 's',
        _("Yes, and save game before sleeping."));
    as_m.entries.emplace_back(2, true, get_option<bool>("FORCE_CAPITAL_YN") ? 'N' : 'n', _("No."));
...
auto start_spellcasting_activity(player& u, spell& sp) -> void {
    auto cast_spell = std::make_unique<player_activity>(ACT_SPELLCASTING, sp.casting_time(u));
...
auto cast_spell() -> void {
    ...
    if (spells.empty()) {
        add_msg(game_message_params{m_bad, gmf_bypass_cooldown},
                _("You don't know any spells to "
                  "cast."));
        return;
    }
    ...
    if (!can_cast_spells) {
        add_msg(game_message_params{m_bad, gmf_bypass_cooldown},
                _("You can't cast any of the "
                  "spells you know!"));
        return;
    }
    if (!has_brawler_spell && u.has_trait(trait_BRAWLER)) {
        add_msg(game_message_params{m_bad, gmf_bypass_cooldown},
                _("You don't know any spells you "

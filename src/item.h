...
public:
    ...
    inline static detached_ptr<item> spawn(JsonIn& jsin) {
        ...
        p->deserialize(jsin);
        if (p->is_null()) { return detached_ptr<item>(); }
        return p;
    }

    inline static detached_ptr<item> spawn(const item& source) {
        if (source.is_null()) { return detached_ptr<item>(); }
        return detached_ptr<item>(new item(source));
    }

    template <typename... T> inline static detached_ptr<item> spawn(T... args) {
        return detached_ptr<item>(new item(std::forward<T>(args)...));
    }

    inline static item* spawn_temporary(const item& source) {
        if (source.is_null()) { return &null_item_reference(); }
        detached_ptr<item> p = item::spawn(source);
        return &*p;
    }

    template <typename... T> inline static item* spawn_temporary(T... args) {
    ...
    /**
     * Splits a count-by-charges item, taking qty charges away from it and creating a new (detached)
     * item from them. If the entire stack is taken, or the item doesn't count by charges then it
     * returns itself detached.
    ...
    virtual bool attempt_detach(std::function<detached_ptr<item>(detached_ptr<item>&&)>) override;

    /**
     * Similar to attempt_detach except this splits a count-by-charges item, taking qty charges away
     * from it and creating a new (detached) item from them. This detached item is then passed to
     * the lambda. Whatever is returned by the lambda is then merged back into the original item,
     * even if all charges were taken originally. Trying to call this on a non-count-by-charges item
     * or returning an item dissimilar to the argument will result in a debugmsg.
     */
    bool attempt_split(int qty, const std::function<detached_ptr<item>(detached_ptr<item>&&)>& cb);
     ...
     */
    /*@{*/
    static detached_ptr<item> make_corpse(
        const mtype_id& mt = string_id<mtype>::NULL_ID(), time_point turn = calendar::turn,
        const std::string& name = "", int upgrade_time = -1);
    /*@}*/
    /**
     ...
     */
    std::string tname(
        unsigned int quantity = 1, bool with_prefix = true, unsigned int truncate = 0) const;
    std::string display_money(
        unsigned int quantity, unsigned int total,
    ...
    std::vector<iteminfo> info(
        const iteminfo_query& parts, int batch, temperature_flag temperature) const;
    ...
    std::string info_string(
        const iteminfo_query& parts, int batch = 1,
    ...
    bool stacks_with(
        const item& rhs, bool check_components = false, bool skip_type_check = false) const;
    /**
     ...
     * NOTE: Result is rounded up to next nearest milliliter when working with stackable (@ref
     * count_by_charges) items that have fractional volume per charge. If trying to determine how
     * many of an item can fit in a given space, @ref charges_per_volume should be used instead.
    ...
    /**
     * Simplified, faster volume check for when processing time is important and exact volume is
     * not. NOTE: Result is rounded up to next nearest milliliter when working with stackable (@ref
     * count_by_charges) items that have fractional volume per charge. If trying to determine how
     * many of an item can fit in a given space, @ref charges_per_volume should be used instead.
     */
     ...
     */
    std::map<std::string, double> dps(
        bool for_display, bool for_calc, const player& guy, const attack_statblock& attack) const;
    std::map<std::string, double> dps(
        bool for_display, bool for_calc, const attack_statblock& attack) const;
     ...
     * Note this function returns null if the item is a gun for which you can use gun_skill()
     * instead.
     */
     ...
     * @return true if this item should be deleted (count-by-charges items with no remaining
     * charges)
     */
    static detached_ptr<item> use_charges(
        detached_ptr<item>&& self, const itype_id& what, int& qty,
        std::vector<detached_ptr<item>>& used, const tripoint_bub_ms& pos,
     ...
     * @param filter Must return true for use to occur.
     */
    static detached_ptr<item> use_amount(
        detached_ptr<item>&& self, const itype_id& it, int& quantity,
     ...
     */
    int get_remaining_capacity_for_liquid(
        const item& liquid, bool allow_bucket = false, std::string* err = nullptr) const;
    int get_remaining_capacity_for_liquid(
        const item& liquid, const Character& who, std::string* err = nullptr) const;
    /**
     ...
     * returns this. It's intended to be used like \code newitem =
     * newitem.in_its_container();\endcode You must pass the detached_ptr representing the current
     * object, so that it can be placed inside its container.
     */
    static detached_ptr<item> in_its_container(detached_ptr<item>&& self);
    static detached_ptr<item> in_container(
        const itype_id& container_type, detached_ptr<item>&& self);
    /*@}*/
    ...
    /**
     * Whether the item has to be removed as it has rotten away completely. May change the item as
     * it calls process_rot()
     ...
     */
    static detached_ptr<item> actualize_rot(
     ...
     * This function should not be called directly. since it does not have all the needed checks or
     * temperature calculations. If you need to calc rot of item call process_rot instead.
    ...
    static detached_ptr<item> process_rot(
        detached_ptr<item>&& self, bool seals, const tripoint_bub_ms& pos, player* carrier,
    ...
    void update_rot(
        const tripoint_bub_ms& pos, temperature_flag temperature,
     ...
     * Regular shelf life perishable foods rot away completely at 2x shelf life. Corpses last 10
     * days
    ...
    inline bool operator==(const item& rhs) const { return this == &rhs; };
    /** LUA: We need this operator defined for Lua bindings to compile. */
    ...
    void set_var(const std::string& name, const char* value) { item_vars_.set(name, value); };
    ...
    void set_var( const std::string &name, long long value ) { item_vars_.set<long long>( name,
    value ); };
    ...
    item_vars_.set<tripoint>( name, value ); }; void set_var( const std::string &name, const
    std::string &value ) { item_vars_.set( name, value ); };

    double get_var( const std::string &name, double default_value ) const { return
    item_vars_.get<double>( name, default_value ); } tripoint get_var( const std::string &name,
    const tripoint_bub_ms &default_value ) const { return item_vars_.get<tripoint>( name,
    default_value ); } std::string get_var( const std::string &name, const std::string
    &default_value ) const { return item_vars_.get( name, default_value ); } std::string get_var(
    const std::string &name ) const { return item_vars_.get( name, "" ); }
    */
    ...
    bool is_sided() const;
    /**
     *  Returns side item currently worn on. Returns BOTH if item is not sided or no side currently
     * set
     */
    side get_side() const;
    /**
    ...
    /**
     * Returns the highest coverage that any piece of data that this item has that covers the
     * bodypart. Values range from 0 (not covering anything) to 100 (covering the whole body part).
    ...
    /**
     * Returns the weight capacity modifier (@ref islot_armor::weight_capacity_modifier) that this
     * item provides when worn. For non-armor it returns 1. The modifier is multiplied with the
     * weight capacity of the character that wears the item.
     */
    float get_weight_capacity_modifier() const;
    /**
     * Returns the weight capacity bonus (@ref islot_armor::weight_capacity_modifier) that this item
     * provides when worn. For non-armor it returns 0. The bonus is added to the total weight
     * capacity of the character that wears the item.
     */
    ...
    /** Get default ammo for the first ammotype common to an item and its current magazine or "NULL"
     * if none exists
     * @param conversion whether to include the effect of any flags or mods which convert the type
     * @return itype_id of default ammo for the first ammotype common to an item and its current
     * magazine or "NULL" if none exists */
     ...
     *  @param conversion whether to include the effect of any flags or mods which convert item's
     * ammo type
     *  @return magazine type or "null" if item has integral magazine or no magazines for current
     * ammo type */
    ...
    /** Whether this is a (nearly) silent gun (a tiny bit of sound is allowed). Non-guns are always
     * silent. */
    ...
    /**
     * Summed projectile speed value (m/s) of a gun, including values from mods. Returns 10 on
     * non-gun items.
     */
    int gun_speed(bool with_ammo = true) const;
    /**
     * Summed bonus to the aimed critical base multiplier, including values from mods. Returns 0 on
     * non-gun items.
     */
    double gun_aimed_crit_bonus(bool with_ammo = true) const;
    /**
     * Summed bonus to the aimed critical max potential multiplier value of a gun, including values
     * from mods. Returns 0 on non-gun items.
     */
    ...
    /**
     * Summed ranged damage, armor piercing, and multipliers for both, of a gun, including values
     * from mods. Returns empty instance on non-gun items.
     */
    ...
    inline void set_old_owner(const faction_id& temp_owner) { old_owner = temp_owner; }
    inline void remove_old_owner() const { old_owner = faction_id::NULL_ID(); }
    inline void set_owner(const faction_id& new_owner) { owner = new_owner; }
    void set_owner(const Character& c);
    inline void remove_owner() const { owner = faction_id::NULL_ID(); }
     ...
     */
    double bonus_from_enchantments(
        const Character& owner, double base, enchant_vals::mod value, bool round = false) const;

    /**
     * Calculate bonus from enchantments that affect this item only,
     * assume it's wielded and all enchantments' conditions are satisfied.
     */
    double bonus_from_enchantments_wielded(
        double base, enchant_vals::mod value, bool round = false) const;
...
private:
    ...
    /**
     * Splits an item similar to split, however it must be called on a count-by-charges item and if
     * the entire stack is taken it will leave a 0 charge item behind. This should be used with
     ...
     * nothing if old.charges isn't 0. The typical flow of calls is to use unsafe_split. Then use
     * functions like pour_into etc. Then use merge_charges to merge any remaining charges back into
     * the old item. Then finally use this function to handle the case that no charges are
     * remaining.
     */
...
protected:
    ...
    static detached_ptr<item> process_corpse(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    static detached_ptr<item> process_litcig(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    static detached_ptr<item> process_extinguish(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    // Place conditions that should remove fake smoke item in this sub-function
    static detached_ptr<item> process_fake_smoke(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    static detached_ptr<item> process_fake_cloning_vat(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    static detached_ptr<item> process_fake_mill(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    static detached_ptr<item> process_cable(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    static detached_ptr<item> process_UPS(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);
    static detached_ptr<item> process_blackpowder_fouling(
        detached_ptr<item>&& self, player* carrier);
    static detached_ptr<item> process_tool(
        detached_ptr<item>&& self, player* carrier, const tripoint_bub_ms& pos);

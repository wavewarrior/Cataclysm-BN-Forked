...
enum TAB_MODE { NORMAL, FILTERED, BATCH };
...
std::string peek_related_recipe(
    ...
    if( !is_hidden ) {
        ...
        for( const std::string subcat_id : jsobj.get_array( "recipe_subcategories" ) ) {
            if( !subcat_id.starts_with( "CSC_" + cat_name + "_" ) && subcat_id != "CSC_ALL" ) {
                jsobj.throw_error( "Crafting sub-category id has to be prefixed with "
                                   "CSC_<category_name>_" );
            }
            if( std::find( craft_subcat_list[category].begin(), craft_subcat_list[category].end(),
                           subcat_id )
                == craft_subcat_list[category].end() ) {
...
static std::string get_subcat_unprefixed( const std::string& cat,
...
template <typename T> list_circularizer<T> make_circular( std::vector<T> &vec )
{
    ...
    int indent, const expanded_list_options& opts ) -> void
{
    ...
    std::ranges::for_each( recp->nested_category_data, [&]( const recipe_id & nested_id ) {
        ...
        if( !availability_cache.contains( nested_rec ) ) {
            ...
            availability_cache
            .emplace( nested_rec, availability( opts.crafter, nested_rec, 1, known ) );
        }
        if( nested_rec->is_nested() ) {
    ...
    opts.highlight_unread_recipes]( const recipe * const a, const recipe * const b ) {
    ...
    const nc_color& color )
{
    ...
    auto format_multiplier = [&]( const std::string & label, float multiplier ) -> std::string {
        ...
        const auto assist_mult =
            static_cast<float>( base_total_moves ) / static_cast<float>( assist_total_moves );
        const auto total_mult =
            bench_mult * assist_mult * tools_mult * light_mult * morale_mult * mutation_mult
            * game_opt_mult;
        ...
        const auto show_total =
            verbose_multipliers || total_percent != 100 || !multiplier_lines.empty();
        if( show_total ) {
            multiplier_lines
            .insert( multiplier_lines.begin(), format_multiplier( _( "Total" ), total_mult ) );
        }
        ...
        if( multiplier_lines.empty() && !verbose_multipliers ) {
            ...
            std::ranges::for_each( multiplier_lines, [&]( const auto & line ) { oss << line; } );
        }
    }

    const int makes = recp.makes_amount();
    if( makes > 1 ) { oss << string_format( _( "Recipe makes: <color_cyan>%d</color>\n" ), makes ); }

    oss << string_format(
            _( "Craftable in the dark?  <color_cyan>%s</color>\n" ),
            ...
            : recp.has_flag( flag_BLIND_IMPOSSIBLE )
            ? _( "Impossible" )
            : _( "Reasonable" ) );
    ...
    if( can_craft_this && too_complex ) {
        oss << _(
                "Due to the complex overlapping requirements, this "
    ...
    if( !can_craft_this && avail.apparently_craftable && avail.known && !recp.is_nested() ) {
        oss << _(
                "<color_red>Cannot be crafted because the same item is needed "
                "for multiple components</color>\n" );
    }
    if( !avail.known ) { oss << _( "<color_red>Not known</color>\n" ); }
    ...
    if( !crafter.knows_recipe( &recp ) ) {
        ...
        const std::string enumerated_books = enumerate_as_string(
                books_with_recipe.begin(), books_with_recipe.end(),
        []( const itype_id & type_id ) { return colorize( item::nname( type_id ), c_cyan ); } );
...
const recipe *select_crafting_recipe( int &batch_size_out, Character& crafter )
{
    ...
    const auto cached_recipe_info =
        [&]( const recipe & recp, const availability & avail, Character & crafter,
             bool show_unavailable, const std::string qry_comps, const int batch_size,
    ...
    [&]( const recipe * rec, const int count, int &scroll_pos ) {
        ...
        const auto category_name =
            ...
            : rec->category;
        const auto subcategory_name =
            normalized_names.contains( rec->subcategory )
            ? normalized_names[rec->subcategory]
            : rec->subcategory;
        const auto category_label =
        ...
        const auto category_entry = std::ranges::find_if( info, [&]( const iteminfo & entry ) {
            return entry.sType == "BASE" && entry.sName == category_name_label;
        } );
        if( category_entry != info.end() ) {
            category_entry->sFmt =
                string_format( "<color_magenta>%s</color>", category_label_upper );
        }
        item_info_data
        data( item_info_cache.dummy->tname( count ), item_info_cache.dummy->type_name( count ),
              info, {}, scroll_pos );
        return data;
    };
    ...
    ui.on_screen_resize( [&]( ui_adaptor & ui ) {
        ...
        if( highlight_unread_recipes ) {
            ...
            add_action_desc( "TOGGLE_UNREAD_RECIPES_FIRST", pgettext( "crafting gui", "Show unread "
                             "recipes "
                             "first" ) );
        }
        ...
        keybinding_tips = foldstring(
                              enumerate_as_string( act_descs, enumeration_conjunction::none ),
        ...
        w_data = catacurses::newwin( dataHeight, width, point( wStart, headHeight + subHeadHeight ) );
    ...
    const recipe_subset & recipes, const std::string & cat, const std::string & subcat ) {
        if( subcat == "CSC_*_FAVORITE" ) {
            ...
            return std::
                   make_pair( recipes.in_category( cat, subcat != "CSC_ALL" ? subcat : "" ), false );
        }
    };
    ...
    const auto sync_rml = [&]() {
        ...
        for( size_t i = 0; i < current.size(); ++i ) {
            ...
            const bool rcp_read =
                !highlight_unread_recipes || !rcp_known
        ...
        if( sel_ok ) {
            ...
            if( isWide ) {
                if( recp.is_nested() ) {
                    ...
                    const long known_items = std::ranges::
                    count_if( recp.nested_category_data, [&]( const recipe_id & nested_id ) {
                        return available_recipes.contains( nested_id.obj() );
                    } );
                    const std::string category_name =
                        normalized_names.contains( recp.category )
                        ? normalized_names[recp.category]
                        : recp.category;
                    ...
                    const std::string s =
                        format_item_info( d.get_item_display(), d.get_item_compare() );
    ...
    rml.open( crafting_rmlui_enabled(), "crafting", ctxt, [&]( Rml::DataModelConstructor & c ) {
        ...
        c.BindEventCallback(
        "on_cat", [&]( Rml::DataModelHandle, Rml::Event &, const Rml::VariantList & args ) {
        ...
        c.BindEventCallback(
        "on_subcat", [&]( Rml::DataModelHandle, Rml::Event &, const Rml::VariantList & args ) {
            if( batch || !filterstring.empty() ) { return; }
            int idx = -1;
            if( !args.empty() ) { args[0].GetInto( idx ); }
    ...
    const auto rml_scroll_info = [&]( int dir ) {
        if( !rml ) { return; }
        for( const char * id : {"recipe-info", "item-info"} ) {
    ...
    do {
        ...
        if( recalc ) {
            ...
            if( batch ) {
                ...
                if( !filterstring.empty() ) {
                    ...
                    do {
                        ...
                        if( qry_filter_str.size() > 2 && qry_filter_str[1] == ':' ) {
                            switch( qry_filter_str[0] ) {
                                case 't':
                                    filtered_recipes = filtered_recipes.reduce(
                                                           qry_filter_str.substr( 2 ), recipe_subset::search_type::tool );
                                    break;

                                case 'c':
                                    filtered_recipes = filtered_recipes.reduce(
                                                           qry_filter_str.substr( 2 ),
                    ...
                    if( enable_nested_categories ) { picking = shown_recipes.nested(); }
                } else {
                ...
                if( show_hidden ) {
                    ...
                    current.clear();
                    std::ranges::
                    copy_if( picking, std::back_inserter( current ), []( const recipe * const recp ) {
                ...
                if( enable_nested_categories ) {
                    ...
                    std::ranges::for_each( current, [&]( const recipe * recp ) {
                        if( recp->is_nested() ) {
                            nested_child_ids
                            .insert( recp->nested_category_data.begin(),
                    ...
                    if( !nested_child_ids.empty() ) {
                        ...
                        current, std::back_inserter( filtered_current ), [&]( const recipe * recp ) {
                            return recp->is_nested()
                                   || !nested_child_ids.contains( recp->ident() );
                        } );
                ...
                for( const recipe * e : current ) {
                    if( !availability_cache.contains( e ) ) {
                        availability_cache.emplace(
                            e, availability( crafter, e, 1, available_recipes.contains( *e ) ) );
                    }
                }
                ...
                std::ranges::for_each( current, [&]( const recipe * recp ) {
                    if( recp->is_nested() ) {
                        ...
                        nested_avail.can_craft = update_nested_can_craft(
                                                     ...
                                                     visiting_nested, available_recipes, show_unavailable );
                    }
                } );
                ...
                if( subtab.cur() != "CSC_*_RECENT" ) {
                    ...
                    highlight_unread_recipes]( const recipe * const a, const recipe * const b ) {
                        if( highlight_unread_recipes && unread_recipes_first ) {
                            const bool a_known = available_recipes.contains( *a );
                            const bool b_known = available_recipes.contains( *b );
                            const bool a_read =
                                !a_known || uistate.read_recipes.count( a->ident() );
                            const bool b_read =
                                !b_known || uistate.read_recipes.count( b->ident() );
                            if( a_read != b_read ) { return !a_read; }
                        }
                ...
                std::ranges::transform( current, std::back_inserter( available ), [&]( const recipe * e ) {
        ...
        if( highlight_unread_recipes && recalc_unread ) {
            if( filterstring.empty() ) {
                for( const std::string& cat : craft_cat_list ) {
                    ...
                    for( const std::string& subcat : craft_subcat_list[cat] ) {
                        ...
                        for( const recipe * const rcp : recipes ) {
                            ...
                            if( uistate.read_recipes.count( rcp_id ) ) { continue; }
                ...
                for( const recipe * const rcp : current ) {
                    const recipe_id& rcp_id = rcp->ident();
                    if( !available_recipes.contains( *rcp ) ) { continue; }
                    if( uistate.hidden_recipes.contains( rcp_id ) ) { continue; }
                    if( uistate.read_recipes.count( rcp_id ) ) { continue; }
        ...
        if( action == "SCROLL_RECIPE_INFO_UP" ) {
            ...
            if( available.empty()
                || !( available[line].can_craft || available[line].is_nested_category ) ) {
            ...
            } else if( !crafter.check_eligible_containers_for_crafting(
                           *current[line], ( batch ) ? line + 1 : 1 ) ) {
            ...
            if( current.empty() || available[line].is_nested_category
                ...
                for( npc * guy : nearby ) {
                    ...
                    any_knows = true;
                    if( guy->can_make( rec, bs ) ) { candidates.push_back( guy ); }
                }
                if( candidates.empty() ) {
                    if( nearby.empty() ) {
                    ...
                    std::sort( candidates.begin(), candidates.end(), [&]( const npc * a, const npc * b ) {
                        return rl_dist( a->bub_pos(), crafter.bub_pos() )
                               < rl_dist( b->bub_pos(), crafter.bub_pos() );
                    } );
                    npc* target = nullptr;
                    ...
                    if( target != nullptr ) {
                        ...
                        add_msg( m_good, _( "%s starts crafting %s." ), target->get_name(),
                                 rec->result_name() );
                        chosen = nullptr;
                        done = true;
                    }
            ...
            item_info_data data =
                item_info_data_from_recipe( current[line], 1, item_info_scroll_popup );
            ...
            if( selected != nullptr ) { game_menus::inv::compare( *selected, *crafted_item ); }
            recalc = true;
            keepline = true;
            ...
            std::vector<SearchPrefix> prefixes = {
                ...
                {
                    'd', _( "reach attack" ),
                    _( "<color_cyan>full description</color> of resulting item (slow)" )
                },
            ...
            std::string description = _(
                                          ...
                                          "can be used with a colon <color_red>:</color> to search in other ways.  "
                                          "Additional filters "
            ...
            for( const auto& prefix : prefixes ) {
                ...
                description += string_format(
                                   ...
                                   prefix.example, padding, spaces, prefix.description );
            }

            description += _( "\nUse <color_red>up/down arrow</color> to go through your search "
                              "history." );
            ...
            if( !available_recipes.contains( *current[line] ) ) { continue; }
            ...
            if( !current_list_has_unread ) {
                query_str = _(
                                "<color_yellow>/!\\</color> Mark all recipes as read?  "
    ...
    const recipe* current, const recipe_subset& available, const Character& crafter )
{
    ...
    uilist rel_menu;
    int np_last = -1;
    if( !related_components.empty() ) { rel_menu.addentry( ++np_last, false, -1, _( "COMPONENTS" ) ); }
    np_last = related_menu_fill( rel_menu, related_components, available );
    if( !related_results.empty() ) { rel_menu.addentry( ++np_last, false, -1, _( "RESULTS" ) ); }
...
static bool query_is_yes( const std::string& query )
{
    ...
    return subquery == "yes" || subquery == "y" || subquery == "1" || subquery == "true"
           || subquery == "t" || subquery == "on"
...
bool lcmatch_any( const std::vector<std::vector<T>> &list_of_list, const std::string& filter )
{
    for( auto& list : list_of_list ) {
        for( auto& comp : list ) {
            if( lcmatch( item::nname( comp.type ), filter ) ) { return true; }
        }
    }
    return false;
}

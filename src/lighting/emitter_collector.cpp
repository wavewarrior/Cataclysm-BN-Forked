...
static constexpr Uint32 EMITTER_BUF_BYTES = static_cast<Uint32>( MAX_EMITTERS ) *
                                ...
                                std::vector<float>      occ )
{
    ...
    pending_sun_sdf_      = std::move( sun_sdf );
...
void emitter_collector::flush_to_render_cb( SDL_GPUCommandBuffer *cb )
{
    ...
    std::vector<float>       sun_sdf      = std::move( pending_sun_sdf_ );
    ...
    if( !xfer_ || !emitter_buf_ ) {
        return;
    }

    const int count = std::min( static_cast<int>( data.size() ),
                                static_cast<int>( MAX_EMITTERS ) );
    ...
    if( rs_.sdf().ready() && !transparency.empty() && !sdf.empty()
        ...
        rs_.sdf().upload( cp, rs_.device().raw(),
                          runtime_w, runtime_h,
                          transparency, sdf, sky_vis, vis, sun_sdf, occ );
    }

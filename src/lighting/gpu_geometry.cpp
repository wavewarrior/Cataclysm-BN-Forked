...
class gpu_geometry_impl
{
    public:
        ...
        void init( gpu_device &d ) {
            if( !d.ready() ) {
                throw std::runtime_error( "gpu_geometry::init: gpu_device not ready" );
            }
            dev = &d;
            ...
            if( !white ) {
                throw std::runtime_error( std::string( "gpu_geometry: create white tex: " ) +
                                          SDL_GetError() );
            }
            ...
            if( !xfer ) {
                throw std::runtime_error( std::string( "gpu_geometry: xfer alloc: " ) +
                                          SDL_GetError() );
            }
...
void gpu_geometry::shutdown() noexcept { if( p ) { p->shutdown(); } }
SDL_GPUTexture *gpu_geometry::white_texture() const noexcept { return p ? p->white : nullptr; }

void gpu_geometry::rect( sprite_batcher &dst, float x, float y, float w, float h,
...
void gpu_geometry::horizontal_line( sprite_batcher &dst, float x, float y, float x2,
                                    float thickness, const float rgba[4] ) const
{
    if( x2 < x ) {
        std::swap( x, x2 );
    }
...
void gpu_geometry::vertical_line( sprite_batcher &dst, float x, float y, float y2,
                                  float thickness, const float rgba[4] ) const
{
    if( y2 < y ) {
        std::swap( y, y2 );

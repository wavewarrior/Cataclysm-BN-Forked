...
// side). These are STATIC: turning on "place lights on click" in the F4
// lighting panel and then left-clicking in the world drops a light at the
// hovered tile, using the cursor brush's radius/intensity/colour. They are
// appended to the emitter snapshot like any other light (so they drive BOTH
// direct lighting and the GI gather). Placement is gated solely by the
// checkbox — the panel can be closed afterward without losing the effect;
// clear them explicitly via the panel's "clear placed" button. Purely a
...
extern std::vector<light> lights;          // placed lights; cleared via UI button

...
struct stripe_packer {
    ...
    stripe_packer( int W, int H, std::uint32_t min )
        : w( W ), h( H ), min_size( min ), y_remainder( H ) {}
    ...
    std::optional<SDL_Rect> pack( std::uint32_t pw, std::uint32_t ph ) {
        ...
        if( it == stripes.end() ) {
            if( rh > y_remainder || y_remainder < min_size ) {
                return std::nullopt;
            }
            it = stripes.insert( stripes.end(), stripe{
                rh,
                static_cast<std::uint32_t>( h ) - y_remainder,
        ...
        SDL_Rect r{
            static_cast<int>( x_off ),
            static_cast<int>( it->y_offset ),
            static_cast<int>( pw ),
            static_cast<int>( ph )
        };
        it->x_remainder -= pw;
        if( it->x_remainder < min_size ) {
            it->x_remainder = 0;
        }
        return r;
    }
...
class gpu_atlas_impl
{
    public:
        ...
        gpu_atlas_impl( int pw, int ph, int mw, int mh )
            : page_w( pw ), page_h( ph ), min_w( mw ), min_h( mh ) {}
        ...
        gpu_atlas_slot allocate( int w, int h ) {
            ...
            for( std::size_t i = 0; i < pages.size(); ++i ) {
                if( auto r = pages[i].packer.pack( w, h ) ) {
                    return make_slot( i, *r );
                }
            }
            // Need a new page.
            page np{};
        ...
        bool upload_surface( SDL_GPUCommandBuffer *cb, const gpu_atlas_slot &slot,
                             SDL_Surface *surf ) {
            if( !dev || !cb || !slot || !surf ) {
                return false;
            }
            if( slot.page_index >= pages.size() ||
                pages[slot.page_index].tex != slot.page ) {
...
void gpu_atlas::init( gpu_device &dev )
{
    p->init( dev );
}
...
gpu_atlas_slot gpu_atlas::allocate( int w, int h )
{
...
gpu_atlas_slot gpu_atlas::upload( SDL_GPUCommandBuffer *cb, SDL_Surface *surf )
{
    ...
    if( !p->upload_surface( cb, s, surf ) ) {
        return {};
    }
    return s;
}

bool gpu_atlas::upload_surface( SDL_GPUCommandBuffer *cb, const gpu_atlas_slot &slot,
                                SDL_Surface *surf )
{
...
std::size_t gpu_atlas::page_count() const noexcept
{
    return p ? p->pages.size() : 0;
}

...
{
    if( !window ) {
        throw std::runtime_error( "gpu_device::init: null window" );
    }
    if( device ) {
    ...
    if( !SDL_ClaimWindowForGPUDevice( device.get(), window ) ) {
        const std::string msg = std::string( "SDL_ClaimWindowForGPUDevice failed: " ) + SDL_GetError();
        dbg( DL::Error ) << msg;
        device.reset();
        throw std::runtime_error( msg );
    }
...
void gpu_device::shutdown() noexcept
{
    if( device && claimed_window ) {
        SDL_ReleaseWindowFromGPUDevice( device.get(), claimed_window );
    }
...
{
    if( !device || !claimed_window || enable == vsync_enabled ) {
        return;
    }
    vsync_enabled = enable;
    const SDL_GPUPresentMode present_mode = enable
                                            ? SDL_GPU_PRESENTMODE_VSYNC
                                            : SDL_GPU_PRESENTMODE_MAILBOX;
    SDL_SetGPUSwapchainParameters( device.get(), claimed_window,
                                   SDL_GPU_SWAPCHAINCOMPOSITION_SDR,
                                   present_mode );
}

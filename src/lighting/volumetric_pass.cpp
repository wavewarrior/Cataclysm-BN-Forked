...
static_assert( sizeof( vol_params ) == 80,
               "vol_params wire-stable with the VolParams cbuffer in vol.frag.hlsl" );
...
bool volumetric_pass::init( gpu_device &dev, SDL_GPUTextureFormat hdr_format )
{
    ...
    auto v = compile_graphics_shader( dev, vert_src, "main",
                                      SDL_SHADERCROSS_SHADERSTAGE_VERTEX, "vol.vert" );
    auto f = compile_graphics_shader( dev, frag_src, "main",
                                      SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "vol.frag" );
    if( !v || !f ) {
        if( v ) {
            SDL_ReleaseGPUShader( dev.raw(), v.shader );
        }
        if( f ) {
            SDL_ReleaseGPUShader( dev.raw(), f.shader );
        }
    ...
    DebugLogFL( DL::Info, DC::Main )
            ...
            << " (expects samplers=0 storage_buffers=2)";
...
void volumetric_pass::record( SDL_GPUCommandBuffer *cb, SDL_GPUTexture *hdr_tex,
                              std::uint32_t full_w, std::uint32_t full_h,
                              SDL_GPUBuffer *sdf_buf, SDL_GPUBuffer *skyvis_buf,
                              const vol_params &params )
{
    if( !ready() || !cb || !hdr_tex || !sdf_buf || !skyvis_buf
        || full_w == 0 || full_h == 0 ) {
        return;
    }
    ...
    const SDL_GPUViewport vp{ 0.0f, 0.0f, static_cast<float>( full_w ),

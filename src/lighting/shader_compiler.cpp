...
{
    bool expected = true;
    if( !compiler_initialised.compare_exchange_strong( expected, false ) ) {
        return;
    }
    ...
    const char *debug_name )
{
    ...
    if( !spirv || spirv_size == 0 ) {
        dbg( DL::Error ) << "HLSL→SPIRV failed ("
    ...
    if( !meta ) {
        dbg( DL::Error ) << "SPIR-V reflect failed ("
                         << ( debug_name ? debug_name : "?" )
                         << "): " << SDL_GetError();
        SDL_free( spirv );
        return out;
    ...
    if( !gpu_shader ) {
        dbg( DL::Error ) << "SPIRV→GPUShader failed ("
                         << ( debug_name ? debug_name : "?" )
                         << "): " << SDL_GetError();
        return out;
    }

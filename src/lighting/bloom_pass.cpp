...
static constexpr SDL_GPUTextureFormat BLOOM_FORMAT =
...
static SDL_GPUGraphicsPipeline *make_pipeline( SDL_GPUDevice *d,
        ...
        SDL_GPUTextureFormat fmt, bool additive )
{
...
bool bloom_pass::init( gpu_device &dev, SDL_GPUTextureFormat hdr_format,
                       std::uint32_t full_w, std::uint32_t full_h )
{
    ...
    auto v = compile_graphics_shader( dev, vert_src, "main",
                                      SDL_SHADERCROSS_SHADERSTAGE_VERTEX, "bloom.vert" );
    auto e = compile_graphics_shader( dev, extract_src, "main",
                                      SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "bloom_extract.frag" );
    auto b = compile_graphics_shader( dev, blur_src, "main",
                                      SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "bloom_blur.frag" );
    auto c = compile_graphics_shader( dev, comp_src, "main",
                                      SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "bloom_composite.frag" );
    if( !v || !e || !b || !c ) {
        if( v ) {
            SDL_ReleaseGPUShader( dev.raw(), v.shader );
        }
        if( e ) {
            SDL_ReleaseGPUShader( dev.raw(), e.shader );
        }
        if( b ) {
            SDL_ReleaseGPUShader( dev.raw(), b.shader );
        }
        if( c ) {
            SDL_ReleaseGPUShader( dev.raw(), c.shader );
        }
    ...
    DebugLogFL( DL::Info, DC::Main )
            ...
            << ") blur(s=" << b.resources.num_samplers
            << ") composite(s=" << c.resources.num_samplers
...
bool bloom_pass::create_textures( std::uint32_t full_w, std::uint32_t full_h )
{
    ...
    if( !SDL_GPUTextureSupportsFormat( dev_->raw(), BLOOM_FORMAT, SDL_GPU_TEXTURETYPE_2D,
...
bool bloom_pass::resize( std::uint32_t full_w, std::uint32_t full_h )
{
    ...
    if( bloom_a_ && bloom_b_ && hw == half_w_ && hh == half_h_ ) {
        return true;
    }
    if( dev_ && dev_->ready() ) {
        if( bloom_a_ ) {
...
{
    if( dev_ && dev_->ready() ) {
        if( extract_pipeline_ ) {
            SDL_ReleaseGPUGraphicsPipeline( dev_->raw(), extract_pipeline_ );
        }
        if( blur_pipeline_ ) {
            SDL_ReleaseGPUGraphicsPipeline( dev_->raw(), blur_pipeline_ );
        }
        ...
        if( extract_frag_ ) {
            SDL_ReleaseGPUShader( dev_->raw(), extract_frag_ );
        }
        if( blur_frag_ ) {
            SDL_ReleaseGPUShader( dev_->raw(), blur_frag_ );
        }
        if( composite_frag_ ) {
...
static void run_subpass( SDL_GPUCommandBuffer *cb, SDL_GPUGraphicsPipeline *pipe,
                         SDL_GPUTexture *src, SDL_GPUSampler *smp, SDL_GPUTexture *dst,
                         std::uint32_t dst_w, std::uint32_t dst_h, bool load )
{
    ...
    const SDL_GPUViewport vp{ 0.0f, 0.0f, static_cast<float>( dst_w ),
...
void bloom_pass::record( SDL_GPUCommandBuffer *cb, SDL_GPUTexture *hdr_tex,
                         std::uint32_t full_w, std::uint32_t full_h,
                         float threshold, float intensity )
{
    if( !ready() || !cb || !hdr_tex || full_w == 0 || full_h == 0 ) {
        return;
    }

    // 1. EXTRACT: world_target → bloom_a_ (half). Uniform: threshold.
    {
        struct { float threshold; float pad0, pad1, pad2; } u{ threshold, 0, 0, 0 };
    ...
    {
        struct { float dx, dy, pad0, pad1; } u{ 1.0f / static_cast<float>( half_w_ ), 0.0f, 0, 0 };
        SDL_PushGPUFragmentUniformData( cb, 0, &u, sizeof( u ) );
        run_subpass( cb, blur_pipeline_, bloom_a_, sampler_, bloom_b_, half_w_, half_h_, false );
    }
    // 3. BLUR V: bloom_b_ → bloom_a_.
    {
        struct { float dx, dy, pad0, pad1; } u{ 0.0f, 1.0f / static_cast<float>( half_h_ ), 0, 0 };
        SDL_PushGPUFragmentUniformData( cb, 0, &u, sizeof( u ) );
    ...
    {
        struct { float intensity; float pad0, pad1, pad2; } u{ intensity, 0, 0, 0 };
        SDL_PushGPUFragmentUniformData( cb, 0, &u, sizeof( u ) );

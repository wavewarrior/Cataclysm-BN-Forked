...
gi_compute_pass::~gi_compute_pass()
{
    shutdown();
}
...
bool gi_compute_pass::init( gpu_device &dev, std::uint32_t max_w, std::uint32_t max_h )
{
    ...
    DebugLogFL( DL::Info, DC::Main )
            ...
            << " uniforms=" << bp.resources.num_uniform_buffers
            << " threads=(" << bp.resources.threadcount_x << ","
            << bp.resources.threadcount_y << "," << bp.resources.threadcount_z
    ...
    field_buf_ = create_buffer( floats, SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ |
                                SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE );
    gi_buf_    = create_buffer( floats, SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE |
                                SDL_GPU_BUFFERUSAGE_GRAPHICS_STORAGE_READ );
    if( !field_buf_ || !gi_buf_ ) {
        return false;
    }
    max_w_ = max_w;
    max_h_ = max_h;
...
SDL_GPUBuffer *gi_compute_pass::create_buffer( std::uint32_t floats,
...
{
    if( field_buf_ && gi_buf_ && max_w == max_w_ && max_h == max_h_ ) {
        return true;
    }
    if( dev_ && dev_->ready() ) {
        if( field_buf_ ) {
...
{
    if( !gi_buf_ || !dev_ || !dev_->ready() || runtime_w == 0 || runtime_h == 0 ) {
        return;
    }

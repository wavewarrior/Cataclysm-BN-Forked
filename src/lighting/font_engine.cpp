...
class font_engine_impl
{
    public:
        ...
        void init( gpu_device &d, SDL_GPUTextureFormat fmt ) {
            if( !d.ready() ) {
                throw std::runtime_error( "font_engine::init: gpu_device not ready" );
            }
            dev = &d;

            engine = TTF_CreateGPUTextEngine( d.raw() );
            if( !engine ) {
                throw std::runtime_error( std::string( "TTF_CreateGPUTextEngine: " ) +
                                          SDL_GetError() );
            }
            ...
            auto v = compile_graphics_shader( d, FONT_VERT_HLSL, "main",
                                              SDL_SHADERCROSS_SHADERSTAGE_VERTEX, "font.vert" );
            auto f = compile_graphics_shader( d, FONT_FRAG_HLSL, "main",
                                              SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "font.frag" );
            if( !v || !f ) {
                if( v ) {
                    SDL_ReleaseGPUShader( d.raw(), v.shader );
                }
                if( f ) {
                    SDL_ReleaseGPUShader( d.raw(), f.shader );
                }
            ...
            if( !sampler ) {
                throw std::runtime_error( std::string( "font sampler: " ) + SDL_GetError() );
            }
        ...
        void draw_text( SDL_GPUCommandBuffer *cb, SDL_GPUTexture *target,
                        std::uint32_t target_w, std::uint32_t target_h,
                        TTF_Text *text, float x, float y, const float rgba[4] ) {
            if( !dev || !cb || !target || !text ) {
                return;
            }
            ...
            for( ; seq; seq = seq->next ) {
                if( seq->image_type != TTF_IMAGE_ALPHA ) {
                    ...
                    dbg( DL::Warn ) << "font_engine: non-ALPHA glyph sequence ("
                                    << static_cast<int>( seq->image_type )
                                    << ") skipped";
                    continue;
                }
                ...
                parts.push_back( draw_part{
                    seq->atlas_texture,
                    first_idx,
                    static_cast<Uint32>( seq->num_indices ),
                    base_vert
                } );
            }
            ...
            for( const draw_part &dp : parts ) {
                ...
                SDL_DrawGPUIndexedPrimitives( rp, dp.num_indices, /*num_instances=*/1,
                                              dp.first_index, dp.vertex_offset,
                                              /*first_instance=*/0 );
            }
...
void font_engine::shutdown() noexcept { if( p ) { p->shutdown(); } }
TTF_TextEngine *font_engine::raw() const noexcept { return p ? p->engine : nullptr; }
void font_engine::begin_frame() { if( p ) { p->begin_frame(); } }

void font_engine::draw_text( SDL_GPUCommandBuffer *cb, SDL_GPUTexture *target,
                             std::uint32_t target_w, std::uint32_t target_h,
                             TTF_Text *text, float x, float y, const float rgba[4] )
{

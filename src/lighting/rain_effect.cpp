...
static SDL_GPUGraphicsPipeline *make_quad_pipeline( SDL_GPUDevice *dev,
        SDL_GPUShader *vert, SDL_GPUShader *frag, SDL_GPUTextureFormat fmt )
{
                        ...
                        std::uint32_t screen_w, std::uint32_t screen_h )
{
    ...
    auto v  = compile_graphics_shader( dev, vert_src, "main",
                                       SDL_SHADERCROSS_SHADERSTAGE_VERTEX, "rain_droplet.vert" );
    auto fd = compile_graphics_shader( dev, droplet_src, "main",
                                       SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "rain_droplet.frag" );
    auto fs = compile_graphics_shader( dev, splash_src, "main",
                                       SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "rain_splash.frag" );
    if( !v || !fd || !fs ) {
    ...
    auto make_buffers = [&]( SDL_GPUTransferBuffer *&xfer, SDL_GPUBuffer *&storage,
    ...
    if( !make_buffers( droplet_xfer_, droplet_storage_, MAX_DROPLETS ) ||
        ...
        if( inst_storage_buf_ ) {
            SDL_ReleaseGPUBuffer( dev.raw(), inst_storage_buf_ );
            inst_storage_buf_ = nullptr;
        }
        if( inst_transfer_buf_ ) {
            SDL_ReleaseGPUTransferBuffer( dev.raw(), inst_transfer_buf_ );
...
{
    if( static_cast<int>( droplets_.size() ) >= MAX_DROPLETS ) {
        return;
    }
...
{
    if( static_cast<int>( splashes_.size() ) >= MAX_SPLASHES ) {
        return;
    }
...
bool rain_effect::upload_instances( SDL_GPUCommandBuffer *cb, SDL_GPUTransferBuffer *xfer,
                                    SDL_GPUBuffer *storage, const std::vector<Inst> &insts )
{
    if( insts.empty() ) {
        return false;
    }
                          ...
                          const rain_params &params )
{
    ...
    while( it != droplets_.end() ) {
        ...
        const bool on_screen = sx > -16.f && sy > -32.f &&
                               sx < static_cast<float>( world_w ) + 16.f &&
                               sy < static_cast<float>( world_h ) + 16.f;
        if( on_screen ) {
            quad_instance qi{};
            qi.dst_x = sx;          qi.dst_y = sy;
            qi.dst_w = 1.6f;        qi.dst_h = 7.f + d.opacity * 7.f;
            qi.src_uw = 1.f;        qi.src_vh = 1.f;
            qi.tint_r = 0.6f;       qi.tint_g = 0.75f;  qi.tint_b = 0.9f;
            qi.tint_a = d.opacity;
    ...
    for( const auto &s : splashes_ ) {
        ...
        const float life = static_cast<float>( s.age ) /
                           static_cast<float>( std::max<uint32_t>( 1u, s.max_age ) );
        ...
        if( sx < -diam || sy < -diam ||
            sx > static_cast<float>( world_w ) + diam ||
            sy > static_cast<float>( world_h ) + diam ) {
            continue;
        }
        quad_instance qi{};
        qi.dst_x = sx - diam * 0.5f;  qi.dst_y = sy - diam * 0.5f;
        qi.dst_w = diam;              qi.dst_h = diam;
        qi.src_uw = 1.f;              qi.src_vh = 1.f;
        qi.tint_r = 0.55f;            qi.tint_g = 0.7f;   qi.tint_b = 0.95f;
        qi.tint_a = fade;
        splash_inst.push_back( qi );
    }

    if( droplet_inst.empty() && splash_inst.empty() ) {
        return;
    }
    ...
    struct { float tw; float th; uint32_t base; uint32_t pad; } fp;
    ...
    if( ++log_throttle >= 120 ) {
        ...
        dbg( DL::Info ) << "rain_effect: droplets=" << droplet_inst.size()
                        << " splashes=" << splash_inst.size()
                        << " (pool d=" << droplets_.size() << " s=" << splashes_.size() << ")";
    }
}

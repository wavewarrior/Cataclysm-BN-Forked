...
ui_composite_target::~ui_composite_target()
{
    shutdown();
}
...
bool ui_composite_target::init( gpu_device &dev, int w, int h )
{
    ...
    if( !dev.ready() || w <= 0 || h <= 0 ) {
        dbg( DL::Warn ) << "ui_composite_target::init: device not ready or bad size "
                        << w << "x" << h;
        return false;
    }
    ...
    if( !tex_ ) {
        dbg( DL::Warn ) << "ui_composite_target::init: SDL_CreateGPUTexture: "
                        << SDL_GetError();
        return false;
    }
...
{
    if( tex_ && dev_ && dev_->ready() ) {
        SDL_ReleaseGPUTexture( dev_->raw(), tex_ );
    }
    tex_ = nullptr;
    w_   = 0;
    h_   = 0;
...
void ui_composite_target::resize( int w, int h )
{
    ...
    if( static_cast<std::uint32_t>( w ) == w_ &&
        static_cast<std::uint32_t>( h ) == h_ && tex_ ) {
        return;
    }
    if( !dev_ ) {
        return;

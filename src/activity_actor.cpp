...
void burrow_activity_actor::do_turn(player_activity&, Character& who) {
    ...
    map& here = get_map();
    sfx::play_activity_sound("activity", "burrow", sfx::get_heard_volume(here.abs_to_bub(target)));
    if (calendar::once_every(1_minutes)) {
...
void jackhammer_activity_actor::do_turn(player_activity&, Character& who) {
    ...
    map& here = get_map();
    sfx::play_activity_sound("tool", "jackhammer", sfx::get_heard_volume(here.abs_to_bub(target)));
    if (calendar::once_every(1_minutes)) {
...
void jackhammer_activity_actor::finish(player_activity& act, Character& who) {
    ...
    apply_mining_exertion(who, progress.get_moves_total());
    who.add_msg_player_or_npc(m_good, _("You finish drilling."), _("<npcname> finishes drilling."));
    act.set_to_null();
    if (!tool) {
    ...
    if (reloadable.is_gun()) {
        ...
        if (reloadable.type->gun->reload_noise_volume > 0) {
            sfx::play_variant_sound(
                "reload", reloadable.typeId().str(), sfx::get_heard_volume(p.bub_pos()));
            sounds::ambient_sound(
                p.bub_pos(), reloadable.type->gun->reload_noise_volume, sounds::sound_t::activity,
...
void wood_chop_activity_actor::finish(player_activity& act, Character& who) {
    ...
    switch (chop_type) {
        case wood_chop_type::TREE: {
            ...
            sfx::play_variant_sound(
                "misc", "timber", sfx::get_heard_volume(here.abs_to_bub(placement)));
...
move_loot_activity_actor::move_loot_activity_actor() noexcept
    : stage( -1 ), num_processed( 0 ), placement( tripoint_abs_ms::zero() )
{}
...
operation_activity_actor::operation_activity_actor() noexcept
    : difficulty( 0 ), success( 0 ), max_power_level( 0 ), pl_skill( 0 ),
      autodoc( false )
{}

operation_activity_actor::operation_activity_actor( int diff, int succ, int power, int skill,
        const std::string &type, const bionic_id &b,
        const std::string &installer, bool auto_ )
    : difficulty( diff ), success( succ ), max_power_level( power ), pl_skill( skill ),
      operation_type( type ), bid( b ), installer_name( installer ), autodoc( auto_ )
{}
...
void operation_activity_actor::do_turn( player_activity &act, Character &who )
{
    ...
    const bool u_see = g->u.sees( p->bub_pos() )
    ...
    if ( time_left > half_op_duration ) {
        ...
        if ( operation_type == "uninstall" ) {
            ...
            if ( p->has_bionic( bid ) ) {
                p->perform_uninstall( bid, difficulty, success,
                    units::from_joule( max_power_level ), pl_skill );
            } else {
            ...
            if ( bid.is_valid() ) {
                p->perform_install( bid, upbid, difficulty, success, pl_skill,
...
const std::unordered_map<activity_id, std::unique_ptr<activity_actor> (*)(JsonIn&)> deserialize_functions =

...
item_penalties get_item_penalties(
    const location_vector<item>::const_iterator& worn_item_it, const Character& c,
    const bodypart_id&) {
    ...
    for (const bodypart_id& bp : c.get_all_body_parts()) {
        if (!worn_item->covers(bp)) { continue; }
        const int num_items =
            std::count_if(c.worn.begin(), c.worn.end(), [&layer, &bp, &c](item* const& i) {
                return i->get_layer() == layer && i->covers(bp)
                    && !(i->has_flag(flag_SEMITANGIBLE) || is_compact(*i, c));
            });
        if (num_items > 1) { body_parts_with_stacking_penalty.push_back(bp); }
    ...
    if (lists_of_bad_items_within.empty()) { lists_of_bad_items_within.emplace_back(); }
...
std::vector<std::string> clothing_properties(
    ...
    const int coverage =
        bp.id().is_null() ? worn_item.get_avg_coverage() : worn_item.get_coverage(bp);
    const int encumbrance =
        bp.id().is_null() ? worn_item.get_avg_encumber(c) : worn_item.get_encumber(c, bp);
    props.push_back(string_format("<color_c_green>[%s]</color>", _("Properties")));
    props.push_back(name_and_value(space + _("Coverage:"), string_format("%3d", coverage), width));
    props.push_back(
        name_and_value(space + _("Encumbrance:"), string_format("%3d", encumbrance), width));
    props.push_back(
        name_and_value(space + _("Warmth:"), string_format("%3d", worn_item.get_warmth()), width));
    props.push_back(name_and_value(
        space + string_format(_("Storage (%s):"), volume_units_abbr()),
...
std::vector<std::string> clothing_protection(const item& worn_item, const int width) {
    ...
    prot.push_back(
        name_and_value(space + _("Bash:"), string_format("%3d", worn_item.bash_resist()), width));
    prot.push_back(
        name_and_value(space + _("Cut:"), string_format("%3d", worn_item.cut_resist()), width));
    prot.push_back(name_and_value(
        space + _("Ballistic:"), string_format("%3d", worn_item.bullet_resist()), width));
    prot.push_back(
        name_and_value(space + _("Acid:"), string_format("%3d", worn_item.acid_resist()), width));
    prot.push_back(
        name_and_value(space + _("Fire:"), string_format("%3d", worn_item.fire_resist()), width));
    prot.push_back(name_and_value(
        space + _("Environmental:"), string_format("%3d", worn_item.get_env_resist()), width));
    return prot;
}
...
std::vector<std::string> clothing_flags_description(const item& worn_item) {
    ...
    if (worn_item.has_flag(flag_HOOD)) { description_stack.emplace_back(_("It has a hood.")); }
    if (worn_item.has_flag(flag_POCKETS)) { description_stack.emplace_back(_("It has pockets.")); }
    ...
    if (worn_item.has_flag(flag_FANCY)) { description_stack.emplace_back(_("It looks fancy.")); }
...
struct layering_item_info {
    ...
    bool operator==(const layering_item_info& o) const {
        ...
        return this->penalties.badness() == o.penalties.badness() && this->encumber == o.encumber
            && this->name == o.name;
    }
};
...
static std::vector<layering_item_info> items_cover_bp(const Character& c, const bodypart_id& bp) {
    ...
    for (auto elem_it = c.worn.begin(); elem_it != c.worn.end(); ++elem_it) {
        ...
        if (elem->covers(bp)) {
            s.push_back(
                {get_item_penalties(elem_it, c, bp), elem->get_encumber(c, bp), elem->tname()});
        }
    }
    return s;
}
...
std::vector<std::string> mid_pane_lines(
    location_vector<item>::const_iterator const& worn_item_it, const Character& c,
    const bodypart_id& bp) {
    item* const& worn_item = *worn_item_it;
    const int width = 40;
    std::vector<std::string> out;
    out.push_back(worn_item->type_name(1));
    for (const std::string& s : clothing_properties(*worn_item, width, c, bp)) { out.push_back(s); }
    for (const std::string& s : clothing_protection(*worn_item, width)) { out.push_back(s); }
    const std::string layer = clothing_layer(*worn_item);
    if (!layer.empty()) { out.push_back(layer); }
    for (const std::string& s : clothing_flags_description(*worn_item)) { out.push_back(s); }
    ...
    if (!penalties.body_parts_with_out_of_order_penalty.empty()) {
        const std::string body_parts = body_part_names(
            penalties.body_parts_with_out_of_order_penalty);
...
void register_al_rml_types(Rml::DataModelConstructor& c) {
    if (g_al_types_registered) { return; }
...
void show_armor_layers_ui(Character& who) {
    ...
    for (const bodypart_id& it : all_parts) { armor_cat.insert(it); }
    ...
    for (const bodypart_id& cover : armor_cat) {
        for (const item* const& elem : who.worn) {
            if (elem->covers(cover)) { req_right_h++; }
        }
    }
    ...
    ui.on_screen_resize([&](ui_adaptor& ui) {
        ...
        w_sort_middle =
            catacurses::newwin(cont_h - num_of_parts - 1, middle_w, win + point(2 + left_w, 3));
        w_sort_right = catacurses::newwin(cont_h, right_w, win + point(3 + left_w + middle_w, 3));
        w_encumb = catacurses::newwin(
            num_of_parts + 1, middle_w, win + point(2 + left_w, -1 + 3 + cont_h - num_of_parts));
    ...
    const auto sync_rml = [&]() {
        ...
        data->cat_rml = cata_text_to_rml(
            colorize(_("Sort Armor"), c_white)
        ...
        for (int i = 0; i < leftListSize; i++) {
            ...
            r.name_rml = cata_text_to_rml(
                colorize(worn->display_name(), penalties.color_for_stacking_badness()));
            r.storage_rml = cata_text_to_rml(format_volume(worn->get_storage()));
            r.hidden_rml =
                worn->has_flag(json_flag_HIDDEN)
                    ? cata_text_to_rml(colorize(_("H"), c_cyan))
                    : Rml::String();
        ...
        const auto combine_bp = [&who](const bodypart_id& cover) -> bool {
            ...
            return cover != opposite && items_cover_bp(who, cover) == items_cover_bp(who, opposite);
        };
        ...
        for (const bodypart_id cover : rl) {
            if (cover.id().is_null()) { continue; }
            const bool is_highlighted =
                cover == bp
                || (combine_bp(cover) && static_cast<bodypart_id>(cover.obj().opposite_part) == bp);
            push_line(
                data->right,
    ...
    rml.open(armor_layers_rmlui_enabled(), "sortarmor", ctxt, [&](Rml::DataModelConstructor& c) {
        ...
        c.BindEventCallback(
            "on_left", [&](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList& args) {
                if (selected >= 0) { return; }
                int idx = -1;
                if (!args.empty()) { args[0].GetInto(idx); }
                if (idx >= 0 && idx < leftListSize) { leftListIndex = idx; }
            });
        c.BindEventCallback(
            "on_prev_bp", [&](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
                if (tabindex-- == 0) { tabindex = tabcount - 1; }
                leftListIndex = leftListOffset = 0;
                selected = -1;
            });
        c.BindEventCallback(
            "on_next_bp", [&](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
    ...
    while (!exit) {
        ...
        if (bp.id().is_null()) {
            ...
            int i = 0;
            for (auto it = who.worn.begin(); it != who.worn.end(); ++it) {
                if ((*it)->covers(bp)) { tmp_worn.push_back(i); }
                i++;
            }
        }
        ...
        // This mirrors the recalc_pane()+re-fetch pattern used for advanced_inv.
        if (coop_fiber::active()) {
            tmp_worn.clear();
            const bodypart_id& bp_post = armor_cat[tabindex];
            if (bp_post.id().is_null()) {
                int i = 0;
                for (auto it = who.worn.begin(); it != who.worn.end(); ++it) {
                    tmp_worn.push_back(i++);
                }
                ...
                int i = 0;
                for (auto it = who.worn.begin(); it != who.worn.end(); ++it) {
                    if ((*it)->covers(bp_post)) { tmp_worn.push_back(i); }
                    i++;
                }
            }
            leftListSize = static_cast<int>(tmp_worn.size());
        ...
        if (action == "UP" && leftListSize > 0) {
            if (leftListIndex > 0) {
                leftListIndex--;
                if (leftListIndex < leftListOffset) { leftListOffset = leftListIndex; }
            } else {
            ...
            if (rightListOffset + rightListLines < rightListSize) { rightListOffset++; }
        } else if (action == "PREV_TAB") {
            if (rightListOffset > 0) { rightListOffset--; }
            ...
            std::stable_sort(who.worn.begin(), who.worn.end(), [](item* const& l, item* const& r) {
            ...
            if (loc) {
                ...
                if (equipped) {
                    ...
                    if (tabindex == num_of_parts || loc->covers(bp)) {
                        ...
                        bool found = false;
                        leftListIndex =
                            std::count_if(who.worn.begin(), who.worn.end(), [&](item* const& i) {
                                if (i == loc) { found = true; }
            ...
            popup_getkey(
                ...
                ctxt.get_desc("EQUIP_ARMOR"), ctxt.get_desc("EQUIP_ARMOR_HERE"),

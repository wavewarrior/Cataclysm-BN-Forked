...
template <> const spell_type &string_id<spell_type>::obj() const
{
    return spell_factory.obj( *this );
}

template <> bool string_id<spell_type>::is_valid() const { return spell_factory.is_valid( *this ); }
...
void spell_type::load( const JsonObject& jo, const std::string & )
{
    ...
    for( auto d : temp_vector ) {
        ...
        std::array<damage_type, 8> ele = {
            ...
            DT_PSI,  DT_LIGHT, DT_BIOLOGICAL, DT_ELECTRIC
        }; // True not included
        if( d == "physical" ) {
...
void spell::make_sound( const tripoint_bub_ms& target ) const
{
    ...
    int loudness = std::abs( damage() ) / 3;
        if( has_flag( spell_flag::LOUD ) ) { loudness += 1 + damage() / 3; }

...
enum class dump_mode { TSV, HTML };

enum class monster_activity_ai_mode : int { normal, activity_skip };
...
public:
     ...
     */
    bool travel_to_dimension(
        const std::string& dim_id, const world_type_id& world_type,
     ...
     */
    const std::string& get_current_dimension_id() const { return current_dimension_id_; }
     ...
     */
    template <typename T = Creature> T* critter_by_id(const character_id& id);
    /**
     ...
     */
    template <typename T = Creature> shared_ptr_fast<T> shared_from(const T& critter);
    ...
    monster* place_critter_around(
        const shared_ptr_fast<monster>& mon, const tripoint_bub_ms& center, int radius,
        bool forced = false);
    monster* place_critter_within(const mtype_id& id, const tripoint_range<tripoint_bub_ms>& range);
    monster* place_critter_within(
        const shared_ptr_fast<monster>& mon, const tripoint_range<tripoint_bub_ms>& range);
    /** @} */
    /**
    ...
    public:
        ...
        public:
            ...
            iterator(std::shared_ptr<std::vector<weak_ptr_fast<T>>> ref,
                ...
                : data_ref(std::move(ref)),
                  iter(t) {
                if (data_ref) {
                    while (iter != data_ref->end() && !valid()) { ++iter; }
                }
            }
            ...
            auto operator++() -> iterator& { // *NOPAD*
                ...
                do { ++iter; } while (iter != data_ref->end() && !valid());
                return *this;
            }
...
public:
    ...
    const std::list<shared_ptr_fast<npc>>& raw_npcs() const { return active_npc; }
    ...
    /** Find the npc with the given ID. Returns NULL if the npc could not be found. Searches all
     * loaded overmaps. */
...
public:
    ...
    bool is_processing_npcs() const { return processing_npcs_; }
    ...
    void catch_a_monster(
        monster* fish, const tripoint_bub_ms& pos, Character* who,
     ...
     */
    std::unordered_set<tripoint_bub_ms> get_fishable_locations(
        int radius, const tripoint_bub_ms& fish_pos);
     ...
     * @param center used to calculate the u.view_offset, could center the screen to the position it
     * represents
     * @param start_point  the start point of the targeting zone, also the initial local position of
     * the cursor
     * @param has_first_point should be true if the first point has been selected when editing the
     * zone
     ...
     * @param end_point the end point of the targeting zone, only used if is_moving_zone is true,
     * default is tripoint_zero
     * @return look_around_result
     */
    look_around_result look_around(
        bool show_window, tripoint_bub_ms& center, const tripoint_bub_ms& start_point,
        bool has_first_point, bool select_zone, bool peeking, bool is_moving_zone = false,
        const tripoint_bub_ms& end_point = tripoint_bub_ms::zero(),
        look_around_mode mode = LA_MODE_DEFAULT);

    // Shared method to print "look around" info
    void pre_print_all_tile_info(
        const tripoint_bub_ms& lp, const catacurses::window& w_info, int& line, int last_line,
        const visibility_variables& cache);

    // Colour-tagged text equivalent of the (deleted) curses look-around info,
    // for the look_around
    ...
    std::string print_all_tile_info_text(
        const tripoint_bub_ms& lp, const std::string& area_name, const visibility_variables& cache);
     ...
     */
    item* inv_map_splice(
        const item_filter& filter, const std::string& title, int radius = 0,
     ...
     * @note: Only works for SDL/TILES (otherwise the function returns `false`). A window (more
     * precisely, a viewport) must already exist and the SDL renderer must be valid.
     * @returns `true` if the screenshot generation was successful, `false` otherwise.
     */
    bool take_screenshot(const std::string& file_path) const;
    /** Saves a screenshot of the current viewport, as a PNG file. Filesystem location is derived
     * from the current world and character.
     * @note: Only works for SDL/TILES (otherwise the function returns `false`). A window (more
     * precisely, a viewport) must already exist and the SDL renderer must be valid.
     * @returns `true` if the screenshot generation was successful, `false` otherwise.
     */
    ...
    void toggle_gate(const tripoint_bub_ms& p);
    ...
    void knockback(
        const tripoint_bub_ms& s, const tripoint_bub_ms& t, int force, int stun, int dam_mult,
        Creature* source);
    ...
    void draw_line(
        const tripoint_bub_ms& p, const tripoint_bub_ms& center_point,
    ...
    void draw_item_override(
        const tripoint_bub_ms& p, const itype_id& id, const mtype_id& mid, bool hilite);
    void draw_vpart_override(
        const tripoint_bub_ms& p, const vpart_id& id, int part_mod, units::angle veh_dir,
        bool hilite, tripoint_mnt_veh mount);
    void draw_below_override(const tripoint_bub_ms& p, bool draw);
    void draw_monster_override(
        const tripoint_bub_ms& p, const mtype_id& id, int count, bool more, Attitude att);
...
private:
    ...
    vehicle* place_vehicle_nearby(
        const vproto_id& id, const point_abs_omt& origin, int min_distance, int max_distance,
        const std::vector<std::string>& omt_search_types = {}, bool notwater = false);
...
private:
    ...
    auto monmove(
        monster_activity_ai_mode mode = monster_activity_ai_mode::normal,
...
public:
    ...
    int mostseen = 0; // # of mons seen last turn; if this increases, set safe_mode to
                      // SAFE_MODE_STOP
...
private:
    ...
    /** How far the tileset should be zoomed out, 16 is default. 32 is zoomed in by x2, 8 is zoomed
     * out by x0.5 */
...
private:
    ...
    void activate_dimension_state(const std::string& new_dim_id, const std::string& old_dim_id);

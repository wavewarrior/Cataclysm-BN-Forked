...
public:
    ...
    [[nodiscard]] auto selected_unload_all() const -> bool { return unload_all_selected; }

protected:
    auto handle_action(const std::string& action) -> bool override {
        if (action != "unload_all") { return false; }
...
inventory_filter_preset::inventory_filter_preset(const item_filter& filter): filter(filter) {}

bool inventory_filter_preset::is_shown(const item* location) const { return filter(*location); }

static item* inv_internal(
    player& u, const inventory_selector_preset& preset, const std::string& title, int radius,
    const std::string& none_message, const std::string& hint = std::string(),
    ...
    const std::vector<activity_id> consuming{
        ACT_EAT_MENU, ACT_CONSUME_FOOD_MENU, ACT_CONSUME_DRINK_MENU, ACT_CONSUME_MEDS_MENU};
    ...
    do {
        ...
        if (include_fake_bionics) { inv_s.add_bionics_items(u); }
        if (has_init_filter) {
...
void game_menus::inv::common(avatar& you) {
    ...
    do {
        ...
        const auto func_pos_x = []() { return 0; };
        const auto func_width = []() { return 50; };
        started_action = examine_item_menu::
            run(*location, func_pos_x, func_width, examine_item_menu::menu_pos_t::right);
    } while (!started_action);
}

item* game_menus::inv::titled_filter_menu(
    const item_filter& filter, avatar& you, const std::string& title,
    const std::string& none_message, int radius) {
    return inv_internal(you, inventory_filter_preset(filter), title, radius, none_message);
}

item* game_menus::inv::titled_menu(
    avatar& you, const std::string& title, const std::string& none_message) {
...
public:
    armor_inventory_preset(player& pl, const std::string& color_in): p(pl), color(color_in) {

        append_cell([this](const item* loc) { return get_number_string(loc->get_avg_encumber(p)); },
                    _("AVG ENCUMBRANCE"));

        append_cell(
            [this](const item* loc) {
                return loc->get_storage() > 0_ml
                         ? string_format("<%s>%s</color>", color, format_volume(loc->get_storage()))
                         : std::string();
            },
            _("STORAGE"));
        ...
        append_cell([this](const item* loc) { return get_number_string(loc->get_warmth()); },
                    _("WARMTH"));

        append_cell([this](const item* loc) { return get_number_string(loc->bash_resist()); },
                    _("BASH"));

        append_cell([this](const item* loc) { return get_number_string(loc->cut_resist()); },
                    _("CUT"));

        append_cell([this](const item* loc) { return get_number_string(loc->bullet_resist()); },
                    _("BULLET"));

        append_cell([this](const item* loc) { return get_number_string(loc->acid_resist()); },
                    _("ACID"));

        append_cell([this](const item* loc) { return get_number_string(loc->fire_resist()); },
                    _("FIRE"));

        append_cell([this](const item* loc) { return get_number_string(loc->get_env_resist()); },
                    _("ENV"));
    }
...
public:
    wear_inventory_preset(player& p, const std::string& color): armor_inventory_preset(p, color) {}

    bool is_shown(const item* loc) const override { return loc->is_armor() && !p.is_worn(*loc); }
...
item* game_menus::inv::wear(player& p) {
    return inv_internal(
        p, wear_inventory_preset(p, "color_yellow"), _("Wear item"), 1,
...
item* game_menus::inv::take_off(avatar& you) {
    return inv_internal(
        you, take_off_inventory_preset(you, "color_red"), _("Take off item"), 1,
        _("You aren't wearing anything."));
}

item* game::inv_map_splice(
    const item_filter& filter, const std::string& title, int radius,
    const std::string& none_message) {
    return inv_internal(u, inventory_filter_preset(filter), title, radius, none_message);
}
...
item* game_menus::inv::container_for(avatar& you, const item& liquid, int radius) {
    ...
    return inv_internal(
        you, inventory_filter_preset(filter),
        string_format(_("Container for %s"), liquid.display_name(liquid.charges)), radius,
        string_format(_("You don't have a suitable container for carrying %s."), liquid.tname()));
}
...
public:
    disassemble_inventory_preset(const player& p, const inventory& inv)
        ...
        append_cell(
            [this](const item* loc) {
                ...
                if (req.is_empty()) { return std::string(); }
                const item* i = loc;
    ...
    bool is_shown(const item* loc) const override { return get_recipe(loc); }

    std::string get_denial(const item* loc) const override {
        const ret_val<bool> ret = crafting::can_disassemble(p, *loc, inv);
        if (!ret.success()) { return ret.str(); }
...
item* game_menus::inv::disassemble(player& p) {
    return inv_internal(
        p, disassemble_inventory_preset(p, p.crafting_inventory()), _("Disassemble item"), 1,
...
public:
    comestible_inventory_preset(const player& p): p(p) {
        ...
        append_cell(
            [this](const item* loc) { return good_bad_none(get_edible_comestible(loc).quench); },
            _("QUENCH"));
        ...
        append_cell(
            [this](const item* loc) {
                ...
                if (spoils > 0_turns) { return to_string_clipped(spoils); }
        ...
        append_cell(
            [this](const item* loc) {
                ...
                const double converted_volume = round_up(
                    convert_volume(it.volume().value() / charges, &converted_volume_scale), 2);
        ...
        append_cell(
            [this](const item* loc) {
                if (g->u.can_estimate_rot()) {
                    ...
                    if (item.spoils > 0_turns) { return get_freshness(loc); }
                    return std::string("---");
                }
                return std::string();
            },
            ...
            [this, &p](const item* loc) {
                ...
                switch (p.get_cbm_rechargeable_with(get_consumable_item(loc))) {
                    ...
                    case rechargeable_cbm::other:
                        std::vector<bionic_id> bids = p.get_bionic_fueled_with(
                            get_consumable_item(loc));
                        if (!bids.empty()) {
                ...
                if (!cbm_name.empty()) { return string_format("<color_cyan>%s</color>", cbm_name); }
    ...
    bool sort_compare(const inventory_entry& lhs, const inventory_entry& rhs) const override {
        ...
        return order_a < order_b || (order_a == order_b && time_a < time_b)
...
protected:
    ...
    time_duration get_time_left(const item* loc) const {
        ...
        if (shelf_life > 0_turns) {
            ...
            // fresh items.
            if (time_left > shelf_life) { time_left = shelf_life; }
        }
    ...
    std::string get_time_left_rounded(const item* loc) const {
        const item& it = get_consumable_item(loc);
        if (it.is_going_bad()) { return _("soon!"); }
...
public:
    comestible_filtered_inventory_preset(const player& p, bool (*predicate)(const item& it))
        : comestible_inventory_preset(p),
          predicate(predicate) {}

    bool is_shown(const item* loc) const override {
        return comestible_inventory_preset::is_shown(loc) && predicate(get_consumable_item(loc));
    }
...
item* game_menus::inv::consume_food(player& p) {
    ...
    return inv_internal(
        ...
        _("Consume food"), 1, _("You have no food to consume."), get_consume_needs_hint(p));
}
...
item* game_menus::inv::consume_drink(player& p) {
    ...
    return inv_internal(
        ...
        _("Consume drink"), 1, _("You have no drink to consume."), get_consume_needs_hint(p));
}
...
item* game_menus::inv::use(avatar& you) {
    return inv_internal(
        you, activatable_inventory_preset(you), _("Use item"), 1,
        _("You don't have any items you can use."), std::string(), true);
}
...
public:
    gunmod_inventory_preset(const player& p, const item& gunmod): p(p), gunmod(gunmod) {
        ...
        append_cell([this](const item* loc) { return good_bad_none(get_odds(loc).second); },
                    _("DAMAGE RISK"));
    }

    bool is_shown(const item* loc) const override { return loc->is_gun() && !loc->is_gunmod(); }
    ...
    std::string get_denial(const item* loc) const override {
        ...
        if (!p.meets_requirements(gunmod, &*loc)) {
            return string_format(
                _("requires at least %s"), p.enumerate_unmet_requirements(gunmod, &*loc));
        }

        if (get_odds(loc).first <= 0) { return _("is too difficult for you to modify"); }
...
item* game_menus::inv::gun_to_modify(player& p, const item& gunmod) {
    return inv_internal(
        p, gunmod_inventory_preset(p, gunmod), _("Select gun to modify"), -1,
...
public:
    read_inventory_preset(const player& p): p(p) {
        ...
        append_cell(
            [this, &p](const item* loc) -> std::string {
                if (loc->type->can_use("MA_MANUAL")) { return _("martial arts"); }
                const islot_book& book = get_book(loc);
                if (!book.skill) { return std::string(); }
                ...
                const auto result = string_format(
                    pgettext("skill", "%1$s to %2$d (%3$d)"), book.skill->name(), book.level,
                    skill.level());
        ...
        append_cell(
            [this](const item* loc) -> std::string {
                const islot_book& book = get_book(loc);
            ...
            _("RECIPES"), unknown);
        append_cell(
            [&p](const item* loc) -> std::string {
                return good_bad_none(character_funcs::get_book_fun_for(p, *loc));
            },
            _("FUN"), unknown);

        append_cell(
            [this, &p, unknown](const item* loc) -> std::string {
                ...
                if (reader == nullptr) { return unknown; }
                ...
                const std::string duration =
                    to_string_approx(time_duration::from_turns(actual_turns), false);

                // Theoretical reading time (in turns) based on the reader speed. Free of penalties.
                const int normal_turns =
                    get_book(loc).time * reader->read_speed() / to_moves<int>(1_turns);
    ...
    bool is_shown(const item* loc) const override { return loc->is_book(); }
    ...
    nc_color get_color(const inventory_entry& entry) const override {
        if (!entry.is_item()) { return inventory_selector_preset::get_color(entry); }
    ...
    bool sort_compare(const inventory_entry& lhs, const inventory_entry& rhs) const override {
        ...
        if (p.is_avatar()) {
            ...
            if (!known_a || !known_b) { return (!known_a && !known_b) ? base_sort : !known_a; }
        }

        struct localized_string {
            std::string s;
            bool operator==(const localized_string& other) const { return s == other.s; }
        ...
        if (info_a.can_still_learn != info_b.can_still_learn) { return info_a.can_still_learn; }
        ...
        if (can_still_learn) {
            const auto a = std::
                make_tuple(skill_a, info_a.is_learnable_already ? 0 : 1, info_a.time_to_levelup);
            const auto b = std::
                make_tuple(skill_b, info_b.is_learnable_already ? 0 : 1, info_b.time_to_levelup);
            return (a == b) ? base_sort : (a < b);
        }

        if (skill_a == skill_b) { return base_sort; }
        return skill_a < skill_b;
    }

private:
    const islot_book& get_book(const item* loc) const { return *loc->type->book; }
...
item* game_menus::inv::steal(avatar& you, player& victim) {
    return inv_internal(
        victim, steal_inventory_preset(you, victim), string_format(_("Steal from %s"), victim.name),
        -1, string_format(_("%s's inventory is empty."), victim.name));
}
...
public:
    weapon_inventory_preset(const player& p): p(p) {
        append_cell(
            [this](const item* loc) {
                if (!loc->is_gun()) { return std::string(); }
                ...
                if (loc->ammo_data() && loc->ammo_remaining()) {
                    ...
                    if (loc->ammo_data()->ammo->damage.damage_units.front().damage_multiplier
                        != 1.0f) {
                        const float ammo_mult =
                            loc->ammo_data()->ammo->damage.damage_units.front().damage_multiplier;

                        return string_format(
                            "%s<color_light_gray>*</color>%s <color_light_gray>=</color> %s",
        ...
        append_cell(
            [this](const item* loc) { return get_damage_string(loc->damage_melee(DT_BASH)); },
            _("BASH"));

        append_cell(
            [this](const item* loc) { return get_damage_string(loc->damage_melee(DT_CUT)); },
            _("CUT"));

        append_cell(
            [this](const item* loc) { return get_damage_string(loc->damage_melee(DT_STAB)); },
            _("STAB"));

        append_cell(
            [this](const item* loc) {
                if (deals_melee_damage(*loc)) { return good_bad_none(loc->type->m_to_hit); }
                return std::string();
            },
            _("MELEE"));
...
item* game_menus::inv::wield(avatar& you) {
    return inv_internal(
        you, weapon_inventory_preset(you), _("Wield item"), 1, _("You have nothing to wield."));
}
...
public:
    ...
    bool is_shown(const item* loc) const override { return actor.can_holster(*loc); }
...
item* game_menus::inv::holster(player& p, item& holster) {
    ...
    if (!actor) {
        const std::string msg =
            string_format(_("You can't put anything into your %s."), holster_name);
        popup(msg, PF_GET_KEY);
        return nullptr;
    }

    const std::string title =
        actor->holster_prompt.empty() ? _("Holster item") : _(actor->holster_prompt);
    const std::string hint = string_format(_("Choose an item to put into your %s"), holster_name);

    return inv_internal(
        p, holster_inventory_preset(p, *actor), title, 1,
        string_format(_("You have no items you could put into your %s."), holster_name), hint);
}
...
public:
    ...
    bool is_shown(const item* loc) const override { return loc->is_gun(); }
...
public:
    salvage_inventory_preset(const player& p, const inventory& inv)
        ...
        append_cell(
            [](const item* loc) {
                return to_string_clipped(
                    time_duration::from_turns(salvage::moves_to_salvage(*loc) / 100));
            },
            _("TIME"));
    }
    ...
    bool is_shown(const item* loc) const override { return loc->is_salvageable(); }
...
item* game_menus::inv::salvage(player& p) {
    ...
    return inv_internal(
        p, salvage_inventory_preset(p, p.crafting_inventory()), _("Salvage what?"), 1,
        _("You have nothing to salvage."));
}

class repair_inventory_preset: public inventory_selector_preset {
public:
    repair_inventory_preset(
        const repair_item_actor* actor, const item* main_tool, player& character)
        : actor(actor),
          main_tool(main_tool),
          character(character) {
        append_cell(
            [this, actor, &character](const item* loc) {
                const auto comp_needed =
                    std::max<int>(1, std::ceil(loc->volume() / 250_ml * actor->cost_scaling));
                auto valid_entries = std::set<material_id>{};
                std::ranges::for_each(actor->materials, [&valid_entries, &loc](const auto& mat) {
                    if (loc->made_of(mat)) { valid_entries.emplace(mat); }
                });
                ...
                std::ranges::for_each(
                    valid_entries,
                    [this, &listed_components, &material_list, &crafting_inv,
                     &comp_needed](const auto& entry) {
                        const auto& component_id = entry.obj().repaired_with();
                        if (listed_components.contains(component_id)) { return; }
                        listed_components.emplace(component_id);
                        const auto num_comp =
                            get_cached_component_count(crafting_inv, component_id);
                        if (num_comp > 0) {
                            material_list.emplace_back(colorize(
                                string_format(_("%s (%d)"), item::nname(component_id), num_comp),
                                num_comp < comp_needed ? c_red : c_unset));
                        }
                    });

                auto ret = join(material_list, ", ");
                if (ret.empty()) { ret = _("<color_red>NONE</color>"); }
                return ret;
            },
        ...
        append_cell(
            [this](const item* loc) {
                ...
                return colorize(
                    string_format("%0.1f%%", 100.0f * chance.first),
        ...
        append_cell(
            [this](const item* loc) {
                const auto chance = get_cached_repair_chance(*loc);
                return colorize(
                    string_format("%0.1f%%", 100.0f * chance.second),
                    chance.second > chance.first
                        ? c_yellow
                        : (chance.second == 0 && chance.first > 0 ? c_light_green : c_unset));
            },
            _("DMG"));
    }
...
private:
    auto get_cached_component_count(
        const inventory& crafting_inv, const itype_id& component_id) const -> int {
...
static auto get_repair_hint(
    const player& character, const repair_item_actor* actor, const item* main_tool) -> std::string {
    auto hint = std::string{};
    hint.append(string_format(_("Tool: <color_cyan>%s</color>"), main_tool->display_name()));
    hint.append(" | ");
    hint.append(string_format(
        _("Skill used: <color_cyan>%s (%d)</color>"), actor->used_skill.obj().name(),
        character.get_skill_level(actor->used_skill)));
    return hint;
}

item* game_menus::inv::repair(player& p, const repair_item_actor* actor, const item* main_tool) {
    return inv_internal(
        p, repair_inventory_preset(actor, main_tool, p), _("Repair what?"), 1,
        string_format(
            _("You have no items that could be repaired with a %s."), main_tool->type_name(1)),
...
item* game_menus::inv::saw_barrel(player& p, item& tool) {
    ...
    return inv_internal(
        p, saw_barrel_inventory_preset(p, tool, *actor), _("Saw barrel"), 1,
        _("You don't have any guns."),
        string_format(_("Choose a weapon to use your %s on"), tool.tname(1, false)));
}
...
item* game_menus::inv::saw_stock(player& p, item& tool) {
    ...
    return inv_internal(
        p, saw_stock_inventory_preset(p, tool, *actor), _("Saw stock"), 1,
        _("You don't have any guns."),
        string_format(_("Choose a weapon to use your %s on"), tool.tname(1, false)));
}
...
std::vector<pickup::pick_drop_selection> game_menus::inv::pickup_from_tile(
    player& p, const tripoint_bub_ms& target) {
    p.inv_restack();
...
void game_menus::inv::compare(player& p, const std::optional<tripoint_rel_ms>& offset) {
    ...
    do {
        ...
        if (to_compare.first == nullptr || to_compare.second == nullptr) { break; }
...
void register_cmp_rml_types(Rml::DataModelConstructor& c) {
    if (g_cmp_types_registered) { return; }
...
void game_menus::inv::compare(const item& l, const item& r) {
    ...
    rml_doc rml;
    rml.open(compare_items_rmlui_enabled(), "compare", ctxt, [&](Rml::DataModelConstructor& c) {
    ...
    const auto scroll_both = [&](float frac) {
        if (!rml) { return; }
        for (const char* id : {"cmp-lhs", "cmp-rhs"}) {
...
void game_menus::inv::reassign_letter(Character& who, item& it, int invlet) {
    ...
    if (!invlet || inv_chars.valid(invlet)) {
        ...
        if (found) { invlets.erase(iter); }
        if (invlet && (!found || it.invlet != invlet)) { invlets[invlet] = it.typeId(); }
    ...
    while (true) {
        ...
        if (invlet == KEY_ESCAPE) {
            ...
            if (auto_setting == "enabled" || (auto_setting == "favorites" && it.is_favorite)) {
                popup_getkey(_(
                    "Note: The Auto Inventory Letters setting might still reassign a letter to "
                    "this item.\n"
...
void game_menus::inv::swap_letters(player& p) {
    ...
    while (true) {
        const std::string invlets = colorize_symbols(
            inv_chars.get_allowed_chars(), [&p](const std::string::value_type& elem) {
...
static item* autodoc_internal(
    player& u, player& patient, const inventory_selector_preset& preset, int radius,
    bool uninstall = false, bool surgeon = false) {
    inventory_pick_selector inv_s(u, preset);
    std::string hint;
    ...
    if (!surgeon) { // surgeon use their own anesthetic, player just need money
        if (patient.has_trait(trait_NOPAIN)) {
            ...
            const int drug_count =
                std::accumulate(a_filter.begin(), a_filter.end(), 0, [](int sum, const item* it) {
                    return sum + it->ammo_remaining();
                });
            hint =
                string_format(_("<color_yellow>Available anesthetic: %i mL</color>"), drug_count);
        }
    }
    ...
    const auto title =
        uninstall ? _("Bionic removal patient: %s") : _("Bionic installation patient: %s");
...
public:
    ...
    std::string get_denial(const item* loc) const override {
        ...
        } else if (bid->upgraded_bionic && !pa.has_bionic(bid->upgraded_bionic)
                   && it->is_upgrade()) {
...
private:
    ...
    std::string get_failure_chance(const item* loc) {
        ...
        std::vector<item*> install_programs = p.crafting_inventory().items_with(
            [loc](const item& it) -> bool {
                return it.typeId() == loc->type->bionic->installation_data;
            });

        const bool has_install_program = !install_programs.empty();

        const int adjusted_skill =
            installer.bionics_adjusted_skill(skill_firstaid, skill_computer, skill_electronics, -1);

        if (g->u.has_trait(trait_DEBUG_BIONICS)) {
            chance_of_failure = 0;
        } else {
            chance_of_failure =
                has_install_program ? 1 : 100 - bionic_manip_cos(adjusted_skill, difficulty);
        }

        return string_format(
            has_install_program ? _("<color_white>*</color> %i%%") : _("%i%%"), chance_of_failure);
    }

    std::string get_anesth_amount(const item* loc) {
        if (!cbm_needs_anesthesia(pa)) { return std::string("-"); }
        const int weight = 7;
        const int duration = loc->type->bionic->difficulty * 2;
        return string_format(_("%i mL"), anesthetic_requirement(duration * weight));
    }
    ...
    std::string get_denial(const item* loc) const override {
        ...
        if (it->has_fault(fault_bionic_nonsterile)) {
            ...
            return _("CBM is already installed.");
                   ...
                   && it->is_upgrade()) {
            return _("No base version installed.");
        } else if (std::any_of(bid->available_upgrades.begin(), bid->available_upgrades.end(),
                               std::bind(&player::has_bionic, &pa, std::placeholders::_1))) {
            return _("Superior version installed.");
        } else if (pa.is_npc() && !bid->has_flag(flag_BIONIC_NPC_USABLE)) {
...
private:
    ...
    std::string get_failure_chance(const item* loc) {
        ...
        const int adjusted_skill =
            installer.bionics_adjusted_skill(skill_firstaid, skill_computer, skill_electronics, 20);
    ...
    std::string get_money_amount(const item* loc) { return format_money(loc->price(true) * 2); }
};

item* game_menus::inv::install_bionic(player& p, player& patient, bool surgeon) {
    if (surgeon) {
        return autodoc_internal(
            p, patient, bionic_install_surgeon_preset(p, patient), 5, false, surgeon);
    } else {
...
public:
    bionic_sterilize_preset(player& pl): p(pl) {}
...
static item* autoclave_internal(player& u, const inventory_selector_preset& preset, int radius) {
    inventory_pick_selector inv_s(u, preset);

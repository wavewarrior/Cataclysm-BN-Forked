...
int main(int argc, char* argv[]) {
    ...
    while (true) {
...
#ifdef COOP_ENABLED
        {
            ...
            constexpr double TICK_INTERVAL_MS = 1000.0;
            constexpr int MAX_CATCH_UP = 3;
            bool game_done = false;

            while (!game_done) {
                const auto now = clk::now();
                g->main_loop_accum_ms_ +=
                ...
                int ticks = 0;
                while (g->main_loop_accum_ms_ >= TICK_INTERVAL_MS && ticks < MAX_CATCH_UP) {
                    g->main_loop_accum_ms_ -= TICK_INTERVAL_MS;
                    ++ticks;
                    ...
                    while (g->u.moves > 0) {
                        if (!g->pending_action_queue_.empty()) {
                            ...
                            // If the action opened a modal fiber, stop draining —
                            // remaining queued actions must wait; the main loop will
                            // route input to resume() before the next tick fires.

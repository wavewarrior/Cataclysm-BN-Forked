...
int map::burn_body_part( player& u, field_entry& cur, body_part bp_token, const int scale )
{
    ...
    if( u.get_armor_type( DT_ACID, bp ) < damage ) {
        const dealt_damage_instance ddi =
            u.deal_damage( nullptr, bp.id(), damage_instance( DT_ACID, damage ) );
...
// This entire function makes very little sense. Why are the rules the way they are? Why does
// walking into some things destroy them but not others?
...
If you wish for a field effect to do something over time (propagate, interact with terrain, etc)
place it in process_subfields
*/
void map::player_in_field( player& u )
{
    ...
    if( u.in_vehicle ) {
        if( const optional_vpart_position vp = veh_at( u.bub_pos() ) ) { inside = vp->is_inside(); }
    }

    // Iterate through all field effects on this tile.
    // Do not remove the field with remove_field, instead set it's intensity to 0. It will be
    // removed later by the field processing, which will also adjust field_count accordingly.
    for( auto& field_list_it : curfield ) {
        field_entry& cur = field_list_it.second;
        if( !cur.is_field_alive() ) { continue; }
        ...
        if( ft == fd_web ) {
            ...
            if( !u.has_trait( trait_WEB_WALKER ) && !u.in_vehicle ) {
                ...
                u.add_effect( effect_webbed, 1_turns, bodypart_str_id::NULL_ID(),
            ...
            if( !u.in_vehicle && !u.has_trait( trait_ACIDPROOF ) ) {
                ...
                if( on_ground && total_damage > 0 ) {
                    u.add_msg_player_or_npc(
                        m_bad, _( "The acid burns your body!" ),
                        _( "The acid burns <npcname>s body!" ) );
                } else if( total_damage > 0 ) {
                    u.add_msg_player_or_npc(
                        m_bad, _( "The acid burns your legs and feet!" ),
            ...
            if( !u.has_active_bionic( bio_heatsink ) && !u.is_wearing( itype_rm13_armor_on ) ) {
                ...
                if( adjusted_intensity >= 1 ) {
                    ...
                    if( !u.is_on_ground() ) {
                        ...
                        for( const auto& bp : all_bps ) { parts_burned.emplace_back( bp ); }
                    }

                    int total_damage = 0;
                    for( const bodypart_id part_burned : parts_burned ) {
                        const dealt_damage_instance dealt = u.deal_damage(
                                                                nullptr, part_burned,
                    ...
                    if( total_damage > 0 ) {
                        u.add_msg_player_or_npc(
                            m_bad, _( player_burn_msg[msg_num] ), _( npc_burn_msg[msg_num] ) );
                    } else {
        ...
        if( ft == fd_tear_gas ) {
            ...
            if( cur.get_field_intensity() > 1 && ( !inside || one_in( 3 ) ) ) {
                u.add_env_effect(
                    effect_blind, body_part_eyes, cur.get_field_intensity() * 2, 10_seconds );
            }
        }
        if( ft == fd_fungal_haze ) {
            if( !u.has_trait( trait_M_IMMUNE ) && ( !inside || one_in( 4 ) ) ) {
                u.add_env_effect(
                    effect_fungus, body_part_mouth, 4, 10_minutes, bodypart_str_id::NULL_ID() );
                u.add_env_effect(
                    effect_fungus, body_part_eyes, 4, 10_minutes, bodypart_str_id::NULL_ID() );
            }
        }
        if( ft == fd_dazzling ) {
        ...
        if( cur.extra_radiation_min() > 0 ) {
            ...
            if( cur.radiation_hurt_damage_min() > 0 && !rad_proof ) {
                ...
                u.hurtall( rng( cur.radiation_hurt_damage_min(), cur.radiation_hurt_damage_max() ),
                           nullptr );
            }
        }
        if( ft == fd_flame_burst ) {
            ...
            if( !inside ) {
                ...
                if( !u.has_active_bionic( bio_heatsink ) && !u.is_wearing( itype_rm13_armor_on ) ) {
                    u.add_msg_player_or_npc(
                        m_bad, _( "You're torched by flames!" ),
                        _( "<npcname> is torched by flames!" ) );
                    u.deal_damage(
                        nullptr, bodypart_id( "leg_l" ), damage_instance( DT_HEAT, rng( 2, 6 ) ) );
                    u.deal_damage(
                        nullptr, bodypart_id( "leg_r" ), damage_instance( DT_HEAT, rng( 2, 6 ) ) );
                    u.deal_damage(
                        nullptr, bodypart_id( "torso" ), damage_instance( DT_HEAT, rng( 4, 9 ) ) );
                    u.check_dead_state();
                } else {
                    u.add_msg_player_or_npc(
        ...
        if( ft == fd_electricity ) {
            ...
            if( !u.is_elec_immune() ) {
                ...
                for( const bodypart_id& bp : u.get_all_body_parts( true ) ) {
                    ...
                    total_damage +=
                        u.deal_damage( nullptr, bp, damage_instance( DT_ELECTRIC, dmg ) )
                        .total_damage();
                }
                ...
                if( total_damage > 0 ) {
                    if( u.has_trait( trait_ELECTRORECEPTORS ) ) {
                        u.add_msg_player_or_npc(
                            m_bad, _( "You're painfully electrocuted!" ), _( "<npcname> is shocked!" ) );
                        u.mod_pain( total_damage / 2 );
                    } else {
                        u.add_msg_player_or_npc(
                            m_bad, _( "You're shocked!" ), _( "<npcname> is shocked!" ) );
                    }
                } else {
                    u.add_msg_player_or_npc(
        ...
        if( ft == fd_fatigue ) {
            // Assume the rift is on the ground for now to prevent issues with the player being
            // unable access vehicle controls on the same tile due to teleportation.
            if( !u.in_vehicle ) {
        ...
        if( ft == fd_shock_vent || ft == fd_acid_vent ) { cur.set_field_intensity( 0 ); }
        if( ft == fd_bees ) {
            ...
            if( !u.is_underwater() ) {
                ...
                for( int i = 0; i < rng( 1, 7 ); i++ ) {
                    ...
                    for( const item * const& i : u.worn ) {
                        if( i->covers( bp ) ) { sum_cover += i->get_coverage( bp ); }
                    }
                    // Get stung if [clothing on a body part isn't thick enough (like t-shirt) OR
                    // clothing covers less than 100% of body part] AND clothing on affected body
                    // part has low environmental protection value
                    if( ( u.get_armor_cut( bp ) <= 1
                          || ( sum_cover < 100 && x_in_y( 100 - sum_cover, 100 ) ) )
                        && u.add_env_effect( effect_stung, bp.id(), intensity, 9_minutes ) ) {
                        u.add_msg_if_player(
                            m_bad, _( "The bees sting you in %s!" ),
        ...
        if( ft == fd_incendiary ) {
            ...
            if( u.has_trait( trait_M_SKIN2 ) || u.has_trait( trait_M_SKIN3 )
                || cur.get_field_intensity() == 1 ) {
                u.add_msg_player_or_npc(
                    m_bad, _( "The incendiary burns you!" ), _( "The incendiary burns <npcname>!" ) );
                u.hurtall( rng( 1, 3 ), nullptr );
            } else {
                u.add_msg_player_or_npc(
                    m_bad, _( "The incendiary melts into your skin!" ),
        ...
        if( ft == fd_fungicidal_gas || ft == fd_insecticidal_gas ) {
            ...
            if( !inside ) {
                ...
                if( !( u.worn_with_flag( STATIC( flag_id( "GAS_PROOF" ) ) )
                    ...
                    const int intensity = cur.get_field_intensity();
                    bool inhaled =
                        u.add_env_effect( effect_poison, body_part_mouth, 5, intensity * 10_seconds );
                    if( u.has_trait( trait_THRESH_MYCUS ) || u.has_trait( trait_THRESH_MARLOSS )
                        ...
                        inhaled |= u.add_env_effect(
                                       effect_badpoison, body_part_mouth, 5, intensity * 10_seconds );
...
void map::creature_in_field( Creature& critter )
{
    ...
    for( auto& field_entry_it : curfield ) {
        ...
        if( !cur_field_entry.is_field_alive() ) { continue; }
        ...
        for( const auto& fe : cur_field_entry.field_effects() ) {
            ...
            if( in_vehicle && !one_in( fe.chance_in_vehicle ) ) { continue; }
            if( inside_vehicle && !one_in( fe.chance_inside_vehicle ) ) { continue; }
            if( !inside_vehicle && !one_in( fe.chance_outside_vehicle ) ) { continue; }
            ...
            if( fe.is_environmental ) {
                effect_added =
                    critter.add_env_effect( fe.id, fe.bp, fe.intensity, fe.get_duration() );
            } else {
                effect_added = true;
            ...
            if( effect_added ) {
                critter.add_msg_player_or_npc(
                    fe.env_message_type, fe.get_message(), fe.get_message_npc() );
            }
        }
    }
}
    ...
    for( auto& field_list_it : curfield ) {
        ...
        if( cur_field_type == fd_web ) {
            if( !z.has_flag( MF_WEBWALK ) ) {
                z.add_effect( effect_webbed, 1_turns, bodypart_str_id::NULL_ID(),
                              cur.get_field_intensity() );
                cur.set_field_intensity( 0 );
            }
        }
        if( cur_field_type == fd_acid ) {
            if( !z.flies() ) {
                ...
                z.check_dead_state();
                if( d > 0 ) { z.add_effect( effect_corroding, 1_turns * rng( d / 2, d * 2 ) ); }
            }
        }
        ...
        if( cur_field_type == fd_sludge ) {
            if( !z.digs() && !z.flies() && !z.has_flag( MF_SLUDGEPROOF ) ) {
        ...
        if( cur_field_type == fd_fire ) {
            ...
            if( z.has_flag( MF_FIREPROOF ) || z.has_flag( MF_FIREY ) ) { return; }
            // TODO: Replace the section below with proper json values
            if( z.made_of_any( Creature::cmat_flesh ) ) { dam += 3; }
            if( z.made_of( material_id( "veggy" ) ) ) { dam += 12; }
            if( z.made_of( LIQUID ) || z.made_of_any( Creature::cmat_flammable ) ) { dam += 20; }
            if( z.made_of_any( Creature::cmat_flameres ) ) { dam += -20; }
            if( z.flies() ) { dam -= 15; }
            ...
            if( cur.get_field_intensity() == 1 ) {
                ...
                if( !z.flies() ) {
                    z.moves -= 20;
                    if( dam > 0 ) { z.add_effect( effect_onfire, 1_turns * rng( dam / 2, dam * 2 ) ); }
                }
        ...
        if( cur_field_type == fd_smoke ) {
            if( !z.has_flag( MF_NO_BREATHE ) ) {
                if( cur.get_field_intensity() == 3 ) { z.moves -= rng( 10, 20 ); }
        ...
        if( cur_field_type == fd_relax_gas ) {
            if( z.made_of_any( Creature::cmat_fleshnveg ) && !z.has_flag( MF_NO_BREATHE ) ) {
                z.add_effect(
                    effect_stunned,
                    rng( cur.get_field_intensity() * 4_turns, cur.get_field_intensity() * 8_turns ) );
            }
        }
        ...
        if( cur_field_type == fd_flame_burst ) {
            ...
            if( z.made_of_any( Creature::cmat_flesh ) ) { dam += 3; }
            if( z.made_of( material_id( "veggy" ) ) ) { dam += 12; }
            if( z.made_of( LIQUID ) || z.made_of_any( Creature::cmat_flammable ) ) { dam += 50; }
            if( z.made_of_any( Creature::cmat_flameres ) ) { dam += -25; }
            dam += rng( 0, 8 );
            z.moves -= 20;
        }
        ...
        if( cur_field_type == fd_incendiary ) {
            ...
            if( z.made_of( material_id( "veggy" ) ) ) { dam += 12; }
            if( z.made_of( LIQUID ) || z.made_of_any( Creature::cmat_flammable ) ) { dam += 20; }
            if( z.made_of_any( Creature::cmat_flameres ) ) { dam += -5; }
        ...
        if( cur_field_type == fd_fungal_haze ) {
            if( !z.type->in_species( FUNGUS ) && !z.type->has_flag( MF_NO_BREATHE )
                && !z.make_fungus() ) {
...
void map::propagate_field(
    const tripoint_bub_ms& center, const field_type_id& type, int amount, int max_intensity )
{
    ...
    while( amount > 0 && !open.empty() ) {
        ...
        while( !open.empty() && get_field_intensity( open.top().second, type ) == cur_intensity ) {
            if( !closed.contains( open.top().second ) ) { gas_front.push_back( open.top() ); }
        ...
        while( !gas_front.empty() ) {
            ...
            for( size_t i = 0; i < 8; i++ ) {
                ...
                if( closed.contains( pt ) ) { continue; }
...
std::ranges::for_each( field_positions, [&]( const point_sm_ms & local ) {
        ...
        for( auto it = curfield.begin(); it != curfield.end(); ) {
            ...
            if( is_newborn ) { cur_fd_type_id = fd_null; }
            ...
            if( cur_fd_type_id == fd_fire ) {
                ...
                const auto can_burn =
                    !no_floor && can_spread && ( check_flammable( ter ) || check_flammable( frn ) );
                const auto is_sealed =
                ...
                if( can_spread && cur.get_field_intensity() > 1 && one_in( 3 ) ) {
                    ...
                    if( cur.get_field_intensity() < maximum_intensity ) {
                        cur.set_field_intensity( cur.get_field_intensity() + 1 );
                        cur.set_field_age(
                            cur.get_field_age() + 10_minutes * cur.get_field_intensity() );
                    }
                }
                ...
                if( can_spread ) {
                    ...
                    std::ranges::for_each( std::views::iota( 0u, 8u ), [&]( size_t c ) {
                        ...
                        if( !dst.valid() ) { return; }
                        if( dst.get_field().find_field( fd_fire ) ) { return; }
                        const auto& dter = dst.get_ter_t();
                        const auto& dfur = dst.get_furn_t();
                        if( in_pit != ( dter.id.id() == t_pit ) ) { return; }
                        auto* nearwebfld = dst.get_field().find_field( fd_web );
                        auto spread_chance = 25 * ( cur.get_field_intensity() - 1 );
                        if( nearwebfld ) { spread_chance = 50 + spread_chance / 2; }
                        const auto dst_has_flammable_items =
                        std::ranges::any_of( dst.get_items().as_vector(), []( const item * i ) {
                            return i && i->flammable();
                        } );
                        ...
                        const auto can_ignite =
                            ...
                            && ( ( power >= 3 && cur.get_field_age() < 0_turns && one_in( 20 ) )
                                 || ( power >= 2 && ter_furn_has_flag( dter, dfur, TFLAG_FLAMMABLE )
                                      && one_in( 2 ) )
                                 || ( power >= 2 && ter_furn_has_flag( dter, dfur, TFLAG_FLAMMABLE_ASH )
                                      && one_in( 2 ) )
                                 || ( power >= 3 && ter_furn_has_flag( dter, dfur, TFLAG_FLAMMABLE_HARD )
                                      && one_in( 5 ) )
                                 || nearwebfld || ( dst_has_flammable_items && one_in( 5 ) ) );
                        if( can_ignite ) {
                            ...
                            if( nearwebfld ) { nearwebfld->set_field_intensity( 0 ); }
                        }
                    } );
                }
                ...
                if( no_floor && pos.z() > -OVERMAP_DEPTH ) {
                    ...
                    if( below_sm ) {
                        ...
                        if( !fire_below ) {
                            ...
                            auto new_i = std::
                                         max( cur.get_field_intensity(), fire_below->get_field_intensity() );
                            if( new_i < 3
            ...
            if( !is_newborn && cur.gas_can_spread() ) {
                ...
                if( gas_pct > 0 && cur.get_field_intensity() > 1
                    && !sm.floor_cache[local.x()][local.y()] && rng( 1, 100 ) <= gas_pct ) {
                    // Try to fall first.
                    ...
                    if( outdoor_speedup > 0_turns ) { cur.mod_field_age( outdoor_speedup ); }
                }
            }
            ...
            if( !is_newborn && cur_fd_type_id == fd_shock_vent ) {
                if( cur.get_field_intensity() > 1 ) {
                    if( one_in( 5 ) ) { cur.set_field_intensity( cur.get_field_intensity() - 1 ); }
                } else {
                    ...
                    for( auto dx = -5; dx <= 5; ++dx ) {
                        for( auto dy = -5; dy <= 5; ++dy ) {
                            ...
                            if( acid && acid->get_field_intensity() == 0 ) {
                                auto new_intensity =
                                    3 - ( std::abs( dx ) + std::abs( dy ) ) / 2 + ( one_in( 3 ) ? 1 : 0 );
                ...
                static const std::array<field_type_id, 18> bee_killers = {
                    {
                        ...
                        fd_insecticidal_gas, fd_fire_vent, fd_flame_burst, fd_electricity, fd_fatigue,
                        fd_shock_vent, fd_plasma, fd_laser
                    }
                };
                const auto killed = std::ranges::any_of( bee_killers, [&]( const field_type_id & k ) {
                ...
                if( killed ) {
                    ...
                    std::ranges::for_each( std::views::iota( 0u, 8u ), [&]( size_t c ) {
                        ...
                        if( dst.valid() && !dst.get_field().find_field( fd_bees ) ) {
                            if( dst.get_field().add_field(
                                    fd_bees, cur.get_field_intensity(), cur.get_field_age() ) ) {
                                ++dst.sm->field_count;
                                dst.sm->field_cache.push_back( dst.local );
                                dst.sm->is_uniform = false;
                            }
            ...
            if( !is_newborn && cur_fd_type.wandering_field ) {
                ...
                for( auto wx = -wradius; wx <= wradius; ++wx ) {
                    for( auto wy = -wradius; wy <= wradius; ++wy ) {
                        ...
                        if( wlocal.x() < 0 || wlocal.x() >= SEEX || wlocal.y() < 0
                            || wlocal.y() >= SEEY ) {
                            continue;
                        }

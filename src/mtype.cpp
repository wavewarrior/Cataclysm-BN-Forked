...
std::string mtype::faction_info_text() const
{
    // Parallel to mtype::faction_display, producing the same content as one
    // colour-tagged string for the RmlUi faction detail pane (curses untouched).
    std::vector<std::string> lines;
    lines.emplace_back( string_format( "%s  %s", colorize( sym, color ), nname() ) );
    ...
    lines.emplace_back( string_format( "%s: %s", colorize( _( "Difficulty" ), c_white ), diff_str ) );

    lines.emplace_back( string_format( "%s: %s", colorize( _( "Origin" ), c_white ),
    ...
    lines.emplace_back( string_format( "%s: %s", colorize( _( "Size" ), c_white ),
                                       size_iter == size_map.end() ? _( "Unknown" ) : _( size_iter->second ) ) );

    lines.emplace_back( string_format( "%s: %s", colorize( _( "Species" ), c_white ),
    ...
    lines.emplace_back( string_format( "%s: %s", colorize( _( "Senses" ), c_white ),
                                       enumerate_as_string( senses_str ) ) );

    if( has_flag( MF_SWIMS ) ) {
        lines.emplace_back( _( "It can swim." ) );
    }
    if( has_flag( MF_FLIES ) ) {
        lines.emplace_back( _( "It can fly." ) );
    }
    if( has_flag( MF_DIGS ) ) {
        lines.emplace_back( _( "It can burrow underground." ) );
    }
    if( has_flag( MF_CLIMBS ) ) {
        lines.emplace_back( _( "It can climb." ) );
    }
    if( has_flag( MF_GRABS ) ) {
        lines.emplace_back( _( "It can grab." ) );
    }
    if( has_flag( MF_VENOM ) ) {
        lines.emplace_back( _( "It can inflict poison." ) );
    }
    if( has_flag( MF_PARALYZE ) ) {
        lines.emplace_back( _( "It can inflict paralysis." ) );
    }

    lines.emplace_back( get_description() );

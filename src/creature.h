...
constexpr I operator+( const creature_size lhs, const I rhs ) { return static_cast<I>( lhs ) + rhs; }

constexpr I operator+( const I lhs, const creature_size rhs ) { return lhs + static_cast<I>( rhs ); }
...
constexpr bool operator<( const creature_size lhs, const I rhs ) { return static_cast<I>( lhs ) < rhs; }

constexpr bool operator<( const I lhs, const creature_size rhs ) { return lhs < static_cast<I>( rhs ); }
...
enum FacingDirection { FD_NONE = 0, FD_LEFT = 1, FD_RIGHT = 2 };
...
void update_animation_state(
    animation_state& state, const animation_tuning& tuning, double wall_now,
    float idle_phase = 0.f );
...
class Creature
{
    public:
        ...
        virtual const monster *as_monster() const { return nullptr; }
        virtual npc *as_npc() { return nullptr; }
        virtual const npc *as_npc() const { return nullptr; }
        virtual Character *as_character() { return nullptr; }
        virtual const Character *as_character() const { return nullptr; }

        /** DANGER: This is a huge landmine.
        This method will return a non-nullptr for NPCs. Do NOT use `critter.as_player() == nullptr` to
        verify that a critter is not the player. Use `is_player` instead. **/
        virtual player *as_player() { return nullptr; }
        /** DANGER: This is a huge landmine.
        This method will return a non-nullptr for NPCs. Do NOT use `critter.as_player() == nullptr` to
        verify that a critter is not the player. Use `is_player` instead. **/
        virtual const player *as_player() const { return nullptr; }
        virtual avatar *as_avatar() { return nullptr; }
        virtual const avatar *as_avatar() const { return nullptr; }
         ...
         * 0.0 means unhittable, 1.0 means all projectiles going through this creature's tile will hit
         * it.
         */
        ...
        virtual bool block_ranged_hit( Creature* source, bodypart_id& bp_hit, damage_instance& dam ) = 0;
        ...
        virtual void deal_melee_hit(
            Creature* source, item* source_weapon, int hit_spread, bool critical_hit, bool is_graze,
            const damage_instance& dam, dealt_damage_instance& dealt_dam );
        virtual void deal_melee_hit(
            Creature* source, int hit_spread, bool critical_hit, bool is_graze,
            const damage_instance& dam, dealt_damage_instance& dealt_dam );
        ...
        virtual void deal_projectile_attack(
            ...
            bool is_graze = false );
        virtual void deal_projectile_attack(
            Creature* source, dealt_projectile_attack& attack, bool is_graze = false );
         ...
         */
        virtual dealt_damage_instance deal_damage(
            Creature* source, bodypart_id bp, const damage_instance& dam, item* source_weapon,
            item* source_projectile, bool is_crit = false, bool is_graze = false );
        virtual dealt_damage_instance deal_damage(
            Creature* source, bodypart_id bp, const damage_instance& dam, item* source_weapon,
            bool is_crit = false, bool is_graze = false );
        virtual dealt_damage_instance deal_damage(
            Creature* source, bodypart_id bp, const damage_instance& dam, bool is_crit = false,
            bool is_graze = false );

        // for each damage type, how much gets through and how much pain do we
        // accrue? mutates damage and pain
        virtual void deal_damage_handle_type(
            const damage_unit& du, bodypart_id bp, int &damage, int &pain );
        // directly decrements the damage. ONLY handles damage, doesn't
        // increase pain, apply effects, etc
        virtual void apply_damage(
            Creature* source, item* source_weapon, item* source_projectile, bodypart_id bp, int amount,
            bool bypass_med = false ) = 0;
        virtual void apply_damage(
            Creature* source, item* source_weapon, bodypart_id bp, int amount,
            bool bypass_med = false ) = 0;
        virtual void apply_damage(
            Creature* source, bodypart_id bp, int amount, bool bypass_med = false ) = 0;
         ...
         */
        virtual void on_hit(
            Creature* source, bodypart_id bp_hit, dealt_projectile_attack const* proj = nullptr ) = 0;
         ...
         */
        virtual bool is_immune_field( const field_type_id & ) const { return false; }
         ...
         */
        bool add_env_effect(
            const efftype_id& eff_id, const bodypart_str_id& vector, int strength,
            const time_duration& dur, const bodypart_str_id& bp, int intensity = 1, bool force = false );
        bool add_env_effect(
            const efftype_id& eff_id, const bodypart_str_id& vector, int strength,
        ...
        /** Remove all effects. */
        void clear_effects();
        /** Check if creature has the matching effect. No bp means to check if the Creature has any
         * effect of the matching type, targeted or untargeted. */
        ...
        /** Handles the uncanny dodge bionic and effects, returns true if the creature successfully
         * dodges */
        virtual bool uncanny_dodge() { return false; }
    ...
    public:
        ...
        virtual void add_msg_if_player(
            const game_message_params & /*params*/, const std::string & /*msg*/ ) const {}
        ...
        template <typename... Args>
        void add_msg_if_player(
            const game_message_params& params, const char *const msg, Args&&... args ) const {
            if( params.type == m_debug && !debug_mode ) { return; }
            return add_msg_if_player( params, string_format( msg, std::forward<Args>( args )... ) );
        }
        template <typename... Args>
        void add_msg_if_player(
            const game_message_params& params, const std::string& msg, Args&&... args ) const {
            if( params.type == m_debug && !debug_mode ) { return; }
            return add_msg_if_player( params, string_format( msg, std::forward<Args>( args )... ) );
        }
        template <typename... Args>
        void add_msg_if_player(
            const game_message_params& params, const translation& msg, Args&&... args ) const {
            if( params.type == m_debug && !debug_mode ) { return; }
            return add_msg_if_player( params, string_format( msg, std::forward<Args>( args )... ) );
        }

        virtual void add_msg_if_npc( const std::string & /*msg*/ ) const {}
        virtual void add_msg_if_npc(
            const game_message_params & /*params*/, const std::string & /*msg*/ ) const {}
        ...
        template <typename... Args> void add_msg_if_npc( const char* const msg, Args&&... args ) const {
            return add_msg_if_npc( string_format( msg, std::forward<Args>( args )... ) );
        }
        template <typename... Args> void add_msg_if_npc( const std::string& msg, Args&&... args ) const {
            return add_msg_if_npc( string_format( msg, std::forward<Args>( args )... ) );
        }
        template <typename... Args> void add_msg_if_npc( const translation& msg, Args&&... args ) const {
            return add_msg_if_npc( string_format( msg, std::forward<Args>( args )... ) );
        }
        template <typename... Args>
        void add_msg_if_npc(
        ...
        void add_msg_player_or_npc(
            const translation & /*player_msg*/, const translation & /*npc_msg*/ ) const;
        void add_msg_player_or_npc(
            const game_message_params & /*params*/, const translation & /*player_msg*/,
        ...
        template <typename... Args>
        void add_msg_player_or_npc(
            const game_message_params& params, const char *const player_msg, const char *const npc_msg,
            Args&&... args ) const {
            if( params.type == m_debug && !debug_mode ) { return; }
            return add_msg_player_or_npc(
                       params, string_format( player_msg, std::forward<Args>( args )... ),
                       string_format( npc_msg, std::forward<Args>( args )... ) );
        }
        template <typename... Args>
        void add_msg_player_or_npc(
        ...
        void add_msg_player_or_say(
            const translation & /*player_msg*/, const translation & /*npc_speech*/ ) const;
        void add_msg_player_or_say(
            const game_message_params & /*params*/, const translation & /*player_msg*/,
        ...
        template <typename... Args>
        void add_msg_player_or_say(
            const game_message_params& params, const char *const player_msg,
    ...
    protected:
        ...
        int speed_base = 0; // only speed needs a base, the rest are assumed at 0 and calculated off
        // skills

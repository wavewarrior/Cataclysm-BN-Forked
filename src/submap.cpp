...
void submap::update_lum_rem(const point_sm_ms& p, const item& i) {
    ...
    for (const auto& it : itm[p.x()][p.y()]) {
        if (it->is_emissive()) { count++; }
    }

    if (count <= 256) { lum[p.x()][p.y()] = static_cast<uint8_t>(count - 1); }
}

void submap::insert_cosmetic(
    const point_sm_ms& p, const std::string& type, const std::string& str) {
    cosmetic_t ins;
...
void submap::update_legacy_computer() {
    if (legacy_computer) {
        for (const auto& p : submap_tiles()) {
            if (ter[p.x()][p.y()] == t_console) { computers.emplace(p, *legacy_computer); }
        }
...
const computer* submap::get_computer(const point_sm_ms& p) const {
    ...
    if (it != computers.end()) { return &it->second; }
    if (legacy_computer && ter[p.x()][p.y()] == t_console) { return legacy_computer.get(); }
    return nullptr;
}
...
void submap::rotate(int turns) {
    ...
    for (auto& elem : cosmetics) { elem.pos = rotate_point(elem.pos); }

    for (auto& elem : spawns) { elem.pos = rotate_point(elem.pos); }
    ...
    for (auto& elem : computers) { rot_comp.emplace(rotate_point(elem.first), elem.second); }
    computers = rot_comp;
    ...
    for (auto& elem : frn_vars) { rot_frn_vars.emplace(rotate_point(elem.first), elem.second); }
    frn_vars = rot_frn_vars;

    std::unordered_map<point_sm_ms, data_vars::data_set> rot_ter_vars;
    for (auto& elem : ter_vars) { rot_ter_vars.emplace(rotate_point(elem.first), elem.second); }
    ...
    std::ranges::for_each(
        std::views::iota(0, SEEX * SEEY) | std::views::transform([](int i) -> point_sm_ms {
            return {i % SEEX, i / SEEX};
        }),
        [this](const point_sm_ms& p) {
            if (trp[p.x()][p.y()] != tr_null) { trap_cache.push_back(p); }
            if (fld[p.x()][p.y()].displayed_field_type()) { field_cache.push_back(p); }
        });
}
        ...
        if ((get_ter(sp).obj().transparent || !get_furn(sp).obj().transparent)) {
            ...
            for (const auto& fld : get_field(sp)) {
                if (!fld.first.is_valid()) {
                    debugmsg(
                        "rebuild_transparency_cache: invalid field type id %d at "
                        "grid(%d,%d,%d) tile(%d,%d) field_count=%d is_uniform=%d",
                        fld.first.to_i(), grid_pos.x(), grid_pos.y(), grid_pos.z(), sp.x(), sp.y(),
                        field_count, static_cast<int>(is_uniform));
                    break;
                }
                const auto& cur = fld.second;
                if (!cur.is_transparent()) { value *= cur.translucency(); }
            }
...
auto submap::set_furn(const point_sm_ms& p, furn_id furn) -> void {
    is_uniform = false;
    emitter_cache = std::nullopt;
    frn[p.x()][p.y()] = furn;

...
void talk_function::clear_mission( npc& p )
{
    ...
    const auto it = std::ranges::find( p.chatbin.missions_assigned, miss );
...
void talk_function::buy_chicken( npc& p ) { spawn_animal( p, mon_chicken ); }
void talk_function::buy_horse( npc& p ) { spawn_animal( p, mon_horse ); }

void talk_function::buy_cow( npc& p ) { spawn_animal( p, mon_cow ); }
...
void talk_function::start_trade( npc& p ) { npc_trading::trade( p, 0, _( "Trade" ) ); }

void talk_function::sort_loot( npc& p )
{
    p.assign_activity(
        std::make_unique<player_activity>( std::make_unique<move_loot_activity_actor>() ) );
}
...
void talk_function::find_mount( npc& p )
{
    ...
    // if we got here and this was prompted by a renewal of the activity, and there are no valid
    // monsters nearby, then cancel whole thing.
    if( p.has_player_activity() ) { p.revert_after_activity(); }
}
...
void talk_function::revert_activity( npc& p ) { p.revert_after_activity(); }

void talk_function::do_craft( npc& p ) { p.do_npc_craft(); }
...
void talk_function::goto_location( npc& p )
{
    ...
    if( index < 0 || index > 2 || index == 1 ) { return; }
    if( index == 1 ) { destination = g->u.abs_omt_pos(); }
    p.goal = destination;
    p.omt_path =
        get_overmapbuffer( p.get_dimension() )
        .get_travel_path( p.abs_omt_pos(), p.goal, overmap_path_params::for_npc() );
...
void talk_function::stop_guard( npc& p )
{
    ...
    p.set_mission( NPC_MISSION_NULL );
    if( p.has_companion_mission() ) { p.reset_companion_mission(); }
...
void talk_function::control_npc( npc& p ) { get_avatar().control_npc( p ); }

void talk_function::reveal_stats( npc& p ) { character_display::disp_info( p ); }
...
void talk_function::bionic_install( npc& p )
{
    ...
    if( !npc_trading::pay_npc( p, price ) ) { return; }
...
void talk_function::bionic_remove( npc& p )
{
    ...
    for( const bionic& bio : all_bio ) {
        if( !std::ranges::contains( bionic_types, bio.info().itype() ) ) {
            ...
            if( bio.info().itype().is_valid() ) {
                ...
                bionic_names.push_back(
                    tmp->tname() + " - " + format_money( 50000 + ( tmp->price( true ) / 4 ) ) );
            } else {
    ...
    int bionic_index = uilist( _( "Which bionic do you wish to uninstall?" ), bionic_names );
    // Did we cancel?
    if( bionic_index < 0 ) {
    ...
    if( bionic_types[bionic_index].is_valid() ) {
        int tmp =
            item::spawn_temporary( bionic_types[bionic_index], calendar::start_of_cataclysm )
            ->price( true );
        price = 50000 + ( tmp / 4 );
    } else {
        price = 50000;
    ...
    if( g->u.can_uninstall_bionic( bionic_id( bionic_types[bionic_index].str() ), p, false ) ) {
        g->u.amount_of( bionic_types[bionic_index] ); // ??? this does nothing, it just queries the
        // count
...
static void give_aid_to( Character& guy )
{
    for( const bodypart_id& bp : guy.get_all_body_parts() ) {
        ...
        if( guy.has_effect( effect_bite, bp_healed ) ) { guy.remove_effect( effect_bite, bp_healed ); }
        if( guy.has_effect( effect_bleed, bp_healed ) ) { guy.remove_effect( effect_bleed, bp_healed ); }
...
static void generic_barber( const std::string& mut_type )
{
    ...
    for( auto elem : hair_muts ) {
        if( g->u.has_trait( elem ) ) { cur_hair = elem; }
        index += 1;
    ...
    if( choice != 0 ) {
        if( g->u.has_trait( cur_hair ) ) { g->u.remove_mutation( cur_hair, true ); }
        g->u.set_mutation( hair_muts[choice - 1] );
        add_msg( m_info, _( "You get a trendy new cut!" ) );
    }
}

void talk_function::barber_beard( npc & /*p*/ ) { generic_barber( "facial_hair" ); }

void talk_function::barber_hair( npc & /*p*/ ) { generic_barber( "hair_style" ); }
...
void talk_function::buy_10_logs( npc& p )
{
    ...
    std::vector<tripoint_abs_omt> places =
        get_overmapbuffer( get_player_character().get_dimension() )
        .find_all( get_player_character().abs_omt_pos(), find_params );
    if( places.empty() ) {
...
void talk_function::follow_only( npc& p ) { p.set_attitude( NPCATT_FOLLOW ); }

void talk_function::deny_follow( npc& p ) { p.add_effect( effect_asked_to_follow, 6_hours ); }

void talk_function::deny_lead( npc& p ) { p.add_effect( effect_asked_to_lead, 6_hours ); }

void talk_function::deny_equipment( npc& p ) { p.add_effect( effect_asked_for_item, 6_hours ); }

void talk_function::deny_train( npc& p ) { p.add_effect( effect_asked_to_train, 6_hours ); }
...
{
    if( p.get_attitude() == NPCATT_KILL ) { return; }

    if( p.sees( g->u ) ) { add_msg( _( "%s turns hostile!" ), p.name ); }
...
void talk_function::remove_stolen_status( npc& p ) { p.set_attitude( NPCATT_NULL ); }
...
{
    if( p.is_hallucination() ) { return; }
    get_map().add_item_or_charges( p.bub_pos(), p.remove_primary_weapon() );
}

void talk_function::player_weapon_away( npc & /*p*/ ) { g->u.i_add( g->u.remove_primary_weapon() ); }
...
void talk_function::player_weapon_drop( npc & /*p*/ )
{
    ...
    for( bionic& i : *g->u.my_bionics ) {
        if( i.powered && i.info().has_flag( flag_BIONIC_WEAPON ) ) { g->u.deactivate_bionic( i ); }
    }
}
...
{
    const auto mission =
        mission::reserve_new( mission_type_id( "MISSION_REACH_SAFETY" ), character_id() );
    mission->assign( g->u );
...
void talk_function::start_training( npc& p )
{
    ...
    if( skill.is_valid() && g->u.get_skill_level( skill ) < p.get_skill_level( skill ) ) {
        ...
        cost =
            p.calc_spell_training_cost( knows, temp_spell.get_difficulty(), temp_spell.get_level() );
        name = temp_spell.id().str();
        expert_multiplier =
            knows ? temp_spell.get_level() - g->u.magic->get_spell( sp_id ).get_level() : 1;
    ...
    auto act = std::make_unique<player_activity>(
                   std::make_unique<train_activity_actor>( name, expert_multiplier ) );
...
npc *pick_follower()
{
    ...
    for( const npc * p : followers ) { menu.addentry( -1, true, MENU_AUTOASSIGN, p->name ); }

    menu.query();
    if( menu.ret < 0 || static_cast<size_t>( menu.ret ) >= followers.size() ) { return nullptr; }
...
void talk_function::copy_npc_rules( npc& p )
{
    ...
    if( other != nullptr && other != &p ) { p.rules = other->rules; }
}

void talk_function::set_npc_pickup( npc& p ) { p.rules.pickup_whitelist->show( p.name ); }

void talk_function::npc_die( npc& p )
{
    p.die( nullptr );
    const shared_ptr_fast<npc> guy = get_overmapbuffer( p.get_dimension() ).find_npc( p.getID() );
    if( guy && !guy->is_dead() ) { guy->marked_for_death = true; }
}
...
void talk_function::clear_overrides( npc& p ) { p.rules.clear_overrides(); }

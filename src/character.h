...
static const std::string DEFAULT_HOTKEYS("1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTU"
                                         "VWXYZ");
...
enum class rechargeable_cbm { none = 0, reactor, furnace, other };
...
struct mountable_status {
    ...
    inline bool can_mount() const { return mountable && skills && size && carry_weight; };
};

/** @relates ret_val */
template <>
struct ret_val<edible_rating>::default_success
    : public std::integral_constant<edible_rating, edible_rating::edible> {};

/** @relates ret_val */
template <>
struct ret_val<edible_rating>::default_failure
    : public std::integral_constant<edible_rating, edible_rating::inedible> {};
...
public:
    ...
    Character* as_character() override { return this; }
    const Character* as_character() const override { return this; }
...
public:
    ...
    // Turned to false for simulating NPCs on distant missions so they don't drop all their gear in
    // sight
    ...
    **/
    ...
    /** Increases hunger, thirst, fatigue and stimulants wearing off. `rate_multiplier` is for
     * retroactive updates. */
    void update_needs(int rate_multiplier);
    needs_rates calc_needs_rates() const;
    /** Kills the player if too hungry, stimmed up etc., forces tired player to sleep and prints
     * warnings. */
    ...
    /** Returns true if the character is wearing something on the entered bodypart_id, ignoring
     * items with the ALLOWS_NATURAL_ATTACKS flag */
    ...
    // melee.cpp
    /** Checks for valid block abilities and reduces damage accordingly. Returns true if the player
     * blocks */
    ...
    /** Broken out of below function for the purpose of using it in other places, namely item info
     * displays */
     ...
     * @param force_technique special technique to use in attack (leave as nullptr to use random
     * technique).
     * @param allow_unarmed always uses the wielded weapon regardless of martialarts style
     */
    void melee_attack(
        Creature& t, bool allow_special, const matec_id* force_technique = nullptr,
    ...
    double crit_chance(
        float roll_hit, float target_dodge, const item& weap, const attack_statblock& attack) const;
...
private:
    ...
    bool valid_aoe_technique(
        Creature& t, const ma_technique& technique, std::vector<Creature*>& targets);
...
public:
    ...
    void apply_damage(
        Creature* source, item* source_weapon, bodypart_id hurt, int dam,
        bool bypass_med = false) override;
    void apply_damage(
        Creature* source, bodypart_id hurt, int dam, bool bypass_med = false) override;
    /** Calls Creature::deal_damage and handles damaged effects (waking up, etc.) */
    dealt_damage_instance deal_damage(
        Creature* source, bodypart_id bp, const damage_instance& d, item* source_weapon,
        item* source_projectile, bool is_crit = false, bool is_graze = false) override;
    dealt_damage_instance deal_damage(
        Creature* source, bodypart_id bp, const damage_instance& d, item* source_weapon,
        bool is_crit = false, bool is_graze = false) override;
    dealt_damage_instance deal_damage(
        Creature* source, bodypart_id bp, const damage_instance& d, bool is_crit = false,
    ...
    std::map<bodypart_id, int> get_armor_fire(
    ...
    /** Removes the appropriate costs (NOTE: will reapply mods & recalc sightlines in case of newly
     * activated mutation). */
    ...
    /** Harms all body parts for dam, with armor reduction. If vary > 0 damage to parts are random
     * within vary % (1-100) */
     ...
     */
    bodypart_str_id body_window(
        const std::string& menu_header, bool show_all, bool precise, int normal_bonus,
        int head_bonus, int torso_bonus, float bleed, float bite, float infect, float bandage_power,
...
public:
    ...
    /** Returns the flat damage bonus to given type from martial arts buffs, applied after the
     * multiplier */
    int mabuff_damage_bonus(damage_type type) const;
    /** Returns the flat penalty to move cost of attacks. If negative, that's a bonus. Applied after
     * multiplier. */
...
public:
    /** Picks a random valid mutation and gives it to the Character, possibly removing/changing
     * others along the way */
    void mutate();
    /** Returns true if the player doesn't have the mutation or a conflicting one and it complies
     * with the force typing */
    ...
    /** Handles process of introducing patient into anesthesia during Autodoc operations. Requires
     * anesthesia kits or NOPAIN mutation */
    void introduce_into_anesthesia(
        const time_duration& duration, Character& installer, bool needs_anesthesia);
    ...
    float bionics_adjusted_skill(
        const skill_id& most_important_skill, const skill_id& important_skill,
        const skill_id& least_important_skill, int skill_level = -1);
    /** Calculate non adjusted skill for (un)installing bionics */
    int bionics_pl_skill(
        const skill_id& most_important_skill, const skill_id& important_skill,
        const skill_id& least_important_skill, int skill_level = -1);
    /**Is the installation possible*/
    bool can_install_bionics(
        const itype& type, Character& installer, bool autodoc = false, int skill_level = -1);
    ...
    bool install_bionics(
        const itype& type, Character& installer, bool autodoc = false, int skill_level = -1);
    /**Success or failure of installation happens here*/
    void perform_install(
        bionic_id bid, bionic_id upbid, int difficulty, int success, int pl_skill,
    ...
    void bionics_install_failure(
        const std::string& installer, int difficulty, int success, float adjusted_skill);

    /**Is The uninstallation possible*/
    bool can_uninstall_bionic(
        const bionic_id& b_id, Character& installer, bool autodoc = false, int skill_level = -1);
    /** Initialize all the values needed to start the operation player_activity */
    bool uninstall_bionic(
        const bionic_id& b_id, Character& installer, bool autodoc = false, int skill_level = -1);
    /**Succes or failure of removal happens here*/
    void perform_uninstall(
        bionic_id bid, int difficulty, int success, const units::energy& power_lvl, int pl_skill);
    ...
    bool uninstall_bionic(
        const bionic& target_cbm, monster& installer, Character& patient, float adjusted_skill);
    /**When a monster fails the surgery*/
    void bionics_uninstall_failure(
        monster& installer, Character& patient, int difficulty, int success, float adjusted_skill);
    ...
    /**Consume fuel used by remote powered bionic, return amount of request unfulfilled (0 if
     * totally successful).*/
    ...
    static int worn_position_to_index(int position) { return -2 - position; }
    ...
    /**
     * Calculate (but do not deduct) the number of moves required when handling (e.g. storing,
     * drawing etc.) an item
     * @param it Item to calculate handling cost for
     * @param penalties Whether item volume and temporary effects (e.g. GRABBED, DOWNED) should be
     * considered.
     * @param base_cost Cost due to storage type.
     * @return cost in moves ranging from 0 to MAX_HANDLING_COST
     */
    int item_handling_cost(
        const item& it, bool penalties = true, int base_cost = INVENTORY_HANDLING_PENALTY) const;

    /**
     * Calculate (but do not deduct) the number of moves required when storing an item in a
     * container
     ...
     */
    int item_store_cost(
        const item& it, const item& container, bool penalties = true,
     ...
     * removed from the liquid item. The return value is the remaining liquid that could not be
     * transferred. The functions do not consume any move points.
    ...
    /// Alternate version if you need to specify a different orign point for nearby vehicle sources
    /// of lifting used for operations on distant objects (e.g. vehicle installation/uninstallation)
    ...
    virtual void drop(
        const drop_locations& what, const tripoint_bub_ms& target, bool stash = false);
     ...
     * @note items with integral magazines are reloadable if free capacity permits (+/- ammo
     * matches)
     */
    bool can_reload(const item& it, const itype_id& ammo = itype_id()) const;

    /**
     * Calculate (but do not deduct) the number of moves required to reload an item with specified
     * quantity of ammo
     * @param it Item to calculate reload cost for
     * @param ammo either ammo or magazine to use when reloading the item
     * @param qty maximum units of ammo to reload. Capped by remaining capacity and ignored if
     * reloading using a magazine.
     */
    ...
    bool worn_with_flag(
        const flag_id& flag, const bodypart_id& bp = bodypart_str_id::NULL_ID()) const;
    /** Returns the first worn item with a given flag. */
    const item* item_worn_with_flag(
        const flag_id& flag, const bodypart_id& bp = bodypart_str_id::NULL_ID()) const;
    /** Returns true if the player is wearing an item with the given flag. */
    ...
    bool worn_with_id(
        const itype_id& item_id, const bodypart_id& bp = bodypart_str_id::NULL_ID()) const;
    /** Returns the first worn item with a given id. */
    const item* item_worn_with_id(
        const itype_id& item_id, const bodypart_id& bp = bodypart_str_id::NULL_ID()) const;
    ...
    time_point get_time_died() const { return time_died; }
...
public:
    ...
    void on_effect_int_change(
        const efftype_id& effect_type, int intensity, const bodypart_str_id& bp) override;
    ...
    /** Checks to see if the player is using floor items to keep warm, and return the name of one
     * such item if so */
    ...
    /** Correction factor of the body temperature due to traits and mutations for player lying on
     * the floor **/
    ...
    /** Swap side on which item is worn; returns false on fail. If interactive is false, don't alert
     * player or drain moves */
    bool change_side(item& it, bool interactive = true);
    bool change_side(item* it, bool interactive = true);

    bool get_check_encumbrance() const { return check_encumbrance; }
    void set_check_encumbrance(bool new_check) { check_encumbrance = new_check; }
    ...
    void add_morale(
        const morale_type& type, int bonus, int max_bonus = 0,
        const time_duration& duration = 1_hours, const time_duration& decay_start = 30_minutes,
    ...
    const inventory& crafting_inventory(
        const tripoint_bub_ms& src_pos = tripoint_bub_ms::zero(), int radius = PICKUP_RANGE,
...
public:
    ...
    recipe_subset get_recipes_from_books(
        const inventory& crafting_inv, const recipe_filter& filter = nullptr) const;
    /**
    ...
    void make_craft(
        const recipe_id& id, int batch_size, const tripoint_bub_ms& loc = tripoint_bub_ms::zero());
    void make_all_craft(
        const recipe_id& id, int batch_size, const tripoint_bub_ms& loc = tripoint_bub_ms::zero());

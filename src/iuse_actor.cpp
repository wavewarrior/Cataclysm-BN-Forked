...
void holster_actor::load(const JsonObject& obj) {
    ...
    std::transform(tmp.begin(), tmp.end(), std::back_inserter(skills), [](const std::string& elem) {
...
void ammobelt_actor::info(const item&, std::vector<iteminfo>& dump) const {
    dump.emplace_back("AMMO", string_format(_("Can be used to assemble: %s"), item::nname(belt)));
}
...
auto iuse_flowerpot_plant::calculate_growth_time(const itype_id& seed_id, const int used_fert) const
    -> time_duration {
...
auto iuse_portal_link::can_use(const Character&, const item& it, bool, const tripoint_bub_ms&) const
    -> ret_val<bool> {
...
concept is_painter = requires(
    ...
    { painter.get_cost(thing) } -> std::same_as<float>;
    { painter.can_paint(thing) } -> std::same_as<bool>;
    { painter.get_color(thing) } -> std::same_as<RGBColorPair>;
    { painter.set_color(thing, color, layer) } -> std::same_as<bool>;
    { painter.describe(thing) } -> std::same_as<std::string>;
};

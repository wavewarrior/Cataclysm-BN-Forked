...
public:
    ...
    static std::unique_ptr<aim_activity_actor> use_bionic(
    ...
    activity_id get_type() const override { return activity_id("ACT_AIM"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_AUTODRIVE"); }
...
protected:
    ...
    bool equivalent_activity(const craft_activity_actor& other) const {
        return location == other.location && rec == other.rec && batch_size == other.batch_size;
    }

public:
    craft_activity_actor() = default;
    explicit craft_activity_actor(
        const recipe* rec, int batch_size = 1, int craft_counter = 0,
        const tripoint_abs_ms& location = tripoint_abs_ms::zero(),
        std::vector<comp_selection<item_comp>> item_selections = {},
        std::vector<comp_selection<tool_comp>> tool_selections = {}, bool tools_prepaid = false,
        bool is_long = false);

    activity_id get_type() const override { return activity_id("ACT_CRAFT"); }
    ...
    act_progress_message get_progress_message(
        const player_activity& act, const Character& who) const override;

    void serialize(JsonOut& jsout) const override;
    static std::unique_ptr<activity_actor> deserialize(JsonIn& jsin);

private:
    auto find_in_progress_craft(
...
private:
    ...
    bool equivalent_activity(const dig_activity_actor& other) const {
        return location == other.location && result_terrain == other.result_terrain
...
public:
    dig_activity_actor(
        int dig_moves, const tripoint_bub_ms& dig_loc, const std::string& resulting_ter,
        const tripoint_bub_ms& dump_loc, const std::string& dump_item_group)
        : moves_total(dig_moves),
          location(dig_loc),
          result_terrain(resulting_ter),
          byproducts_location(dump_loc),
          byproducts_item_group(dump_item_group) {}

    activity_id get_type() const override { return activity_id("ACT_DIG"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_DIG_CHANNEL"); }
...
public:
    ...
    disassemble_activity_actor(
        std::vector<iuse_location>&& targets, tripoint_abs_ms pos, bool recursive)
        : targets(std::move(targets)),
          pos(pos),
          recursive(recursive) {}
    ~disassemble_activity_actor() = default;

    activity_id get_type() const override { return activity_id("ACT_DISASSEMBLE"); }
    void calc_all_moves(player_activity& act, Character& who) override;
    void start(player_activity& act, Character& who) override;
    void do_turn(player_activity&, Character&) override;
    void finish(player_activity& act, Character& who) override;
...
public:
    ...
    drop_activity_actor(
        Character& ch, const drop_locations& items, bool force_ground,
        const tripoint_rel_ms& relpos);

    activity_id get_type() const override { return activity_id("ACT_DROP"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_HACKING"); }
...
public:
    ...
    repair_item_activity_actor(safe_reference<item> tool, const std::string& iuse_name)
        : tool_item(tool),
          iuse_name_string(iuse_name) {}

    void set_hack_vehicle(const tripoint_abs_ms& pos, int crafter_idx, const itype_id& tool_type) {
...
public:
    ...
    mend_item_activity_actor(
        safe_reference<item> target, const std::string& fault, const std::string& method)
        : target_item(target),
          fault_id_str(fault),
          method_id(method) {}

    activity_id get_type() const override { return activity_id("ACT_MEND_ITEM"); }
...
public:
    ...
    toolmod_add_activity_actor(safe_reference<item> tool, safe_reference<item> mod)
        : base_tool(tool),
          mod_item(mod) {}

    activity_id get_type() const override { return activity_id("ACT_TOOLMOD_ADD"); }
...
public:
    ...
    gunmod_add_activity_actor(
        safe_reference<item> gun, safe_reference<item> mod, int success_chance, int damage_chance,
        const itype_id& tool, int charges_qty)
        : gun_item(gun),
          mod_item(mod),
          roll(success_chance),
          risk(damage_chance),
...
public:
    ...
    reload_activity_actor(safe_reference<item> target, safe_reference<item> ammo, int quantity)
        : target_item(target),
          ammo_item(ammo),
          qty(quantity) {}
...
public:
    ...
    wear_activity_actor(std::vector<safe_reference<item>> items, std::vector<int> quantities)
        : items(std::move(items)),
          quantities(std::move(quantities)) {}

    activity_id get_type() const override { return activity_id("ACT_WEAR"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_ARMOR_LAYERS"); }
...
public:
    explicit hacksaw_activity_actor(const tripoint_bub_ms& target, const safe_reference<item>& tool)
        : target(target),
          tool(tool) {};

    activity_id get_type() const override { return activity_id("ACT_HACKSAW"); }
...
public:
    explicit boltcutting_activity_actor(
        const tripoint_bub_ms& target, const safe_reference<item> tool)
        : target(target),
          tool(tool) {};

    activity_id get_type() const override { return activity_id("ACT_BOLTCUTTING"); }
...
private:
    ...
    lockpick_activity_actor(
        int moves_total, safe_reference<item> lockpick, detached_ptr<item>&& fake_lockpick,
        const tripoint_abs_ms& target)
        : moves_total(moves_total),
          lockpick(lockpick),
          fake_lockpick(new fake_item_location()),
          target(target) {
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_LOCKPICK"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_MIGRATION_CANCEL"); }
...
public:
    move_items_activity_actor(
        std::vector<item*> items, std::vector<int> quantities, bool to_vehicle,
        tripoint_rel_ms relative_destination)
        : quantities(quantities),
          to_vehicle(to_vehicle),
          relative_destination(relative_destination) {

        for (item*& it : items) { target_items.emplace_back(it); }
    }

    activity_id get_type() const override { return activity_id("ACT_MOVE_ITEMS"); }
...
public:
    toggle_gate_activity_actor(int gate_moves, const tripoint_bub_ms& gate_placement)
        : moves_total(gate_moves),
          placement(gate_placement) {}

    activity_id get_type() const override { return activity_id("ACT_TOGGLE_GATE"); }
...
public:
    pickup_activity_actor(
        const std::vector<pickup::pick_drop_selection>& target_items,
        const std::optional<tripoint_bub_ms>& starting_pos)
        : target_items(target_items),
          starting_pos(starting_pos) {}

    activity_id get_type() const override { return activity_id("ACT_PICKUP"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_STASH"); }
...
public:
    ...
    throw_activity_actor(item& target, std::optional<tripoint_bub_ms> blind_throw_from_pos)
        : target(&target),
          blind_throw_from_pos(blind_throw_from_pos) {}
    ~throw_activity_actor() = default;

    activity_id get_type() const override { return activity_id("ACT_THROW"); }
...
public:
    explicit oxytorch_activity_actor(
        const tripoint_bub_ms& target, const safe_reference<item>& tool)
        : target(target),
          tool(tool) {};

    activity_id get_type() const override { return activity_id("ACT_OXYTORCH"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const oxytorch_activity_actor& actor = static_cast<const oxytorch_activity_actor&>(other);
        return actor.target == target;
    }
};
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_BUILD"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_ASSIST"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_BURROW"); }
...
public:
    explicit pickaxe_activity_actor(const tripoint_abs_ms& target, const safe_reference<item> tool)
        : target(target),
          tool(tool) {};

    activity_id get_type() const override { return activity_id("ACT_PICKAXE"); }
...
public:
    explicit jackhammer_activity_actor(
        const tripoint_abs_ms& target, const safe_reference<item> tool)
        : target(target),
          tool(tool) {};

    activity_id get_type() const override { return activity_id("ACT_JACKHAMMER"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const jackhammer_activity_actor& actor = static_cast<const jackhammer_activity_actor&>(
            other);
        return actor.target == target && actor.tool == tool;
    }
};
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_CHURN"); }
...
public:
    explicit fill_pit_activity_actor(const tripoint_abs_ms& target, const safe_reference<item> tool)
        : target(target),
          tool(tool) {};

    activity_id get_type() const override { return activity_id("ACT_FILL_PIT"); }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_PRY_NAILS"); }
...
public:
    explicit plant_seed_activity_actor(const tripoint_abs_ms& target, const itype_id& seed)
        : target(target),
          seed_id(seed) {};

    activity_id get_type() const override { return activity_id("ACT_PLANT_SEED"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const plant_seed_activity_actor& actor = static_cast<const plant_seed_activity_actor&>(
            other);
...
public:
    explicit forage_activity_actor(const tripoint_abs_ms& target, bool auto_resume)
        : target(target),
          auto_resume(auto_resume) {};

    activity_id get_type() const override { return activity_id("ACT_FORAGE"); }
...
public:
    explicit hand_crank_activity_actor(
        const safe_reference<item> tool, const std::vector<int>& values,
        const std::vector<std::string>& str_values)
        : tool(tool),
          values(values),
          str_values(str_values) {};

    activity_id get_type() const override { return activity_id("ACT_HAND_CRANK"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const hand_crank_activity_actor& actor = static_cast<const hand_crank_activity_actor&>(
            other);
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_FILL_LIQUID"); }
...
public:
    ...
    explicit fertilize_plot_activity_actor(const std::string& fertilizer_str)
        : str_value(fertilizer_str) {};

    activity_id get_type() const override { return activity_id("ACT_FERTILIZE_PLOT"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const fertilize_plot_activity_actor& actor =
            static_cast<const fertilize_plot_activity_actor&>(other);
...
public:
    ...
    salvage_activity_actor(iuse_locations&& targets, tripoint_abs_ms pos)
        : targets(std::move(targets)),
          pos(pos) {}

    ~salvage_activity_actor() = default;

    activity_id get_type() const override { return activity_id("ACT_LONGSALVAGE"); }
...
public:
    ...
    butchery_activity_actor(
        butcher_type type, const std::vector<item*>& targets,
        const tripoint_abs_ms& placement = tripoint_abs_ms())
        : type(type),
          placement(placement) {
        for (item* it : targets) { this->targets.emplace_back(it); }
    }

    butchery_activity_actor(
        butcher_type type, std::vector<safe_reference<item>>&& targets,
        const tripoint_abs_ms& placement = tripoint_abs_ms())
        : type(type),
          targets(std::move(targets)),
          placement(placement) {}
...
public:
    ...
    wood_chop_activity_actor(
        wood_chop_type type, const tripoint_abs_ms& place, int moves_,
...
public:
    ...
    explicit vibe_activity_actor(const safe_reference<item>& item): vibrator(item) {}

    activity_id get_type() const override { return activity_id("ACT_VIBE"); }
...
public:
    activity_id get_type() const override { return activity_id("ACT_ATM"); }
...
public:
    ...
    shear_activity_actor(
        const std::vector<tripoint_abs_ms>& positions, const safe_reference<item>& tool,
...
public:
    activity_id get_type() const override { return activity_id("ACT_FIND_MOUNT"); }
...
public:
    activity_id get_type() const override { return activity_id("ACT_ADV_INVENTORY"); }

    void start(player_activity&, Character&) override {}
    void do_turn(player_activity& act, Character&) override { act.set_to_null(); }
    void finish(player_activity&, Character&) override {}
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_CRACKING"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const wait_stamina_activity_actor& actor = static_cast<const wait_stamina_activity_actor&>(
            other);
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_WAIT_STAMINA"); }
...
public:
    activity_id get_type() const override { return activity_id("ACT_READ"); }
...
public:
    activity_id get_type() const override { return activity_id("ACT_TRY_SLEEP"); }
...
public:
    activity_id get_type() const override { return activity_id("ACT_TRAVELLING"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const start_fire_activity_actor& actor = static_cast<const start_fire_activity_actor&>(
            other);
        return actor.placement == placement;
    }
...
public:
    ...
    activity_id get_type() const override { return activity_id("ACT_FISH"); }
...
private:
    ...
    bool can_resume_with_internal(
        const activity_actor& other, const Character& /*who*/) const override {
        const make_zlave_activity_actor& actor = static_cast<const make_zlave_activity_actor&>(
            other);
        return actor.str_value == str_value;
    }
...
public:
    ...
    train_activity_actor(const std::string& n, int mult = 0): name(n), expert_multiplier(mult) {}
...
private:
    weak_ptr_fast<monster> target;
    std::string target_name;
    int moves = 0;

public:
    robot_control_activity_actor() = default;
    robot_control_activity_actor(const weak_ptr_fast<monster>& t, const std::string& name, int m)
        : target(t),
          target_name(name),
          moves(m) {}
...
private:
    ...
    std::string mode;             // "study" or "learn"
    bool gain_level_mode = false; // "gain_level" str_value[0]
    int total_exp_gained = 0;
    int total_levels_gained = 0;
    int dark_flag = 0; // -1 = too dark
    int turn_counter = 0;
    int last_exp_displayed = 0;
    int moves = 0;

public:
    study_spell_activity_actor() = default;
    study_spell_activity_actor(
        const std::string& name, const std::string& mode_, bool gain_level, int m)
        : spell_name(name),
          mode(mode_),
          gain_level_mode(gain_level),
          moves(m) {}
...
private:
    std::string spell_name;
    int level_override = -1; // -1 for player spells, actual level for item spells
    bool no_fail = false;
    bool no_mana = true; // 1 = normal mana cost, 0 = override (no mana cost)
    int moves = 0;

public:
    spellcasting_activity_actor() = default;
    spellcasting_activity_actor(const std::string& name, int level, bool fail, bool mana, int m)
        : spell_name(name),
...
public:
    ...
    void start(player_activity& act, Character&) override { act.index = num_corpses; }
    void do_turn(player_activity& act, Character& who) override;
    void finish(player_activity& act, Character& who) override;

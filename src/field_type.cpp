...
void field_type::load( const JsonObject &jo, const std::string & )
{
    ...
    for( const JsonObject jao : jo.get_array( "intensity_levels" ) ) {
        ...
        if( jao.has_array( "light_color" ) ) {
            ...
            intensity_level.light_color.r = static_cast<float>( jarr.get_int( 0 ) ) / 255.0f;
            intensity_level.light_color.g = static_cast<float>( jarr.get_int( 1 ) ) / 255.0f;
            intensity_level.light_color.b = static_cast<float>( jarr.get_int( 2 ) ) / 255.0f;
        } else {

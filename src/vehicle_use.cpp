    ...
    const std::string& name, char key, const std::string& flag )
{
    ...
    auto msg = string_format( state ? _( "Turn on %s" ) : colorize( _( "Turn off %s" ), c_pink ),
                              name );
...
void handbrake()
{
    ...
    if( !is_on_rails && veh->last_turn != 0_degrees
        && std::lround( static_cast<double>( rng( 15,
                        60 ) * 100 ) * 0.44704 ) < std::abs( veh->velocity ) ) {
        veh->skidding = true;
    ...
    std::vector<uilist_entry> &options, std::vector<std::function<void()>> &actions )
{
    ...
    add_toggle( pgettext( "electronics menu option", "reactor" ), keybind( "TOGGLE_REACTOR" ),
                "REACTOR" );
    ...
    add_toggle( pgettext( "electronics menu option", "freezer" ), keybind( "TOGGLE_FREEZER" ),
                "FREEZER" );
    ...
    add_toggle( pgettext( "electronics menu option", "planter" ), keybind( "TOGGLE_PLANTER" ),
                "PLANTER" );
    ...
    if( is_locked ) {
        ...
        if( crafting_inv.has_quality( quality_id( "SCREW" ) ) ) {
            if( query_yn( _( "You don't find any keys in the %s. Attempt to hotwire vehicle?" ),
                ...
                g->u.assign_activity(
                    std::make_unique<player_activity>( std::make_unique<hotwire_car_activity_actor>(
                            abs_veh_pos.x() + q.x(), abs_veh_pos.y() + q.y(),
                            g->u.get_skill_level( skill_mechanics ), moves ) ) );
            } else {
...
void vehicle::use_controls( const tripoint_bub_ms& pos )
{
    ...
    if( has_electronic_controls ) {
        set_electronics_menu_options( options, actions );
        options.emplace_back( _( "Control multiple electronics" ), keybind( "CONTROL_MANY_"
                                 "ELECTRONICS" ) );
        actions.emplace_back( [&] {
            control_electronics();
            refresh();
    ...
    if( !droppers.empty() ) {
        options.emplace_back( _( "Activate all item droppers (Drop Everything)" ), keybind( "DROPPER_"
                                 "ALL" ) );
        actions.emplace_back( [&] {
        ...
        options.emplace_back( _( "Activate one item dropper (Drop Everything)" ), keybind( "DROPPER_"
                                 "SINGLE_"
                                 "ALL" ) );
        actions.emplace_back( [&] {
        ...
        options.emplace_back( _( "Activate one item dropper (Drop One Thing)" ), keybind( "DROPPER_"
                                 "SINGLE" ) );
        actions.emplace_back( [&] {
...
bool vehicle::start_engine( const int e )
{
    ...
    if( einfo.engine_backfire_threshold() ) {
        if( ( 1 - dmg ) < einfo.engine_backfire_threshold() && one_in( einfo.engine_backfire_freq() ) ) {
            ...
            sounds::sound(
                pos, start_moves / 10, sounds::sound_t::movement,
    ...
    if( eng.faults_potential().contains( fault_starter ) ) {
        if( eng.faults().contains( fault_starter ) ) {
            sounds::sound(
                pos, noise, sounds::sound_t::alarm,
    ...
    if( eng.faults().contains( fault_pump ) || eng.faults().contains( fault_diesel ) ) {
        sounds::sound(
            pos, noise, sounds::sound_t::movement,
    ...
    sounds::sound(
        pos, noise, sounds::sound_t::movement, string_format( _( "the %s starting" ), eng.name() ) );
...
void vehicle::start_engines( const bool take_control, const bool autodrive )
{
    ...
    if( !autodrive ) {
        g->u.assign_activity( std::make_unique<player_activity>(
                                  std::make_unique <
                                  start_engines_activity_actor > ( starting_engine_position, take_control, start_time ) ) );
    }
}
...
void vehicle::honk_horn()
{
    ...
    for( const vpart_reference& vp : get_avail_parts( "HORN" ) ) {
        ...
        if( horn_type.bonus >= 110 ) {
            ...
            //~ Moderate horn sound
            sounds::sound( horn_pos, horn_type.bonus, sounds::sound_t::alarm, _( "BEEEP!" ), false,
                           "vehicle", "horn_medium" );
        } else {
            //~ Weak horn sound
            sounds::sound( horn_pos, horn_type.bonus, sounds::sound_t::alarm, _( "honk." ), false,
                           "vehicle", "horn_low" );
        }
    }
...
void vehicle::beeper_sound()
{
    ...
    for( const vpart_reference& vp : get_avail_parts( "BEEPER" ) ) {
        ...
        //~ Beeper sound
        sounds::sound( vp.pos(), vp.info().bonus, sounds::sound_t::alarm, _( "beep!" ), false,
                       "vehicle", "rear_beeper" );
    }
}
...
void vehicle::play_chimes()
{
    ...
    for( const vpart_reference& vp : get_enabled_parts( "CHIMES" ) ) {
        sounds::sound(
            vp.pos(), 40, sounds::sound_t::music,
...
void vehicle::operate_reaper()
{
    for( const vpart_reference& vp : get_enabled_parts( "REAPER" ) ) {
        ...
        sounds::sound( reaper_pos, rng( 10, 25 ), sounds::sound_t::combat, _( "Swish" ), false,
                       "vehicle", "reaper" );
        ...
        for( auto it = v.begin(); it != v.end(); it++ ) {
            ...
            if( i->is_seed() ) {
                ...
                if( g->m.ter( loc ) != t_dirtmound && vp.has_feature( "ADVANCED_PLANTER" ) ) {
                    ...
                    sounds::sound( loc, rng( 10, 20 ), sounds::sound_t::combat, _( "Clink" ), false,
                                   "smash_success", "hit_vehicle" );
                }
    ...
    for( const vpart_reference& vp : get_enabled_parts( "SCOOP" ) ) {
        ...
        for( const tripoint_bub_ms& position : parts_points ) {
            ...
            if( one_in( chance_to_damage_item )
                ...
                sounds::sound(
                    position,
                    rng( 10, that_item_there->volume() / units::legacy_volume_factor * 2 + 10 ),
...
void vehicle::alarm()
{
    if( one_in( 4 ) ) {
        ...
        if( found_alarm ) {
            ...
            sounds::sound( bub_ms_location(), rng( 45, 80 ), sounds::sound_t::alarm,
                           random_entry_ref( sound_msgs ), false, "vehicle", "car_alarm" );
...
void vehicle::interact_with( const tripoint_bub_ms& pos, int interact_part )
{
    ...
    switch( choice ) {
        ...
        case RELOAD_TURRET: {
            ...
            if( opt ) {
                you.assign_activity(
                    std::make_unique<player_activity>( std::make_unique<reload_activity_actor>(
                            safe_reference<item>( turret.base() ), safe_reference<item>( *opt.ammo ),
                            opt.qty() ) ) );
            }
            return;
        }

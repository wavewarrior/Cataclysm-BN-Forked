...
int visibility_distance(int dx, int dy, int dz) {
    if (trigdist == 0u) { return max(dx, max(dy, dz)); }
...
bool blocked_by_player_vision_adjustment(int x, int y, int z) {
    if (vision_block_mask == 0u || z != player_z_idx) { return false; }
    uint bit = adjacent_block_bit(x - player_x, y - player_y);
    return bit != 0u && (vision_block_mask & bit) != 0u;
}

int tile_index(int x, int y, int z) { return z * cache_xy + x * cache_y + y; }
...
bool blocked_by_vehicle_diagonal(int from_x, int from_y, int from_z, int to_x, int to_y, int to_z) {
    ...
    if (z < 0 || z >= z_count || abs(dx) != 1 || abs(dy) != 1) { return false; }
    if (dx == -1 && dy == -1) {
...
[numthreads(8, 8, 1)] void main(uint3 group_id : SV_GroupID, uint3 thread_id : SV_GroupThreadID) {
    // Map to target tile.
    ...
    if (tx < 0 || ty < 0 || tx >= cache_x || ty >= cache_y || group_id.z >= (uint)dispatch_z_count
        || tz >= z_count) {
        return;
    }
    ...
    int dist_i =
        visibility_distance(abs(tx - player_x), abs(ty - player_y), abs(tz - player_z_idx));
    ...
    for (int i = 1; i < steps; ++i) {
        ...
        int prev_iz =
            player_z_idx + round_nearest_int((float)sdz * ((float)(i - 1) / (float)steps));
        ...
        if (blocked_by_vehicle_diagonal(prev_ix, prev_iy, prev_iz, ix, iy, iz)) {
            // Vehicle-diagonal corner block: hard-stop the ray, mirroring the CPU
            // cast_zlight_segment `return` (shadowcasting.cpp:589-591).  The 3D
            // caster is the ground truth for the seen cache; the 2D castLight
            // `continue` does NOT apply here.
            blocked = true;
            break;
        }

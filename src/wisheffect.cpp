...
struct entry_data {
    entry_data( const bodypart_str_id& body_part ): body_part( body_part ) {}
...
template <
    class T,
    typename F = std::pair <
        const std::string, void ( T::* )( const input_context &, const input_event &, int, uilist & ) >>
...
void foreach_effect(
    Creature& c, const entry_data& entry, const std::function<void( const effect & )> &fun )
{
    if( entry.eff ) {
        fun( **entry.eff );
    } else {
        ...
        for( const auto& pr : c.get_all_effects() ) {
            for( const std::pair<const bodypart_str_id, effect> &inner : pr.second ) {
                if( inner.first == entry.body_part ) { fun( inner.second ); }
            }
        }
    }
}
...
std::optional<bodypart_str_id> query_body_part( Creature& c )
{
    ...
    uilist menu;
    size_t i = 0;
    for( const bodypart_id& bp : all_bps ) { menu.addentry( i++, true, -1, bp.id().str() ); }
    menu.query();
...
std::optional<int> query_intensity()
{
    ...
    try {
        last_val.intensity =
            string_input_popup().title( _( "Input new intensity" ) ).only_digits( true ).query_int();
...
std::optional<time_duration> query_duration()
{
    ...
    if( !popup.confirmed() ) { return std::nullopt; }
...
{
    private:
        using effect_select_fun =
            void ( effect_select_callback::* )( const input_context &, const input_event &, int, uilist & );
    ...
    public:
        effect_select_callback( Creature& c ): c( c ) {}

        static const std::map<std::string, effect_select_fun> &get_handled_actions() {
            return handled_actions;
        }

        bool key( const input_context& ctx, const input_event& key, int entnum, uilist* parent_menu )
        override {
        ...
        void change_body_part( const input_context &, const input_event &, int, uilist & ) {
            query_body_part( c );
        }
        void change_intensity( const input_context &, const input_event &, int, uilist & ) {
            query_intensity();
        }
        void change_duration( const input_context &, const input_event &, int, uilist & ) {
            query_duration();
        }
        void toggle_force( const input_context &, const input_event &, int, uilist & ) {
            toggle_effect_force();
        }
        void draw_rml( uilist* menu, Rml::ElementDocument* doc ) override {
            ...
            ss << string_format(
                   "[%s] <bold>Body part</bold>: %s\n", ctxt.get_desc( "CHANGE_BODY_PART" ),
                   last_val.bodypart ? last_val.bodypart->name.translated().c_str() : "Global" );

            time_duration dur =
                last_val.duration <= 0_seconds ? eff_type->get_max_duration() : last_val.duration;
            ss << string_format(
                   "[%s] <bold>Duration</bold>: %10d (max: %d)\n", ctxt.get_desc( "CHANGE_DURATION" ),
                   to_turns<int>( dur ), to_turns<int>( eff_type->get_max_duration() ) );

            ss << string_format(
                   "[%s] <bold>Intensity</bold>: %d\n", ctxt.get_desc( "CHANGE_INTENSITY" ),
                   std::max( last_val.intensity, 1 ) );

            ss << string_format(
                   "[%s] <bold>Force</bold>: %s\n", ctxt.get_desc( "TOGGLE_FORCE" ),
                   last_val.force ? "Yes" : "No" );

            ss << '\n';
            ss << string_format( "Permanent: %s\n", eff_type->is_permanent() ? "Yes" : "No" );
...
{
    private:
        using effect_edit_fun =
            void ( effect_edit_callback::* )( const input_context &, const input_event &, int, uilist & );
    ...
    public:
        effect_edit_callback(
            Creature& c, const std::vector<entry_data> &meta,
            const std::function<void()> &on_creature_changed )
            : c( c ),
              meta( meta ),
              on_creature_changed( on_creature_changed ) {}

        static std::map<std::string, effect_edit_fun> get_handled_actions() { return handled_actions; }
    ...
    private:
        void add_effect_submenu( const input_context &, const input_event &, int, uilist& menu ) {
            wisheffect_state& last_val = uistate.debug_menu.effect;
            uilist submenu;
            submenu.w_x_setup = 0;
            submenu.w_width_setup = []() -> int { return TERMX; };
            submenu.pad_right_setup = []() -> int { return TERMX - 40; };
            effect_select_callback callback( c );
            register_callback( submenu, callback );
            submenu.input_category = menu.input_category;
            int i = 0;
            for( const efftype_id& eff : all_effects ) { submenu.addentry( i++, true, -1, eff.str() ); }
        ...
        void clear_effects( const input_context &, const input_event &, int, uilist & ) {
            on_creature_changed();
            c.clear_effects();
            if( !c.get_all_effects().empty() ) { debugmsg( "Couldn't clear effects" ); }
        }
        void remove( const input_context &, const input_event &, int entnum, uilist & ) {
            on_creature_changed();
            foreach_effect( c, meta[entnum], [&]( const effect & eff ) {
                bool removed = c.remove_effect( eff.get_id(), eff.get_bp() );
                if( !removed ) {
                    debugmsg( "Couldn't remove %s from %s", eff.get_id().str(), eff.get_bp().str() );
                }
            } );
        }
        void change_body_part(
            const input_context &, const input_event &, int entnum, uilist& parent_menu ) {
            ...
            if( bp ) {
                ...
                foreach_effect( c, meta[entnum], [&]( const effect & eff ) { to_move.push_back( eff ); } );
        ...
        void change_intensity( const input_context &, const input_event &, int entnum, uilist & ) {
            std::optional<int> new_intensity = query_intensity();
            if( new_intensity ) {
                on_creature_changed();
                foreach_effect( c, meta[entnum], [&]( const effect & eff ) {
                    effect& e = c.get_effect( eff.get_id(), eff.get_bp() );
                    if( !e.is_null() ) { e.set_intensity( *new_intensity ); }
                } );
            }
        }
        void change_duration( const input_context &, const input_event &, int entnum, uilist & ) {
            std::optional<time_duration> dur = query_duration();
            if( dur ) {
                on_creature_changed();
                foreach_effect( c, meta[entnum], [&]( const effect & eff ) {
                    effect& e = c.get_effect( eff.get_id(), eff.get_bp() );
                    if( !e.is_null() ) { e.set_duration( *dur ); }
                } );
            }
        }
        void toggle_force( const input_context &, const input_event &, int, uilist & ) {
    ...
    while( stay_in_menu ) {
        ...
        for( const auto& pr : em ) {
            for( const auto& inner : pr.second ) {
                ...
                if( !inner.first.is_null()
                    ...
                    debugmsg( "Creature %s has effect on body part %s, not in anatomy", c.get_name(),
                              inner.first.str() );
                }
            }
        }
        ...
        const auto add_effect_entry = [&menu, &i]( const effect & eff ) {
            ...
            std::string effect_description = string_format(
                                                 "- %s %d, %d s", eff.get_id().str(), eff.get_intensity(),
        ...
        const auto on_creature_changed = [&not_dirty]() { not_dirty = false; };

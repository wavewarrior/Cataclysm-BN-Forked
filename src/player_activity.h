...
class player_activity
{
    private:
        ...
        friend activity_ptr;
        /** This keeps track of if the activity is currently running so we can avoid deleting it until
         * it's done. */
        bool active = false;
    ...
    public:
        ...
        player_activity(
            activity_id, int turns = 0, int Index = -1, int pos = INT_MIN,
        ...
        explicit operator bool() const { return !type.is_null(); }
         ...
         * parameters) again.
         */
        bool is_suspendable() const { return type->suspendable(); }

        bool is_multi_type() const { return type->multi_activity(); }

        /** This replaces the former usage `act.type = ACT_NULL` */
        void set_to_null();

        const activity_id &id() const { return type; }
        ...
        std::string get_stop_phrase() const { return type->stop_phrase(); }

        const translation &get_verb() const { return type->verb(); }
        ...
        void calc_moves( const Character& who ) { speed.calc_moves( who ); }
        ...
        template <typename T> T *get_actor() const { return dynamic_cast<T*>( actor.get() ); }

...
static const requirement_id requirement_add_grid_connection = requirement_id("add_grid_connection");
...
int iuse::pickaxe(player* p, item* it, bool, const tripoint_bub_ms& pos) {
    ...
    p->add_msg_if_player(_("You strike the %1$s with your %2$s."), g->m.tername(pnt), it->tname());
...
static void set_cable_active(player* const who, item* const it, const cable_connection_data& data) {
    data.set_vars(it);
    it->activate();

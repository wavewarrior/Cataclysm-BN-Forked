          ...
          menu_pos_t menu_pos )
{
    ...
    const hint_rating rate_wield_item = cant_acquare ? hint_rating::cant : hint_rating::good;
    const hint_rating rate_drop_item = cant_dispose_of ? hint_rating::cant : hint_rating::good;
    ...
    */
    ...
    // RmlUi render path. data/doc declared here so the redraw can sync.
    // Item info rendered via item_info_rml_lines (draw_item_info removed).
    // Single exit (return ret_val) → explicit rml.close() there.

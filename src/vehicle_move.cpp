...
void vehicle::stop(bool update_cache) {
    ...
    map& here = get_map();
    for (const auto& p : get_points()) { here.set_memory_seen_cache_dirty(here.abs_to_bub(p)); }
}

bool vehicle::collision(
    ...
    if (dp.z() == -1 && !bash_floor) {
        ...
        if (collision(colls, tripoint_rel_ms::zero(), just_detect, true)) { return true; }
    }
...
auto vehicle::part_collision(const vehicle_part_collision_options& options) -> veh_collision {
    ...
    if (is_body_collision) {
        ...
        if (here.has_flag("SWIMMABLE", critter->bub_pos())) {
            ...
            while (g->critter_at(end_pos, true) || cur_points.contains(here.bub_to_abs(end_pos))) {
                start_pos = end_pos;
    ...
    do {
        ...
        if (std::abs(impulse_obj - impulse_veh) > 5) {
            add_msg(m_debug, "Conservation of momentum violated, impulse values between object and "
                             "vehicle are not equal! ");
        ...
        add_msg(m_debug,
                "Vehicle mass of %.2f Kg with a pre-collision velocity of %.2f m/s, collision "
                "object mass of %.2f Kg, with a velocity of %.2f m/s. Predicted deformation "
                "distance is %.2f meters.",
                mass, cmps_to_mps(prev_velocity), mass2, vel2, deformation_distance);
        add_msg(m_debug,
                "Vehicle impulse of %.2f Ns resulted in Part collision damage %.2f of a maximum of "
                "%.2f, Object impulse of %.2f resulted in damage of %.2f (dmg mod %0.2i)",
        ...
        if (ret.type == veh_coll_bashable) {
            ...
            if (ph != nullptr) {
                ...
                add_msg(m_debug,
                        "Vehicle received impulse of %.2f Nm dealing %.2f damage of maximum %.2f, "
                        "Critter received impulse of %.2f Nm dealing %.2f damage. ",
            ...
            if (!vert_coll) {
                if (std::fabs(vel2_a) > 10.0f
                    ...
                    // Vehicle momentum too low to push the critter.
                    // Set smashed so the do-while exits cleanly; without this the
                    // loop re-evaluates at v=0 and the critter can phase through.
                    velocity = 0;
                    smashed = true;
                    break;
                }
    ...
    if (critter != nullptr) {
        if (!critter->is_hallucination()) {
            ...
            if (part_flag(ret.part, "SHARP")) {
                ...
                sounds::sound(
                    p, 20, sounds::sound_t::combat, snd, false, "smash_success", "hit_vehicle");
            }
        }
    } else {
        if (pl_ctrl) {
...
bool vehicle::check_heli_descend(Character& who) {
    ...
    for (const tripoint_abs_ms& abs : get_points(true)) {
        ...
        if (here.has_zlevels()
            && (pt.z() < -OVERMAP_DEPTH || !here.has_flag_ter_or_furn(TFLAG_NO_FLOOR, pt))) {
        ...
        if (here.impassable_ter_furn(below) || here.has_flag_ter_or_furn(TFLAG_RAMP_DOWN, below)
            ...
            who.add_msg_if_player(m_bad, _("It would be unsafe to try and land when there are "
                                           "obstacles below you."));
            return false;
        }
    }
...
bool vehicle::check_heli_ascend(Character& who) {
    ...
    if (!is_flying_in_air() && check_on_ramp()) {
        who.add_msg_if_player(m_bad, _("It would be unsafe to try and take off from an uneven "
                                       "surface."));
        return false;
    }
    map& here = get_map();
...
void vehicle::pldrive(Character& driver, tripoint_rel_veh p) {
    ...
    if (turn_delta != 0_degrees) {
        float eff = steering_effectiveness();
        if (eff == -2) {
            driver.add_msg_if_player(m_info, _("You cannot steer an animal-drawn vehicle with no "
                                               "animal harnessed."));
            return;
        }

        if (eff < 0) {
            driver.add_msg_if_player(m_info, _("This vehicle has no steering system installed, you "
                                               "can't turn it."));
            return;
        }
        ...
        if (penalty > skill || cost > 400) {
            ...
            if (cost >= driver.get_speed() * 2) {
                driver.add_msg_if_player(m_warning, _("It takes you a very long time to steer that "
                                                      "vehicle!"));
            } else if (cost >= driver.get_speed() * 1.5f) {
                driver.add_msg_if_player(m_warning, _("It takes you a long time to steer that "
                                                      "vehicle!"));
            }
        }

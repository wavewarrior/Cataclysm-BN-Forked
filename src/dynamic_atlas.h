...
class dynamic_atlas
{
    public:
        ...
        struct sprite_sheet {
            ...
            /// CPU-side mirror of the legacy SDL_Texture's pixels. Populated by
            /// readback_load() for UV remapping (readback_find) and, when
            /// is_batching is set, pre-populated at page creation and kept in
            /// sync by blit_sprite/end_batch so batched sprite writes land on
            /// this surface directly instead of a lock-per-sprite render call.
            ...
            /// SDL_Texture / `surface` readback. The stripe packer is bounded

...
tripoint_omt_ms rotate_point( const tripoint_omt_ms &p, int rotations )
{
    if( p.x() < 0 || p.x() >= SEEX * 2 ||
        ...
        return tripoint_omt_ms( 0, 0, p.z() );
    }

    rotations = rotations % 4;

    tripoint_omt_ms ret = p;
    switch( rotations ) {
        case 0:
            break;
        case 1:
...
void mapgen_null( mapgendata &dat )
{
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            dat.m.ter_set( point_omt_ms( i, j ), t_null );
            dat.m.set_radiation( point_omt_ms( i, j ), 0 );
        }
    }
}
...
void mapgen_test( mapgendata &dat )
{
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            dat.m.ter_set( point_omt_ms( i, j ), t_grass );
            dat.m.set_radiation( point_omt_ms( i, j ), 0 );
        }
    }
}
...
{
    mapgen_constructor *const m = &dat.m;
    for( int i = 0; i < 4; i++ ) {
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            if( rng( 0, dat.w_fac ) <= i && rng( 0, dat.e_fac ) <= SEEX * 2 - 1 - i &&
                ...
                m->ter_set( point_omt_ms( i, j ), t_pit_shallow );
                m->make_rubble( point_omt_ms( i, j ), f_rubble_rock );
                m->set_radiation( point_omt_ms( i, j ), rng( 0, 4 ) * rng( 0, 2 ) );
            } else {
                m->ter_set( point_omt_ms( i, j ), dat.groundcover() );
                m->set_radiation( point_omt_ms( i, j ), rng( 0, 2 ) * rng( 0, 2 ) * rng( 0, 2 ) );
            }
        }
    }
    m->place_items( item_group_id( "wreckage" ), 83, point_omt_ms::zero(), point_omt_ms( SEEX * 2 - 1,
                    SEEY * 2 - 1 ),
                    true, dat.when() );
}

// TODO: make void map::ter_or_furn_set(const int x, const int y, const ter_furn_id & tfid);
static void ter_or_furn_set( mapgen_constructor *m, const point_omt_ms &p, const ter_furn_id &tfid )
{
    if( tfid.ter != t_null ) {
...
void mapgen_field( mapgendata &dat )
{
    ...
    m->place_items( item_group_id( "field" ), 60, point_omt_ms::zero(), point_omt_ms( SEEX * 2 - 1,
                    SEEY * 2 - 1 ), true,
                    dat.when() );
}
...
void mapgen_hive( mapgendata &dat )
{
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            int rn = rng( 0, 14 );
            if( rn > 13 ) {
                m->ter_set( point_omt_ms( i, j ), t_tree );
            } else if( rn > 11 ) {
                m->ter_set( point_omt_ms( i, j ), t_tree_young );
            } else if( rn > 10 ) {
                m->ter_set( point_omt_ms( i, j ), t_underbrush );
            } else {
                m->ter_set( point_omt_ms( i, j ), dat.groundcover() );
            }
        }
    ...
    for( int j = 5; j < SEEY * 2 - 5; j += 6 ) {
        for( int i = ( j == 5 || j == 17 ? 3 : 6 ); i < SEEX * 2 - 5; i += 6 ) {
            if( !one_in( 8 ) ) {
                ...
                m->ter_set( point_omt_ms( i, j - 5 ), t_wax );
                m->ter_set( point_omt_ms( i, j + 5 ), t_wax );
                for( int k = -2; k <= 2; k++ ) {
                    for( int l = -1; l <= 1; l++ ) {
                        m->ter_set( point_omt_ms( i + k, j + l ), t_floor_wax );
                    }
                }
                m->add_spawn( mon_bee, 2, { i, j } );
                m->add_spawn( mon_beekeeper, 1, { i, j } );
                m->ter_set( point_omt_ms( i, j - 3 ), t_floor_wax );
                m->ter_set( point_omt_ms( i, j + 3 ), t_floor_wax );
                m->ter_set( point_omt_ms( i - 1, j - 2 ), t_floor_wax );
                m->ter_set( point_omt_ms( i, j - 2 ), t_floor_wax );
                m->ter_set( point_omt_ms( i + 1, j - 2 ), t_floor_wax );
                m->ter_set( point_omt_ms( i - 1, j + 2 ), t_floor_wax );
                m->ter_set( point_omt_ms( i, j + 2 ), t_floor_wax );
                m->ter_set( point_omt_ms( i + 1, j + 2 ), t_floor_wax );
                ...
                m->ter_set( point_omt_ms( i - 1, j - 4 ), t_wax );
                m->ter_set( point_omt_ms( i, j - 4 ), t_wax );
                m->ter_set( point_omt_ms( i + 1, j - 4 ), t_wax );
                m->ter_set( point_omt_ms( i - 2, j - 3 ), t_wax );
                m->ter_set( point_omt_ms( i - 1, j - 3 ), t_wax );
                m->ter_set( point_omt_ms( i + 1, j - 3 ), t_wax );
                m->ter_set( point_omt_ms( i + 2, j - 3 ), t_wax );
                m->ter_set( point_omt_ms( i - 3, j - 2 ), t_wax );
                m->ter_set( point_omt_ms( i - 2, j - 2 ), t_wax );
                m->ter_set( point_omt_ms( i + 2, j - 2 ), t_wax );
                m->ter_set( point_omt_ms( i + 3, j - 2 ), t_wax );
                m->ter_set( point_omt_ms( i - 3, j - 1 ), t_wax );
                m->ter_set( point_omt_ms( i - 3, j ), t_wax );
                m->ter_set( point_omt_ms( i - 3, j - 1 ), t_wax );
                m->ter_set( point_omt_ms( i - 3, j + 1 ), t_wax );
                m->ter_set( point_omt_ms( i - 3, j ), t_wax );
                m->ter_set( point_omt_ms( i - 3, j + 1 ), t_wax );
                ...
                if( skip1 == 0 || skip2 == 0 ) {
                    m->ter_set( point_omt_ms( i - 1, j - 4 ), t_floor_wax );
                }
                if( skip1 == 1 || skip2 == 1 ) {
                    m->ter_set( point_omt_ms( i, j - 4 ), t_floor_wax );
                }
                if( skip1 == 2 || skip2 == 2 ) {
                    m->ter_set( point_omt_ms( i + 1, j - 4 ), t_floor_wax );
                }
                if( skip1 == 3 || skip2 == 3 ) {
                    m->ter_set( point_omt_ms( i - 2, j - 3 ), t_floor_wax );
                }
                if( skip1 == 4 || skip2 == 4 ) {
                    m->ter_set( point_omt_ms( i - 1, j - 3 ), t_floor_wax );
                }
                if( skip1 == 5 || skip2 == 5 ) {
                    m->ter_set( point_omt_ms( i + 1, j - 3 ), t_floor_wax );
                }
                ...
                if( skip1 == 7 || skip2 == 7 ) {
                    m->ter_set( point_omt_ms( i - 3, j - 2 ), t_floor_wax );
                }
                if( skip1 == 8 || skip2 == 8 ) {
                    m->ter_set( point_omt_ms( i - 2, j - 2 ), t_floor_wax );
                }
                if( skip1 == 9 || skip2 == 9 ) {
                    m->ter_set( point_omt_ms( i + 2, j - 2 ), t_floor_wax );
                }
                ...
                if( skip1 == 11 || skip2 == 11 ) {
                    m->ter_set( point_omt_ms( i - 3, j - 1 ), t_floor_wax );
                }
                if( skip1 == 12 || skip2 == 12 ) {
                    m->ter_set( point_omt_ms( i - 3, j ), t_floor_wax );
                }
                ...
                if( is_center ) {
                    m->place_items( item_group_id( "hive_center" ), 90, point_omt_ms( i - 2, j - 2 ),
                                    point_omt_ms( i + 2, j + 2 ),
                                    false,
                                    dat.when() );
                } else {
                    m->place_items( item_group_id( "hive" ), 80, point_omt_ms( i - 2, j - 2 ), point_omt_ms( i + 2,
                                    j + 2 ), false,
                                    dat.when() );
                }
            }
    ...
    if( is_center ) {
        m->place_npc( point_omt_ms( SEEX, SEEY ), string_id<npc_template>( "apis" ) );
    }
}
...
void mapgen_road( mapgendata &dat )
{
    ...
    if( diag ) {
            ...
            for( int y = 0; y < SEEY * 2; y++ ) {
                for( int x = 0; x < SEEX * 2; x++ ) {
                    if( x > y - 4 && ( x < 4 || y > SEEY * 2 - 5 || y >= x ) ) {
                        m->ter_set( point_omt_ms( x, y ), t_sidewalk );
                    }
                }
            }
        }
            ...
            for( int x = 0; x < SEEX * 2; x++ ) {
                if( x > y && // definitely only draw in the upper right half of the map
                    ...
                    if( ( x + rot / 2 ) % 4 && ( x - y == SEEX - 1 + ( 1 - ( rot / 2 ) ) ||
                                                 x - y == SEEX + ( 1 - ( rot / 2 ) ) ) ) {
                        m->ter_set( point_omt_ms( x, y ), t_pavement_y );
                    } else {
                        m->ter_set( point_omt_ms( x, y ), t_pavement );
                    }
                }
            }
        }
        ...
        for( int dir = 0; dir < 4; dir++ ) {
            if( roads_nesw[dir] ) {
                ...
                if( sidewalks_neswx[( dir + 3 ) % 4     ] ||   // has_sidewalk west?
                    ...
                    square( m, t_sidewalk, point_omt_ms( x1, y1 ), point_omt_ms( x2, y2 ) );
                }
        ...
        if( dead_end_extension > 0 && sidewalks_neswx[ 2 ] ) {
            square( m, t_sidewalk, point_omt_ms( 0, SEEY + dead_end_extension ), point_omt_ms( SEEX * 2 - 1,
        ...
        for( int dir = 0; dir < 4; dir++ ) {
            if( roads_nesw[dir] ) {
                ...
                coord_rotate_cw( x1, y1, dir );
                coord_rotate_cw( x2, y2, dir );
                square( m, t_pavement, point_omt_ms( x1, y1 ), point_omt_ms( x2, y2 ) );
                if( curvedir_nesw[dir] != 0 ) {
                    for( int x = 1; x < 4; x++ ) {
                        for( int y = 0; y < x; y++ ) {
                            int ty = y, tx = ( curvedir_nesw[dir] == -1 ? x : SEEX * 2 - 1 - x );
                            coord_rotate_cw( tx, ty, dir );
                            m->ter_set( point_omt_ms( tx, ty ), t_pavement );
                        }
                    }
                }
            }
                ...
                for( int x = SEEX - 1; x <= SEEX; x++ ) {
                    for( int y = 0; y < max_y; y++ ) {
                        if( ( y + ( ( dir + rot ) / 2 % 2 ) ) % 4 ) {
                            ...
                            m->ter_set( point_omt_ms( xn, yn ), t_pavement_y );
                        }
                    }
                }
            }
        ...
        if( plaza_dir > -1 ) {
            if( plaza_dir == 8 ) {
                ...
                // plaza side
                square( m, t_pavement, point_omt_ms( 0, SEEY - 10 ), point_omt_ms( SEEX * 2 - 1, SEEY - 1 ) );
                square( m, t_sidewalk, point_omt_ms( 0, SEEY - 2 ), point_omt_ms( SEEX * 2 - 1, SEEY * 2 - 1 ) );
                if( one_in( 3 ) ) {
                    line( m, t_tree_young, point_omt_ms( 1, SEEY ), point_omt_ms( SEEX * 2 - 2, SEEY ) );
                }
                if( one_in( 3 ) ) {
                    line_furn( m, f_bench, point_omt_ms( 2, SEEY + 2 ), point_omt_ms( 5, SEEY + 2 ) );
                    line_furn( m, f_bench, point_omt_ms( 10, SEEY + 2 ), point_omt_ms( 13, SEEY + 2 ) );
                    line_furn( m, f_bench, point_omt_ms( 18, SEEY + 2 ), point_omt_ms( 21, SEEY + 2 ) );
                }
            } else { // plaza corner
                circle( m, t_pavement, point_omt_ms( 0, SEEY * 2 - 1 ), 21 );
                circle( m, t_sidewalk, point_omt_ms( 0, SEEY * 2 - 1 ), 13 );
                if( one_in( 3 ) ) {
                    circle( m, t_tree_young, point_omt_ms( 0, SEEY * 2 - 1 ), 11 );
                    circle( m, t_sidewalk,   point_omt_ms( 0, SEEY * 2 - 1 ), 10 );
                }
                if( one_in( 3 ) ) {
                    circle( m, t_water_sh, point_omt_ms( 4, SEEY * 2 - 5 ), 3 );
                }
            }
        }
    }
    ...
    if( neighbor_sidewalks ) {
        m->place_spawns( GROUP_ZOMBIE, 2, point_omt_ms::zero(), point_omt_ms( SEEX * 2 - 1, SEEX * 2 - 1 ),
                         dat.monster_density() );
        // 1 per 10 overmaps
        if( one_in( 10000 ) ) {
    ...
    m->place_items( item_group_id( plaza ? "trash" : "road" ), 5, point_omt_ms::zero(),
                    point_omt_ms( SEEX * 2 - 1,
                                  SEEX * 2 - 1 ), plaza,
                    dat.when() );

    // add a manhole if appropriate
    if( dat.terrain_type() == "road_nesw_manhole" ) {
        m->ter_set( point_omt_ms( rng( 6, SEEX * 2 - 6 ), rng( 6, SEEX * 2 - 6 ) ), t_manhole_cover );
    }
...
void mapgen_subway( mapgendata &dat )
{
    ...
    switch( num_dirs ) {
        case 4:
            // 4-way intersection
            mapf::formatted_set_simple( m, point_omt_ms::zero(),
                                        "...^X^^^X^....^X^^^X^...\n"
                                        "..^DX^^DX^....^XD^^XD^..\n"
                                        ".^D^X^D^X^....^X^D^X^D^.\n"
                                        "^D^^XD^^X^^^^^^X^^DX^^D^\n"
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            if( one_in( 12 ) &&
                m->ter( point_omt_ms( i, j ) ) == t_rock_floor ) {
                m->ter_set( point_omt_ms( i, j ), t_moss_underground );
                // Some of that moss has mushrooms too.
                if( one_in( 15 ) ) {
                    m->furn_set( point_omt_ms( i, j ), f_cave_mushrooms );
                }
            }
        }
    }
...
void mapgen_sewer( mapgendata &dat )
{
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            ...
            m->ter_set( point_omt_ms( i, j ), fill ? t_rock : t_sewage );
        }
    }
    m->place_items( item_group_id( "sewer" ), 4 + ( num_dirs * 6 ), point_omt_ms::zero(),
                    point_omt_ms( SEEX * 2 - 1, SEEY * 2 - 1 ), true, dat.when() );
}
...
void mapgen_highway( mapgendata &dat )
{
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            if( i < 3 || i >= SEEX * 2 - 3 ) {
                ...
                m->ter_set( point_omt_ms( i, j ), t_railing );
            } else {
                if( ( i == SEEX - 1 || i == SEEX ) && j % 4 != 0 ) {
                    m->ter_set( point_omt_ms( i, j ), t_pavement_y );
                } else {
                    m->ter_set( point_omt_ms( i, j ), t_pavement );
                }
            }
        }
    }
    ...
    m->place_items( item_group_id( "road" ), 8, point_omt_ms::zero(), point_omt_ms( SEEX * 2 - 1,
                    SEEX * 2 - 1 ), false,
                    dat.when() );
}
...
void mapgen_river_curved_not( mapgendata &dat )
{
    ...
    for( int x = north_edge; x < SEEX * 2; x++ ) {
        for( int y = 0; y < east_edge; y++ ) {
            ...
            if( circle_edge <= 8 ) {
                m->ter_set( point_omt_ms( x, y ), grass_or_dirt() );
            } else if( circle_edge == 9 && one_in( 20 ) ) {
                m->ter_set( point_omt_ms( x, y ), clay_or_sand() );
            } else if( circle_edge <= 36 ) {
                m->ter_set( point_omt_ms( x, y ), t_water_moving_sh );
            }
        }
    }
...
void mapgen_river_straight( mapgendata &dat )
{
    ...
    for( int x = 0; x < SEEX * 2; x++ ) {
        ...
        line( m, grass_or_dirt(), point_omt_ms( x, 0 ), point_omt_ms( x, ground_edge ) );
        if( one_in( 20 ) ) {
            m->ter_set( point_omt_ms( x, ++ground_edge ), clay_or_sand() );
        }
        line( m, t_water_moving_sh, point_omt_ms( x, ++ground_edge ), point_omt_ms( x, shallow_edge ) );
    }
...
void mapgen_river_curved( mapgendata &dat )
{
    ...
    for( int y = 0; y < SEEY * 2; y++ ) {
        ...
        line( m, grass_or_dirt(), point_omt_ms( ground_edge, y ), point_omt_ms( SEEX * 2 - 1, y ) );
        if( one_in( 20 ) ) {
            m->ter_set( point_omt_ms( --ground_edge, y ), clay_or_sand() );
        }
        line( m, t_water_moving_sh, point_omt_ms( shallow_edge, y ), point_omt_ms( --ground_edge, y ) );
    }
...
void mapgen_cavern( mapgendata &dat )
{
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            if( ( j < dat.n_fac || j > dat.s_fac || i < dat.w_fac || i > dat.e_fac ) &&
                ...
                m->ter_set( point_omt_ms( i, j ), t_rock );
            } else {
                m->ter_set( point_omt_ms( i, j ), t_rock_floor );
            }
        }
    }
    ...
    for( int n = 0; n < rn; n++ ) {
        point_omt_ms p{ rng( 5, SEEX * 2 - 6 ), rng( 5, SEEY * 2 - 6 ) };
    ...
    m->place_items( item_group_id( "cavern" ), 60, point_omt_ms::zero(), point_omt_ms( SEEX * 2 - 1,
                    SEEY * 2 - 1 ),
                    false, dat.when() );
    if( one_in( 6 ) ) { // Miner remains
        ...
        while( !one_in( 3 ) ) {
            for( int i = 0; i < 3; ++i ) {
                m->put_items_from_loc( item_group_id( "cannedfood" ), p2, dat.when() );
            }
        }
    }
...
{
    fill_background( &dat.m, ter_id( "t_pd_border" ) );
}
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            if( ( dat.n_fac < 0 && j < dat.n_fac * -1 ) || ( dat.s_fac < 0 && j >= SEEY * 2 - dat.s_fac ) ||
                ...
                m->ter_set( point_omt_ms( i, j ), t_lava );
            }
        }
    }
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            ...
            if( i >= SEEX - 1 && i <= SEEX && j >= SEEY - 1 && j <= SEEY ) {
                m->ter_set( point_omt_ms( i, j ), t_slope_down );
            }
        }
    }
    switch( rng( 0, 4 ) ) { // Randomly chosen "altar" design
        case 0:
            for( int i = 7; i <= 16; i += 3 ) {
                m->ter_set( point_omt_ms( i, 6 ), t_rock );
                m->ter_set( point_omt_ms( i, 17 ), t_rock );
                m->ter_set( point_omt_ms( 6, i ), t_rock );
                m->ter_set( point_omt_ms( 17, i ), t_rock );
                if( i > 7 && i < 16 ) {
                    m->ter_set( point_omt_ms( i, 10 ), t_rock );
                    m->ter_set( point_omt_ms( i, 13 ), t_rock );
                } else {
                    m->ter_set( point_omt_ms( i - 1, 6 ), t_rock );
                    m->ter_set( point_omt_ms( i - 1, 10 ), t_rock );
                    m->ter_set( point_omt_ms( i - 1, 13 ), t_rock );
                    m->ter_set( point_omt_ms( i - 1, 17 ), t_rock );
                }
            }
            break;
        case 1:
            for( int i = 6; i < 11; i++ ) {
                m->ter_set( point_omt_ms( i, i ), t_lava );
                m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, i ), t_lava );
                m->ter_set( point_omt_ms( i, SEEY * 2 - 1 - i ), t_lava );
                m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, SEEY * 2 - 1 - i ), t_lava );
                if( i < 10 ) {
                    m->ter_set( point_omt_ms( i + 1, i ), t_lava );
                    m->ter_set( point_omt_ms( SEEX * 2 - i, i ), t_lava );
                    m->ter_set( point_omt_ms( i + 1, SEEY * 2 - 1 - i ), t_lava );
                    m->ter_set( point_omt_ms( SEEX * 2 - i, SEEY * 2 - 1 - i ), t_lava );

                    m->ter_set( point_omt_ms( i, i + 1 ), t_lava );
                    m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, i + 1 ), t_lava );
                    m->ter_set( point_omt_ms( i, SEEY * 2 - i ), t_lava );
                    m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, SEEY * 2 - i ), t_lava );
                }
                if( i < 9 ) {
                    m->ter_set( point_omt_ms( i + 2, i ), t_rock );
                    m->ter_set( point_omt_ms( SEEX * 2 - i + 1, i ), t_rock );
                    m->ter_set( point_omt_ms( i + 2, SEEY * 2 - 1 - i ), t_rock );
                    m->ter_set( point_omt_ms( SEEX * 2 - i + 1, SEEY * 2 - 1 - i ), t_rock );

                    m->ter_set( point_omt_ms( i, i + 2 ), t_rock );
                    m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, i + 2 ), t_rock );
                    m->ter_set( point_omt_ms( i, SEEY * 2 - i + 1 ), t_rock );
                    m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, SEEY * 2 - i + 1 ), t_rock );
                }
            }
            break;
        case 2:
            for( int i = 7; i < 17; i++ ) {
                ...
                if( i != 7 && i != 16 && i != 11 && i != 12 ) {
                    m->ter_set( point_omt_ms( i, 8 ), t_rock );
                    m->ter_set( point_omt_ms( 8, i ), t_rock );
                    m->ter_set( point_omt_ms( i, 15 ), t_rock );
                    m->ter_set( point_omt_ms( 15, i ), t_rock );
                }
                if( i == 11 || i == 12 ) {
                    m->ter_set( point_omt_ms( i, 10 ), t_rock );
                    m->ter_set( point_omt_ms( 10, i ), t_rock );
                    m->ter_set( point_omt_ms( i, 13 ), t_rock );
                    m->ter_set( point_omt_ms( 13, i ), t_rock );
                }
            }
            break;
        case 3:
            for( int i = 6; i < 11; i++ ) {
                for( int j = 6; j < 11; j++ ) {
                    m->ter_set( point_omt_ms( i, j ), t_lava );
                    m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, j ), t_lava );
                    m->ter_set( point_omt_ms( i, SEEY * 2 - 1 - j ), t_lava );
                    m->ter_set( point_omt_ms( SEEX * 2 - 1 - i, SEEY * 2 - 1 - j ), t_lava );
                }
            }
            break;
    }
...
void mapgen_tutorial( mapgendata &dat )
{
    ...
    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            if( j == 0 || j == SEEY * 2 - 1 ) {
                m->ter_set( point_omt_ms( i, j ), t_wall );
            } else if( i == 0 || i == SEEX * 2 - 1 ) {
                m->ter_set( point_omt_ms( i, j ), t_wall );
            } else if( j == SEEY ) {
                if( i % 4 == 2 ) {
                    m->ter_set( point_omt_ms( i, j ), t_door_c );
                } else if( i % 5 == 3 ) {
                    m->ter_set( point_omt_ms( i, j ), t_window_domestic );
                } else {
                    m->ter_set( point_omt_ms( i, j ), t_wall );
                }
            } else {
                m->ter_set( point_omt_ms( i, j ), t_floor );
            }
        }
    }
    m->furn_set( point_omt_ms( 7, SEEY * 2 - 4 ), f_rack );
    m->place_gas_pump( point_omt_ms( SEEX * 2 - 2, SEEY * 2 - 4 ), rng( 500, 1000 ) );
    if( dat.zlevel() < 0 ) {
        m->ter_set( point_omt_ms( SEEX - 2, SEEY + 2 ), t_stairs_up );
        m->ter_set( point_omt_ms( 2, 2 ), t_water_sh );
        m->ter_set( point_omt_ms( 2, 3 ), t_water_sh );
        m->ter_set( point_omt_ms( 3, 2 ), t_water_sh );
        m->ter_set( point_omt_ms( 3, 3 ), t_water_sh );
    } else {
        m->spawn_item( point_omt_ms( 5, SEEY + 1 ), itype_id( "helmet_bike" ) );
        m->spawn_item( point_omt_ms( 4, SEEY + 1 ), itype_id( "backpack" ) );
        m->spawn_item( point_omt_ms( 3, SEEY + 1 ), itype_id( "pants_cargo" ) );
        m->spawn_item( point_omt_ms( 7, SEEY * 2 - 4 ), itype_id( "machete" ) );
        m->spawn_item( point_omt_ms( 7, SEEY * 2 - 4 ), itype_id( "9mm" ) );
        m->spawn_item( point_omt_ms( 7, SEEY * 2 - 4 ), itype_id( "9mmP" ) );
        m->spawn_item( point_omt_ms( 7, SEEY * 2 - 4 ), itype_id( "uzi" ) );
        m->spawn_item( point_omt_ms( 7, SEEY * 2 - 4 ), itype_id( "uzimag" ) );
        ...
        m->ter_set( point_omt_ms( SEEX - 2, SEEY + 2 ), t_stairs_down );
    }
}
...
void mapgen_forest_trail_straight( mapgendata &dat )
{
    ...
    point_omt_ms center( SEEX + center_offset(), SEEY + center_offset() );

    for( int i = 0; i < SEEX * 2; i++ ) {
        for( int j = 0; j < SEEY * 2; j++ ) {
            if( i > center.x() - width_offset() && i < center.x() + width_offset() ) {
                m->furn_set( point_omt_ms( i, j ), f_null );
                m->ter_set( point_omt_ms( i, j ), *dat.region.forest_trail.trail_terrain.pick() );
            }
        }
    }
    ...
    m->place_items( item_group_id( "forest_trail" ), 75, center + point_rel_ms( -2, -2 ),
                    center + point_rel_ms( 2, 2 ), true, dat.when() );
}
...
void mapgen_lake_shore( mapgendata &dat )
{
    ...
    const int max_b = coords::map_squares_per( coords::scale::overmap_terrain ) - 1;
    const int sector_length = max_b / 3;

    // Define the corners of the map. These won't change.
    // NOLINTNEXTLINE(cata-point-initialization, cata-use-named-point-constants)
    static constexpr point_omt_ms nw_corner( 0, 0 );
    static constexpr point_omt_ms ne_corner( max_b, 0 );
    static constexpr point_omt_ms se_corner( max_b, max_b );
    static constexpr point_omt_ms sw_corner( 0, max_b );

    static constexpr point_rel_ms river_beach_x( sector_length / 2, 0 );
    static constexpr point_rel_ms river_beach_y( 0, sector_length / 2 );
    static constexpr point_rel_ms lake_beach_x( sector_length * 2, 0 );
    static constexpr point_rel_ms lake_beach_y( 0, sector_length * 2 );

    std::vector<point_omt_ms> shore_points;
    std::map<int, point_omt_ms> slots;
    ...
    std::vector<std::vector<point_omt_ms>> line_segments;
    ...
    if( line_segments.empty() ) {
        auto nw_inner = nw_corner + point_rel_ms( sector_length, sector_length );
        auto ne_inner = ne_corner + point_rel_ms( -sector_length, sector_length );
        auto se_inner = se_corner + point_rel_ms( -sector_length, -sector_length );
        auto sw_inner = sw_corner + point_rel_ms( sector_length, -sector_length );
    ...
    static constexpr inclusive_rectangle<point_omt_ms> map_boundaries( nw_corner, se_corner );
    ...
    const auto draw_shallow_water = [&]( point_omt_ms  from, point_omt_ms  to ) {
        std::vector<point_omt_ms> points = line_to( from, to );
        for( auto &p : points ) {
            for( point_omt_ms bp : closest_points_first( p, 1 ) ) {
    ...
    const auto jittered_midpoint = [&]( const point_omt_ms & from, const point_omt_ms & to ) {
        const int jitter = rl_dist( from, to ) / 4;
        std::uniform_int_distribution<int> roll( -jitter, jitter );
        const point_omt_ms midpoint( ( from.x() + to.x() ) / 2 + roll( prng ),
    ...
    for( auto &ls : line_segments ) {
        const auto mp1 = jittered_midpoint( ls[0], ls[1] );
        const auto mp2 = jittered_midpoint( ls[0], mp1 );
        const auto mp3 = jittered_midpoint( mp1, ls[1] );
    ...
    std::unordered_set<point_omt_ms> visited;

    const auto should_fill = [&]( const point_omt_ms & p ) {
...
void mremove_trap( mapgen_constructor *m, const point_omt_ms &p )
{
    m->remove_trap( p );
}

void mtrap_set( mapgen_constructor *m, const point_omt_ms &p, trap_id type )
{
    m->trap_set( p, type );
}

void madd_field( mapgen_constructor *m, const point_omt_ms &p, field_type_id type, int intensity )
{
    m->add_field( p, type, intensity, 0_turns );
}

void resolve_regional_terrain_and_furniture( const mapgendata &dat )
{
    for( const point_omt_ms &p : dat.m.points_on_zlevel() ) {

...
static int move_cost(const item& it, const tripoint_bub_ms& src, const tripoint_bub_ms& dest) {
    if (g->u.get_grab_type() == OBJECT_VEHICLE) {
        ...
        if (const std::optional<vpart_reference> vp =
                get_map().veh_at(cart_position).part_with_feature("CARGO", false)) {
...
static bool vehicle_activity(player& p, const tripoint_bub_ms& src_loc, int vpindex, char type) {
    ...
    if (vpindex >= veh->part_count()) {
        // if parts got removed during our work, we can't just carry on removing, we want to repair
        // parts! so just bail out, as we don't know if the next shifted part is suitable for
        // repair.
        if (type == 'r') {
            return false;
        } else if (type == 'o') {
            vpindex = veh->get_next_shifted_index(vpindex, p);
            if (vpindex == -1) { return false; }
        }
    }
    ...
    const auto abs_loc = here.bub_to_abs(src_loc);
    const tripoint_mnt_veh cursor_pos = tripoint_mnt_veh(0, 0, 0);
    std::unique_ptr<player_activity> act = std::make_unique<
        player_activity>(std::make_unique<vehicle_activity_actor>(
        abs_loc, cursor_pos, veh->index_of_part(&veh->part(vpindex)), vp.get_id(), type,
        time_to_take));
    ...
    for (const auto pt : veh->get_points(true)) { act->coord_set.insert(pt); }
...
std::vector<tripoint_bub_ms> route_adjacent(const player& p, const tripoint_bub_ms& dest) {
    ...
    for (const tripoint_bub_ms& tp : here.points_in_radius(dest, 1)) {
        if (tp != p.bub_pos() && here.passable(tp)
...
static std::vector<construction_id> get_group_roots(
    const std::vector<construction_id>& all, const construction_id& base) {
    // Get group members
    ...
    if (group_members.empty()) { return std::vector<construction_id>(); }
    ...
    if (same_output) { return group_members; }

    std::vector<construction_id> ret;
    std::ranges::copy_if(group_members, std::back_inserter(ret), [&](auto& con) {
        auto it = std::find_if(group_members.begin(), group_members.end(), [&](auto& next) {
            return con->post_terrain == next->pre_terrain
...
static activity_reason_info find_base_construction(
    const std::vector<construction_id>& list_constructions, player& p, const inventory& inv,
    const tripoint_bub_ms& loc, const std::optional<construction_id>& part_con_id,
    const construction_id id, bool strict = true) {
    // already done?
    map& here = get_map();
    ...
    for (const auto& rid : roots) {
        ...
        if ((!rid_build.post_terrain.is_empty() && rid_build.post_terrain.id() == ter)
    ...
    for (const auto& con : list_constructions) {
        if (con->group.is_empty()) { continue; }
    ...
    while (!pq.empty()) {
        ...
        if ((!con_build.post_terrain.is_empty() && con_build.post_terrain.id() == ter)
        ...
        // we don't have the skill to do this one, but might be able to do one of the others left in
        // PQ
        if (!has_skill) {
            reason_result =
                activity_reason_info::build(do_activity_reason::DONT_HAVE_SKILL, false, con);
            continue;
        }
        ...
        if (cc) {
            if (pcb) {
                ...
                return activity_reason_info::
                    build(do_activity_reason::CAN_DO_CONSTRUCTION, true, con);
            }
            // can't build with current inventory. Might be able to build other options
            reason_result =
                activity_reason_info::build(do_activity_reason::NO_COMPONENTS, false, con);
            continue;
        }

        if (!pcb) {
            // Don't have the components to construct this, but might be able to construct something
            // else so set our reason in case this is the last thing we check.
            reason_result =
                activity_reason_info::build(do_activity_reason::NO_COMPONENTS, false, con);
        }
        ...
        if (con_build.pre_terrain.is_empty() && con_build.pre_furniture.is_empty()
            ...
            reason_result =
                activity_reason_info::build(do_activity_reason::BLOCKING_TILE, false, con);
            continue;
        }
    ...
    if (reason_result.reason != do_activity_reason::UNKNOWN_ACTIVITY) {
        if (reason_result.reason == do_activity_reason::NO_COMPONENTS) {
            return activity_reason_info::
                build(do_activity_reason::NO_COMPONENTS_PREREQ, false, *reason_result.con_idx);
        }
        return activity_reason_info::build(reason_result.reason, false, *reason_result.con_idx);
    }
    if (reason_result.reason == do_activity_reason::NO_COMPONENTS) { return reason_result; }
...
static bool are_requirements_nearby(
    ...
    static const auto check_weight_if = [](const activity_id& id) {
        return id == ACT_MULTIPLE_FARM || id == ACT_MULTIPLE_CHOP_PLANKS
            || id == ACT_MULTIPLE_BUTCHER || id == ACT_VEHICLE_DECONSTRUCTION
            || id == ACT_VEHICLE_REPAIR || id == ACT_MULTIPLE_CHOP_TREES || id == ACT_MULTIPLE_FISH
            || id == ACT_MULTIPLE_MINE;
    };
    const bool check_weight =
        check_weight_if(activity_to_restore)
        || (!p.backlog.empty() && check_weight_if(p.backlog.front()->id()));
    bool found_welder = false;
    for (item* elem : p.inv_dump()) {
        if (elem->has_quality(qual_WELD)) { found_welder = true; }
        temp_inv.add_item(*elem, true);
    }
    map& here = get_map();
    for (const auto& elem : loot_spots) {
        // if we are searching for things to fetch, we can skip certain things.
        // if, however they are already near the work spot, then the crafting / inventory functions
        // will have their own method to use or discount them.
        if (in_loot_zones) {
            // skip tiles in IGNORE zone and tiles on fire
            // (to prevent taking out wood off the lit brazier)
            // and inaccessible furniture, like filled charcoal kiln
            if (mgr.has(zone_type_LOOT_IGNORE, here.bub_to_abs(elem))
                || here.dangerous_field_at(elem) || !here.can_put_items_ter_furn(elem)) {
                continue;
            }
        }
        for (const auto& elem2 : here.i_at(elem)) {
            if (in_loot_zones && elem2->made_of(LIQUID)) { continue; }
            if (check_weight) {
                // this fetch task will need to pick up an item. so check for its weight/volume
                // before setting off.
                if (in_loot_zones
                    && (elem2->volume() > volume_allowed || elem2->weight() > weight_allowed)) {
                    continue;
                }
            }
        ...
        if (!in_loot_zones) {
            if (const std::optional<vpart_reference> vp =
                ...
                for (auto& it : src_veh.get_items(src_part)) { temp_inv.add_item(*it, true); }
            }
        }
    }
    // use nearby welding rig without needing to drag it or position yourself on the right side of
    // the vehicle.
    if (!found_welder) {
        for (const auto& elem : here.points_in_radius(src_loc, PICKUP_RANGE - 1)) {
            ...
            if (vp) {
                ...
                if (weldpart) {
                    item* welder =
                        item::spawn_temporary(itype_welder, calendar::start_of_cataclysm);
                    welder->charges = veh.fuel_left(itype_battery, true);
                    welder->set_flag(flag_PSEUDO);
                    temp_inv.add_item(*welder, false);
                    item* soldering_iron =
                        item::spawn_temporary(itype_soldering_iron, calendar::start_of_cataclysm);
...
static auto construction_activity_name(const construction_id& con_id) -> std::string {
    if (con_id->group.is_empty()) { return _("construction site"); }
...
static auto construction_activity_name(const zone_data* zone, const tripoint_bub_ms& src_loc)
    -> std::string {
    if (zone) {
...
static auto farm_plot_name(const zone_data* zone) -> std::string {
    ...
    return seed.is_valid() && !seed.is_empty()
             ? string_format(_("farm plot for %s"), item::nname(seed))
             : _("farm plot");
}
...
static void set_activity_failure_message(
    player& p, const std::string& msg, bool* notice_sent = nullptr) {
    if (msg.empty()) { return; }
    if (npc* guy = p.as_npc()) {
        guy->set_suppress_activity_complete_message(true);
        const std::string existing = guy->peek_activity_failure_message();
        if (existing.empty()) { guy->set_activity_failure_message(msg); }
    }
...
static activity_reason_info can_do_activity_there(
    const activity_id& act, player& p, const tripoint_bub_ms& src_loc,
    const int distance = ACTIVITY_SEARCH_DISTANCE, bool* failure_notice_sent = nullptr) {
    ...
    if (act == ACT_VEHICLE_DECONSTRUCTION || act == ACT_VEHICLE_REPAIR) {
        ...
        if (std::abs(veh->velocity) > 100 || veh->player_in_control(g->u)) {
            set_activity_failure_message(
                p, _("Vehicle work blocked: the vehicle is moving or under control."),
                failure_notice_sent);
        ...
        for (const npc& guy : g->all_npcs()) {
            ...
            if (guy.has_player_activity() && guy.activity->placement != tripoint_abs_ms::min()) {
            ...
            if (here.abs_to_bub(g->u.activity->placement) == src_loc || guy_work_spot == src_loc
                || guy.bub_pos() == src_loc || (p.is_npc() && g->u.bub_pos() == src_loc)) {
                set_activity_failure_message(
                    p, _("Vehicle work blocked: someone is already working here."),
                    failure_notice_sent);
        ...
        if (act == ACT_VEHICLE_DECONSTRUCTION) {
            ...
            for (vehicle_part* part_elem : parts) {
                ...
                // if part is not on this vehicle, or if its attached to another part that needs to
                // be removed first.
                if (vpindex == -1 || !veh->can_unmount(vpindex)) { continue; }
                // this is the same part that somebody else wants to work on, or already is.
                if (std::ranges::find(already_working_indexes, vpindex)
                    != already_working_indexes.end()) {
                    continue;
                }
                ...
                if (!has_skill_for_vehicle_work(vpinfo.removal_skills, p)) {
                    ...
                    if (!skill_text.empty()) {
                        set_activity_failure_message(
                            p,
                            string_format(
                                _("Vehicle deconstruction blocked: requires %s."), skill_text),
                            failure_notice_sent);
                    } else {
                        set_activity_failure_message(
                            p,
                ...
                const bool use_str =
                    character_funcs::can_lift_with_helpers(p, base.lift_strength());
                if (!(use_aid || use_str)) {
                ...
                // temporarily store the intended index, we do this so two NPCs don't try and work
                // on the same part at same time.
                ...
                if (!can_make) {
                    ...
                    if (!missing_list.empty()) {
                        ...
                        set_activity_failure_message(
                            p, _("Vehicle deconstruction missing requirements nearby."),
                            failure_notice_sent);
                    }
            ...
            for (vehicle_part* part_elem : parts) {
                ...
                if (!has_skill_for_vehicle_work(vpinfo.repair_skills, p)) {
                    ...
                    if (!skill_text.empty()) {
                        set_activity_failure_message(
                            p, string_format(_("Vehicle repair blocked: requires %s."), skill_text),
                            failure_notice_sent);
                    } else {
                        set_activity_failure_message(
                            p, _("Vehicle repair blocked: insufficient skill for this task."),
                            failure_notice_sent);
                    }
                    vehicle_skill_blocked = true;
                    continue;
                ...
                if (!can_make) {
                    ...
                    if (!missing_list.empty()) {
                        set_activity_failure_message(
                            p,
                            string_format(
                                _("Vehicle repair missing requirements:\n%s"), missing_list),
                            failure_notice_sent);
                    } else {
                        set_activity_failure_message(
                            p, _("Vehicle repair missing requirements nearby."),
                            failure_notice_sent);
                    }
        ...
        if (vehicle_any_found) {
            set_activity_failure_message(
                p, _("Vehicle work blocked: no suitable part to work on here."),
                failure_notice_sent);
        }
        return activity_reason_info::fail(do_activity_reason::NO_ZONE);
    }
    ...
    if (act == ACT_MULTIPLE_BUTCHER) {
        ...
        for (const auto& pt : here.points_in_radius(src_loc, 2)) {
        ...
        if (!corpses.empty()) {
            if (big_count > 0 && small_count == 0) {
                const inventory& inv = p.crafting_inventory();
                if (!b_rack_present
                    || !(here.has_nearby_table(src_loc, PICKUP_RANGE)
            ...
            if ((big_count > 0 && small_count > 0) || (big_count == 0)) {
                // there are small corpses here, so we can ignore any big corpses here for the
                // moment.
    ...
    if (act == ACT_TIDY_UP) {
        if (mgr.has_near(zone_type_LOOT_UNSORTED, here.bub_to_abs(src_loc), distance)) {
            return activity_reason_info::ok(do_activity_reason::CAN_DO_FETCH);
        }
        return activity_reason_info::fail(do_activity_reason::NO_ZONE);
    }
    if (act == ACT_MULTIPLE_CONSTRUCTION) {
        zones = mgr.get_zones(zone_type_CONSTRUCTION_BLUEPRINT, here.bub_to_abs(src_loc));
        ...
        zones = mgr.get_zones(zone_type_FARM_PLOT, here.bub_to_abs(src_loc));
        for (const zone_data& zone : zones) {
            const auto& opt = zone.get_options();
            ...
            if (here.has_flag_furn(flag_GROWTH_HARVEST, src_loc)) {
                ...
                } else if (!warm_enough_to_plant(abs_loc)) {
                    return activity_reason_info::fail(do_activity_reason::NEEDS_WARM_WEATHER);
                    // Plants underground need to be either valid to plant underground, or given
                    // artificial heating
                } else if (!seed.obj().has_flag(flag_CAN_PLANT_UNDERGROUND) && src_loc.z() < 0
                           && get_weather().get_temperature(abs_loc) < 10_c) {
                    return activity_reason_info::fail(do_activity_reason::NEEDS_ABOVE_GROUND);
                } else {
                    // do we have the required seed on our person?
                    // If its a farm zone with no specified seed, and we've checked for tilling and
                    // harvesting. then it means no further work can be done here
                    if (seed.is_empty()) {
    ...
    // Shouldn't get here because the zones were checked previously. if it does, set enum reason as
    // "no zone"
    return activity_reason_info::fail(do_activity_reason::NO_ZONE);
}
...
static std::vector<std::tuple<tripoint_bub_ms, itype_id, int>> requirements_map(
    ...
    for (const auto& elem :
         mgr.get_point_set_loot(here.bub_to_abs(p.bub_pos()), distance, p.is_npc())) {
        const auto bub_loc = here.abs_to_bub(elem);
        // if there is a loot zone that's already near the work spot, we don't want it to be added
        // twice.
        if (std::ranges::find(already_there_spots, bub_loc) != already_there_spots.end()) {
            // construction tasks don't need the loot spot *and* the already_there/combined spots
            // both added. but a farming task will need to go and fetch the tool no matter if its
            // near the work spot. whereas the construction will automatically use what's nearby
            // anyway.
            if (pickup_task) {
                loot_spots.push_back(bub_loc);
            } else {
                continue;
            }
        } else {
            loot_spots.push_back(bub_loc);
            combined_spots.push_back(bub_loc);
        }
    }
    // if the requirements aren't available, then stop.
    if (!are_requirements_nearby(
            pickup_task ? loot_spots : combined_spots, things_to_fetch_id, p, activity_to_restore,
            pickup_task, src_loc)) {
        return requirement_map;
    }
    // if the requirements are already near the work spot and its a construction/crafting task, then
    // no need to fetch anything more.
    if (!pickup_task
        && are_requirements_nearby(
            already_there_spots, things_to_fetch_id, p, activity_to_restore, false, src_loc)) {
        return requirement_map;
    }
    ...
    for (const auto& point_elem : pickup_task ? loot_spots : combined_spots) {
            ...
            for (std::vector<item_comp>& elem : req_comps) {
                for (item_comp& comp_elem : elem) {
                    if (comp_elem.type == stack_elem->typeId()) {
                        ...
                        // that is fine, we will choose which "line" to fulfill later, and the
                        // decrement will count towards that then.
                        if (!pickup_task
                            && std::ranges::find(already_there_spots, point_elem)
                                   != already_there_spots.end()) {
        ...
        for (const auto& map_elem : temp_map) {
            ...
            // if its a construction/crafting task, we can discount any items already near the work
            // spot. we don't need to fetch those, they will be used automatically in the
            // construction. a shovel for tilling, for example, however, needs to be picked up, no
            // matter if its near the spot or not.
            if (!pickup_task) {
                if (std::ranges::find(already_there_spots, point_elem)
                    != already_there_spots.end()) {
                    continue;
                }
            }
    ...
    for (const std::vector<item_comp>& elem : req_comps) {
        ...
        for (const item_comp& comp_elem : elem) {
            ...
            if (line_found) {
                while (true) {
                    // go back over things
                    if (it == requirement_map.begin()) { break; }
                    if (remainder <= 0) {
                        line_found = true;
                        break;
                    }
...
static bool construction_activity(
    player& p, const zone_data* /*zone*/, const tripoint_bub_ms& src_loc,
    const activity_reason_info& act_info, const activity_id& activity_to_restore) {
    ...
    p.assign_activity(std::make_unique<player_activity>(
        std::make_unique<construction_activity_actor>(here.bub_to_abs(src_loc))));
    return true;
}

static bool tidy_activity(
    player& p, const tripoint_bub_ms& src_loc, const activity_id& activity_to_restore,
    ...
    if (!zone_src_set.empty()) {
        ...
        for (auto& src_elem : zone_src_sorted) {
            if (!here.can_put_items_ter_furn(here.abs_to_bub(src_elem))) { continue; }
            loot_src_lot = here.abs_to_bub(src_elem);
            break;
        }
    }
    if (loot_src_lot == tripoint_bub_ms::zero()) { return false; }
    auto items_there = here.i_at(src_loc);
    for (auto& it : items_there) {
        if (it->has_var("activity_var") && it->get_var("activity_var", "") == p.name) {
            move_item(p, *it, it->count(), src_loc, loot_src_lot, activity_to_restore);
            break;
        }
    }
    // we are adjacent to an unsorted zone, we came here to just drop items we are carrying
    if (mgr.has(zone_type_LOOT_UNSORTED, here.bub_to_abs(src_loc))) {
        for (item* inv_elem : p.inv_dump()) {
            if (inv_elem->has_var("activity_var")) {
                inv_elem->erase_var("activity_var");
                put_into_vehicle_or_drop(
                    p, item_drop_reason::deliberate, inv_elem->detach(), src_loc);
            }
        }
    }
    return true;
...
static bool fetch_activity(
    ...
    const int distance = ACTIVITY_SEARCH_DISTANCE) {
    map& here = get_map();
    if (!here.can_put_items_ter_furn(here.abs_to_bub(p.backlog.front()->coords.back()))) {
        return false;
    }
                ...
                if (std::get<0>(elem) == src_loc && veh_elem->typeId() == std::get<1>(elem)) {
                    if (!p.backlog.empty()
                        ...
                        move_item(
                            ...
                            src_loc, here.abs_to_bub(p.backlog.front()->coords.back()),
                            activity_to_restore);
                        return true;
                    }
                }
            ...
            if (std::get<0>(elem) == src_loc && it.typeId() == std::get<1>(elem)) {
                ...
                if (!p.backlog.empty() && p.backlog.front()->id() == ACT_MULTIPLE_CONSTRUCTION) {
                    move_item(
                        ...
                        here.abs_to_bub(p.backlog.front()->coords.back()), activity_to_restore);
                    return true;
...
static bool butcher_corpse_activity(
    player& p, const tripoint_bub_ms& src_loc, const do_activity_reason& reason) {
    map& here = get_map();
    ...
    for (auto& elem : items) {
        if (elem->is_corpse() && !elem->has_var("activity_var")) {
            ...
            p.assign_activity(std::make_unique<player_activity>(
                std::make_unique<
                    butchery_activity_actor>(BUTCHER_FULL, targets, here.bub_to_abs(src_loc))));
            return true;
        }
    }
    return false;
        ...
        if (i->typeId() == itype_log) {
            ...
            p.assign_activity(
                std::make_unique<player_activity>(std::make_unique<wood_chop_activity_actor>(
                    wood_chop_type::PLANKS, here.bub_to_abs(src_loc), moves)));
            return true;
        }
    }
    return false;
...
void activity_on_turn_move_loot(player_activity& act, player& p) {
    ...
    if (stage == THINK) {
        ...
        for (auto& src : src_sorted) {
            ...
            // (to prevent taking out wood off the lit brazier)
            // and inaccessible furniture, like filled charcoal kiln
            if (mgr.has(zone_type_LOOT_IGNORE, src) || here.get_field(src_loc, fd_fire) != nullptr
    ...
    if (stage == DO) {
        ...
        for (auto it = items.begin() + num_processed; it < items.end(); ++it) {
            ...
            if (thisitem.made_of(LIQUID)) { continue; }

            // skip favorite items in ignore favorite zones
            if (thisitem.is_favorite && mgr.has(zone_type_LOOT_IGNORE_FAVORITES, src)) { continue; }

            const zone_type_id id =
                mgr.get_near_zone_type_for_item(thisitem, abspos, ACTIVITY_SEARCH_DISTANCE);
            ...
            for (const auto& dest : dest_set) {
                ...
                if (free_space >= thisitem.volume()) {
                    ...
                    if (num_processed > 0) { --num_processed; }
                    break;
                }
            }
            if (p.moves <= 0) { return; }
...
static bool mine_activity(player& p, const tripoint_bub_ms& src_loc) {
    ...
    for (item* elem : mining_inv) {
        if (chosen_item == nullptr) {
            chosen_item = elem;
            if (elem->type->can_use("JACKHAMMER")) { powered = true; }
        } else {
    ...
    if (powered) {
        p.assign_activity(
            std::make_unique<player_activity>(std::make_unique<jackhammer_activity_actor>(
                here.bub_to_abs(src_loc), safe_reference<item>(*chosen_item))));
    } else {
        p.assign_activity(
...
static bool chop_tree_activity(player& p, const tripoint_bub_ms& src_loc) {
    ...
    if (here.has_flag(flag_TREE, src_loc)) {
        p.assign_activity(
            std::make_unique<player_activity>(std::make_unique<wood_chop_activity_actor>(
                wood_chop_type::TREE, here.bub_to_abs(src_loc), moves, best_qual)));
        return true;
    } else if (ter == t_trunk || ter == t_stump) {
        p.assign_activity(
            std::make_unique<player_activity>(std::make_unique<wood_chop_activity_actor>(
                wood_chop_type::LOGS, here.bub_to_abs(src_loc), moves, best_qual)));
        return true;
    }
    return false;
}
...
static zone_type_id get_zone_for_act(
    const tripoint_bub_ms& src_loc, const zone_manager& mgr, const activity_id& act_id) {
    zone_type_id ret = zone_type_id("");
    if (act_id == ACT_VEHICLE_DECONSTRUCTION) { ret = zone_type_VEHICLE_DECONSTRUCT; }
    if (act_id == ACT_VEHICLE_REPAIR) { ret = zone_type_VEHICLE_REPAIR; }
    if (act_id == ACT_MULTIPLE_CHOP_TREES) { ret = zone_type_CHOP_TREES; }
    if (act_id == ACT_MULTIPLE_CONSTRUCTION) { ret = zone_type_CONSTRUCTION_BLUEPRINT; }
    if (act_id == ACT_MULTIPLE_FARM) { ret = zone_type_FARM_PLOT; }
    if (act_id == ACT_MULTIPLE_BUTCHER) { ret = zone_type_LOOT_CORPSE; }
    if (act_id == ACT_MULTIPLE_CHOP_PLANKS) { ret = zone_type_LOOT_WOOD; }
    if (act_id == ACT_MULTIPLE_FISH) { ret = zone_type_FISHING_SPOT; }
    if (act_id == ACT_MULTIPLE_MINE) { ret = zone_type_MINING; }
    if (src_loc != tripoint_bub_ms::zero() && act_id == ACT_FETCH_REQUIRED) {
        const zone_data* zd = mgr.get_zone_at(get_map().bub_to_abs(src_loc));
        if (zd) { ret = zd->get_type(); }
    }
    return ret;
}
...
static std::unordered_set<tripoint_abs_ms> generic_multi_activity_locations(
    ...
    if (act_id == ACT_TIDY_UP) {
        ...
        if (!unsorted_set.empty()) { unsorted_spot = here.abs_to_bub(random_entry(unsorted_set)); }
        bool found_one_point = false;
        bool found_route = true;
        for (const tripoint_bub_ms& elem :
             here.points_in_radius(localpos, ACTIVITY_SEARCH_DISTANCE)) {
        ...
        if (src_set.empty() && unsorted_spot != tripoint_bub_ms::zero()) {
            for (const item* inv_elem : p.inv_dump()) {
                if (inv_elem->has_var("activity_var")) {
                    // we've gone to tidy up all the things lying around, now tidy up the things we
                    // picked up.
                    src_set.insert(here.bub_to_abs(unsorted_spot));
                    break;
                }
            }
        }
        ...
        // multiple construction will form a list of targets based on blueprint zones and unfinished
        // constructions
        if (act_id == ACT_MULTIPLE_CONSTRUCTION) {
            ...
            for (const tripoint_bub_ms& elem :
                ...
                if (pc) { src_set.insert(here.bub_to_abs(elem)); }
            }
            std::erase_if(src_set, [&](const tripoint_abs_ms& point) {
                return mgr.has(zone_type_CONSTRUCTION_IGNORE, point);
            });
            if (before_filter_count > 0 && src_set.empty()) {
                if (npc* guy = p.as_npc()) {
                    guy->set_suppress_activity_complete_message(true);
                    guy->set_activity_failure_message(_("Construction tasks skipped: all targets "
                                                        "are inside a Construction: Ignore zone.  "
                                                        "Remove that zone or move the blueprint."));
                }
        ...
        for (const auto& elem : mental_map) {
            ...
            src_set.insert(here.bub_to_abs(elem_point));
        }
    }
    ...
    if (!pre_dark_check && post_dark_check) {
        ...
        if (act_id == ACT_MULTIPLE_FARM) {
            set_activity_failure_message(p, _("Farming blocked for nearby farm plots: it is too "
                                              "dark to work."));
        }
    }
    if (act_id == ACT_MULTIPLE_FARM && src_set.empty()) {
        set_activity_failure_message(p, _("No nearby farm plots are available for this order.  Add "
                                          "a Farm Plot zone to give the order."));
    }
    if (act_id == ACT_MULTIPLE_CONSTRUCTION && src_set.empty()) {
        if (npc* guy = p.as_npc()) {
            guy->set_suppress_activity_complete_message(true);
            guy->set_activity_failure_message(_("No construction sites are available.  Add a "
                                                "Construction: Blueprint zone or start a "
...
static requirement_check_result generic_multi_activity_check_requirement(
    ...
    const bool is_vehicle_activity =
        act_id == ACT_VEHICLE_DECONSTRUCTION || act_id == ACT_VEHICLE_REPAIR;
    ...
    const auto record_construction_failure = [&](const std::string& msg) {
        if (act_id != ACT_MULTIPLE_CONSTRUCTION) { return; }
        record_activity_failure(msg);
    };
    const auto record_farm_failure = [&](const std::string& msg) {
        if (act_id != ACT_MULTIPLE_FARM) { return; }
        record_activity_failure(msg);
    };
    const auto record_vehicle_failure = [&](const std::string& msg) {
        if (!is_vehicle_activity) { return; }
        record_activity_failure(msg);
    };

    map& here = get_map();
    const auto abspos = p.abs_pos();
    const auto src_abs = here.bub_to_abs(src_loc);
    zone_manager& mgr = zone_manager::get_manager();

    bool& can_do_it = act_info.can_do;
    const do_activity_reason& reason = act_info.reason;
    const zone_data* zone = mgr.get_zone_at(src_abs, get_zone_for_act(src_loc, mgr, act_id));

    const bool needs_to_be_in_zone =
        act_id == ACT_FETCH_REQUIRED || act_id == ACT_MULTIPLE_FARM
        || act_id == ACT_MULTIPLE_BUTCHER || act_id == ACT_MULTIPLE_CHOP_PLANKS
        || act_id == ACT_MULTIPLE_CHOP_TREES || act_id == ACT_VEHICLE_DECONSTRUCTION
        || act_id == ACT_VEHICLE_REPAIR || act_id == ACT_MULTIPLE_FISH
        || act_id == ACT_MULTIPLE_MINE
        || (act_id == ACT_MULTIPLE_CONSTRUCTION && !here.partial_con_at(tripoint_bub_ms(src_loc)));
    // some activities require the target tile to be part of a zone.
    // tidy up activity doesn't - it wants things that may not be in a zone already - things that
    ...
    if (can_do_it) { return CAN_DO_LOCATION; }
    if (reason == do_activity_reason::DONT_HAVE_SKILL || reason == do_activity_reason::NO_ZONE
        || reason == do_activity_reason::ALREADY_DONE || reason == do_activity_reason::BLOCKING_TILE
        ...
        if (reason == do_activity_reason::DONT_HAVE_SKILL) {
            if (npc* guy = p.as_npc()) {
                if (!guy->peek_activity_failure_message().empty()) { return SKIP_LOCATION; }
            }
            ...
            if (act_id == ACT_MULTIPLE_CONSTRUCTION && act_info.con_idx) {
                ...
                failure_msg =
                    skill_text.empty()
                        ? string_format(_("Construction blocked for %s: insufficient skill for the "
                                          "selected build."),
                                        construction_activity_name(act_info.con_idx))
            ...
            if (!item_name.empty()) {
                ...
                if (act_id == ACT_MULTIPLE_FARM) {
                    record_farm_failure(string_format(
                        _("Farming blocked for %1$s: %2$s is lying on the plot."),
                ...
                if (act_id == ACT_MULTIPLE_FARM) {
                    record_farm_failure(string_format(
                        _("Farming blocked for %s: the plot is occupied."), farm_plot_name(zone)));
                }
            }
            record_vehicle_failure(_("Vehicle work blocked: something is occupying the work "
                                     "site."));
            ...
            if (act_id == ACT_MULTIPLE_FARM) {
                record_farm_failure(string_format(
                    _("Farming blocked for %s: it is too cold to plant here."),
                    farm_seed_name(zone)));
            }
        ...
        for (const auto& elem :
            ...
            const auto bub_loc = here.abs_to_bub(elem);
            loot_zone_spots.push_back(bub_loc);
            combined_spots.push_back(bub_loc);
        }
        for (const auto& elem : here.points_in_radius(src_loc, PICKUP_RANGE - 1)) {
            combined_spots.push_back(elem);
        ...
        if ((reason == do_activity_reason::NO_COMPONENTS
            ...
            requirement_data reqs_data =
                requirement_data(tool_comp_vector, quality_comp_vector, requirement_comp_vector);
            const std::string ran_str = random_string(10);
            const requirement_id req_id(ran_str);
        ...
        if (!are_requirements_nearby(
                tool_pickup ? loot_zone_spots : combined_spots, what_we_need, p, act_id,
                tool_pickup, src_loc)) {
            ...
            const std::string missing_text =
                ...
                : needs_farming && reason == do_activity_reason::NEEDS_PLANTING
                    ? string_format(
                          _("Farming skipped for %s: missing nearby seeds."), farm_plot_name(zone))
            ...
            if (needs_construction || needs_farming) {
                if (npc* guy = p.as_npc()) { guy->set_suppress_activity_complete_message(true); }
                auto combined = missing_text;
                if (!missing_list.empty()) { combined += "\n" + missing_list; }
                ...
                if (npc* guy = p.as_npc()) {
                    guy->set_suppress_activity_complete_message(true);
                    const std::string missing_list = what_we_need->list_missing();
                    std::string combined = _("Vehicle work skipped: required components or tools "
                                             "are missing nearby.");
                    if (!missing_list.empty()) { combined += "\n" + missing_list; }
            ...
            if (!check_only) {
                ...
                if (act_prev.coords.empty()) {
                    ...
                    for (const auto& point_elem :
                        ...
                        // ( or someone else's ) construction spots
                        if (!p.sees(point_elem)
                            || (std::ranges::find(local_src_set, point_elem) != local_src_set.end())
                            || !here.can_put_items_ter_furn(point_elem)) {
                            continue;
                        }
                    ...
                    act_prev.coords.push_back(here.bub_to_abs(
...
static bool generic_multi_activity_do(
    player& p, const activity_id& act_id, const activity_reason_info& act_info,
    const tripoint_abs_ms& src, const tripoint_bub_ms& src_loc) {
    ...
    if (reason == do_activity_reason::NEEDS_HARVESTING
        ...
        std::vector<zone_data> zones = mgr.get_zones(zone_type_FARM_PLOT, here.bub_to_abs(src_loc));
        for (const zone_data& zone : zones) {
            const auto& opt = zone.get_options();
            ...
            if (seed_inv.empty()) { continue; }
            if (!seed.is_valid() || !seed.obj().seed) { continue; }
        ...
        if (!tidy_activity(p, src_loc, act_id, ACTIVITY_SEARCH_DISTANCE)) { return false; }
        ...
        if (fetch_activity(p, src_loc, act_id, ACTIVITY_SEARCH_DISTANCE)) {
            if (!p.is_npc()) {
                // Npcs will automatically start the next thing in the backlog, players need to be
                // manually prompted Because some player activities are necessarily not marked as
                // auto-resume.
...
bool generic_multi_activity_handler(player_activity& act, player& p, bool check_only) {
    ...
    for (const tripoint_abs_ms& src : src_sorted) {
        ...
        if (!here.inbounds(src_loc) && !check_only) {
            ...
            if (route.empty()) {
                const zone_data* zone =
                    mgr.get_zone_at(src, get_zone_for_act(src_loc, mgr, activity_to_restore));
                // can't get there, can't do anything, skip it
        ...
        activity_reason_info act_info = can_do_activity_there(
            activity_to_restore, p, src_loc, ACTIVITY_SEARCH_DISTANCE, &failure_notice_sent);
        ...
        const requirement_check_result req_res = generic_multi_activity_check_requirement(
            p, activity_to_restore, act_info, src, src_loc, src_set, check_only,
            &failure_notice_sent);
        ...
        // and we can't check player.pos() for darkness before they've traveled to where they are
        // going to be. but now we are here, we check
        if (activity_to_restore != ACT_TIDY_UP && activity_to_restore != ACT_MOVE_LOOT
    ...
    if (!check_only) {
        ...
        // if we got here, we need to revert otherwise NPC will be stuck in AI Limbo and have a head
        // explosion.
        if (p.backlog.empty() || src_set.empty()) {
            if (is_multi_construction && !construction_progress) {
                if (npc* guy = p.as_npc()) {
                    ...
                    const std::string existing = guy->peek_activity_failure_message();
                    if (existing.empty()) {
                        guy->set_activity_failure_message(_("Construction task ended with no "
                                                            "progress.  Check zones and required "
                                                            "components."));
                    } else {
            ...
            if (npc* guy = p.as_npc()) {
                ...
                const std::string existing = guy->peek_activity_failure_message();
                if (existing.empty()) {
                    guy->set_activity_failure_message(_("Construction task ended with no progress. "
                                                        " Check zones and required components."));
                } else {
                    guy->set_activity_failure_message(existing);
                }
            }
...
static std::optional<tripoint_bub_ms> find_best_fire(
    ...
    for (const tripoint_bub_ms& pt : from) {
        ...
        if (here.has_flag_furn(TFLAG_FIRE_CONTAINER, pt)) { return pt; }
    }

    return best_fire;
}

static inline bool has_clear_path_to_pickup_items(
    const tripoint_bub_ms& from, const tripoint_bub_ms& to) {
    map& here = get_map();
    return here.has_items(to) && here.accessible_items(to)
...
static std::optional<tripoint_bub_ms> find_refuel_spot_zone(const tripoint_bub_ms& center) {
    ...
    for (const auto& tile_abs : tiles_abs) {
        ...
        if (has_clear_path_to_pickup_items(center, tile)) { return tile; }
    }
...
bool find_auto_consume(player& p, const consume_type type) {
    ...
    const auto ok_to_consume = [&p, type](const item* it) -> bool {
        ...
        /* not enjoyable.         */
        if (p.fun_for(comest).first < -5) { return false; }
        /* cannot consume.        */
        if (!p.can_consume(comest)) { return false; }
        ...
        if (comest.has_use()) { return false; }
        return true;
    };
    ...
    std::optional<item*> stalest =
        mgr.get_near(consume_type_zone, here.bub_to_abs(pos), ACTIVITY_SEARCH_DISTANCE)
    ...
    if (item_loc && item_loc->is_container()) { item_loc->on_contents_changed(); }
    return true;
}
...
void try_fuel_fire(player_activity& act, player& p, const bool starting_fire) {
    ...
    auto best_fire = starting_fire ? here.abs_to_bub(act.placement) : find_best_fire(adjacent, pos);

    if (!best_fire || !here.accessible_items(*best_fire)) { return; }

    auto refuel_spot = find_refuel_spot_zone(pos);
    if (!refuel_spot) {
        refuel_spot = find_refuel_spot_trap(adjacent, pos);
        if (!refuel_spot) { return; }
    }
    ...
    if (!starting_fire && (fd.fuel_produced >= 1.0f || fire_age < 10_minutes)) { return; }
    ...
    for (item*& it : potential_fuel) {
        ...
        // If we specifically need tinder to start this fire, grab it the instant it's found and
        // ignore any other fuel
        if (starting_fire) {
    ...
    if (found) {
        const int quantity =
            std::max(1, std::min(found->charges, found->charges_per_volume(250_ml)));
        // Note: move_item() handles messages (they're the generic "you drop x")
        move_item(p, *found, quantity, *refuel_spot, *best_fire);
    } else if (found_tinder) {
        const int quantity =
            std::max(1, std::min(found_tinder->charges, found_tinder->charges_per_volume(250_ml)));

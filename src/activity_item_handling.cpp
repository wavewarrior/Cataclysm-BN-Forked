...
static bool vehicle_activity( player& p, const tripoint_bub_ms& src_loc, int vpindex, char type )
{
    ...
    const auto abs_loc = bub_to_abs( src_loc );
    ...
    const tripoint_bub_ms& src_loc )
{
    ...
    for( const auto& elem : loot_spots ) {
        ...
        if( in_loot_zones ) {
            ...
            if( mgr.has( zone_type_LOOT_IGNORE, bub_to_abs( elem ) )
    ...
    const int distance = ACTIVITY_SEARCH_DISTANCE, bool* failure_notice_sent = nullptr )
{
    ...
    if( act == ACT_VEHICLE_DECONSTRUCTION || act == ACT_VEHICLE_REPAIR ) {
        ...
        for( const npc& guy : g->all_npcs() ) {
            ...
            if( guy.has_player_activity() && guy.activity->placement != tripoint_abs_ms::min() ) {
                guy_work_spot = abs_to_bub( guy.activity->placement );
            }
            ...
            if( abs_to_bub( g->u.activity->placement ) == src_loc || guy_work_spot == src_loc
    ...
    if( act == ACT_TIDY_UP ) {
        if( mgr.has_near( zone_type_LOOT_UNSORTED, bub_to_abs( src_loc ), distance ) ) {
    ...
    player& p, const int distance = ACTIVITY_SEARCH_DISTANCE )
{
    ...
    for( const auto& elem :
         mgr.get_point_set_loot( p.abs_pos(), distance, p.is_npc() ) ) {
    ...
    const int distance = ACTIVITY_SEARCH_DISTANCE )
{
    ...
    map& here = get_map();
    auto loot_abspos = bub_to_abs( src_loc );
    ...
    if( !zone_src_set.empty() ) {
        ...
        for( auto& src_elem : zone_src_sorted ) {
            if( !here.can_put_items_ter_furn( abs_to_bub( src_elem ) ) ) { continue; }
            loot_src_lot = abs_to_bub( src_elem );
            break;
        }
    }
    ...
    if( mgr.has( zone_type_LOOT_UNSORTED, bub_to_abs( src_loc ) ) ) {
    ...
    const tripoint_bub_ms& src_loc, const zone_manager& mgr, const activity_id& act_id )
{
    ...
    if( src_loc != tripoint_bub_ms::zero() && act_id == ACT_FETCH_REQUIRED ) {
        const zone_data* zd = mgr.get_zone_at( bub_to_abs( src_loc ) );
    ...
    player& p, const activity_id& act_id )
{
    ...
    map& here = get_map();
    const auto abspos = bub_to_abs( localpos );
    if( act_id == ACT_TIDY_UP ) {
            ...
            for( const item * const& stack_elem : here.i_at( elem ) ) {
                if( stack_elem->has_var( "activity_var" )
                    ...
                    if( !f.has_flag( flag_PLANT ) ) {
                        src_set.insert( bub_to_abs( elem ) );
        ...
        if( src_set.empty() && unsorted_spot != tripoint_bub_ms::zero() ) {
            for( const item * inv_elem : p.inv_dump() ) {
                if( inv_elem->has_var( "activity_var" ) ) {
                    ...
                    // picked up.
                    src_set.insert( bub_to_abs( unsorted_spot ) );
                    break;
                }
            }
        }
        ...
        if( act_id == ACT_MULTIPLE_CONSTRUCTION ) {
            ...
            for( const tripoint_bub_ms& elem :
                ...
                if( pc ) { src_set.insert( bub_to_abs( elem ) ); }
            }
    ...
    for( auto it2 = src_set.begin(); it2 != src_set.end(); ) {
        ...
        const auto set_pt = abs_to_bub( *it2 );
...
static std::optional<tripoint_bub_ms> find_refuel_spot_zone( const tripoint_bub_ms& center )
{
    ...
    for( const auto& tile_abs : tiles_abs ) {
        const auto tile = abs_to_bub( tile_abs );
...
void try_fuel_fire( player_activity& act, player& p, const bool starting_fire )
{
    ...
    auto best_fire = starting_fire ? abs_to_bub( act.placement ) : find_best_fire( adjacent, pos );

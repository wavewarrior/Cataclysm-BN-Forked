...
{
    if( g_veh_rml_types_registered ) { return; }
...
void veh_interact::cache_tool_availability_update_lifting( const tripoint_bub_ms&
        world_cursor_pos )
{
    ...
    while( true ) {
        ...
        if( action == "INSTALL" || action == "CONFIRM" ) {
            if( can_install ) {
                switch( reason ) {
                    case DOUBLE_STACK:
                        msg = _( "It's too hard to reach the vehicle with the other vehicle in the "
                                 "way" );
                        return;
                    case LOW_MORALE:
                ...
                if( veh->is_foldable() && !sel_vpart_info->has_flag( "FOLDABLE" )
                    && !query_yn( _(
                                      "Installing this part will make the vehicle unfoldable.  "
                                      "Continue?" ) ) ) {
                    return;
                }
                const auto& shapes =
...
void veh_interact::do_refill()
{
    switch( cant_do( 'f' ) ) {
        case DOUBLE_STACK:
            msg = _( "You can't refill that fuel tank, it's too hard to reach with the other "
                     "vehicle in the way" );
            return;
...
void veh_interact::calc_overview()
{
    ...
    for( const vpart_reference& vpr : veh->get_all_parts() ) {
        if( vpr.part().is_engine() && vpr.part().is_available() ) {
            overview_opts.emplace_back( "ENGINE", &vpr.part(), next_hotkey( vpr.part(), hotkey ) );
        }
    }
    ...
    for( const vpart_reference& vpr : veh->get_all_parts() ) {
        if( vpr.part().is_battery() && vpr.part().is_available() ) {
            overview_opts.emplace_back( "BATTERY", &vpr.part(), next_hotkey( vpr.part(), hotkey ) );
        }
    }
    ...
    for( const vpart_reference& vpr : veh->get_all_parts() ) {
        if( vpr.part().is_perpetual_power_source() && !vpr.part().is_reactor()
            ...
            overview_opts
            .emplace_back( "POWER_GENERATOR", &vpr.part(), next_hotkey( vpr.part(), hotkey ) );
        }
    }
    ...
    for( const vpart_reference& vpr : veh->get_all_parts() ) {
        if( vpr.part().is_seat() && vpr.part().is_available() ) {
            overview_opts.emplace_back( "SEAT", &vpr.part(), next_hotkey( vpr.part(), hotkey ) );
        }
    }
}
...
void veh_interact::overview( const overview_enable_t &enable, const overview_action_t &action )
{
    ...
    while( true ) {
        ...
        // Display engine faults inline (preserves visible content while removing dead callback
        // mechanism)
        if( overview_pos >= 0 && static_cast<size_t>( overview_pos ) < overview_opts.size()
            && overview_opts[overview_pos].key == "ENGINE" ) {
...
void veh_interact::do_change_shape()
{
    if( cant_do( 'p' ) == task_reason::INVALID_TARGET ) {
        msg = _( "No parts here or the part visible from your position does not have different "
                 "shapes" );
        return;
    }
...
std::vector<std::string> veh_interact::stats_lines() const
{
    ...
    const auto stat = [&]( const std::string & s ) { lines.emplace_back( colorize( s, c_light_gray ) ); };
    ...
    stat( string_format(
              _( "Mass: <color_light_blue>%5.0f</color> %s" ), convert_weight( veh->total_mass() ),
              weight_units() ) );
    if( veh->has_lift() ) {
        units::mass lift_as_mass = units::from_newton(
                                       veh->total_lift( true, false, !veh->engine_on ) );
        stat( string_format(
                  _( "Maximum Lift: <color_light_blue>%5.0f</color> %s" ), convert_weight( lift_as_mass ),
                  weight_units() ) );
        if( veh->has_part( "WING" ) ) {
            stat( string_format(
                      _( "Liftoff Speed: <color_light_blue>%3d</color> %s" ), veh->get_takeoff_speed(),
                      velocity_units( VU_VEHICLE ) ) );
        }
    }
    if( is_boat ) {
    ...
    lines.emplace_back(
        colorize( _( "Status:" ), c_light_gray ) + " "
    ...
    stat(
        string_format( _( "Air drag:       <color_light_blue>%5.2f</color>" ), veh->coeff_air_drag() ) );
    if( is_boat ) {
        stat( string_format(
                  _( "Water drag:     <color_light_blue>%5.2f</color>" ), veh->coeff_water_drag() ) );
    }
    if( is_ground ) {
        stat( string_format(
                  _( "Rolling drag:   <color_light_blue>%5.2f</color>" ), veh->coeff_rolling_drag() ) );
    }
    stat(
        string_format( _( "Static drag:    <color_light_blue>%5d</color>" ), veh->static_drag( false ) ) );
    stat( string_format(
    ...
    if( is_boat ) {
        ...
        std::string draft_string =
            water_clearance > 0
            ? _( "Draft/Clearance:<color_light_blue>%4.2f</color>m/<color_light_blue>%4.2f</"
             "color>m" )
            : _( "Draft/Clearance:<color_light_blue>%4.2f</color>m/<color_light_red>%4.2f</"
             "color>m" );
...
static std::string veh_overview_header_label( vehicle& veh, const std::string& key, int epower_w )
{
    if( key == "ENGINE" ) {
        return string_format(
                   ...
                   veh.total_power_w( true, true ) / 1000, health_color( false ), veh.total_power_w() / 1000 );
    } else if( key == "TANK" ) {
        return _( "Tanks" );
    } else if( key == "BATTERY" ) {
        ...
        return string_format(
                   _( "Batteries: %s%+4.1f kW</color>" ), health_color( epower_w >= 0 ), epower_w / 1000.0 );
...
std::string veh_interact::overview_detail( const vehicle_part& pt, const std::string& key ) const
{
    if( key == "ENGINE" ) {
        const std::string s = string_format(
                                  ...
                                  right_justify(
                                      pt.enabled ? pgettext( "vehicle part enabled value", "Yes" )
        ...
        if( pt.is_tank() ) {
            if( !pt.ammo_current().is_null() ) {
                ...
                return colorize(
                           string_format(
                               fmtstring, specials, pt_ammo_cur->nname( 1 ), round_up( to_liter( vol ), 1 ) ),
                           pt_ammo_cur->color );
        ...
        if( !pt.ammo_current().is_null() ) {
            ...
            if( pt.is_leaking() ) { fmtstring = "%s  " + leak_marker + "%5.1fL" + leak_marker; }
            return colorize(
        ...
        if( pt.is_leaking() ) { fmtstring = "%i   " + leak_marker + "%3i%%" + leak_marker; }
        ...
        if( pt.ammo_remaining() ) {
            ...
            if( pt.is_leaking() ) { fmtstring = "%s  " + leak_marker + "%5i" + leak_marker; }
            return colorize(
...
std::vector<std::string> veh_interact::diagram_lines() const
{
    ...
    if( cols <= 0 || rows <= 0 ) { return std::vector<std::string>(); }
    ...
    // overwrite them.
    for( int y = 0; y < rows; ++y ) { put( h_size.x, y, veh_sym_to_utf8( LINE_XOXO ), c_dark_gray ); }
    for( int x = 0; x < cols; ++x ) { put( x, h_size.y, veh_sym_to_utf8( LINE_OXOX ), c_dark_gray ); }

    // One glyph per structural part (each square hit once).
    for( const int p : veh->all_standalone_parts() ) {
        const units::angle part_direction = normalize(
        ...
        if( q == point_rel_veh::zero() ) { pcol = hilite( pcol ); }
    ...
    // Centre marker = the cursor tile's part, red when the tile is obstructed.
    {
        ...
        std::string csym = " ";
        if( cpart >= 0 ) {
            const units::angle dir = normalize(
...
{
    if( !install_info ) { return std::string(); }
    std::string out;
...
{
    if( part == nullptr ) { return std::string(); }
    std::vector<std::string> lines;
    lines.emplace_back( colorize( part->name(), c_light_green ) );
    lines.emplace_back( colorize(
                            string_format( _( "Durability: <color_light_gray>%d</color>" ), part->durability ), c_white ) );
    lines.emplace_back( colorize(
                            string_format( _( "Damage: <color_light_gray>%d%%</color>" ), part->dmg_mod ), c_white ) );
    lines.emplace_back( colorize(
                            string_format( _( "Weight: <color_light_gray>%.1f%s</color>" ),
                                           convert_weight( part->item->weight ), weight_units() ),
                            c_white ) );
    if( part->folded_volume != 0_ml ) {
        lines.emplace_back( colorize(
                                string_format( _( "Folded Volume: <color_light_gray>%s %s</color>" ),
                                               format_volume( part->folded_volume ), volume_units_abbr() ),
                                c_white ) );
    }
    if( part->size > 0_ml && part->has_flag( VPFLAG_CARGO ) ) {
        lines.emplace_back( colorize(
                                string_format( _( "Capacity: <color_light_gray>%s %s</color>" ), format_volume( part->size ),
                                               volume_units_abbr() ),
                                c_white ) );
    }
    if( part->bonus > 0 ) {
        ...
        if( !label.empty() ) {
            lines.emplace_back( colorize(
                                    string_format( "%s: <color_light_gray>%d</color>", label, part->bonus ), c_white ) );
        }
    }
    if( part->has_flag( VPFLAG_WHEEL ) ) {
        const cata::value_ptr<islot_wheel> &whl = part->item->wheel;
        lines.emplace_back( colorize(
                                string_format( _( "Wheel Diameter: <color_light_gray>%s</color>" ),
                                               wheel_dimensions::format_for_display( whl ? whl->diameter : 0 ) ),
                                c_white ) );
        lines.emplace_back( colorize(
                                string_format( _( "Wheel Width: <color_light_gray>%s</color>" ),
                                               wheel_dimensions::format_for_display( whl ? whl->width : 0 ) ),
                                c_white ) );
    }
    if( part->epower != 0 ) {
        lines.emplace_back( colorize(
                                string_format( _( "Electric Power: <color_light_gray>%+4d</color>" ), part->epower ),
                                c_white ) );
    }
    if( !part->fuel_type.is_null() ) {
        lines.emplace_back( colorize(
                                string_format( _( "Charge: <color_light_gray>%s</color>" ), item::nname( part->fuel_type ) ),
                                c_white ) );
    }
    if( part->energy_consumption != 0 ) {
        lines.emplace_back( colorize(
                                string_format( _( "Drain: <color_light_gray>%+8d</color>" ), -part->energy_consumption ),
                                c_white ) );
    }
    ...
    if( !flag_line.empty() ) { lines.emplace_back( colorize( flag_line, c_yellow ) ); }
    if( part->fuel_type == itype_battery && !part->has_flag( VPFLAG_ENGINE )
        ...
        lines.emplace_back( colorize(
                                string_format( _( "Battery Capacity: <color_light_gray>%8d</color>" ), battery->capacity ),
                                c_white ) );
    } else {
        ...
        if( part_power != 0 ) {
            lines.emplace_back( colorize(
                                    string_format( _( "Power: <color_light_gray>%+8d</color>" ), part_power ), c_white ) );
        }
    }
    std::string out;
...
void veh_interact::sync_rml()
{
    ...
    rml_data->name_rml = cata_text_to_rml(
                             colorize( _( "Name: " ), c_light_gray ) + colorize( name, name_col ) );
    ...
    if( !install_info ) {
        ...
        for( int idx = 0; idx != static_cast<int>( overview_opts.size() ); ++idx ) {
            ...
            if( last != opt.key ) {
                ...
                hdr.is_header = true;
                hdr.text = cata_text_to_rml(
                               colorize( veh_overview_header_label( *veh, opt.key, epower_w ), c_light_gray ) );
                hdr.detail = cata_text_to_rml(
                                 colorize( veh_overview_caption( opt.key ), c_light_gray ) );
            ...
            row.text = cata_text_to_rml( colorize(
                                             string_format(
                                                 "<color_dark_gray>%c </color>%s", opt.hotkey ? opt.hotkey : ' ', pt.name() ),
                                             col ) );
    ...
    if( install_info ) {
        ...
        for( size_t i = 0; i < list.size(); i++ ) {
            ...
            row.text = cata_text_to_rml(
                           colorize( sym, info.color ) + " " + colorize( info.name(), namecol ) );

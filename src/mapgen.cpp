...
static std::map<std::pair<std::string, int>, mapgen_palette::placing_map> palette_placings_cache;
...
mapgen_palette mapgen_palette::load_internal(
    ...
    if (cache_it != palette_placings_cache.end()) {
        // Cache hit: format_placings is already built from a previous parse of
        // this exact JSON location.  The furniture/items/toilets/etc. members on
        // jo will never be visited in this branch, so tell the JsonObject not to
        // report them as unvisited — they were legitimately consumed on the first
        // pass that populated the cache.
        jo.allow_omitted_members();

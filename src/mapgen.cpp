...
jmapgen_int::jmapgen_int(const JsonObject& jo, const std::string& tag, int def_val, int def_valmax)
    : val(def_val),
      valmax(def_valmax) {
    if (jo.has_array(tag)) {
        JsonArray sparray = jo.get_array(tag);
...
public:
    ...
    jmapgen_vending_machine(const JsonObject& jsi): reinforced(jsi.get_bool("reinforced", false)) {
...
void jmapgen_objects::finalize() {
    ...
    std::stable_sort(objects.begin(), objects.end(), [](const jmapgen_obj& l, const jmapgen_obj& r) {
...
std::unique_ptr<vehicle> map::add_vehicle_to_map(
    ...
    for (std::vector<int>::const_iterator part = frame_indices.begin(); part != frame_indices.end();
         part++) {
...
void map::rotate(int turns, const bool setpos_safe) {
    ...
    for (int j = 0; j < 2; ++j) {
        for (int i = 0; i < 2; ++i) {
            ...
            for (auto& veh : sm->vehicles) { veh->abs_sm_pos = bub_to_abs(p); }

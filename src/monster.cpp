...
static std::vector<player*> find_targets_to_ungrab(const tripoint_bub_ms& pos) {
    ...
    for (auto& player_pos : g->m.points_in_radius(pos, 1, 0)) {
        ...
        if (!p || !p->has_effect(effect_grabbed)) { continue; }
        bool grabbed = false;
...
monster::monster()
    : corpse_components(new monster_component_item_location(this)),
...
monster::monster(const mtype_id& id): monster() {
    ...
    if (monster::has_flag(MF_AQUATIC)) { fish_population = dice(1, 20); }
}

monster::monster(const mtype_id& id, const tripoint_bub_ms& p): monster(id) { spawn(p); }
...
monster::monster(const monster& source)
    ...
    for (const item* const& it : source.inv) { inv.push_back(item::spawn(*it)); }
...
void monster::hasten_upgrade() {
    if (!can_upgrade() || upgrade_time < 1) { return; }
    ...
    if (has_effect(effect_well_fed)) { reduction = static_cast<int>(reduction * 1.3); }

    upgrade_time -= reduction;
    if (upgrade_time < 0) { upgrade_time = 0; }
}

int monster::get_upgrade_time() const { return upgrade_time; }

// Sets time to upgrade to 0.
void monster::allow_upgrade() { upgrade_time = 0; }
...
int monster::next_upgrade_time() {
    ...
    if (get_option<bool>("ALWAYS_EVOLVE") && day >= 0) { return day; }
...
void monster::try_upgrade(bool pin_time) {
    ...
    while (true) {
        ...
        if (type->upgrade_into) {
            ...
            if (MonsterGroupManager::monster_is_blacklisted(type->upgrade_into)) { return; }
            poly(type->upgrade_into);
        } else {
            const mtype_id& new_type = MonsterGroupManager::GetRandomMonsterFromGroup(
                type->upgrade_group);
            if (new_type) { poly(new_type); }
        }
...
void monster::try_reproduce() {
    ...
    int chance = -1;
    while (true) {
        if (*baby_timer > calendar::turn) { return; }
...
void monster::reproduce() {
    ...
    if (has_effect(effect_well_fed)) {
        ...
        if (x_in_y(fractional_percent, 100)) { max_babies++; }
    }
    ...
    const spawn_disposition disposition =
        is_pet() ? spawn_disposition::SpawnDisp_Pet : spawn_disposition::SpawnDisp_Default;

    if (type->baby_monster) {
        g->m.add_spawn(type->baby_monster, spawn_cnt, bub_pos(), disposition);
    } else {
        ...
        g->m.add_item_or_charges(bub_pos(), std::move(item_to_spawn), true);
    }
}
...
std::string monster::skin_name() const { return name_with_armor(); }
...
std::pair<std::string, nc_color> monster::get_attitude() const {
    ...
    const auto att = attitude_names.at(
        ply.has_trait(trait_INATTENTIVE) ? MATT_UNKNOWN : attitude(&g->u));
    return {_(att.first), all_colors.get(att.second)};
}
...
static std::pair<std::string, nc_color> speed_description(
    float mon_speed_rating, bool immobile = false) {
    if (immobile) { return std::make_pair(_("It is immobile."), c_green); }
    ...
    if (player_knows) {
        ...
        if (player_runcost == 0) { player_runcost = 1.0f; }

        // determine tiles per turn (tpt)
        const float player_tpt = ply.get_speed() / player_runcost;
        const float ratio = player_tpt == 0 ? 2.00f : mon_speed_rating / player_tpt;
...
std::string monster::print_info_text() const {
    ...
    std::string head = colorize(bar_str, hp_color) + " " + colorize(name(), basic_symbol_color());
    const std::string effect_status = get_effect_status();
    if (!effect_status.empty()) { head += "  " + colorize(effect_status, h_white); }
    head += "  " + colorize(att.first, att.second);
    lines.emplace_back(head);

    lines.emplace_back(colorize(
        sees(g->u) ? _("Can see to your current location") : _("Can't see to your current location"),
    ...
    if (debug_mode) {
        lines.emplace_back(
            colorize(_(" Difficulty ") + std::to_string(type->difficulty), c_light_gray));
    }
    if (display_mod_source) {
        const std::string mod_src = enumerate_as_string(
            type->src.begin(), type->src.end(),
    ...
    if (has_effect(effect_ridden) && mounted_player) {
        lines.emplace_back(
            colorize(string_format(_("Rider: %s"), mounted_player->disp_name()), c_white));
    }
    if (size_bonus > 0) {
        lines.emplace_back(
            colorize(string_format(_("It is %s."), size_names.at(get_size())), c_light_gray));
    }
...
std::string monster::extended_description() const {
    ...
    if (display_object_ids) { ss += colorize(string_format("[%s]", type->id.str()), c_light_blue); }
    ...
    const std::pair<std::string, nc_color> speed_desc =
        speed_description(speed_rating(), has_flag(MF_IMMOBILE));
    ...
    if (type->has_flag(MF_ANIMAL)) { types.emplace_back(_("an animal")); }
    if (!types.empty()) { ss += string_format(_("It is %s."), enumerate_as_string(types)) + "\n"; }

    using flag_description = std::pair<m_flag, std::string>;
    const auto describe_flags =
        [this,
    ...
    const auto describe_properties =
        [&ss](std::string_view format, const std::vector<property_description>& property_names,
              std::string_view if_empty = "") {
            std::string property_descriptions = enumerate_as_string(
                property_names.begin(), property_names.end(),
                [](const property_description& pd) { return pd.first ? pd.second : ""; });
    ...
    describe_flags(
        _("It has the following senses: %s."),
        {
    ...
    ss += string_format(
        _("Deal average damage per second: <stat>%.1f</stat>"),
    ...
    if (debug_mode) {
        ...
        ss += string_format(_("Current Time: Turn %1$d | Day: %2$d"), to_turns<int>(current_time),
                            to_days<int>(current_time))
            + "\n";

        ss +=
            string_format(
                _("Upgrade Time: %1$d (turns left: %2$d) %3$s"), upgrade_time,
        ...
        if (baby_timer.has_value()) {
            ss +=
                string_format(
                    _("Reproduction time: %1$d (turns left: %2$d) %3$s"),
...
nc_color monster::symbol_color() const { return color_with_effects(); }

bool monster::is_symbol_highlighted() const { return friendly != 0; }
...
nc_color monster::color_with_effects() const {
    ...
    if (has_effect(effect_beartrap) || has_effect(effect_stunned) || has_effect(effect_downed)
        || has_effect(effect_tied) || has_effect(effect_lightsnare)
        || has_effect(effect_heavysnare)) {
        ret = hilite(ret);
    }
    if (has_effect(effect_pacified)) { ret = invert_color(ret); }
    if (has_effect(effect_onfire)) { ret = red_background(ret); }
    return ret;
}
...
bool monster::can_see() const { return has_flag(MF_SEES) && !effect_cache[VISION_IMPAIRED]; }

bool monster::can_hear() const { return has_flag(MF_HEARS) && !has_effect(effect_deaf); }

bool monster::can_submerge() const {
    return (has_flag(MF_NO_BREATHE) || swims() || has_flag(MF_AQUATIC)) && !has_flag(MF_ELECTRONIC);
}

bool monster::can_drown() const {
    return !swims() && !has_flag(MF_AQUATIC) && !has_flag(MF_NO_BREATHE) && !flies();
}

bool monster::can_climb() const { return climbs() || flies(); }

bool monster::digging() const { return digs() || (can_dig() && is_underwater()); }

bool monster::can_dig() const { return has_flag(MF_CAN_DIG); }

bool monster::digs() const { return has_flag(MF_DIGS); }

bool monster::flies() const { return has_flag(MF_FLIES); }

bool monster::climbs() const { return has_flag(MF_CLIMBS); }

bool monster::swims() const { return has_flag(MF_SWIMS); }
...
bool monster::made_of_any(const std::set<material_id>& ms) const { return type->made_of_any(ms); }

bool monster::made_of(phase_id p) const { return type->phase == p; }

void monster::set_goal(const tripoint_bub_ms& p) {
    const map& here = get_map();
    if (!here.inbounds(p)) { return; }

    if (p != goal && p != bub_pos()) { repath_requested = true; }
    goal = p;
}
...
auto monster::shift(point_rel_sm sm_shift) -> void {
    ...
    if (wandf > 0) { wander_pos = wander_pos - ms_shift; }
    ...
    if (!this->path.empty()) {
        std::ranges::for_each(path, [ms_shift](tripoint_bub_ms& p) { p = p - ms_shift; });
...
Creature* monster::attack_target() {
    ...
    if (target == nullptr || target == this || attitude_to(*target) == Attitude::A_FRIENDLY
        || !sees(*target)) {
        return nullptr;
    }
...
Attitude monster::attitude_to(const Creature& other) const {
    ...
    if (m != nullptr) {
        if (m == this) { return Attitude::A_FRIENDLY; }
        ...
        if ((friendly != 0 && m->friendly != 0)
        ...
        } else if (g->u.has_trait(trait_PROF_FERAL)
                   && (faction == faction_zombie || type->in_species(ZOMBIE))
                   && (m->faction == faction_zombie || m->type->in_species(ZOMBIE))) {
            // Zombies ignoring a feral survivor aren't quite the same as friendly
            // Ignore actually-friendly zombies/ferals but not other friendlies like reprogramed
            // bots
            return Attitude::A_FRIENDLY;
            ...
            if (has_flag(MF_FACTION_MEMORY)) { effective_anger = get_faction_anger(m->faction); }

            if ((friendly == 0 && m->friendly == 0 && faction_att == MFA_NEUTRAL) || morale < 0
                || effective_anger < 10) {
...
template <> std::string io::enum_to_string<monster_attitude>(monster_attitude att) {
    switch (att) {
        case MATT_NULL:
            return "MATT_NULL";
        case MATT_FRIEND:
...
auto monster::attitude(const Character* u) const -> monster_attitude {
    if (friendly != 0) {
        ...
        if (np != nullptr && np->is_hallucination()) { return MATT_IGNORE; }
    }
    if (effect_cache[FLEEING]) { return MATT_FLEE; }
    ...
    if (u != nullptr) {
        ...
        if (faction == faction_zombie || type->in_species(ZOMBIE)) {
            if (u->has_trait(trait_PROF_FERAL)
                && !u->has_effect(effect_feral_infighting_punishment)) {
                return MATT_FRIEND;
            }
        }
        ...
        if (type->in_species(FUNGUS)
            && (u->has_trait(trait_THRESH_MYCUS) || u->has_trait(trait_MYCUS_FRIEND))) {
            return MATT_FRIEND;
        }
        ...
        if (u->has_trait(trait_TERRIFYING)) { effective_morale -= 10; }

        if (has_flag(MF_ANIMAL)) {
            if (u->has_trait(trait_PROF_FERAL)) {
                // We want all wildlife to amp their fight-or-flight response up to eleven, so anger
                // adjustments in general won't cut it.
                if (effective_anger >= -10) {
                    return MATT_ATTACK;
                } else {
                    return MATT_FLEE;
                ...
                effective_anger -= 10;
                if (effective_anger < 10) { effective_morale += 55; }
            } else if (u->has_trait(trait_ANIMALEMPATH2)) {
                effective_anger -= 20;
                if (effective_anger < 20) { effective_morale += 80; }
                ...
                if (effective_anger >= 20) { effective_anger += 20; }
                if (effective_anger < 20) { effective_morale -= 5; }
            }
        }

        for (const trait_id& mut : u->get_mutations()) {
            for (const std::pair<const species_id, int>& elem : mut.obj().anger_relations) {
                if (type->in_species(elem.first)) { effective_anger += elem.second; }
            }
        }

        for (const trait_id& mut : u->get_mutations()) {
            for (const species_id& spe : mut.obj().ignored_by) {
                if (type->in_species(spe)) { return MATT_IGNORE; }
            }
        }
    }
    ...
    if (u != nullptr && !aggro_character && !u->is_monster()) { return MATT_IGNORE; }
...
auto monster::generic_npc_attitude_to() const -> Attitude {
    ...
    if (friendly != 0) {
        if (has_effect(effect_docile)) { return Attitude::A_FRIENDLY; }
        if (!type->in_species(ZOMBIE)) { return Attitude::A_FRIENDLY; }
    }
    if (effect_cache[FLEEING]) { return Attitude::A_NEUTRAL; }
    if (has_effect(effect_pacified)) { return Attitude::A_FRIENDLY; }
    ...
    if (effective_morale < 0) { return Attitude::A_NEUTRAL; }
    if (effective_anger <= 0) { return Attitude::A_NEUTRAL; }
    if (effective_anger < 10) { return Attitude::A_NEUTRAL; }
    if (!aggro_character) { return Attitude::A_NEUTRAL; }

    return Attitude::A_HOSTILE;
}

int monster::hp_percentage() const { return get_hp(bodypart_id("torso")) * 100 / get_hp_max(); }

void monster::process_triggers() {
    process_trigger(mon_trigger::STALK, [this]() { return anger > 0 && one_in(5) ? 1 : 0; });
...
void monster::process_trigger(mon_trigger trig, int amount, mfaction_id target_faction) {
    ...
    if (type->has_fear_trigger(trig)) { morale -= amount; }
...
void monster::process_trigger(
    mon_trigger trig, const std::function<auto()->int>& amount_func, mfaction_id target_faction) {
    if (type->has_anger_trigger(trig)) {
        if (has_flag(MF_FACTION_MEMORY) && target_faction.is_valid()) {
    ...
    if (type->has_fear_trigger(trig)) { morale -= amount_func(); }
    if (type->has_placate_trigger(trig)) {
        if (has_flag(MF_FACTION_MEMORY) && target_faction.is_valid()) {
            add_faction_anger(target_faction, -amount_func());
        } else {
...
void monster::trigger_character_aggro_chance(int chance, const char* reason) {
    if (x_in_y(chance, 100)) { trigger_character_aggro(reason); }
}

bool monster::is_underwater() const { return Creature::is_underwater() && can_submerge(); }
...
bool monster::is_warm() const { return has_flag(MF_WARM); }

bool monster::in_species(const species_id& spec) const { return type->in_species(spec); }

bool monster::is_elec_immune() const { return is_immune_damage(DT_ELECTRIC); }

bool monster::is_immune_effect(const efftype_id& effect) const {
    if (effect == effect_onfire) {
        return is_immune_damage(DT_HEAT) || made_of(LIQUID) || has_flag(MF_FIREY);
    }

    if (effect == effect_corroding) { return is_immune_damage(DT_ACID); }

    if (effect == effect_bleed) { return !has_flag(MF_WARM) || !made_of(material_id("flesh")); }

    if (effect == effect_paralyzepoison || effect == effect_badpoison || effect == effect_poison) {
    ...
    if (effect == effect_stunned || effect == effect_dazed) { return has_flag(MF_STUN_IMMUNE); }

    if (effect == effect_smoke || effect == effect_teargas || effect == effect_migo_atmosphere) {
...
resistances monster::resists() const {
    ...
    res.set_resist(
        DT_BULLET, type->armor_bullet + armor_bullet_bonus + get_worn_armor_val(DT_BULLET));
...
void monster::melee_attack(Creature& target, float accuracy) {
    ...
    if (stab_cut > 0 && has_flag(MF_BADVENOM)) {
        target.add_msg_if_player(
            m_bad,
            _("You feel venom flood your body, wracking you with "
              "pain…"));
...
int monster::heal(const int delta_hp, bool overheal) {
    ...
    if (delta_hp <= 0 || (hp >= maxhp && !overheal)) { return 0; }
...
void monster::set_hp(const int hp) { this->hp = hp; }
...
bool monster::movement_impaired() { return effect_cache[MOVEMENT_IMPAIRED]; }
...
bool monster::move_effects(bool) {
    ...
    if (has_effect(effect_tied)) {
        ...
        const bool immediate_break =
            type->in_species(FISH) || type->in_species(MOLLUSK) || type->in_species(ROBOT)
            || type->bodytype == "snake" || type->bodytype == "blob";
        const float struggle_multiplier =
            tied_drop
                ? (tied_drop->typeId() == itype_id("net") ? 0.5f
                   : tied_drop->typeId() == itype_id("bolas")
                       ? 2.0f
                       : 1.0f)
                : 1.0f;
        const float struggle_threshold =
            type->melee_dice * type->melee_sides * 1.5f * struggle_multiplier;
        if (!immediate_break && rng(0, 900) > struggle_threshold) {
            if (u_see_me) { add_msg(_("The %s struggles to break free of its bonds."), name()); }
        } else if (immediate_break) {
            remove_effect(effect_tied);
            if (tied_item) {
                if (u_see_me) { add_msg(_("The %s easily slips out of its bonds."), name()); }
                g->m.add_item_or_charges(bub_pos(), remove_tied_item());
            }
        } else {
            if (tied_item) {
                ...
                const bool broken =
                    rng(type->melee_dice * type->melee_sides,
                        std::min(10000, type->melee_dice * type->melee_sides * 250))
                    > 800;
                if (!broken) {
                    g->m.add_item_or_charges(bub_pos(), it->detach());
                } else {
                    it->detach();
                }
                if (u_see_me) {
    ...
    if (has_effect(effect_downed)) {
        if (rng(0, 40) > type->melee_dice * type->melee_sides * 1.5) {
            if (u_see_me) { add_msg(_("The %s struggles to stand."), name()); }
        } else {
            if (u_see_me) { add_msg(_("The %s climbs to its feet!"), name()); }
    ...
    if (has_effect(effect_webbed)) {
        if (x_in_y(type->melee_dice * type->melee_sides, 6 * get_effect_int(effect_webbed))) {
            if (u_see_me) { add_msg(_("The %s breaks free of the webs!"), name()); }
    ...
    if (has_effect(effect_lightsnare)) {
        if (x_in_y(type->melee_dice * type->melee_sides, 12)) {
            ...
            g->m.spawn_item(bub_pos(), "string_36");
            g->m.spawn_item(bub_pos(), "snare_trigger");
            if (u_see_me) { add_msg(_("The %s escapes the light snare!"), name()); }
        }
        return false;
    }
    if (has_effect(effect_heavysnare)) {
        if (type->melee_dice * type->melee_sides >= 7) {
            if (x_in_y(type->melee_dice * type->melee_sides, 32)) {
                ...
                g->m.spawn_item(bub_pos(), "rope_6");
                g->m.spawn_item(bub_pos(), "snare_trigger");
                if (u_see_me) { add_msg(_("The %s escapes the heavy snare!"), name()); }
            }
        }
        return false;
    }
    if (has_effect(effect_beartrap)) {
        if (type->melee_dice * type->melee_sides >= 18) {
            if (x_in_y(type->melee_dice * type->melee_sides, 200)) {
                ...
                g->m.spawn_item(bub_pos(), "beartrap");
                if (u_see_me) { add_msg(_("The %s escapes the bear trap!"), name()); }
            }
        }
        return false;
    }
    if (has_effect(effect_crushed)) {
        if (x_in_y(type->melee_dice * type->melee_sides, 100)) {
            ...
            if (u_see_me) { add_msg(_("The %s frees itself from the rubble!"), name()); }
        }
        return false;
    }
    ...
    if (has_effect(effect_in_pit)) {
        if (rng(0, 40) > type->melee_dice * type->melee_sides) {
            ...
            if (u_see_me) { add_msg(_("The %s escapes the pit!"), name()); }
    ...
    if (has_effect(effect_grabbed)) {
        if (dice(type->melee_dice + type->melee_sides, 3) < get_effect_int(effect_grabbed)
            ...
            if (u_see_me) { add_msg(_("The %s breaks free from the grab!"), name()); }
...
void monster::add_effect(
    const efftype_id& eff_id, const time_duration& dur, const bodypart_str_id&, int intensity,
    bool force, bool deferred) {
...
bool monster::has_grab_break_tec() const { return false; }
...
float monster::stability_roll() const {
    ...
    if (has_effect(effect_stunned)) { stability -= rng(1, 5); }
    return stability;
}
...
int monster::get_grab_strength() const { return type->grab_strength; }
...
void monster::reset_special(const std::string& special_name) {
    ...
    if (iter != type->special_attacks.end()) { set_special(special_name, iter->second->cooldown); }
}
...
void monster::normalize_ammo(const int old_ammo) {
    ...
    for (const auto& ammo_entry : type->starting_ammo) { total_ammo += ammo_entry.second; }
    if (total_ammo == 0) {
...
void monster::set_summon_time(const time_duration& length) { summon_time_limit = length; }

void monster::decrement_summon_timer() {
    if (!summon_time_limit) { return; }
...
void monster::process_turn() {
        ...
        for (const std::pair<const emit_id, time_duration>& e : type->emit_fields) {
            ...
            if (emid == emit_id("emit_shock_cloud")) {
                if (has_effect(effect_emp)) {
                    ...
                    g->m.emit_field(bub_pos(), emit_id("emit_shock_cloud_big"));
                    continue;
                }
            }
            g->m.emit_field(bub_pos(), emid);
        }
    }
    ...
    for (const auto& sp_type : type->special_attacks) {
        ...
        if (local_iter == special_attacks.end()) { continue; }
    ...
    if (has_effect(effect_grabbing)) {
        for (auto& dest : g->m.points_in_radius(bub_pos(), 1, 0)) {
            ...
            if (p && p->has_effect(effect_grabbed)) { add_effect(effect_grabbing, 2_turns); }
        }
    }
    ...
    if (has_flag(MF_ELECTRIC_FIELD)) {
        if (has_effect(effect_emp)) {
            if (calendar::once_every(10_turns)) {
                sounds::sound(bub_pos(), 5, sounds::sound_t::combat, _("hummmmm."), false,
                              "humming", "electric");
            }
        } else {
            for (const auto& zap : g->m.points_in_radius(bub_pos(), 1)) {
                ...
                for (const auto& item : items) {
                    if (item->made_of(LIQUID) && item->flammable()) { // start a fire!
                        ...
                        sounds::sound(bub_pos(), 30, sounds::sound_t::combat, _("fwoosh!"), false,
                                      "fire", "ignition");
                        break;
                    }
                }
                if (zap != bub_pos()) {
                    explosion_handler::emp_blast(zap); // Fries electronics due to the intensity of
                    // the field
                }
                ...
                if (t == ter_str_id("t_gas_pump") || t == ter_str_id("t_gas_pump_a")) {
                    if (one_in(4)) {
                        ...
                        if (player_sees) {
                            add_msg(m_warning, _("The %s explodes in a fiery inferno!"),
                                    g->m.tername(zap));
                        }
                    } else {
                        if (player_sees) {
            ...
            if (get_weather().lightning_active && !has_effect(effect_supercharged)
                ...
                sounds::sound(bub_pos(), 20, sounds::sound_t::combat, _("vrrrRRRUUMMMMMMMM!"),
                              false, "explosion", "default");
                if (g->u.sees(bub_pos())) {
                ...
                sounds::sound(bub_pos(), 20, sounds::sound_t::combat, _("VMMMMMMMMM!"), false,
                              "humming", "electric");
            }
        }
    }
...
void monster::batch_turns(int n) {
    if (n <= 0 || is_dead_state()) { return; }

    try_upgrade(false);
    if (has_flag(MF_MILKABLE)) { refill_udders(); }
    ...
    for (const auto& sp_type : type->special_attacks) {
        ...
        if (local_iter == special_attacks.end() || !local_iter->second.enabled) { continue; }
    ...
    if (!has_flag(MF_FACTION_MEMORY) && anger != type->agro) {
        ...
        if (std::abs(anger - type->agro) > 15) {
            const int adjust_by_a = std::min((n / 4), (std::abs(anger - type->agro) - 15));
            n -= adjust_by_a * 4;
            if (anger < type->agro) {
                anger += adjust_by_a;
            } else {
        ...
        if (anger > type->agro) {
            anger -= std::min(static_cast<int>(std::ceil(n / 8.0)), std::abs(anger - type->agro));
        } else {
            anger += std::min((n / 8), std::abs(anger - type->agro));
        }
        // If we got angry at characters have a chance at calming down
    ...
    if (has_effect(effect_well_fed)) { regen *= 2.0f; }
    ...
    add_msg(m_debug, "on_load() by %s, %d turns, healed %d hp, %d speed", name(), n, healed,
            healed_speed);

    moves = 0;
}

void monster::erase() { g->remove_zombie(*this); }
    ...
    if (!is_hallucination() && ch != nullptr) {
        ...
        if (ch->has_trait(trait_PROF_FERAL) && has_flag(MF_HUMAN)) {
            ...
            if (faction != faction_zombie && !type->in_species(ZOMBIE)) {
                ...
                if (one_in(3)) {
                    ch->add_msg_if_player(
                        m_bad,
                        _("The rush of blood seems to drive off the smell "
...
int monster::mech_str_addition() const { return type->mech_str_bonus; }
...
void monster::process_one_effect(effect& it, bool is_new) {
    ...
    const auto get_effect = [&it, is_new](const std::string& arg, bool reduced) {
        if (is_new) { return it.get_amount(arg, reduced); }
...
void monster::process_effects_internal() {
    ...
    for (auto& elem : *effects) {
        for (auto& _effect_it : elem.second) {
            if (!_effect_it.second.is_removed()) { process_one_effect(_effect_it.second, false); }
        }
    }
    ...
    for (const auto& regeneration_modifier : type->regeneration_modifiers) {
        if (has_effect(regeneration_modifier.first)) {
            ...
            regen_multiplier =
                1.00 + regeneration_modifier.second.base_modifier
    ...
    if (regeneration_amount < 0) { regeneration_amount = 0; }
    ...
    if (type->regenerates_in_dark) {
        ...
        if (light < 11.0f && one_in(2) && hp < type->hp) {
            ...
            int dark_regen_amount =
                std::min(static_cast<int>(110.0f - (light * 10.0f)), type->hp - hp);
            dark_regen_amount = std::min(dark_regen_amount, 50);
            heal(round(dark_regen_amount));
            if (dark_regen_amount > 0 && g->u.sees(*this)) {
                add_msg(m_warning, _("The %1$s uses the darkness to regenerate %2$s damage."),
                        name(), dark_regen_amount);
            }
        }
    }
    ...
    if (type->regen_morale && hp >= type->hp) {
        ...
        if (morale <= type->morale) { morale += 1; }
        // Don't restore global anger for FACTION_MEMORY monsters
        if (!has_flag(MF_FACTION_MEMORY) && anger <= type->agro) { anger += 1; }
        if (morale < 0) { morale += 5; }
        // Don't restore global anger for FACTION_MEMORY monsters
        if (!has_flag(MF_FACTION_MEMORY) && anger < 0) { anger += 5; }
    }
...
bool monster::make_fungus() {
    if (is_hallucination()) { return true; }
    ...
    if (type->fungalize_into.is_empty()) { return false; }

    const std::string old_name = name();
    poly(type->fungalize_into);

    if (g->u.sees(bub_pos())) {
        add_msg(m_info, _("The spores transform %1$s into a %2$s!"), old_name, name());
    }
...
units::mass monster::weight_capacity() const { return type->weight * type->mountable_weight_ratio; }
...
void monster::add_msg_if_npc(const std::string& msg) const {
    if (g->u.sees(*this)) { add_msg(replace_with_npc_name(msg)); }
}

void monster::add_msg_player_or_npc(
...
void monster::add_msg_if_npc(const game_message_params& params, const std::string& msg) const {
    if (g->u.sees(*this)) { add_msg(params, replace_with_npc_name(msg)); }
}
...
units::mass monster::get_carried_weight() const {
    ...
    if (tack_item) { total_weight += tack_item->weight(); }
    if (storage_item) { total_weight += storage_item->weight(); }
    if (armor_item) { total_weight += armor_item->weight(); }
    for (const item* const& it : inv) { total_weight += it->weight(); }
    return total_weight;
}

units::volume monster::get_carried_volume() const {
    units::volume total_volume = 0_ml;
    for (const item* const& it : inv) { total_volume += it->volume(); }
    return total_volume;
}

bool monster::is_dead() const { return dead || is_dead_state(); }

bool monster::is_nemesis() const { return has_flag(MF_NEMESIS); }
...
void monster::init_from_item(const item& itm) {
    if (itm.typeId() == itype_corpse) {
        ...
        if (!up_time.empty()) { upgrade_time = std::stoi(up_time); }
    } else {
        // must be a robot
        const int damfac = itm.max_damage() - std::max(0, itm.damage()) + 1;
        // One hp at least, everything else would be unfair (happens only to monster with *very* low
        // hp),
...
detached_ptr<item> monster::to_item() const {
    if (type->revert_to_itype.is_empty()) { return detached_ptr<item>(); }
    ...
    if (!unique_name.empty()) { result->set_var("item_label", unique_name); }
    // If it's configured as a pet and not merely friendly, make sure next time we skip re-rolling
    // for a friendly deployment.
    if (has_effect(effect_pet)) { result->set_flag(flag_SPAWN_FRIENDLY); }
    return result;
}
...
int monster::get_hp_max(const bodypart_id&) const { return get_hp_max(); }
...
int monster::get_hp(const bodypart_id&) const { return hp; }

int monster::get_hp() const { return hp; }

float monster::get_mountable_weight_ratio() const { return type->mountable_weight_ratio; }

void monster::hear_sound(const tripoint_bub_ms& source, const int vol, const int dist) {
    if (!can_hear()) { return; }

    static const string_id<monfaction> faction_zombie("zombie");
    const bool feral_friend =
        (faction == faction_zombie || type->in_species(ZOMBIE)) && g->u.has_trait(trait_PROF_FERAL)
        && !g->u.has_effect(effect_feral_infighting_punishment);

    // Hackery: If player is currently a feral and you're a zombie, ignore any sounds close to their
    // position.
    if (feral_friend && rl_dist(g->u.bub_pos(), source) <= 10) { return; }
...
void monster::set_horde_attraction(monster_horde_attraction mha) { horde_attraction = mha; }
...
bool monster::will_join_horde(int size) {
    ...
    if (mha == MHA_NEVER) {
        ...
        return true;
    } else if (!g->m.is_outside(bub_pos())
...
void monster::on_unload() { last_updated = calendar::turn; }
...
std::pair<PathfindingSettings, RouteSettings> monster::get_pathfinding_pair() const {
    return !effect_cache[PATHFINDING_OVERRIDE]
...
const std::vector<item*>& monster::get_items() const { return inv.as_vector(); }
...
detached_ptr<item> monster::remove_item(item* it) {
    ...
    if (iter != inv.end()) { inv.erase(iter, &ret); }
    return ret;
}

location_vector<item>::iterator monster::remove_item(
    location_vector<item>::iterator& it, detached_ptr<item>* result) {
    return inv.erase(it, result);
}

std::vector<detached_ptr<item>> monster::clear_items() { return inv.clear(); }

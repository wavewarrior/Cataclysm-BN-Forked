...
- Remaining backlog (real bespoke screens): ~~**veh_interact**~~ **DONE (all 6 slices, eyeball owed)** —
  ...
  install/repair). **gamemode_defense IN PROGRESS** — slice 1 (setup form) DONE+committed,
  slice 2 (caravan shop) remaining. Left after that: the
...
| `gamemode_defense` **IN PROGRESS** | 51 | 6 | Defense game-mode menus. Two interactive curses screens: `setup()` (settings form) + `caravan()` (between-wave shop). **Slice 1 DONE (build-green Metal, toggle OFF, eyeball owed), UNCOMMITTED→committed:** the SETUP form (`refresh_setup`). Render-only field list — 9 settings (label + value + description) + 6 enemy toggles + 5 needs toggles, section headers, the selected field highlighted; SELCOL/TOGCOL colours reproduced, the cursor + all cycling/toggling/start stay on the keyboard, doc rebuilt each frame (live form). Method-local `rml_doc` + file-static `defense_setup_*` model (distraction-style sync-in-on_redraw). `gamemode_defense_rmlui_enabled()` toggle + F4 "defense mode setup" + devui bind. `refresh_setup` pristine. i18n gap: the "Defense Mode" title is hardcoded English in the RML. REMAINING: slice 2 = the caravan shop (`caravan()` + `draw_caravan_borders`/`draw_caravan_categories`/`draw_caravan_items`). |

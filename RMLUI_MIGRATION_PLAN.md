   ...
   - **creature-info (track A) — SCOPE COLLAPSED + slice 1 DONE (2026-06-21).** The plan over-framed
     ...
     **`Creature::print_info_text()` IS the shared producer** (virtual base empty; `monster`+`npc`
     ...
     producer into its 2 player-facing **hosts** (the dev hosts editmap/wish stay):
       ...
       - **`look_around` creature info pane (`print_creature_info`, game.cpp:8715) — DEFERRED**: the
         giant look overlay's `w_look` has many producers (terrain/trap/vehicle/field/creature);
         migrating it = the whole pane, couples to the map. Do as a later host migration.
...
2. **Migrate the §8.1 gate-blocker backlog** (~12 screens above) — these were never coded at all.
   ...
   Creature-info (track A): `Creature::print_info_text()` IS the shared producer (done); mtype =
   `description_view` (done); `list_monsters` host migrated `95a59a1c24`; only `look_around`'s
   creature pane left (deferred, giant host).

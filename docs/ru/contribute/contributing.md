...
Стиль C++ контролируется helper-скриптами форматирования репозитория. Перед коммитом запустите `just fmt`; см. [CODE_STYLE](./../dev/explanation/code_style.md) для подробностей.
...
В дереве исходных текстов в `tests/` встроен набор тестов. Вы должны запускать набор тестов после **ЛЮБОГО** изменения в исходном коде игры. CMake собирает исполняемый файл тестов в каталоге `tests/` выбранного пресета сборки. Без аргументов он запустит весь набор тестов. С `--help` он выведет ряд опций вызова, которые вы можете использовать для настройки его работы.
...
Я рекомендую привыкнуть вызывать команды сборки и тестирования CMake для выбранного пресета.
...
С помощью этих команд вы сможете воссоздать надлежащие условия для тестирования ваших изменений. [Вики BN](https://cataclysmbn-guide.com/) может содержать полезную информацию о меню отладки.

...
上記の例では、`out/build/` の下にビルドディレクトリを作成していますが、必須ではありません。完全に別の場所に作成することも容易です。
...
`--parallel 2` フラグはビルドの並列処理を制御します。省略することも可能です。
`--config Release` フラグは最適化された Visual Studio 構成を選択します。これを省略すると、代わりに `Debug`構成がビルドされます。
...
したがって、Tiles と Sound サポートを使用して『Cataclysm-BN』をリリースモードでビルドするための CMake コマンドは、リポジトリルートで実行すると次のようになります。
